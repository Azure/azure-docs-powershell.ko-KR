...
### 예제 1: 이름으로 PostgreSql 가상 네트워크 규칙 업데이트
```powershell
...
이 cmdlet은 이름으로 가상 네트워크 규칙을 PostgreSql 업데이트 합니다.

### 예제 2: id를 기준으로 PostgreSql 가상 네트워크 규칙 업데이트
```powershell
...
이러한 cmdlet은 id를 기준으로 PostgreSql 가상 네트워크 규칙을 업데이트 합니다.
...
### PostgreSql. Api20171201. IVirtualNetworkRule에 대 한
