...
## 설명은
**AzStorageBlobContent** cmdlet은 로컬 파일을 Azure Storage blob에 업로드 합니다.
...
**AzStorageBlobContent** Cmdlet은 ContosoUploads 라는 컨테이너에 파일을 업로드 합니다.
...
이 명령은 Get-AzStorageBlob cmdlet을 사용 하 여 ContosoUploads 컨테이너의 Planning2015 라는 blob을 가져온 다음 현재 cmdlet에 전달 합니다.
...
이 명령은 **AzStorageContainer** cmdlet을 사용 하 여 ContosoUpload 문자열로 시작 하는 컨테이너를 가져온 다음 현재 cmdlet에 해당 blob을 전달 합니다.
명령이 ContosoPlanning as Planning2015 라는 파일을 업로드 합니다.
...
### 예제 6: 지정 된 blob 속성을 사용 하 여 파일을 blob에 업로드 하 고 StandardBlobTier을 멋지게 설정 합니다.
```
...
이 명령은 c:\temp\index.html 파일을 지정 된 blob 속성을 갖는 contosouploads 이라는 컨테이너에 업로드 하 고 StandardBlobTier 멋진 것으로 설정 합니다.
이 명령은 [MimeMapping]:: GetMimeMapping () API를 통해 blob 속성으로 설정 된 ContentType 값을 가져옵니다.
...
읽기 권한 없이 SAS 토큰에서 만들어진 저장소 컨텍스트를 사용 하려면-Force 매개 변수를 추가 하 여 blob 존재 검사를 생략 해야 합니다.
