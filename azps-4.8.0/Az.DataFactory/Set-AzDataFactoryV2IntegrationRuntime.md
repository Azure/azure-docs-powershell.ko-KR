...
## 설명은
Set-AzDataFactoryV2IntegrationRuntime cmdlet은 특정 매개 변수를 사용 하 여 통합 런타임을 업데이트 합니다.
...
Cmdlet은 ' selfhost-ir ' 이라는 통합 런타임의 설명을 업데이트 합니다.

### 예제 2: 자체 호스팅 통합 런타임 공유
```
PS C:\> Set-AzDataFactoryV2IntegrationRuntime -ResourceGroupName 'rg-test-dfv2' -DataFactoryName 'test-df-eu2' -Name 'test-selfhost-ir' `
...
Cmdlet은 공유 통합 런타임을 사용 하기 위해 ADF를 추가 합니다. `-SharedIntegrationRuntimeResourceId`매개 변수를 사용 하는 경우에도이를 `-Type` 포함 해야 합니다. Cmdlet을 실행 하기 전에 통합 런타임을 사용할 수 있는 권한을 데이터 팩터리에 부여 해야 합니다.

### 예제 3: Self-Hosted IR을 Azure에 대 한 프록시로 ADF의 SSIS IR로 구성 합니다.
```
...
Cmdlet 업데이트 Azure-SSIS 통합 런타임을 통해 자체 호스팅 통합 런타임을 데이터 프록시로 사용 합니다.

## 변수

### -AuthKey
자체 호스팅 통합 런타임의 인증 키입니다.
...
통합 런타임의 카탈로그 데이터베이스 관리자 자격 증명입니다.
...
### -CatalogPricingTier
통합 런타임의 카탈로그 데이터베이스 가격 책정 계층입니다.
...
### -DataFlowComputeType
데이터 흐름 작업을 실행할 데이터 흐름 클러스터의 계산 유형입니다.
...
### -DataFlowCoreCount
데이터 흐름 작업을 실행할 데이터 흐름 클러스터의 핵심 개수입니다.
...
### -DataFlowTimeToLive
데이터 흐름 작업을 실행할 데이터 흐름 클러스터의 ttl (time to live) 설정 시간 (분)입니다.
...
프록시로 사용 되는 Self-Hosted 통합 런타임 이름입니다.
...
Self-Hosted 및 Azure-SSIS 통합 런타임 간에 데이터를 이동할 때 사용할 준비 데이터 저장소를 참조 하는 Azure Blob 저장소 연결 된 서비스 이름입니다.
...
### -DataProxyStagingPath
스테이징 데이터 저장소에서 Self-Hosted와 Azure-SSIS 통합 런타임 간에 데이터를 이동할 때 사용할 경로를 지정 하지 않으면 기본 컨테이너가 사용 됩니다.
...
### -에디션
표준 또는 엔터프라이즈 일 수 있는 SSIS 통합 런타임의 edition이 지정 되지 않은 경우 기본값은 Standard입니다.
...
### -LicenseType
SSIS IR에 대해 선택할 라이선스 유형입니다. LicenseIncluded 또는 BasePrice의 두 가지 유형이 있습니다. Azure 하이브리드 사용 혜택 (AHUB) 가격 책정에 대해 자격이 있는 경우 BasePrice를 선택 하세요. 그렇지 않은 경우에는 LicenseIncluded를 선택 하세요.
...
관리 전용 통합 런타임에 대 한 노드당 최대 병렬 실행 수입니다.
...
### -SetupScriptContainerSasUri
사용자 지정 설치 스크립트를 포함 하는 Azure blob 컨테이너의 SAS URI입니다.
