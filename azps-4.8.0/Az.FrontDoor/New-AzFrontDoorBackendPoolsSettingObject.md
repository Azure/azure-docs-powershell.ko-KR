...
## 설명은
**AzFrontDoorBackendpoolsSettingObject** Cmdlet은 전방 문을 만들기 위해 새로운 PSBackendPoolsSettings 개체를 만듭니다.

## 예제의

### 예제 1: 기본값을 사용 하 여 BackendPoolsSettings 개체 만들기
```powershell
...
### 예제 2: 사용자가 지정한 값을 사용 하 여 BackendPoolsSettings 개체 만들기
```powershell
...
모든 백 엔드 풀에 대 한 HTTPS 요청에서 인증서 이름을 적용할지 여부를 확인 합니다.
비 HTTPS 요청에는 영향을 주지 않습니다.
...
요청을 백 엔드로 전달 하는 동안 보내기 및 받기 시간 제한. 제한 시간에 도달 하면 요청이 실패 하 고 반환 됩니다.
...
### FrontDoor. PSBackendPoolsSetting/.
