...
## 설명은
**AzApiManagementSubscription** cmdlet은 지정 된 구독 또는 구독이 지정 되지 않은 경우 모든 구독을 가져옵니다.
키가 결과 세부 정보에 포함 되지 않습니다. 키를 가져오려면 Get-help를 **AzApiManagementSubscriptionKey** 를 사용 합니다.
...
이 명령은 전역 api 범위에 대해 구성 된 모든 구독을 가져옵니다.

### 예제 6: 제품 및 사용자 범위에 대 한 모든 구독 가져오기
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-East-US" -ServiceName "contoso"
...
### -범위
범위 식별자입니다. Api 범위/Apis/{apiid} 또는 Product Scope/products/{productId} 또는 전역 범위/전역 scope/api 인지 여부에 관계 없이 구독의 범위입니다.
