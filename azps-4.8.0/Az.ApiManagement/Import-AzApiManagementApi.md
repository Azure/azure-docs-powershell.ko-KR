...
이 명령은 지정 된 Open 3.0 specification 링크에서 API를 가져옵니다.

### 예제 5: 열려 있는 Api 링크에서 ApiVersion 집합으로 API 가져오기
...
이 명령은 지정 된 Open 3.0 사양 문서에서 API를 가져와 새 ApiVersion을 만듭니다.
...
### -프로토콜
웹 API 프로토콜 (http, https). 사용할 수 있는 API에 대 한 프로토콜입니다. 이 매개 변수는 선택 사항입니다. 제공 하는 경우 사양 문서에 지정 된 프로토콜을 재정의 합니다.
...
### -ServiceUrl
API를 노출 하는 웹 서비스의 URL입니다. 이 URL은 Azure API Management에만 사용 되며 공개 되지 않습니다. 이 매개 변수는 선택 사항입니다. 제공 하는 경우 사양 문서에 지정 된 ServiceUrl을 재정의 합니다.
