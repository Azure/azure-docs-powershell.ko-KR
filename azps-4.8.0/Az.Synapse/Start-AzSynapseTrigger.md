...
## 설명은
**AzSynapseTrigger** cmdlet은 작업 영역에서 트리거를 시작 합니다. 트리거가 ' 중지 됨 ' 상태인 경우 cmdlet은 트리거를 시작 하 고 결과적으로 해당 정의에 따라 파이프라인을 호출 합니다. 트리거가 이미 ' 시작 됨 ' 상태에 있는 경우에는이 cmdlet이 적용 되지 않습니다.
...
작업 영역 ContosoWorkspace에서 ContosoTrigger 이라는 트리거를 시작 합니다.
...
Workspace ContosoWorkspace to 파이프라인에서 ContosoTrigger 이라는 트리거를 시작 합니다.
