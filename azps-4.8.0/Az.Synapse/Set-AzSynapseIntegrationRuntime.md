...
### SetByIntegrationRuntimeName (기본값)
```
...
## 설명은
**AzSynapseIntegrationRuntime** cmdlet은 특정 매개 변수를 사용 하 여 통합 런타임을 업데이트 합니다.
...
Cmdlet은 공유 통합 런타임을 사용 하는 작업 영역을 추가 합니다. `-SharedIntegrationRuntimeResourceId`매개 변수를 사용 하는 경우에도이를 `-Type` 포함 해야 합니다. Cmdlet을 실행 하기 전에 통합 런타임을 사용할 수 있는 권한을 작업 영역에 부여 해야 합니다.
...
Self-Hosted와 Azure-SSIS 통합 런타임 간에 데이터를 이동할 때 사용할 준비 데이터 저장소를 참조 하는 Azure Blob 저장소 연결 된 서비스 이름입니다.
...
스테이징 데이터 저장소에서 Self-Hosted와 Azure-SSIS 통합 런타임 간에 데이터를 이동할 때 사용할 경로입니다. 지정 하지 않으면 기본 컨테이너가 사용 됩니다.
...
### -ExpressCustomSetup
사용자 지정 설치 스크립트 없이 구성 및 타사 구성 요소를 설정 하는 데 사용할 수 있는 SSIS 통합 런타임의 빠른 사용자 지정 설정입니다.
...
### -LicenseType
SSIS IR에 대해 선택할 라이선스 유형입니다.
LicenseIncluded 또는 BasePrice의 두 가지 유형이 있습니다.
Azure 하이브리드 사용 혜택 (AHUB) 가격 책정에 대해 자격이 있는 경우 BasePrice를 선택 하세요.
그렇지 않은 경우에는 LicenseIncluded를 선택 하세요.
