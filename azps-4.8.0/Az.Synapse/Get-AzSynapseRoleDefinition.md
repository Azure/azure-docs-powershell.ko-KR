...
### GetByWorkspaceNameAndIdParameterSet (기본값)
```
...
## 설명은
**AzSynapseRoleDefinition** Cmdlet은 Azure Synapse Analytics 역할 정의를 가져옵니다.
역할 이름 또는 역할 Id를 지정 하지 않으면이 cmdlet은 모든 역할 정의를 가져옵니다.
...
이 명령은 작업 영역 아래에 있는 모든 역할 정의를 가져옵니다.
...
이 명령은 name ContosoRole를 사용 하 여 작업 영역 ContosoWorkspace에서 역할 정의를 가져옵니다.
...
이 명령은 파이프라인을 통해 작업 영역 아래에 있는 모든 역할 정의를 가져옵니다.
