...
### GetSparkJobsByIdParameterSet (기본값)
```
...
## 설명은
**AzSynapseSparkJob** Cmdlet은 Azure Synapse Analytics Spark 작업을 가져옵니다.
...
이 명령은 Spark 풀 아래에 있는 모든 작업을 가져옵니다.
...
이 명령은 지정 된 응용 프로그램 ID를 갖는 작업을 가져옵니다.
...
이 명령은 파이프라인을 통해 Spark 풀 아래에 있는 모든 작업을 가져옵니다.
