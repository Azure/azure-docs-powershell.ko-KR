...
## 설명은
**AzBatchTask** Cmdlet은 일괄 작업에 대 한 Azure 일괄 처리 작업을 가져옵니다.
*JobId* 매개 변수 또는 *작업* 매개 변수 중 하나를 통해 작업을 지정 합니다.
단일 작업을 가져오려면 *Id* 매개 변수를 지정 합니다.
*Filter* 매개 변수를 지정 하 여 OData (Open Data Protocol) 필터와 일치 하는 작업을 가져올 수 있습니다.
...
이 명령은 작업 Job01에서 ID Task03를 사용 하 여 작업을 가져옵니다.
Get-AzBatchAccountKey cmdlet을 사용 하 여 컨텍스트를 $Context 변수에 할당 합니다.

### 예제 2: 지정한 작업에서 완료 된 모든 작업 가져오기
```
...
이 명령은 ID Job02가 있는 작업에서 완료 된 작업을 가져옵니다.
...
### -확장
OData expand 절을 지정 합니다.
가져올 주 엔터티의 연결 된 엔터티를 가져오려면이 매개 변수에 대 한 값을 지정 합니다.
...
필터를 지정 하지 않으면이 cmdlet은 일괄 처리 계정 또는 작업에 대 한 모든 작업을 반환 합니다.
...
### -JobId
이 cmdlet이 가져오는 작업을 포함 하는 작업의 ID를 지정 합니다.
