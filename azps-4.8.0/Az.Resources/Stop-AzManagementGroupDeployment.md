...
## 설명은
**AzManagementGroupDeployment** cmdlet은 시작 되었지만 관리 그룹에서 완료 되지 않은 배포를 취소 합니다.
배포를 중지 하려면 배포에는 프로비저닝 등의 완료 되지 않은 상태 (예: 프로 비전 됨 또는 실패)가 아닌 완전 한 프로비저닝 상태가 있어야 합니다.

관리 그룹에서 배포를 만들려면 New-AzManagementGroupDeployment cmdlet을 사용 합니다.
...
이 명령은 관리 그룹 "myMG"에서 실행 중인 배포 "deployment01"을 취소 합니다.
...
이 명령은 관리 그룹 "myMG"의 배포 "deployment01"를 가져와 취소 합니다. 
