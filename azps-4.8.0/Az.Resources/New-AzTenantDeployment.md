...
## 설명은
**AzTenantDeployment** cmdlet은 현재 테 넌 트 범위에 배포를 추가 합니다.

테 넌 트 범위에서 배포를 추가 하려면 위치 및 템플릿을 지정 합니다.
...
구독에 리소스를 추가 하려면 구독 범위에서 배포를 만드는 **새 AzSubscriptionDeployment** 를 사용 하 여 구독 수준 리소스를 배포 합니다.
관리 그룹에 리소스를 추가 하려면 관리 그룹에서 배포를 만드는 **New AzManagementGroupDeployment** 를 사용 합니다.
...
이 명령은 정의 된 태그 매개 변수를 사용 하 여 사용자 지정 서식 파일과 디스크의 서식 파일을 사용 하 여 현재 테 넌 트 범위에서 새 배포를 만듭니다.
...
이 명령은 현재 테 넌 트에서 사용자 지정 템플릿을 사용 하 여 새 배포를 만들고 메모리 내 hashtable으로 변환 된 디스크의 템플릿 파일을 만듭니다.
...
### -WhatIfResultFormat
What-If 결과 서식입니다. -WhatIf 또는-Confirm 스위치가 설정 된 경우 적용 됩니다.
