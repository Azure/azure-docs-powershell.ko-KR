...
### 예제 1: ResourceId를 사용 하 여 VM 시작 호출
...
이 명령은 {ResourceId}를 사용 하 여 가상 머신을 시작 합니다.

### 예제 2: Context.resourcename을 사용 하 여 VM에서 poweroffing 호출
...
이 명령은 가상 컴퓨터에서 {ResourceId}를 중지 합니다.
명령에서 *Force* 매개 변수를 지정 하므로 확인을 요청 하는 메시지가 표시 되지 않습니다.

### 예제 3: ResourceId를 사용 하 여 리소스 공급자 등록 호출
...
이 명령은 리소스 공급자 "Microsoft. 네트워크"를 등록 합니다.
명령에서 *Force* 매개 변수를 지정 하므로 확인을 요청 하는 메시지가 표시 되지 않습니다.
