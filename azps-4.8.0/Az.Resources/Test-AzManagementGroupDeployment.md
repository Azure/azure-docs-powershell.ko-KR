...
## 설명은
**테스트 AzManagementGroupDeployment** cmdlet은 관리 그룹에서 배포 템플릿과 해당 매개 변수 값이 유효한 지 여부를 결정 합니다.
...
이 명령은 지정 된 서식 파일 및 매개 변수 파일을 사용 하 여 관리 그룹 "myMG"에서 배포를 테스트 합니다.
...
이 명령은 지정 된 템플릿 파일과 매개 변수 파일에서 만든 메모리 내 해시 테이블을 사용 하 여 관리 그룹 "myMG"에서 배포를 테스트 합니다.
