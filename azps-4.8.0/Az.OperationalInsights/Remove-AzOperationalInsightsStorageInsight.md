...
## 설명은
**AzOperationalInsightsStorageInsight** cmdlet은 작업 영역에서 저장소 정보를 삭제 합니다.

## 예제의

### 예제 1: 이름으로 저장 된 저장소를 제거 합니다.
```
...
이 명령은 지정 된 리소스 그룹의 MyWorkspace 라는 작업 영역에서 MyStorageInsight 이라는 저장소 정보를 제거 합니다.
명령에서 *Force* 매개 변수를 지정 하지 않으므로 저장소 정보를 제거 하기 전에 확인을 요청 하는 메시지가 표시 됩니다.

### 예제 2: 확인 하지 않고 저장소 정보 제거
```
...
첫 번째 명령은 Get-AzOperationalInsightsWorkspace cmdlet을 사용 하 여 MyWorkspace 라는 작업 영역을 가져오고이를 $Workspace 변수에 저장 합니다. 두 번째 명령은 확인 메시지가 표시 되지 않고 $Workspace에서 MyStorageInsight 이라는 저장소 정보를 제거 합니다.
...
### -WorkspaceName
저장소 정보를 포함 하는 작업 영역의 이름을 지정 합니다.
