...
PeerAsn 65000를 사용 하 여 시애틀 시설에서 단일 연결을 사용 하 여 새 직접 피어 링을 만듭니다.
...
### -DirectConnection
New-AzExchangePeeringConnection 및 파이프를 사용 하 여이 명령에 대 한 새 직접 연결을 만듭니다.
...
### -ExchangeConnection
New-AzExchangePeeringConnection를 사용 하 여 새 Exchange 연결을 만들고이 명령에 파이프 합니다.
...
### -InputObject
이 개체를 검색 하려면 Get-AzLegacyPeering를 사용 합니다.
...
### -MicrosoftNetwork
피어를 추가할 Microsoft 네트워크를 선택 합니다.
...
### -PeerAsnResourceId
피어 Asn 리소스 Id입니다. Get-AzPeerAsn를 사용 하 여 Id를 검색 합니다.
...
### -PeeringLocation
Azure 지역과는 다른 실제 위치입니다.
Get-AzPeeringLocation을 사용 하 여 \<kind\> 도시 이름을 키로 사용 합니다.
...
### -Sku
다른 옵션을 명시적으로 선택 하지 않는 한 Basic_Direct_Free 또는 Premium_Direct_Free를 선택 합니다.
