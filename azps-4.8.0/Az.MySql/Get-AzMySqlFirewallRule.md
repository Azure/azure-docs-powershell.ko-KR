...
### 예제 1: 지정 된 MySql 서버의 모든 방화벽 규칙 나열
```powershell
...
이 cmdlet에는 지정 된 MySql 서버의 모든 방화벽 규칙이 나열 됩니다.

### 예제 2: 이름별로 방화벽 규칙 가져오기
```powershell
...
이 cmdlet은 이름별로 방화벽 규칙을 가져옵니다.

### 예제 3: id 별 방화벽 규칙 가져오기
```powershell
