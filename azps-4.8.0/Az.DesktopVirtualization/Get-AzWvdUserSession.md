...
### 예제 1: 이름별로 Windows 가상 데스크톱 UserSession 가져오기
```powershell
...
이 명령은 세션 호스트에서 Windows 가상 데스크톱 UserSession을 가져옵니다.

### 예제 2: Windows 가상 데스크톱 UserSessions 목록 표시
```powershell
...
이 명령은 세션 호스트에 Windows 가상 데스크톱 UserSessions를 나열 합니다.

### 예제 3: 호스트 풀에 Windows 가상 데스크톱 UserSessions 나열
```powershell
...
이 명령은 호스트 풀의 세션 호스트에 Windows 가상 데스크톱 UserSessions를 나열 합니다.
...
### -필터
OData 필터 식입니다.
유효한 필터링 속성은 userprincipalname 및 sessionstate입니다.
...
### -SessionHostName
지정 된 호스트 풀 내에 있는 세션 호스트의 이름입니다.
...
### Api20191210Preview to. i i m. IUserSession
