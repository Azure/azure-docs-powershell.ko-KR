...
### ListRegistriesParameterSet (기본값)
```
...
## 설명은
Get-AzContainerRegistry cmdlet은 지정 된 컨테이너 레지스트리 또는 리소스 그룹 또는 구독에 있는 모든 컨테이너를 가져옵니다.

## 예제의

### 예제 1: 지정 된 컨테이너 레지스트리 가져오기
```powershell
...
이 명령은 지정 된 컨테이너 레지스트리를 가져옵니다.

### 예제 2: 리소스 그룹의 모든 컨테이너 레지스트리 가져오기
```powershell
...
이 명령은 리소스 그룹의 모든 컨테이너 레지스트리를 가져옵니다.

### 예제 3: 구독에서 모든 컨테이너의 레지스트리 가져오기
```powershell
...
이 명령은 구독의 모든 컨테이너 레지스트리를 가져옵니다.
...
### -IncludeDetail
컨테이너 레지스트리에 대 한 자세한 정보를 표시 합니다.
