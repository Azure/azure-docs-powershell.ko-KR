...
## 설명은
**AzSqlDatabaseAudit** Cmdlet은 Azure SQL 데이터베이스의 감사 설정을 가져옵니다.
Cmdlet을 사용 하려면 *ResourceGroupName* , *ServerName* 및 *DatabaseName* 매개 변수를 사용 하 여 데이터베이스를 식별 합니다.

## 예제의

### 예제 1: Azure SQL 데이터베이스의 감사 설정 가져오기
```
...
### 예제 2: Azure SQL 데이터베이스의 감사 설정, 파이프라인 가져오기
```
...
### 예제 3: Azure SQL 데이터베이스의 감사 설정 가져오기
```
PS C:\>Get-AzSqlDatabaseAudit -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -DatabaseName "Database01"
ServerName                          : server01
DatabaseName                        : database01
