...
## 설명은
**AzSqlElasticPoolAdvisor** Cmdlet은 Azure Sql 탄력적 풀에 대 한 하나 이상의 Azure Sql 탄력적인 풀 확인 프로그램을 가져옵니다.

## 예제의

### 예제 1: 지정 된 탄력적 풀의 모든 관리자 목록 표시
```
...
명령에 WIRunnerPool 이라는 탄력적 풀에 대 한 모든 관리자가 나열 됩니다.

### 예제 2: 지정 된 탄력적 풀에 대 한 단일 관리자 가져오기
```
...
이 명령은 WIRunnerPool 이라는 탄력적 풀에 대 한 CreateIndex 라는 관리자를 가져옵니다.
...
이 명령은 응답에 포함 된 권장 작업과 함께 탄력적 풀에 대 한 모든 서비스를 가져옵니다.
...
### 예제 5: 필터링을 사용 하 여 지정 된 탄력적 풀의 모든 관리자 목록 표시
```
...
명령에는 "d" 문자로 시작 하는 WIRunnerPool 이라는 탄력적 풀에 대 한 모든 관리자가 나열 됩니다.
...
### -ElasticPoolName
이 cmdlet이 관리자를 요청 하는 탄력적 풀의 이름을 지정 합니다.
...
Cmdlet에 해당 응답의 관리자에 대 한 권장 작업이 포함 되어 있음을 나타냅니다.
...
## 상속자
* 키워드: azure, azurerm, arm, resource, 관리, manager, sql, elasticpool, mssql, advisor
