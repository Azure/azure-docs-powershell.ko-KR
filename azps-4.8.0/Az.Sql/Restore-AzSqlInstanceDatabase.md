...
## 설명은
**Restore-AzSqlInstanceDatabase** cmdlet은 지역 중복 백업, 라이브 데이터베이스의 특정 시점 또는 장기 보존 백업에서 인스턴스 데이터베이스를 복원 합니다.
...
### 예제 4: 지정 된 시점에서 삭제 한 인스턴스 데이터베이스 복원
```
...
첫 번째 명령은 ' managedInstance1 ' 인스턴스에서 ' DB1 ' 이라는 삭제 된 인스턴스 데이터베이스를 가져옵니다.
두 번째 명령은 지정 된 특정 시점 백업에서 Database01_restored 이라는 인스턴스 데이터베이스로 페치된 데이터베이스를 복원 합니다.

### 예제 5: 삭제 된 인스턴스 데이터베이스를 지정 된 시점에서 복원
```
PS C:\> $deletedDatabase = Get-AzSqlDeletedInstanceDatabaseBackup -ResourceGroupName "ResourceGroup01" -InstanceName "managedInstance1" -DatabaseName "DB1"
...
첫 번째 명령은 ' managedInstance1 ' 인스턴스에서 ' DB1 ' 이라는 삭제 된 인스턴스 데이터베이스를 가져옵니다.
두 번째 명령은 지정 된 특정 시점 백업에서 입력 개체를 사용 하 여 이름이 Database01_restored 인 인스턴스 데이터베이스로 페치된 데이터베이스를 복원 합니다.
...
지정 된 리소스 ID (Get-AzSqlInstanceDatabaseLongTermRetentionBackup를 실행 하 여 찾을 수 있음)로 LTR 백업을 복원 합니다.
