...
## 설명은
Get-AzAutomationSoftwareUpdateRun cmdlet은 소프트웨어 업데이트 실행 목록을 반환 합니다. 소프트웨어 업데이트 구성에는 연결 된 일정이 있으므로 여러 번 트리거될 수 있습니다. 일정을 트리거할 때마다 업데이트 실행이 생성 됩니다. 업데이트 실행은 모든 컴퓨터 실행의 결과를 집계 한 것입니다. SoftwareUpdateConfigurationName 매개 변수를 전달 하 여 특정 소프트웨어 업데이트 구성에 대 한 실행을 가져올 수 있습니다. 특정 실행을 얻으려면 name 매개 변수를 전달 해야 합니다. 특정 상태의 실행, 특정 운영 체제를 대상으로 하는 실행, 적절 한 매개 변수를 전달 하 여 특정 시간 이후에 시작 된 실행을 나열할 수도 있습니다.
...
### 시스템 Null 허용 ' 1 [[UpdateManagement]. OperatingSystemType, Microsoft azure. PowerShell. t i = 1.0.0.0, Culture = 중립, PublicKeyToken = null]]

### 시스템 Null 허용 ' 1 [[UpdateManagement]. SoftwareUpdateRunStatus, Microsoft azure. PowerShell. t i = 1.0.0.0, Culture = 중립, PublicKeyToken = null]]
