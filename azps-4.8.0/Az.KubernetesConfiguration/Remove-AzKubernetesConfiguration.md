...
## SYNOPSIS
이렇게 하면 소스 제어 구성을 설정 하는 데 사용 되는 YAML 파일이 삭제 되므로 이후 원본 리포지토리에서의 동기화가 중지 됩니다.
...
### 예제 1: 이름으로 kubernetes 클러스터의 configuation 제거
```powershell
...
이 명령은 kubernetes 클러스터의 configuation 이름을 이름으로 제거 합니다.

### 예제 2: kubernetes 클러스터의 configuation를 개체 별로 제거
```powershell
...
이 명령은 kubernetes 클러스터의 configuation를 개체 별로 제거 합니다.
