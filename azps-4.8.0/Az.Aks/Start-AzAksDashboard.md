...
## 설명은
관리 되는 클러스터의 대시보드로 Kubectl SSH 터널을 만듭니다. SSH 터널은 Kubectl-Tunnel 라는 PowerShell 작업에 설정 되어 있으며, 실행 하 여 찾을 수 있습니다 `Get-Job` . 터널은를 통해 액세스할 수 있어야 합니다 [http://127.0.0.1:8001](http://127.0.0.1:8001) .

## 예제의

### SSH 터널을 시작 하 고 브라우저를 Kubernetes 대시보드로 엽니다.
```
...
### -DisableBrowser
Kubectl 포트 전달을 설정한 후에는 pop를 열지 마세요.
...
### -ListenPort
대시보드의 수신 대기 포트입니다. 기본값은 8003입니다.
...
### -PassThru
Cmdlet이 전달 된 경우 KubeTunnelJob를 반환 합니다.
