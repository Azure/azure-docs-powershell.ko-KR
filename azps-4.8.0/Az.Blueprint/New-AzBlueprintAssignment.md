...
`$blueprintObject`정의 된 매개 변수 및 리소스 그룹 사전을 사용 하 여 지정 된 구독 내에서 청사진 정의에 대 한 새 청사진 과제를 만듭니다. 시스템 할당 id를 사용 합니다. 위치는 관리 되는 id를 만들 영역을 정의 합니다.
...
정의 된 매개 변수 및 리소스 그룹 사전을 사용 하 여 지정 된 구독 내에 청사진 정의에 대 한 새 청사진 과제를 만들고 리소스 `$blueprintObject` 잠금을 **allresources** 로 구성 합니다. 시스템 할당 id를 사용 하는 기본값입니다.  위치는 관리 되는 id를 만들 영역을 정의 합니다.
...
`$blueprintObject`지정 된 사용자 할당 id id를 사용 하 여 정의 된 매개 변수 및 리소스 그룹 사전을 사용 하 여 지정한 구독에서 청사진 정의에 대 한 새 청사진 과제를 만듭니다.
...
과제 파일을 통해 청사진 과제를 만듭니다. 과제물 파일의 형식은 다음의 요청/응답 샘플에서 찾을 수 있습니다. https://github.com/Azure/azure-rest-api-specs/tree/master/specification/blueprint/resource-manager/Microsoft.Blueprint/preview/2018-11-01-preview/examples
...
`$blueprintObject`정의 된 매개 변수를 사용 하 여 지정 된 관리 그룹 내에서 지정 된 구독을 대상으로 하는 청사진 정의에 대 한 새 청사진 과제를 만듭니다.

## 변수

### -AssignmentFile
디스크의 JSON 형식에서 할당 파일의 위치입니다.
...
### -위치
에서 만들 관리 되는 id의 영역입니다.
자세한 정보 aka.ms/blueprintmsi
...
리소스 그룹 아티팩트에 전달할 매개 변수의 해시 테이블입니다.
...
KeyVault 리소스 id, 이름 및 버전에 대 한 보안 문자열 매개 변수입니다.
...
### -SubscriptionId
청사진 정의를 할당 하는 구독 Id입니다.
SubscriptionId 문자열의 쉼표로 구분 된 목록이 될 수 있습니다.
...
시스템 할당 id (MSI)를 통해 아티팩트를 배포 합니다.
...
사용자가 할당 한 id (MSI)를 사용 하 여 아티팩트를 배포 합니다.
