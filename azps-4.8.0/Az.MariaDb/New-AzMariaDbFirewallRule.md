...
이 명령은 그 아래에 있는 방화벽 규칙을 만듭니다.

### 예제 2:-ClientIPAddress를 사용 하 여 새 Iadb 방화벽 규칙을 만듭니다.
```powershell
...
이 cmdlet은-ClientIPAddress를 사용 하 여로이를 생성 합니다.

### 예제 3: 모든 Ip를 허용 하는 새로운 작업을 새로 하는 Adb 방화벽 규칙 만들기
```powershell
...
이 cmdlet은 모든 Ip를 허용 하는 새로운을 위한 새 \ * 방화벽 규칙을 만듭니다.

## 변수

### -AllowAll
모든 범위 Ip를 허용 하기 위해 제공 합니다 (0.0.0.0 ~ 255.255.255.255).
...
### -이름
서버 방화벽 규칙의 이름입니다.
지정 하지 않으면 기본값이 정의 되지 않습니다.
AllowAll이 있으면 기본 이름은 AllowAll_yyyy dd_HH-mm-ss입니다.
...
### Microsoft. Api20180601Preview. IFirewallRule에 대 한. b a r i.
