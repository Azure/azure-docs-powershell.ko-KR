...
## 설명은
Get-AzDataLakeStoreVirtualNetworkRule cmdlet은 지정 된 Data Lake Store에서 지정 된 가상 네트워크 규칙을 가져옵니다.
가상 네트워크 규칙이 지정 되지 않은 경우 계정에 대 한 모든 가상 네트워크 규칙이 나열 됩니다.
...
계정 "dl"에서 "myVNET" 이라는 가상 네트워크 규칙을 반환 합니다.

## 변수

### -계정
가상 네트워크 규칙을 가져오는 Data Lake Store 계정
