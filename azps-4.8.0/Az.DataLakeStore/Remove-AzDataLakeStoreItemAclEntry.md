...
## 설명은
**AzDataLakeStoreItemAclEntry** Cmdlet은 Data Lake store에 있는 파일 또는 폴더의 ACL (액세스 제어 목록)에서 항목 (ACE)을 제거 합니다.
...
### 예제 3: Acl 개체를 재귀적으로 사용 하 여 ACE에 대 한 사용 권한 제거
```
...
이 명령은 루트에서 Patti의 사용자 ACE를 제거 하 고 계정 ContosoADL의 모든 하위 디렉터리 및 파일에서 재귀적으로 사용 합니다.
...
### -재귀적으로
하위 디렉터리 및 파일에 재귀적으로 제거할 ACL을 나타냅니다.
...
### -ShowProgress
전달 되 면 진행 상태를 보여 주는 것입니다. 재귀 Acl 제거를 수행 하는 경우에만 적용 됩니다.
