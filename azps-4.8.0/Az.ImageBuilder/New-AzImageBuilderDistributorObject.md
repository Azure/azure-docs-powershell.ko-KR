...
이 명령은 공유 이미지 배포자를 만듭니다.

## 변수

### -ArtifactTag
배포자에 의해 생성/업데이트 된 후 아티팩트에 적용 되는 태그입니다.
...
### -ExcludeFromLatest
생성 된 이미지 버전을 최신에서 제외할지 여부를 나타내는 플래그입니다.
Default (false)를 사용 하지 않는 경우 생략 합니다.
...
### -위치
이미지가 이미 있는 경우 이미지에 대 한 Azure 위치가 일치 해야 합니다.
...
### -StorageAccountType
공유 이미지를 저장 하는 데 사용 되는 저장소 계정 유형입니다.
Default (Standard_LRS)를 사용 하려면 생략 합니다.
...
### Api20200214. IImageTemplateDistributor에 대 한 Microsoft.
