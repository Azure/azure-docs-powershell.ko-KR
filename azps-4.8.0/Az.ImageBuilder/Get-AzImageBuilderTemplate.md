...
이 명령은 구독 아래의 모든 서식 파일을 나열 합니다.

### 예제 2: 자원 그룹 아래에 모든 서식 파일 나열
```powershell
...
이 명령은 리소스 그룹 아래에 모든 서식 파일을 나열 합니다.

### 예제 3: 리소스 그룹에서 서식 파일 가져오기
```powershell
...
이 명령은 리소스 그룹의 템플릿을 가져옵니다.

### 예제 4: 리소스 그룹에서 서식 파일 가져오기
```powershell
...
INPUTOBJECT <IImageBuilderIdentity> : Identity 매개 변수
  - `[Id <String>]`: 리소스 id 경로
  - `[ImageTemplateName <String>]`: 이미지 서식 파일의 이름입니다.
  - `[ResourceGroupName <String>]`: 리소스 그룹의 이름입니다.
  - `[RunOutputName <String>]`: 실행 출력의 이름입니다.
