...
## 설명은
**Add-AzApplicationGatewayIPConfiguration** cmdlet은 응용 프로그램 게이트웨이에 IP 구성을 추가 합니다.
...
첫 번째 명령은 가상 네트워크를 가져옵니다.
두 번째 명령은 이전에 만든 가상 네트워크를 사용 하 여 서브넷을 가져옵니다.
세 번째 명령은 응용 프로그램 게이트웨이를 가져와 $AppGw 변수에 저장 합니다.
네 번째 명령은 $AppGw에 저장 된 응용 프로그램 게이트웨이에 IP 구성을 추가 합니다.
