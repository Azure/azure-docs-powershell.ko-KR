...
## 설명은
Add-AzApplicationGatewayBackendHttpSetting cmdlet은 응용 프로그램 게이트웨이에 백 엔드 HTTP 설정을 추가 합니다.
백 엔드 HTTP 설정은 풀의 모든 백 엔드 서버에 적용 됩니다.

## 예제의

### 예제 1: 응용 프로그램 게이트웨이에 백 엔드 HTTP 설정 추가
```powershell
...
첫 번째 명령은 ResourceGroup01 이라는 리소스 그룹에 속하는 ApplicationGateway01 라는 응용 프로그램 게이트웨이를 가져온 다음이를 $AppGw 변수에 저장 합니다. 두 번째 명령은 응용 프로그램 게이트웨이에 대 한 백 엔드 HTTP 설정을 추가 하 고, 포트를 88로 설정 하 고, 프로토콜을 HTTP로 설정한 다음, 설정의 이름을 Setting02.

### 예제 2

응용 프로그램 게이트웨이에 백 엔드 HTTP 설정을 추가 합니다. 생성
...
### -ApplicationGateway
이 cmdlet이 설정을 추가 하는 응용 프로그램 게이트웨이의 이름을 지정 합니다.
...
이 매개 변수에 허용 되는 값은 Disabled, Enabled입니다.
...
### -이름
이 cmdlet이 추가 하는 백 엔드 HTTP 설정의 이름을 지정 합니다.
...
### -프로토콜
응용 프로그램 게이트웨이 및 백 엔드 서버 간 통신을 위한 프로토콜을 지정 합니다.
