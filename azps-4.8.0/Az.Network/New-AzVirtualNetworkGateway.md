...
### 예제 2: 외부 Radius 구성을 사용 하 여 가상 네트워크 게이트웨이 만들기
```powershell
New-AzResourceGroup -Location "UK West" -Name "vnet-gateway"
...
### 예제 3: P2S 설정을 사용 하 여 가상 네트워크 게이트웨이 만들기
```powershell
New-AzResourceGroup -Location "UK West" -Name "vnet-gateway"
$subnet = New-AzVirtualNetworkSubnetConfig -Name 'gatewaysubnet' -AddressPrefix '10.254.0.0/27'
...
### 예제 4: 가상 네트워크 게이트웨이의 VpnClient에 대 한 AAD 인증 구성을 사용 하 여 가상 네트워크 게이트웨이 만들기
```powershell
New-AzResourceGroup -Location "UK West" -Name "vnet-gateway"
New-AzVirtualNetworkSubnetConfig -Name 'gatewaysubnet' -AddressPrefix '10.254.0.0/27'
...
### 예제 5: VpnGatewayGeneration를 사용 하 여 가상 네트워크 게이트웨이 만들기
```powershell
New-AzResourceGroup -Location "UK West" -Name "vnet-gateway"
$subnet = New-AzVirtualNetworkSubnetConfig -Name 'gatewaysubnet' -AddressPrefix '10.254.0.0/27'
...
### 예제 6: IpConfigurationBgpPeeringAddresses를 사용 하 여 가상 네트워크 게이트웨이 만들기
```powershell
...
위의 방법을 통해 리소스 그룹을 만들고, 공용 IP 주소를 요청 하 고, 가상 네트워크와 서브넷을 만들고, Azure에서 가상 네트워크 게이트웨이를 만듭니다.
ngwipconfig에 생성 되 고 저장 되는 게이트웨이 ipconfiguration의 ipconfigurationId1.
게이트웨이는 "영국 서쪽"의 리소스 그룹 "resourcegroup1resourcegroup1"에서 "gateway1" ("gw1ipconfBgp1" 변수에 저장 된 IP 구성 Bgppeering 주소와 "VPN" 게이트웨이 형식, vpn 유형 "RouteBased", sku "VpnGw4" 및 VpnGatewayGeneration Generation2 enabled 중에서 "" 이름 "으로 호출 됩니다.
...
### -Asn
VPN을 통한 BGP에 대 한 가상 네트워크 게이트웨이의 ASN
...
가상 네트워크 게이트웨이에서 개인 IPAddress를 사용 하도록 설정 하는 플래그
...
### -게이트웨이 종류
이 가상 네트워크 게이트웨이의 유형: Vpn, Express 경로
...
### -IpConfigurations
가상 네트워크 게이트웨이의 IpConfigurations.
...
이 VirtualNetwork VPN 게이트웨이의 세대입니다.
(%)은 (는)이 (가) VPN이 아닌 경우 없음 이어야 합니다.
...
### -VpnType
Vpn: PolicyBased/RouteBased의 유형
