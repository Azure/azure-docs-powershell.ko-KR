...
응용 프로그램 게이트웨이에 백 엔드 HTTP 설정을 추가 합니다.
...
응용 프로그램 게이트웨이에 다시 쓰기 규칙 집합을 추가 합니다.
...
### [추가-AzExpressRouteCircuitPeeringConfig](Add-AzExpressRouteCircuitPeeringConfig.md)
Express 경로 회로에 피어 링 구성을 추가 합니다.

### [추가-AzExpressRouteCrossConnectionPeering](Add-AzExpressRouteCrossConnectionPeering.md)
...
NetworkInterface와 연결 된 TapConfiguration 리소스를 만듭니다. 기존 VirtualNetworkTap 리소스를 참조 합니다.
...
### [추가-AzServiceEndpointPolicyDefinition](Add-AzServiceEndpointPolicyDefinition.md)
지정 된 정책에 서비스 끝점 정책 정의를 추가 합니다.

### [추가-AzVirtualHubRoute](Add-AzVirtualHubRoute.md)
Add-AzVirtualHubRouteTable 명령에 매개 변수로 전달 될 수 있는 VirtualHubRoute 개체를 만듭니다. 

### [추가-AzVirtualHubRouteTable](Add-AzVirtualHubRouteTable.md)
VirtualHub의 자식인 가상 허브 경로 테이블 리소스를 만듭니다.

### [추가-AzVirtualNetworkGatewayIpConfig](Add-AzVirtualNetworkGatewayIpConfig.md)
가상 네트워크 게이트웨이에 IP 구성을 추가 합니다.

### [추가-AzVirtualNetworkPeering](Add-AzVirtualNetworkPeering.md)
두 가상 네트워크 간에 피어 링을 만듭니다.

### [추가-AzVirtualNetworkSubnetConfig](Add-AzVirtualNetworkSubnetConfig.md)
가상 네트워크에 서브넷 구성을 추가 합니다.

### [추가-AzVirtualRouterPeer](Add-AzVirtualRouterPeer.md)
Azure VirtualRouter에 피어 추가
...
### [연결 해제-AzP2SVpnGatewayVpnConnection](Disconnect-AzP2SVpnGatewayVpnConnection.md)
지정 된 p2s vpn 게이트웨이를 사용 하 여 연결 된 특정 vpn 클라이언트 연결 연결 끊기

### [연결 해제-AzVirtualNetworkGatewayVpnConnection](Disconnect-AzVirtualNetworkGatewayVpnConnection.md)
지정 된 가상 네트워크 게이트웨이와 연결 된 특정 vpn 클라이언트 연결을 끊습니다.
...
백 엔드 HTTP 설정 개체의 연결 드레이닝 구성을 가져옵니다.
...
응용 프로그램 게이트웨이의 HTTP 수신기를 가져옵니다.
...
응용 프로그램 게이트웨이의 다시 작성 규칙 집합을 가져옵니다.
...
응용 프로그램 게이트웨이에 대 한 SSL 인증서를 가져옵니다.
...
자동 승인 된 개인 끝점에 연결할 수 있는 개인 링크 서비스 id의 배열을 가져옵니다.
...
Express 경로 회로에 대 한 라우트 테이블 요약을 가져옵니다.

### [Get-AzExpressRouteCircuitStat](Get-AzExpressRouteCircuitStat.md)
Express 경로 회로의 사용 통계를 가져옵니다.

### [Get-AzExpressRouteConnection](Get-AzExpressRouteConnection.md)
ExpressRouteGateway에 연결 된 모든 Express 경로 연결을 이름 또는 이름별로 연결을 가져오거나 나열 합니다.
...
Express 간 교차 연결의 경로 테이블 요약을 가져옵니다.

### [Get-AzExpressRouteGateway](Get-AzExpressRouteGateway.md)
ResourceGroupName 및 ExpressRouteGateway Name을 사용 하 여 리소스를 가져오거나 ResourceGroupName 또는 SubscriptionId에의 한 모든 게이트웨이를 나열 합니다.
...
ExpressRoutePort에 할당 된 id를 가져옵니다.
...
부하 분산 장치에서 하나 이상의 인바운드 NAT 풀 구성을 가져옵니다.
...
부하 분산 장치에서 아웃 바운드 규칙 구성을 가져옵니다.

### [Get-AzLoadBalancerProbeConfig](Get-AzLoadBalancerProbeConfig.md)
부하 분산 장치에 대 한 프로브 구성을 가져옵니다.

### [Get-AzLoadBalancerRuleConfig](Get-AzLoadBalancerRuleConfig.md)
부하 분산 장치에 대 한 규칙 구성을 가져옵니다.
...
지정 된 구독 및 지역에서 흐름 로그 리소스 또는 흐름 로그 리소스 목록을 가져옵니다.
...
리소스 그룹에 있는 리소스 및 해당 관계의 네트워크 수준 보기를 가져옵니다.
...
P2SVpnGateway에서 사이트 연결 상태 정보에 대 한 현재 aggregared 지점을 가져옵니다.

### [Get-AzP2sVpnGatewayDetailedConnectionHealth](Get-AzP2sVpnGatewayDetailedConnectionHealth.md)
P2SVpnGateway에서 현재 지점의 사이트 연결에 대 한 자세한 정보를 가져옵니다.
...
이름별로 Azure VirtualHub를 가져옵니다. ResourceGroupName 또는 ResourceGroupName/구독으로 모든 가상 허브가 나열 됩니다.

### [Get-AzVirtualHubRouteTable](Get-AzVirtualHubRouteTable.md)
가상 허브에서 가상 허브 경로 테이블을 가져오거나 가상 허브의 모든 경로 테이블을 나열 합니다.
...
Azure 가상 네트워크 게이트웨이의 BGP 피어를 나열 합니다.
...
Azure VirtualRouter 가져오기

### [Get-AzVirtualRouterPeer](Get-AzVirtualRouterPeer.md)
Azure VirtualRouter에서 VirtualRouter 피어를 가져옵니다.
...
VpnConnections를 통해이 WAN에 연결 된 VpnSites 하위 집합의 Vpn 구성을 가져옵니다. 생성 된 Vpn 구성을 고객이 지정한 저장소 blob에 업로드 합니다.

### [Get-AzVirtualWanVpnServerConfiguration](Get-AzVirtualWanVpnServerConfiguration.md)
이 VirtualWan과 연결 된 모든 VpnServerConfigurations 목록을 가져옵니다.

### [Get-AzVirtualWanVpnServerConfigurationVpnProfile](Get-AzVirtualWanVpnServerConfigurationVpnProfile.md)
사이트 클라이언트 설치 지점에 대 한 VirtualWan-VpnServerConfiguration 수준에서 Vpn 프로필을 생성 하 고 다운로드 합니다.

### [Get-AzVpnClientConfiguration](Get-AzVpnClientConfiguration.md)
사용자가 New-AzVpnClientConfiguration를 사용 하 여 생성 된 Vpn 프로필 패키지를 쉽게 다운로드할 수 있습니다.
...
이름으로 vpn 연결을 가져오거나 VpnGateway에 연결 된 모든 vpn 연결 목록을 표시 합니다.

### [Get-AzVpnGateway](Get-AzVpnGateway.md)
ResourceGroupName 및 VpnGateway Name을 사용 하 여 리소스를 가져오거나 ResourceGroupName 또는 SubscriptionId에의 한 모든 게이트웨이를 나열 합니다.

### [Get-AzVpnServerConfiguration](Get-AzVpnServerConfiguration.md)
사이트 연결을 위한 기존 VpnServerConfiguration를 가져옵니다.
...
### [AzNetworkWatcherNetworkConfigurationDiagnostic-호출](Invoke-AzNetworkWatcherNetworkConfigurationDiagnostic.md)
대상 리소스의 지정 된 네트워크 프로필에 대 한 네트워크 구성 진단 세션을 호출 합니다.

### [이동-AzExpressRouteCircuit](Move-AzExpressRouteCircuit.md)
기본 배포 모델에서 리소스 관리자 배포 모델로 Express 경로를 이동 합니다.

### [새로운 AzApplicationGateway](New-AzApplicationGateway.md)
응용 프로그램 게이트웨이를 만듭니다.

### [새로운 AzApplicationGatewayAuthenticationCertificate](New-AzApplicationGatewayAuthenticationCertificate.md)
응용 프로그램 게이트웨이에 대 한 인증 인증서를 만듭니다.
...
### [새로운 AzApplicationGatewayConnectionDraining](New-AzApplicationGatewayConnectionDraining.md)
...
### [새로운 AzApplicationGatewayFirewallCondition](New-AzApplicationGatewayFirewallCondition.md)
사용자 지정 규칙에 대 한 일치 조건을 만듭니다.

### [새로운 AzApplicationGatewayFirewallCustomRule](New-AzApplicationGatewayFirewallCustomRule.md)
응용 프로그램 게이트웨이 방화벽 정책에 대 한 새 사용자 지정 규칙을 만듭니다.

### [새로운 AzApplicationGatewayFirewallDisabledRuleGroupConfig](New-AzApplicationGatewayFirewallDisabledRuleGroupConfig.md)
사용할 수 없도록 설정 된 새 규칙 그룹 구성을 만듭니다.

### [새로운 AzApplicationGatewayFirewallExclusionConfig](New-AzApplicationGatewayFirewallExclusionConfig.md)
Application gateway waf에 대 한 새 제외 규칙 목록 만들기

### [새로운 AzApplicationGatewayFirewallMatchVariable](New-AzApplicationGatewayFirewallMatchVariable.md)
방화벽 조건에 대 한 일치 변수를 만듭니다.

### [새로운 AzApplicationGatewayFirewallPolicy](New-AzApplicationGatewayFirewallPolicy.md)
응용 프로그램 게이트웨이 방화벽 정책을 만듭니다.

### [새로운 AzApplicationGatewayFirewallPolicyExclusion](New-AzApplicationGatewayFirewallPolicyExclusion.md)
방화벽 정책에서 제외를 만듭니다.

### [새로운 AzApplicationGatewayFirewallPolicyManagedRule](New-AzApplicationGatewayFirewallPolicyManagedRule.md)
방화벽 정책에 대 한 ManagedRules를 만듭니다.

### [새로운 AzApplicationGatewayFirewallPolicyManagedRuleGroupOverride](New-AzApplicationGatewayFirewallPolicyManagedRuleGroupOverride.md)
방화벽 정책에 대 한 ManagedRuleSets 항목을 만듭니다.

### [새로운 AzApplicationGatewayFirewallPolicyManagedRuleOverride](New-AzApplicationGatewayFirewallPolicyManagedRuleOverride.md)
RuleGroupOverrideGroup entry에 대 한 managedRuleOverride 항목을 만듭니다.

### [새로운 AzApplicationGatewayFirewallPolicyManagedRuleSet](New-AzApplicationGatewayFirewallPolicyManagedRuleSet.md)
FirewallPolicy에 대 한 ManagedRuleSet 집합을 만듭니다.

### [새로운 AzApplicationGatewayFirewallPolicySetting](New-AzApplicationGatewayFirewallPolicySetting.md)
방화벽 정책에 대 한 정책 설정 만들기
...
### [새로운 AzApplicationGatewayIdentity](New-AzApplicationGatewayIdentity.md)
응용 프로그램 게이트웨이에 대 한 id 개체를 만듭니다. 이는 사용자가 할당 한 id에 대 한 참조를 유지 합니다.
...
### [새로운 AzApplicationGatewayProbeHealthResponseMatch](New-AzApplicationGatewayProbeHealthResponseMatch.md)
...
응용 프로그램 게이트웨이에 대 한 리디렉션 구성을 만듭니다.
...
Application gateway에 대 한 재작성 규칙을 만듭니다.

### [새로운 AzApplicationGatewayRewriteRuleActionSet](New-AzApplicationGatewayRewriteRuleActionSet.md)
응용 프로그램 게이트웨이에 대 한 재작성 규칙 동작 집합을 만듭니다.

### [새로운 AzApplicationGatewayRewriteRuleCondition](New-AzApplicationGatewayRewriteRuleCondition.md)
...
응용 프로그램 게이트웨이에 대 한 재작성 규칙 헤더 구성을 만듭니다.

### [새로운 AzApplicationGatewayRewriteRuleSet](New-AzApplicationGatewayRewriteRuleSet.md)
응용 프로그램 게이트웨이에 대 한 요청 라우팅 규칙을 만듭니다.

### [새로운 AzApplicationGatewayRewriteRuleUrlConfiguration](New-AzApplicationGatewayRewriteRuleUrlConfiguration.md)
...
응용 프로그램 게이트웨이에 대 한 SSL 정책을 만듭니다.
...
백 엔드 서버 풀에 대 한 URL 경로 매핑의 배열을 만듭니다.
...
### [새로운 AzBastion](New-AzBastion.md)
방호 리소스를 만듭니다.
...
### [새로운 AzExpressRouteCircuitPeeringConfig](New-AzExpressRouteCircuitPeeringConfig.md)
이 (가) Express 회로에 추가할 새 피어 링 구성을 만듭니다.

### [새로운 AzExpressRouteConnection](New-AzExpressRouteConnection.md)
이 (가) 구내 게이트웨이를 온-프레미스 Express 경로 회로에 연결 하는 Express 경로 연결을 만듭니다.

### [새로운 AzExpressRouteGateway](New-AzExpressRouteGateway.md)
확장 가능한 Express 가능 게이트웨이를 만듭니다.
...
Azure ExpressRoutePortIdentity를 만듭니다.
...
### [새로운 AzFirewallHubIpAddress](New-AzFirewallHubIpAddress.md)
가상 허브의 방화벽에 assoicated Ip 주소

### [새로운 AzFirewallHubPublicIpAddress](New-AzFirewallHubPublicIpAddress.md)
가상 허브의 방화벽에 대 한 공용 Ip assoicated

### [새로운 AzFirewallNatRule](New-AzFirewallNatRule.md)
방화벽 NAT 규칙을 만듭니다.

### [새로운 AzFirewallNatRuleCollection](New-AzFirewallNatRuleCollection.md)
방화벽 NAT 규칙의 컬렉션을 만듭니다.
...
### [새로운 AzFirewallPolicy](New-AzFirewallPolicy.md)
새 Azure 방화벽 정책을 만듭니다.

### [새로운 AzFirewallPolicyApplicationRule](New-AzFirewallPolicyApplicationRule.md)
새 Azure 방화벽 정책 응용 프로그램 규칙 만들기

### [새로운 AzFirewallPolicyFilterRuleCollection](New-AzFirewallPolicyFilterRuleCollection.md)
새 Azure 방화벽 정책 필터 규칙 모음 만들기

### [새로운 AzFirewallPolicyNatRuleCollection](New-AzFirewallPolicyNatRuleCollection.md)
새 Azure 방화벽 정책 Nat 규칙 모음 만들기

### [새로운 AzFirewallPolicyNetworkRule](New-AzFirewallPolicyNetworkRule.md)
새 Azure 방화벽 정책 네트워크 규칙 만들기

### [새로운 AzFirewallPolicyNetworkRule](New-AzFirewallPolicyNatRule.md)
새 Azure 방화벽 정책 NAT 규칙 만들기

### [새로운 AzFirewallPolicyRuleCollectionGroup](New-AzFirewallPolicyRuleCollectionGroup.md)
새 Azure 방화벽 정책 규칙 모음 그룹 만들기

### [새로운 AzFirewallPublicIpAddress](New-AzFirewallPublicIpAddress.md)
Azure 방화벽에서 다중 pip에 사용할 수 있는 Ip 주소의 자리 표시자입니다.

### [새로운 AzFirewallPolicyThreatIntelWhitelist](New-AzFirewallPolicyThreatIntelWhitelist.md)
Azure 방화벽 정책에 대 한 새 위협 인텔리전스 허용 목록 만들기

### [새로운 AzFirewallThreatIntelWhitelist](New-AzFirewallThreatIntelWhitelist.md)
Azure 방화벽에 대 한 새 위협 인텔리전스 허용 목록 만들기

### [새로운 AzIpAllocation](New-AzIpAllocation.md)
Azure IpAllocation을 만듭니다.

### [새로운 AzIpConfigurationBgpPeeringAddressObject](New-AzIpConfigurationBgpPeeringAddressObject.md)
새 IpconfigurationBgpPeeringAddressObject를 만듭니다.

### [새로운 AzIpGroup](New-AzIpGroup.md)
Azure IpGroup을 만듭니다.

### [새로운 AzIpsecPolicy](New-AzIpsecPolicy.md)
IPSec 정책을 만듭니다.

### [새로운 AzIpsecTrafficSelectorPolicy](New-AzIpsecTrafficSelectorPolicy.md)
트래픽 선택기 정책을 만듭니다.
...
부하 분산 장치에 대 한 인바운드 NAT 풀 구성을 만듭니다.
...
### [새로운 AzNatGateway](New-AzNatGateway.md)
공용 Ip 주소/공용 Ip 접두사, IdleTimeoutInMinutes 및 Sku 속성이 있는 새 Nat 게이트웨이 리소스를 만듭니다.
...
### [새로운 AzNetworkProfile](New-AzNetworkProfile.md)
새 네트워크 프로필을 만듭니다.
...
새 네트워크 감시자 리소스를 만듭니다.

### [새로운 AzNetworkWatcherConnectionMonitor](New-AzNetworkWatcherConnectionMonitor.md)
연결 모니터 리소스를 만듭니다.

### [새로운 AzNetworkWatcherConnectionMonitorEndpointFilterItemObject](New-AzNetworkWatcherConnectionMonitorEndpointFilterItemObject.md)
연결 모니터 끝점 필터 항목을 만듭니다.

### [새로운 AzNetworkWatcherConnectionMonitorEndpointObject](New-AzNetworkWatcherConnectionMonitorEndpointObject.md)
연결 모니터 끝점을 만듭니다.

### [새로운 AzNetworkWatcherConnectionMonitorObject](New-AzNetworkWatcherConnectionMonitorObject.md)
연결 모니터 V2 개체를 만듭니다.

### [새로운 AzNetworkWatcherConnectionMonitorOutputObject](New-AzNetworkWatcherConnectionMonitorOutputObject.md)
연결 모니터 출력 대상 개체를 만듭니다.

### [새로운 AzNetworkWatcherConnectionMonitorProtocolConfigurationObject](New-AzNetworkWatcherConnectionMonitorProtocolConfigurationObject.md)
TCP, HTTP 또는 ICMP를 통해 테스트 평가를 수행 하는 데 사용 되는 프로토콜 구성을 만듭니다.

### [새로운 AzNetworkWatcherConnectionMonitorTestConfigurationObject](New-AzNetworkWatcherConnectionMonitorTestConfigurationObject.md)
연결 모니터 테스트 구성을 만듭니다.

### [새로운 AzNetworkWatcherConnectionMonitorTestGroupObject](New-AzNetworkWatcherConnectionMonitorTestGroupObject.md)
연결 모니터 테스트 그룹을 만듭니다.

### [새로운 AzNetworkWatcherFlowLog](New-AzNetworkWatcherFlowLog.md)
지정 된 네트워크 보안 그룹에 대 한 흐름 로그 리소스를 만들거나 업데이트 합니다.

### [새로운 AzNetworkWatcherNetworkConfigurationDiagnosticProfile](New-AzNetworkWatcherNetworkConfigurationDiagnosticProfile.md)
...
### [새로운 AzPrivateDnsZoneConfig](New-AzPrivateDnsZoneConfig.md)
개인 dns 영역 그룹의 DNS 영역 구성을 만듭니다.

### [새로운 AzPrivateDnsZoneGroup](New-AzPrivateDnsZoneGroup.md)
지정 된 개인 끝점에 개인 DNS 영역 그룹을 만듭니다.
...
### [새로운 AzPrivateLinkServiceConnection](New-AzPrivateLinkServiceConnection.md)
개인 링크 서비스 연결 구성을 만듭니다.

### [새로운 AzPrivateLinkServiceIpConfig](New-AzPrivateLinkServiceIpConfig.md)
개인 링크 서비스 ip 구성을 만듭니다.

### [새로운 AzPublicIpAddress](New-AzPublicIpAddress.md)
공용 IP 주소를 만듭니다.

### [새로운 AzPublicIpPrefix](New-AzPublicIpPrefix.md)
공용 IP 접두사를 만듭니다.

### [새로운 AzPublicIpTag](New-AzPublicIpTag.md)
IP 태그를 만듭니다.

### [새로운 AzRadiusServer](New-AzRadiusServer.md)
외부 radius 서버 구성을 만듭니다.
...
### [새로운 AzSecurityPartnerProvider](New-AzSecurityPartnerProvider.md)
Azure SecurityPartnerProvider를 만듭니다.

### [새로운 AzServiceEndpointPolicy](New-AzServiceEndpointPolicy.md)
서비스 끝점 정책을 만듭니다.

### [새로운 AzServiceEndpointPolicyDefinition](New-AzServiceEndpointPolicyDefinition.md)
서비스 끝점 정책 정의를 만듭니다.

### [새로운 AzVirtualHub](New-AzVirtualHub.md)
Azure VirtualHub 리소스를 만듭니다.

### [새로운 AzVirtualHubRoute](New-AzVirtualHubRoute.md)
Azure 가상 허브 경로 개체를 만듭니다.

### [새로운 AzVirtualHubRouteTable](New-AzVirtualHubRouteTable.md)
Azure 가상 허브 경로 테이블 개체를 만듭니다.

### [새로운 AzVirtualHubVnetConnection](New-AzVirtualHubVnetConnection.md)
New-AzVirtualHubVnetConnection cmdlet은 가상 네트워크를 Azure 가상 허브에 피어 HubVirtualNetworkConnection 리소스를 만듭니다.
...
가상 네트워크 게이트웨이와 프레미스 VPN 장치 사이에 사이트 간 VPN 연결을 만듭니다.
...
### [새로운 AzVirtualRouter](New-AzVirtualRouter.md)
Azure VirtualRouter를 만듭니다.

### [새로운 AzVirtualWan](New-AzVirtualWan.md)
Azure 가상 WAN을 만듭니다.

### [새로운 AzVpnClientConfiguration](New-AzVpnClientConfiguration.md)
...
### [새로운 AzVpnClientIpsecPolicy](New-AzVpnClientIpsecPolicy.md)
...
VpnGateway를 RM에 VpnSite로 표시 되는 원격 고객 분기에 연결 하는 IPSec 연결을 만듭니다.

### [새로운 AzVpnGateway](New-AzVpnGateway.md)
확장 가능한 VPN 게이트웨이를 만듭니다.

### [새로운 AzVpnServerConfiguration](New-AzVpnServerConfiguration.md)
사이트 연결을 위한 새 VpnServerConfiguration를 만듭니다.
...
### [새로운 AzVpnSiteLink](New-AzVpnSiteLink.md)
Azure VpnSiteLink 개체를 만듭니다.

### [새로운 AzVpnSiteLinkConnection](New-AzVpnSiteLinkConnection.md)
Azure VpnSiteLinkConnection 개체를 만듭니다.

### [제거-AzApplicationGateway](Remove-AzApplicationGateway.md)
...
응용 프로그램 게이트웨이에서 자동 크기 조정 구성을 제거 합니다.
...
백 엔드 HTTP 설정 개체의 연결 드레이닝 구성을 제거 합니다.

### [제거-AzApplicationGatewayCustomError](Remove-AzApplicationGatewayCustomError.md)
응용 프로그램 게이트웨이에서 사용자 지정 오류를 제거 합니다.

### [제거-AzApplicationGatewayFirewallPolicy](Remove-AzApplicationGatewayFirewallPolicy.md)
응용 프로그램 게이트웨이 방화벽 정책을 제거 합니다.
...
### [제거-AzApplicationGatewayIdentity](Remove-AzApplicationGatewayIdentity.md)
응용 프로그램 게이트웨이에서 id를 제거 합니다.
...
Azure application gateway에서 SSL 인증서를 제거 합니다.

### [제거-AzApplicationGatewaySslPolicy](Remove-AzApplicationGatewaySslPolicy.md)
...
응용 프로그램 게이트웨이에서 신뢰할 수 있는 루트 인증서를 제거 합니다.
...
응용 프로그램 보안 그룹을 제거 합니다.

### [제거-AzBastion](Remove-AzBastion.md)
방호 리소스를 제거 합니다.

### [제거-AzDdosProtectionPlan](Remove-AzDdosProtectionPlan.md)
DDoS 보호 계획을 제거 합니다.
...
Express 경로 회로 연결 구성을 제거 합니다.

### [제거-AzExpressRouteCircuitPeeringConfig](Remove-AzExpressRouteCircuitPeeringConfig.md)
Express 경로 회로 피어 링 구성을 제거 합니다.

### [제거-AzExpressRouteConnection](Remove-AzExpressRouteConnection.md)
ExpressRouteConnection를 제거 합니다.

### [제거-AzExpressRouteCrossConnectionPeering](Remove-AzExpressRouteCrossConnectionPeering.md)
Express 경로 간 연결 피어 링 구성을 제거 합니다.

### [제거-AzExpressRouteGateway](Remove-AzExpressRouteGateway.md)
Remove-AzExpressRouteGateway cmdlet은 Azure Express 경로 게이트웨이를 제거 합니다. Azure 가상 WAN의 소프트웨어 정의 연결과 관련 된 게이트웨이입니다.

### [제거-AzExpressRoutePort](Remove-AzExpressRoutePort.md)
ExpressRoutePort를 제거 합니다.

### [제거-AzExpressRoutePortIdentity](Remove-AzExpressRoutePortIdentity.md)
ExpressRoutePort에서 id를 제거 합니다.

### [제거-AzFirewall](Remove-AzFirewall.md)
방화벽을 제거 합니다.

### [제거-AzFirewallPolicy](Remove-AzFirewallPolicy.md)
Azure 방화벽 정책 제거

### [제거-AzFirewallPolicyRuleCollectionGroup](Remove-AzFirewallPolicyRuleCollectionGroup.md)
Azure 방화벽 정책에서 Azure 방화벽 정책 규칙 모음 그룹을 제거 합니다.

### [제거-AzIpAllocation](Remove-AzIpAllocation.md)
Azure IpAllocation을 삭제 합니다.

### [제거-AzIpGroup](Remove-AzIpGroup.md)
...
부하 분산 장치에서 프런트 엔드 IP 구성을 제거 합니다.

### [제거-AzLoadBalancerInboundNatPoolConfig](Remove-AzLoadBalancerInboundNatPoolConfig.md)
...
부하 분산 장치에서 아웃 바운드 규칙 구성을 제거 합니다.
...
로컬 네트워크 게이트웨이를 삭제 합니다.

### [제거-AzNatGateway](Remove-AzNatGateway.md)
Nat 게이트웨이 리소스를 제거 합니다.
...
### [제거-AzNetworkProfile](Remove-AzNetworkProfile.md)
네트워크 프로필을 제거 합니다.
...
기존 P2SVpnGateway를 제거 합니다.

### [제거-AzPrivateDnsZoneGroup](Remove-AzPrivateDnsZoneGroup.md)
DNS 영역 그룹을 제거 합니다.
...
### [제거-AzPrivateLinkService](Remove-AzPrivateLinkService.md)
개인 링크 서비스 제거

### [제거-AzPublicIpAddress](Remove-AzPublicIpAddress.md)
공용 IP 주소를 제거 합니다.

### [제거-AzPublicIpPrefix](Remove-AzPublicIpPrefix.md)
공용 IP 접두사를 제거 합니다.
...
경로 필터에서 경로 필터 규칙을 제거 합니다.

### [제거-AzRouteTable](Remove-AzRouteTable.md)
경로 테이블을 제거 합니다.

### [제거-AzSecurityPartnerProvider](Remove-AzSecurityPartnerProvider.md)
Azure SecurityPartnerProvider를 삭제 합니다.

### [제거-AzServiceEndpointPolicy](Remove-AzServiceEndpointPolicy.md)
서비스 끝점 정책을 제거 합니다.

### [제거-AzServiceEndpointPolicyDefinition](Remove-AzServiceEndpointPolicyDefinition.md)
서비스 끝점 정책 정의를 제거 합니다.

### [제거-AzVirtualHub](Remove-AzVirtualHub.md)
Azure VirtualHub 리소스를 제거 합니다.

### [제거-AzVirtualHubRouteTable](Remove-AzVirtualHubRouteTable.md)
가상 허브에 연결 된 가상 허브 경로 테이블 리소스를 삭제 합니다.

### [제거-AzVirtualHubVnetConnection](Remove-AzVirtualHubVnetConnection.md)
Remove-AzVirtualHubVnetConnection cmdlet은 원격 VNET을 허브 VNET에 피어 하는 Azure 가상 네트워크 연결을 제거 합니다.

### [제거-AzVirtualNetwork](Remove-AzVirtualNetwork.md)
가상 네트워크를 제거 합니다.

### [제거-AzVirtualNetworkGateway](Remove-AzVirtualNetworkGateway.md)
가상 네트워크 게이트웨이를 삭제 합니다.

### [제거-AzVirtualNetworkGatewayConnection](Remove-AzVirtualNetworkGatewayConnection.md)
가상 네트워크 게이트웨이 연결을 삭제 합니다.

### [제거-AzVirtualNetworkGatewayDefaultSite](Remove-AzVirtualNetworkGatewayDefaultSite.md)
가상 네트워크 게이트웨이에서 기본 사이트를 제거 합니다.

### [제거-AzVirtualNetworkGatewayIpConfig](Remove-AzVirtualNetworkGatewayIpConfig.md)
가상 네트워크 게이트웨이에서 IP 구성을 제거 합니다.

### [제거-AzVirtualNetworkPeering](Remove-AzVirtualNetworkPeering.md)
가상 네트워크 피어 링을 제거 합니다.
...
### [제거-AzVirtualRouter](Remove-AzVirtualRouter.md)
Azure VirtualRouter를 삭제 합니다.

### [제거-AzVirtualRouterPeer](Remove-AzVirtualRouterPeer.md)
Azure VirtualRouter 피어를 제거 합니다.

### [제거-AzVirtualWan](Remove-AzVirtualWan.md)
Azure 가상 WAN을 제거 합니다.

### [제거-AzVpnClientIpsecParameter](Remove-AzVpnClientIpsecParameter.md)
가상 네트워크 게이트웨이 리소스에 설정 되어 있는 Vpn 사용자 지정 ipsec 정책을 제거 합니다.
...
Remove-AzVpnGateway cmdlet은 Azure VPN 게이트웨이를 제거 합니다. Azure 가상 WAN의 소프트웨어 정의 연결과 관련 된 게이트웨이입니다.

### [제거-AzVpnServerConfiguration](Remove-AzVpnServerConfiguration.md)
...
### [다시 설정-AzVirtualNetworkGatewayConnectionSharedKey](Reset-AzVirtualNetworkGatewayConnectionSharedKey.md)
가상 네트워크 게이트웨이 연결의 공유 키를 다시 설정 합니다.
...
응용 프로그램 게이트웨이의 백 엔드 주소 풀을 업데이트 합니다.
...
백 엔드 HTTP 설정 개체의 연결 드레이닝 구성을 수정 합니다.
...
응용 프로그램 게이트웨이 방화벽 정책을 업데이트 합니다.
...
응용 프로그램 게이트웨이의 http 수신기에서 사용자 지정 오류를 업데이트 합니다.

### [Set-AzApplicationGatewayIdentity](Set-AzApplicationGatewayIdentity.md)
응용 프로그램 게이트웨이에 할당 된 id를 업데이트 합니다.
...
Application gateway에 대 한 요청 라우팅 규칙을 수정 합니다.

### [Set-AzApplicationGatewayRewriteRuleSet](Set-AzApplicationGatewayRewriteRuleSet.md)
응용 프로그램 게이트웨이에 대 한 재작성 규칙 집합을 수정 합니다.
...
Application gateway의 신뢰할 수 있는 루트 인증서를 업데이트 합니다.

### [Set-AzApplicationGatewayUrlPathMapConfig](Set-AzApplicationGatewayUrlPathMapConfig.md)
백 엔드 서버 풀에 대 한 URL 경로 매핑 배열의 구성을 설정 합니다.
...
Express 경로 게이트웨이와 온-프레미스 express 경로 회로 피어 링 간에 생성 된 express 경로 연결을 업데이트 합니다.

### [Set-AzExpressRouteCrossConnection](Set-AzExpressRouteCrossConnection.md)
Express 경로 간 연결을 수정 합니다.
...
수정 된 azure 방화벽 정책 규칙 모음 그룹을 저장 합니다.

### [Set-AzIpAllocation](Set-AzIpAllocation.md)
수정 된 IpAllocation을 저장 합니다.
...
부하 분산 장치에 대 한 인바운드 NAT 풀 구성을 설정 합니다.
...
부하 분산 장치에 대 한 아웃 바운드 규칙 구성을 설정 합니다.
...
개인 링크 서비스에서 개인 끝점 연결 상태를 업데이트 합니다.
...
수정 된 Azure SecurityPartnerProvider를 저장 합니다.
...
### [시작-AzApplicationGateway](Start-AzApplicationGateway.md)
...
가상 네트워크 게이트웨이 연결에서 패킷 캡처 작업을 시작 합니다.

### [시작-AzVirtualnetworkGatewayPacketCapture](Start-AzVirtualnetworkGatewayPacketCapture.md)
가상 네트워크 게이트웨이에서 패킷 캡처 작업을 시작 합니다.
...
### [중지-AzVirtualNetworkGatewayConnectionPacketCapture](Stop-AzVirtualNetworkGatewayConnectionPacketCapture.md)
가상 네트워크 게이트웨이 연결의 패킷 캡처 작업을 중지 합니다.

### [중지-AzVirtualNetworkGatewayPacketCapture](Stop-AzVirtualNetworkGatewayPacketCapture.md)
가상 네트워크 게이트웨이에서 패킷 캡처 작업을 중지 합니다.

### [테스트-AzDnsAvailability](Test-AzDnsAvailability.md)
...
### [테스트-AzPrivateIPAddressAvailability](Test-AzPrivateIPAddressAvailability.md)
가상 네트워크에서 개인 IP 주소의 사용 가능성을 테스트 합니다.

### [테스트-AzPrivateLinkServiceVisibility](Test-AzPrivateLinkServiceVisibility.md)
**테스트 AzPrivateLinkServiceVisibility** 는 현재 사용에 대해 개인 링크 서비스를 표시할지 여부를 확인 합니다.

### [업데이트-AzP2sVpnGateway](Update-AzP2sVpnGateway.md)
사이트 연결에 대 한 VirtualHub의 기존 P2SVpnGateway를 업데이트 합니다.

### [업데이트-AzVirtualHub](Update-AzVirtualHub.md)
가상 허브를 업데이트 합니다.

### [업데이트-AzVirtualHubVnetConnection](Update-AzVirtualHubVnetConnection.md)
기존 HubVirtualNetworkConnection를 업데이트 합니다.

### [업데이트-AzVirtualRouterPeer](Update-AzVirtualRouterPeer.md)
Azure VirtualRouter 피어 업데이트

### [업데이트-AzVirtualWan](Update-AzVirtualWan.md)
Azure 가상 WAN을 업데이트 합니다.

### [업데이트-AzVpnConnection](Update-AzVpnConnection.md)
VPN 연결을 업데이트 합니다.

### [업데이트-AzVpnGateway](Update-AzVpnGateway.md)
확장 가능한 VPN 게이트웨이를 업데이트 합니다.

### [업데이트-AzVpnServerConfiguration](Update-AzVpnServerConfiguration.md)
기존 VpnServerConfiguration를 업데이트 합니다.
