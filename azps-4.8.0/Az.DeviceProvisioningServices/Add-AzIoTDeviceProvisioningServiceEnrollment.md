...
## 설명은
Azure IoT Hub 디바이스 프로비저닝 서비스에서 디바이스 등록을 만듭니다.
...
증명 형식 SymmetricKey를 사용 하 여 등록 만들기
...
증명 형식 SymmetricKey 및 초기 트윈 상태를 사용 하 여 등록을 만듭니다.
...
### -ApiVersion
사용자 지정 할당 요청에서 제공 하는 프로비저닝 서비스의 API 버전입니다.
...
여러 IoT Hub에 공백으로 구분 된 목록을 사용 합니다.
...
### -PrimaryCAName
주 루트 CA 인증서의 이름입니다.
루트 CA 인증서가 있는 증명을 원하는 경우 루트 ca 이름을 제공 해야 합니다.
...
### -PrimaryCertificate
기본 인증서를 포함 하는 파일에 대 한 경로입니다.
Base-64는 X509 인증서 .cer 파일 또는 pem 파일 경로의 표시입니다.
...
### -ReprovisionPolicy
다른 Iot Hub로 다시 프로 비전 할 때 처리 될 장치 데이터입니다.
...
### -RootCertificate
루트 인증서를 사용 하 여 X509attestation를 만들 수 있습니다.
