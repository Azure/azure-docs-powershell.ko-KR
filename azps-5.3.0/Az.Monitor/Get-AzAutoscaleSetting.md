...
## 설명
**Get-AzAutoscaleSetting** cmdlet은 리소스 그룹 또는 지정된 자동 크기 조정 설정과 연결된 모든 자동 크기 조정 설정을 얻습니다.
...
이 명령은 리소스 그룹에 할당된 자동 조정 설정을 Default-Web-EastUS로 이동합니다.

### 예제 2: 이름에 따라 자동 조정 설정 사용
```
...
이 명령은 DefaultServerFarm-Default-Web-EastUS라는 자동 조정 설정을 얻습니다.
...
### -DetailedOutput
이 작업이 출력에 전체 세부 정보를 표시하는지 나타냅니다.
