...
## 설명
**New-AzAutoscaleProfile** cmdlet은 자동 크기 조정 프로필을 만듭니다.

## 예제

### 예제 1: 고정된 날짜로 단일 프로필 만들기
```
...
첫 번째 명령은 요청이라는 자동 조정 규칙을 만든 다음, $Rule 변수에 저장합니다.
두 번째 명령은 2010에서 규칙을 사용하여 고정된 날짜로 Profile01이라는 프로필을 $Rule.

### 예제 2: 일정을 사용하여 프로필 만들기
```
PS C:\>$Rule = New-AzAutoscaleRule -MetricName "Requests" -MetricResourceId "/subscriptions/b93fb07a-6f93-30be-bf3e-4f0deca15f4f/resourceGroups/Default-Web-EastUS/providers/microsoft.web/sites/mywebsite" -Operator GreaterThan -MetricStatistic Average -Threshold 10 -TimeGrain 00:01:00 -ScaleActionCooldown 00:05:00 -ScaleActionDirection Increase -ScaleActionScaleType ChangeCount -ScaleActionValue "1" 
...
첫 번째 명령은 요청이라는 자동 조정 규칙을 만든 다음, $Rule 변수에 저장합니다.
두 번째 명령은 다음 명령의 규칙을 사용하여 두 번째 일정으로 SecondProfileName이라는 프로필을 $Rule.

### 예제 3: 두 가지 규칙으로 프로필 만들기
```
...
처음 두 명령은 규칙을 만들고 각각 $Rule 1 및 $Rule 2 변수에 저장합니다.
세 번째 명령은 Rule1 및 Rule2의 규칙을 사용하여 ProfileName이라는 프로필을 만든 다음 $Profile 1 변수에 저장합니다.
마지막 명령은 Rule1 및 Rule2의 규칙을 사용하여 SecondProfileName이라는 프로필을 만든 다음 $Profile 2 변수에 저장합니다.

### 예제 4: 일정 또는 고정 날짜가 없는 프로필 만들기
```
PS C:\>$Rule = New-AzAutoscaleRule -MetricName "Requests" -MetricResourceId "/subscriptions/b93fb07a-6f93-30be-bf3e-4f0deca15f4f/resourceGroups/Default-Web-EastUS/providers/microsoft.web/sites/mywebsite" -Operator GreaterThan -MetricStatistic Average -Threshold 10 -TimeGrain 00:01:00 -ScaleActionCooldown 00:05:00 -ScaleActionDirection Increase -ScaleActionScaleType ChangeCount -ScaleActionValue "1" 
...
첫 번째 명령은 요청이라는 자동 조정 규칙을 만든 다음, $Rule 변수에 저장합니다.
두 번째 명령은 일정이나 고정 날짜 없이 프로필을 만든 다음, $Profile 변수에 저장합니다.
