...
## 설명
**New-AzBatchPool** cmdlet은 *BatchContext* 매개 변수로 지정된 계정의 Azure Batch 서비스에 풀을 만듭니다.
...
이 명령은 TargetDedicated 매개 변수 집합을 사용하여 ID MyPool로 새 풀을 만듭니다.
대상 할당은 3개의 계산 노드입니다.
풀은 Windows-2016-STANDARD_D1_V2 운영 체제 이미지와 함께 가상 머신을 사용하도록 구성됩니다.
...
이 명령은 자동 조정 매개 변수 집합을 사용하여 ID AutoScalePool로 새 풀을 만듭니다.
풀은 Windows-2016-STANDARD_D1_V2 운영 체제 이미지와 함께 가상 머신을 사용하도록 구성하고 계산 노드의 대상 수는 자동 확장 수식에 의해 결정됩니다.
...
풀 크기가 자동 크기 조정 수식에 따라 자동으로 조정되기 전에 경과되는 시간(분)을 지정합니다.
기본값은 15분, 최소값은 5분입니다.
...
이 cmdlet이 전용 계산 노드 간의 직접 통신을 위해 풀을 설정하는지 나타냅니다.
...
단일 계산 노드에서 실행할 수 있는 최대 태스크 수를 지정합니다.
...
### -MountConfiguration
풀의 각 노드에 탑재할 파일 시스템 목록입니다. Azure Files, NFS, CIFS/SMB 및 Blobfuse를 지원 합니다.
...
### -ResizeTimeout
풀에 계산 노드를 할당하기 위한 시간 시간을 지정합니다.
...
시작 태스크는 계산 노드가 풀에 조인하거나 계산 노드가 재부팅되거나 다시이미지될 때 실행됩니다.
...
ComputeNodeFillType과 같은 태스크 스컬링 정책을 지정합니다.
...
가상 머신 인프라에서 풀에 대한 구성 설정을 지정합니다.
...
### -VirtualMachineSize
풀에 있는 가상 머신의 크기를 지정합니다.
가상 머신 크기에 대한 자세한 내용은 가상 머신의 크기(Microsoft Azure 사이트에서)를 https://azure.microsoft.com/en-us/documentation/articles/virtual-machines-size-specs/ https://azure.microsoft.com/en-us/documentation/articles/virtual-machines-size-specs/) 참조하세요.
