...
## 설명
**Set-AzBatchComputeNodeUser** cmdlet은 Azure Batch 계산 노드에서 사용자 계정의 속성을 수정합니다.
...
이 명령은 ContosoPool이라는 풀에 지정된 ID가 있는 계산 노드에서 PFuller라는 사용자 계정을 수정합니다.
이 명령은 계정 암호 및 만료 시간을 변경합니다.
...
### -ComputeNodeId
이 cmdlet이 작동하는 계산 노드의 ID를 지정합니다.
...
### -Name
이 cmdlet이 수정하는 사용자 계정의 이름을 지정합니다.
...
### -PoolId
이 cmdlet이 작동하는 계산 노드를 포함하는 풀의 ID를 지정합니다.
