...
**New-AzServiceFabricCluster** 명령은 사용자가 제공하거나 시스템에서 자체 서명된 인증서를 생성한 인증서를 사용하여 새 Service Fabric 클러스터를 설정합니다. 사용되는 템플릿은 기본 템플릿 또는 사용자가 제공하는 사용자 지정 템플릿일 수 있습니다. 자체 서명된 인증서를 내보낼 폴더를 지정하거나 나중에 키 자격 증명 모음에서 해당 인증서를 인출할 수 있습니다.
사용자 지정 템플릿 및 매개 변수 파일을 지정하는 경우 매개 변수 파일에 인증서 정보를 제공할 필요가 없습니다. 시스템에서 이러한 매개 변수를 채우게 됩니다.
네 가지 옵션은 아래에 자세히 설명됩니다. 각 매개 변수에 대한 설명을 위해 아래로 스크롤합니다.

## 예제

### 예제 1: 클러스터를 배포할 클러스터 크기, 인증 주체 이름 및 OS만 지정합니다.
...
이 명령은 클러스터를 배포할 클러스터 크기, 인증 주체 이름 및 OS만 지정합니다.

### 예제 2: 키 자격 증명 모음의 기존 인증서 리소스 및 클러스터를 배포할 사용자 지정 템플릿 지정
...
### 예제 3: 사용자 지정 템플릿을 사용하여 새 클러스터 만들기 키 자격 증명 모음에 다른 리소스 그룹 이름을 지정하고 시스템에 새 인증서를 업로드하게 합니다.
...
이 명령은 사용자 지정 템플릿을 사용하여 새 클러스터를 만듭니다. 키 자격 증명 모음에 다른 리소스 그룹 이름을 지정하고 시스템에 새 인증서를 업로드하게 합니다.
...
Azure Key Vault 이름이 지정되지 않은 경우 리소스 그룹 이름으로 기본값이 지정됩니다.
...
Azure Key Vault 리소스 그룹 이름이 지정되지 않은 경우 리소스 그룹 이름으로 기본값이 지정됩니다.
...
클러스터의 이름을 지정합니다. 지정하지 않으면 리소스 그룹 이름과 동일합니다.
...
### -Thumbprint
SecretIdentifier에 대한 인증서 correspoinding에 대한 지문입니다. 키 자격 증명 모음에 인증서만 비밀로 저장되고 cmdlet이 지문을 검색할 수 없는 경우 인증서가 관리되지 않는 경우 이 방법을 사용 합니다.
