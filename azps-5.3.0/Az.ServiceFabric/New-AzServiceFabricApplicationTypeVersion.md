...
## 설명
이 cmdlet은 -PackageUrl에 지정된 패키지를 사용하여 새 애플리케이션 유형 버전을 만듭니다. 배포 중에 사용할 Azure Resource Manager의 REST 엔드포인트를 통해 액세스할 수 있으며 확장 .sfpkg로 패키지 및 압축된 애플리케이션을 포함해야 합니다. 이 명령은 애플리케이션 유형이 아직 없는 경우 생성합니다.
...
이 예제에서는 "testAppType" 형식 아래에 애플리케이션 유형 버전 "v1"을 생성합니다. 패키지에 포함된 애플리케이션 매니페스트의 버전은 -Version에 지정된 버전과 동일한 버전을 포함해야 합니다.
