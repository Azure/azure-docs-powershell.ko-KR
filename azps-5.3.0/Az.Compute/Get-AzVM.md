...
## 설명
**Get-AzVM** cmdlet은 Azure 가상 머신의 모델 보기 또는 인스턴스 보기를 얻습니다.
모델 보기는 가상 머신의 사용자 지정 속성입니다.
인스턴스 보기는 가상 머신의 인스턴스 수준 상태입니다.
상태 매개 *변수를* 지정하여 기본값인 모델 보기 대신 가상 머신의 인스턴스 보기를 얻습니다.

## 예제

### 예제 1: 모델 및 인스턴스 보기 속성 얻기
```
...
이 명령은 VirtualMachine07이라는 가상 머신의 모델 보기 및 인스턴스 보기 속성을 얻습니다.
...
이 명령은 VirtualMachine07이라는 가상 머신의 속성을 얻습니다.
이 명령은 상태 매개 *변수를 지정합니다.*
따라서 이 명령은 인스턴스 보기 속성만 얻습니다.

### 예제 3: 리소스 그룹의 모든 가상 머신에 대한 속성 얻기
```
...
이 명령은 ResourceGroup11이라는 리소스 그룹의 모든 가상 머신에 대한 속성을 얻습니다.

### 예제 4: 구독의 모든 가상 머신을 얻습니다.
```
PS C:\> Get-AzVM
...
이 명령은 구독의 모든 가상 머신을 얻습니다.

### 예제 5: 위치에 있는 모든 가상 머신을 얻습니다.
```
...
이 명령은 미국 서부 지역의 모든 가상 머신을 얻습니다.

### 예제 6: 필터링을 사용하여 모든 가상 머신을 얻습니다.
```
...
이 명령은 "test"로 시작하는 구독의 모든 가상 머신을 얻습니다.
...
### -DisplayHint
가상 머신 개체가 표시되는 방법을 결정 합니다.
유효한 값은 -- 압축: 최상위 속성만 표시 -- 확장: 모든 수준의 모든 속성을 표시
