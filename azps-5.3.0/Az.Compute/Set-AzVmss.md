...
이 cmdlet에서 지원하는 유일한 작업은 다시Image입니다.
...
이 명령은 ContosoGroup이라는 리소스 그룹에 속한 ContosoVMSS라는 VMSS를 다시 이식합니다.

### 예제 2

지정된 VMSS에 대한 특정 작업을 설정합니다. (자동Generated)
...
### -PerformMaintenance
이 cmdlet이 VMSS에서 하나 이상의 가상 머신 유지 관리 수행을 나타냅니다.
...
### -재배포
cmdlet이 VMSS에서 하나 이상의 가상 머신을 다시 재배포하는지 나타냅니다.
...
### -VMScaleSetName
이 cmdlet이 작업을 설정하는 VMSS의 이름 종류입니다.
