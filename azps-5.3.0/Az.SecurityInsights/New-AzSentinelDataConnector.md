...
만들 경고 규칙의 종류를 지정하려면 매개 변수 중 하나(예: -AzureActiveDirectory)를 지정해야 합니다.  각 종류에는 서로 다른 필수 패러마스터가 있습니다.
*Confirm* 매개 변수 및 $ConfirmPreference Windows PowerShell cmdlet에서 확인을 요청하는지 여부를 제어할 수 있습니다.
참고: 포털에서 사용할 수 있는 일부 데이터 커넥터는 API를 통해 사용할 수 없습니다.
...
이 예제에서는 지정된 작업 영역의 Azure Security Center용 **DataConnector를** 만든 다음 $DataConnector 변수에 저장합니다.
...
이 예제에서는 지정된 작업 영역의 Microsoft Cloud App Security용 **DataConnector를** 만든 다음 $DataConnector 변수에 저장합니다.
...
### -AmazonWebServicesCloudTrail
Data Connector Amazon 웹 서비스 클라우드 내보
...
### -MicrosoftCloudAppSecurity
Data Connector Microsoft 클라우드 앱 보안
