...
만들 경고 규칙의 종류를 지정하려면 *Fusion,* *Scheduled* 또는 *MicrosoftSecurityIncidentCreation의* 세 가지 매개 변수 중 하나를 지정해야 합니다.  각 종류에는 서로 다른 필수 패러마스터가 있습니다.
...
이 예제에서는 고급 *Multistage* 공격 탐지용 템플릿을 기반으로 Fusion 종류의 **AlertRule을** 만든 다음, *Fusion* 변수에 $AlertRule 저장합니다.
...
이 예제에서는 *IoT용 Azure Security Center* 경고를 기반으로 인시던트 만들기 템플릿을 기반으로 *MicrosoftSecurityIncidentCreation* 종류의 **AlertRule을** 만든 다음, 새 경고에 $AlertRule 저장합니다.
