...
이 명령은 MyCache라는 Redis Enterprise Cache에 대한 데이터베이스의 클라이언트 프로토콜을 업데이트합니다.

### 예제 2: 클라이언트 프로토콜 및 지우기 정책 업데이트
```powershell
...
이 명령은 MyCache라는 Redis Enterprise Cache에 대한 데이터베이스의 클라이언트 프로토콜 및 삭제 정책을 업데이트합니다.
...
### -EvictionPolicy
Redis eviction policy - 기본값: VolatileLRU
