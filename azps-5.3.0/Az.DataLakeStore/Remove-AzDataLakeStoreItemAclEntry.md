...
## 설명
**Remove-AzDataLakeStoreItemAclEntry** cmdlet은 Data Lake Store의 파일 또는 폴더의 ACL(액세스 제어 목록)에서 ACE(항목)를 제거합니다.
...
### 예제 3: Acl 개체를 사용하여 재발적으로 ACE에 대한 사용 권한 제거
```
...
이 명령은 루트에서 Patti Fuller에 대한 사용자 ACE를 제거하고 ContosoADL 계정에 대한 모든 하위디렉터 및 파일에서 재발적으로 제거합니다.
...
### -Recurse
자식 하위 간접 및 파일에 재발적으로 제거할 ACL을 나타냅니다.
...
### -ShowProgress
전달된 경우 진행 상태가 표시됩니다. 재발 Acl 제거가 완료된 경우만 해당됩니다.
