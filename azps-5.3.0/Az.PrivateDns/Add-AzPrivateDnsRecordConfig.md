...
## 설명
Add-AzPrivateDnsRecordConfig cmdlet은 RecordSet 개체에 DNS(개인 도메인 이름 시스템) 레코드를 추가합니다. RecordSet 개체는 오프라인 개체로, Microsoft Azure 개인 DNS 서비스에 대한 변경 내용을 유지하기 위해 Set-AzPrivateDnsRecordSet cmdlet을 실행한 후에만 개인 DNS 응답을 변경하지 않습니다. SOA 레코드는 사설 DNS 영역이 만들어지며 사설 DNS 영역이 삭제될 때 제거됩니다. SOA 레코드를 추가하거나 제거할 수는 없지만 편집할 수 있습니다. RecordSet 개체를 매개 변수로 또는 파이프라인 연산자를 사용하여 이 cmdlet에 전달할 수 있습니다.
