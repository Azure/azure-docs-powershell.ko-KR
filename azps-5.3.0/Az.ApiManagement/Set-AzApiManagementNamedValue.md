...
## 설명
**Set-AzApiManagementNamedValue** cmdlet은 Azure API Management 명명된 값을 수정합니다.
...
첫 번째 명령은 변수 변수에 두 $Tags 할당합니다.
두 번째 명령은 ID Property11이 있는 명명된 값을 수정합니다.
이 명령은 명명된 값에 대한 태그로 $Tags 문자열을 할당합니다.

### 예제 2: 비밀 값을 하도록 명명된 값 수정
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
API Management 명명된 값을 수정합니다. (자동Generated)
...
명명된 값의 이름입니다.
최대 길이는 100자입니다.
문자, 숫자, 마주, 대시 및 밑면 문자만 포함할 수 있습니다.
이 매개 변수는 선택 사항입니다.
...
### -PassThru
지정된 경우 수정된 속성을 나타내는 Microsoft.Azure.Commands.ApiManagement.ServiceManagement.Models.PsApiManagementProperty 형식의 인스턴스가 출력에 기록됩니다.
...
### -Secret
명명된 값이 비밀인지와 해당 값을 암호화해야 하는지 여부입니다.
이 매개 변수는 선택 사항입니다.
