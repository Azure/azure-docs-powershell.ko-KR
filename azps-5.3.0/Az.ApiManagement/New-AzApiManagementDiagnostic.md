...
## 설명
**New-AzApiManagementDiagnostic** cmdlet은 전역 범위 또는 특정 API 범위에서 진단 엔터티를 만듭니다.
...
위의 예제에서는 헤더 및 100 Bytes의 본문을 로거에 기록하는 API에 `httpbin` 대한 `azuremonitor` 진단을 작성합니다.

## PARAMETERS

### -AlwaysLog
적용할 수 없는 메시지 샘플링 설정 유형을 지정합니다.
이 매개 변수는 선택 사항입니다.
