...
## 설명
**Remove-AzApiManagementPolicy** cmdlet은 지정된 범위에서 API Management 정책을 제거합니다.
...
이 명령은 API Management에서 테넌트 수준 정책을 제거합니다.
...
이 명령은 API Management에서 제품 범위 정책을 제거합니다.
...
이 명령은 API Management에서 API 범위 정책을 제거합니다.
...
이 명령은 API Management에서 작업 범위 정책을 제거합니다.

## PARAMETERS

### -ApiId
기존 API의 식별자를 지정합니다.
이 매개 변수를 지정하면 cmdlet에서 API 범위 정책을 제거합니다.
...
### -OperationId
기존 작업의 식별자를 지정합니다.
*ApiId* 매개 변수를 사용하여 이 매개 변수를 지정하면 이 cmdlet은 작업 범위 정책을 제거합니다.
...
### -PassThru
이 cmdlet이 성공하는 경우 $True 값이 반환되거나, 그렇지 않으면 $False 반환합니다.
...
### -ProductId
기존 제품의 식별자를 지정합니다.
이 매개 변수를 지정하면 cmdlet에서 제품 범위 정책을 제거합니다.
