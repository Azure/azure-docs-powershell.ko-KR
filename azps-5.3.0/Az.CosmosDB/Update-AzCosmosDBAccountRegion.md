...
## 설명
CosmosDB 계정의 지역을 업데이트합니다. 위치는 PSLocation 형식의 개체 또는 장애 조치 우선 순위에 따라 순서가 지정되는 위치 이름 문자열로 제공될 수 있습니다.
LocationObject 매개 변수는 추가할 새 위치에 해당하는 새 LocationObjects가 추가한 현재 위치(장애 조치 우선 순위 포함) 목록을 예상합니다.
위치 매개 변수에는 현재 위치 목록(장애 조치 우선 순위에 따라 순서가 지정) 및 새 위치가 예상됩니다. 지역 추가만 지원됩니다. Location 또는 LocationObject를 제공하세요.
