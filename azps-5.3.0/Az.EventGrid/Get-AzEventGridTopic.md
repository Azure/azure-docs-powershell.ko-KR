...
토픽 이름이 제공되지 않은 경우 토픽 목록이 반환됩니다. 이 목록에 반환되는 요소 수는 Top 매개 변수에 의해 제어됩니다. 상위 값이 지정되지 않은 경우 또는 $null 목록에 모든 토픽 항목이 포함되어 있습니다. 그렇지 않으면 Top은 목록에 반환할 최대 요소 수를 나타냅니다.
더 많은 토픽을 계속 사용할 수 있는 경우 다음 호출에서 NextLink의 값을 사용하여 항목의 다음 페이지를 얻을 수 있습니다.
