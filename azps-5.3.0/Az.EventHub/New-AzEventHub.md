...
캡처 설명 속성을 사용하여 Eventhub를 만들 경우 아래 단계(예제 2)를 따르세요. 
...
리소스 그룹 \` \` \` \` \` MyResourceGroupName을 통해 3일의 메시지 보존 기간과 2개의 파티션이 있는 MyEventHubName이라는 이벤트 허브를 WestUS 위치에 \` 만듭니다.

### 예제 2: Eventhub를 'CaptureDescription'으로 업데이트
```powershell
...
리소스 그룹 \` \` \` \` \` MyResourceGroupName을 사용하여 3일의 메시지 보존 기간, 2개 파티션 및 CaptureDescription 속성을 사용하여 MyEventHubName이라는 이벤트 허브를 \` 만듭니다.
