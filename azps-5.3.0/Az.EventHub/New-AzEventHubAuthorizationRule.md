...
## 설명
New-AzEventHubAuthorizationRule cmdlet은 새 Event Hubs 권한 부여 규칙을 만듭니다.
...
리소스 그룹 MyResourceGroupName을 통해 수신 및 보내기 권한을 네임스페이스 \` \` \` MyNamespaceName에 부여하는 권한 부여 \` \` 규칙 MyAuthRuleName을 \` 만듭니다.
...
리소스 그룹 \` \` \` \` \` \` MyResourceGroupName을 통해 MyNamespaceName 네임스페이스의 이벤트 허브 MyEventHubName에 수신 및 보내기 권한을 부여하는 권한 부여 \` 규칙 MyAuthRuleName을 \` 만듭니다.
