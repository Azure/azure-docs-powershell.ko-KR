...
## 설명
이 Remove-AzEventHub cmdlet은 지정된 네임스페이스에서 지정된 이벤트 허브를 제거하고 삭제합니다.
...
Event Hub \` MyEventHubName을 \` 제거합니다.
