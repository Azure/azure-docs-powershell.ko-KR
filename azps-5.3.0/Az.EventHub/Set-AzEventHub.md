...
## 설명
이 Set-AzEventHub cmdlet은 지정된 이벤트 허브의 속성을 업데이트합니다.

## 예제

### 예제 1
캡처 설명 속성으로 Eventhub를 업데이트하기 위해 아래 단계를 따르세요. 
...
MyCreatedEventHub 개체로 표현되는 Event Hub \` MyEventHubName을 업데이트하여 메시지 보존 \` \` 기간을 4일로, 파티션 수를 \` 2로, CaptureDescription 속성으로 설정
...
MyCreatedEventHub 개체로 표현되는 Event Hub MyEventHubName을 업데이트하여 메시지 보존 기간을 4일로 설정하고 파티션 수를 \` \` \` \` 2로 설정합니다.
