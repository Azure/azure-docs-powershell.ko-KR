...
## 설명
아티팩트를 업데이트합니다. 아티팩트를 업데이트하는 두 가지 방법은 아티팩트 JSON을 입력 파일로 또는 아티팩트에 대한 인라인 매개 변수를 제공하는 것입니다. JSON 메서드는 인라인 매개 변수 메서드를 제공해야 하는 아티팩트 형식이 필요하지는 않은 반면 사용자는 -Type 매개 변수를 통해 아티팩트의 형식을 제공해야 합니다.
...
아티팩트 JSON 파일을 통해 아티팩트를 업데이트합니다.
