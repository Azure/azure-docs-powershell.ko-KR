...
이 명령은 Databricks 작업 영역이 생성됩니다.

### 예제 2: 사용자 지정된 가상 네트워크를 사용하여 Databricks 작업 영역 만들기
```powershell
...
이 명령은 리소스 그룹에 사용자 지정된 가상 네트워크를 사용하여 Databricks 작업 영역이 생성됩니다.

### 예제 3: 암호화를 사용하도록 설정하여 Databricks 작업 영역 만들기
```powershell
...
이 명령은 Databricks 작업 영역이 만들어지며 암호화 준비를 위해 설정됩니다.
암호화에 대한 자세한 설정은 Update-AzDatabricksWorkspace 예제를 참조하세요.
...
### -PrivateSubnetName
Virtual Network 내의 개인 서브넷의 이름입니다.
...
### -PublicSubnetName
Virtual Network 내의 공용 서브넷의 이름입니다.
...
DBFS 루트 파일 시스템을 미사용 데이터에 대한 플랫폼 관리 키를 사용하여 보조 암호화 계층으로 사용할지 여부를 나타내는 부울입니다.
...
### -VirtualNetworkId
이 Databricks 클러스터를 만들어야 하는 Virtual Network의 ID입니다.
