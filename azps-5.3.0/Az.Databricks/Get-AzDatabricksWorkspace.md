...
이 명령은 리소스 그룹에 Databricks 작업 영역이 있습니다.

### 예제 2: 구독의 모든 Databricks 작업 영역 나열
```powershell
...
이 명령은 구독의 모든 Databricks 작업 영역이 나열됩니다.
...
이 명령은 리소스 그룹의 모든 Databricks 작업 영역이 나열됩니다.
...
INPUTOBJECT: <IDatabricksIdentity> ID 매개 변수
  - `[Id <String>]`: 리소스 ID 경로
