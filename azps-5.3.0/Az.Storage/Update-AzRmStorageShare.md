...
## 설명
**New-AzRmStorageShare** cmdlet은 Storage 파일 공유를 수정합니다.

## 예제

### 예제 1: Storage 파일 공유의 메타데이터를 수정하고 저장소 계정 이름 및 공유 이름으로 할당량 공유
```
...
이 명령은 Storage 파일 공유의 메타데이터를 수정하고 할당량과 Storage 계정 이름 및 공유 이름을 공유하고 반환된 파일 공유 개체로 수정 결과를 보여 주며,

### 예제 2: Storage 계정 개체 및 공유 이름을 사용하여 Storage 파일 공유의 메타데이터 수정
```
PS C:\>$accountObject = Get-AzStorageAccount -ResourceGroupName "myResourceGroup" -StorageAccountName "myStorageAccount"
PS C:\>$share = Update-AzRmStorageShare -StorageAccount $accountObject -Name "myshare" -Metadata @{tag0="value0";tag1="value1"}
```

이 명령은 Storage 계정 개체 및 공유 이름을 사용하여 Storage 파일 공유의 메타데이터를 수정합니다.

### 예제 3: 파이프라인이 있는 Storage 계정의 모든 Storage 파일 공유에 대한 공유 할당량 수정
```
...
이 명령은 파이프라인이 있는 Storage 계정의 모든 Storage 파일 공유에 대해 공유 할당량(5000 GiB)을 수정합니다.

### 예제 4: Accesstier를 쿨로 설정하여 Storage 파일 공유 수정
```
...
이 명령은 Accesstier를 쿨로 사용하여 Storage 파일 공유를 수정합니다.
