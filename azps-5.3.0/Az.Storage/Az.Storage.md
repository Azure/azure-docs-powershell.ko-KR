...
# Az.Storage 모듈
## 설명
이 항목에서는 Azure Storage 관리 Cmdlet에 대한 도움말 항목을 제공합니다.

## Az.Storage Cmdlet
### [Add-AzRmStorageContainerLegalHold](Add-AzRmStorageContainerLegalHold.md)
Storage Blob 컨테이너에 법적 보관 태그 추가

### [Add-AzStorageAccountManagementPolicyAction](Add-AzStorageAccountManagementPolicyAction.md)
입력 ManagementPolicy 작업 그룹 개체에 작업을 추가하거나 작업으로 ManagementPolicy 작업 그룹 개체를 만듭니다. 이 개체는 New-AzStorageAccountManagementPolicyRule에서 사용할 수 있습니다.
...
Azure Storage 계정에 대해 정적 웹 사이트를 사용하도록 설정
...
파일 시스템으로 파일 또는 디렉터리의 세부 정보를 얻습니다.
...
Storage 계정의 NetWorkRule 속성을 얻게 됩니다.
...
Blob의 콘텐츠에 구조적 쿼리 언어(SQL) 명령문을 적용하고 데이터의 검색된 하위 집합만 로컬 파일에 저장합니다.
...
Azure 저장소 컨테이너에 대한 저장된 액세스 정책 또는 정책을 얻습니다.

### [Get-AzStorageCORSRule](Get-AzStorageCORSRule.md)
Storage 서비스 유형에 대한 CORS 규칙을 얻습니다.
...
파일 또는 디렉터리를 동일한 Storage 계정의 다른 파일 또는 디렉터리로 이동
...
Azure 저장소 큐에 대해 저장된 액세스 정책을 만듭니다.
...
Storage Blob 컨테이너에서 법적 보류 태그를 제거합니다.
...
Azure 저장소 테이블에서 저장된 액세스 정책을 제거합니다.
...
지정된 경로에서 ACL을 재발적으로 설정합니다. 

### [Set-AzDataLakeGen2ItemAclObject](Set-AzDataLakeGen2ItemAclObject.md)
DataLake gen2 항목 ACL 개체를 만들고 업데이트합니다. 이 개체는 Update-AzDataLakeGen2Item cmdlet에 사용할 수 있습니다.
...
Azure 저장소 컨테이너에 대해 저장된 액세스 정책을 설정합니다.
...
Azure 저장소 큐에 대해 저장된 액세스 정책을 설정합니다.
...
페이지 Blob 스냅숏에서 지정된 대상 페이지 Blob으로 증분 복사 작업을 시작하세요.
