...
## 설명
**Stop-AzDataFactoryV2IntegrationRuntime** cmdlet은 관리되는 전용 통합 런타임이 "시작" 상태로 중지됩니다. 이 상태는 Start-AzDataFactoryV2IntegrationRuntime cmdlet에서 시작됩니다. 리소스가 릴리스된 후 상태가 '중지됨'으로 전송됩니다.

## 예제

### 예제 1: '시작' 상태인 관리되는 통합 런타임 중지
```
...
관리되는 통합 런타임 'test-reserlved-ir'은 '시작' 상태입니다. cmdlet을 Stop-AzDataFactoryV2IntegrationRuntime 실행하면 리소스가 릴리스되고 상태가 '중지됨'으로 전송됩니다.
