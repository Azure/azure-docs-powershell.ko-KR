...
## SYNOPSIS
장애 조치(failover) 및 테스트 장애 조치(failover) 관련 구성 세부 정보를 포함하는 ASR NIC 구성을 만듭니다.
...
## 설명
**New-AzRecoveryServicesAsrVMNicConfig** cmdlet은 장애 조치(failover) 및 테스트 장애 조치 관련 세부 정보를 포함하는 ASR NIC 구성 개체를 만듭니다. 정보가 전달되지 않은 경우 이러한 값이 null로 업데이트되지 않도록 복제 보호 항목에서 해당 값을 선택해야 합니다.
...
NIC에 대해 구성된 장애 조치(failover) 및 테스트 페이오버 네트워킹 설정을 사용하여 ASRVmNicConfig 개체를 만듭니다. 위에서 전달되지 않은 속성은 전달된 보호된 항목에서 페치됩니다.
...
NIC 이름 변경에 대해 구성된 테스트 페이오버 네트워킹 설정을 사용하여 ASRVmNicConfig 개체를 만듭니다. 위에서 전달되지 않은 속성은 전달된 보호된 항목에서 페치됩니다.


### 예제 3

장애 조치(failover) 및 테스트 장애 조치(failover) 관련 구성 세부 정보를 포함하는 ASR NIC 구성을 만듭니다. (자동Generated)
...
복구 NIC에서 가속 네트워킹을 사용할지 여부를 지정합니다.
...
테스트 장애 조치(failover) NIC에서 가속화된 네트워킹을 사용할지 여부를 지정합니다.
...
복구 NIC와 연결된 공용 IP 주소의 ID를 지정합니다.
...
### -ReuseExistingNic
장애 조치(failover) 중에 기존 NIC를 사용할 수 있는지 여부를 지정합니다.
...
테스트 장애 조치(failover) NIC와 연결된 NSG의 ID를 지정합니다.
...
### -TfoNicName
테스트 장애 조치(failover) NIC의 이름을 지정합니다.
...
테스트 장애 조치(failover) NIC 리소스 그룹의 이름을 지정합니다.
...
테스트 장애 조치(failover) NIC의 IP 주소를 지정합니다.
...
테스트 장애 조치(failover) NIC와 연결된 공용 IP 주소의 ID를 지정합니다.
...
### -TfoReuseExistingNic
테스트 장애 조치(failover) 중에 기존 NIC를 사용할 수 있는지 여부를 지정합니다.
...
### -TfoVMNetworkId
테스트 장애 조치(failover) 가상 네트워크의 ID를 지정합니다.
