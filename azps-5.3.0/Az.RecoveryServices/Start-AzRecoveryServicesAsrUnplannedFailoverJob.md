...
## 설명
**Start-AzRecoveryServicesAsrUnplannedFailoverJob** cmdlet은 Azure Site Recovery 복제 보호된 항목 또는 복구 계획의 계획되지 않은 장애 조치(failover)를 시작합니다.
...
지정된 매개 변수를 사용하여 복구 계획에 대한 계획되지 않은 장애 조치(failover) 작업을 시작하고 작업을 추적하는 데 사용되는 ASR 작업을 반환합니다.

### 예제 2

계획되지 않은 장애 조치(failover) 작업을 시작합니다. (자동Generated)
...
보호된 항목의 장애 조치(failover)에 대한 데이터 암호화 기본 인증서 파일 경로를 지정합니다.
...
보호된 항목의 장애 조치(failover)에 대한 데이터 암호화 보조 인증서 파일 경로를 지정합니다.
...
계획되지 않은 장애 조치(failover)를 시작하기 전에 원본 쪽에서 작업을 수행합니다.
...
### -RecoveryPoint
보호된 머신을 장애 조치(failover)할 사용자 지정 복구 지점을 지정합니다.
...
Azure Site Recovery 복제 보호 항목을 지정합니다.
