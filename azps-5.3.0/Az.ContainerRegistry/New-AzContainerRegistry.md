...
이 명령은 리소스 그룹 \` MyResourceGroup에 새 저장소 계정으로 컨테이너 레지스트리를 \` 만듭니다.

### 예제 2: 관리 사용자가 사용하도록 설정된 컨테이너 레지스트리 만들기
```powershell
...
이 명령은 관리자 사용자가 사용하도록 설정된 컨테이너 레지스트리를 만듭니다.
...
### -Tag
Container Registry Tags.Key-value 쌍(해시 테이블 형식).
