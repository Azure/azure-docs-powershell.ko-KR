...
## 설명
이 Update-AzContainerRegistryCredential cmdlet은 컨테이너 레지스트리에 대한 로그인 자격 증명을 다시 생성합니다.

## 예제

### 예제 1: 컨테이너 레지스트리에 대한 로그인 자격 증명 다시 생성
```powershell
...
이 명령은 지정된 컨테이너 레지스트리에 대한 로그인 자격 증명을 다시 생성합니다.
로그인 자격 증명을 다시 생성하려면 컨테이너 레지스트리 MyRegistry에 대해 관리 사용자를 사용하도록 \` \` 설정해야 합니다.
