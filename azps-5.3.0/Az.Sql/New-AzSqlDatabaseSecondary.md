...
## 설명
**New-AzSqlDatabaseSecondary** cmdlet은 데이터베이스에 대한 지역 복제를 설정하는 데 사용할 Start-AzSqlDatabaseCopy cmdlet을 대체합니다. 주 데이터베이스에서 보조 데이터베이스로의 지역 복제 링크 개체를 반환합니다.

## 예제

### 예제 1: 활성 Geo-Replication
```powershell
$database = Get-AzSqlDatabase -DatabaseName $databasename -ResourceGroupName $primaryresourcegroupname -ServerName $primaryservername
$database | New-AzSqlDatabaseSecondary -PartnerResourceGroupName $secondaryresourcegroupname -PartnerServerName $secondaryservername -AllowConnections "All"
```

### 예제 2: Active Geo-Replication 설정하고 원본 데이터베이스 이름과 다를 파트너 데이터베이스 이름을 지정합니다.
```powershell
$database = Get-AzSqlDatabase -DatabaseName $databasename -ResourceGroupName $primaryresourcegroupname -ServerName $primaryservername
...
### -AllowConnections
보조 Azure SQL 데이터베이스의 읽기 의도를 지정합니다.
이 매개 변수에 허용되는 값은
- 아니요
- 모두
...
### -DatabaseName
주 데이터베이스 역할을 할 데이터베이스의 이름을 지정합니다.
...
이 cmdlet이 보조 데이터베이스를 할당하는 Azure 리소스 그룹의 이름을 지정합니다.
...
### -PartnerServerName
보조 역할을 할 Azure SQL 데이터베이스 서버의 이름을 지정합니다.
...
### -ResourceGroupName
이 cmdlet이 주 데이터베이스를 할당할 Azure 리소스 그룹의 이름을 지정합니다.
...
Azure Sql Database 보조 데이터베이스의 계산 생성입니다.
...
보조 데이터베이스를 넣을 탄력적 풀의 이름을 지정합니다.
...
보조 데이터베이스에 할당할 서비스 목표의 이름을 지정합니다.
...
### -SecondaryVCore
Azure Sql Database 보조 데이터베이스의 Vcore 번호입니다.
...
### -ServerName
기본 SQL Server 데이터베이스의 SQL 지정합니다.
...
### -Tags
SQL 데이터베이스 복제 링크와 연결하기 위해 해시 테이블의 형태로 키-값 쌍을 지정합니다. 예: @{key0="value0";key1=$null;key2="value2"}
