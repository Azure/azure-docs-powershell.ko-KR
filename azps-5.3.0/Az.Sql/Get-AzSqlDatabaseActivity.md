...
## 설명
**Get-AzSqlDatabaseActivity** cmdlet은 Azure SQL Database에서 데이터베이스 작업의 상태를 얻습니다.

## 예제

### 예제 1: 모든 SQL 데이터베이스 인스턴스에 대한 상태 확인
```
PS C:\>Get-AzSqlDatabaseActivity -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -ElasticPoolName "ElasticPool01"
```

이 명령은 ElasticPool01이라는 탄력적 SQL 모든 데이터베이스 인스턴스의 작업 상태를 반환합니다.

### 예제 2: 모든 데이터베이스 작업의 SQL
```
PS C:\>Get-AzSqlDatabaseActivity -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -DatabaseName "Database01"
```

이 명령은 데이터베이스의 모든 SQL 데이터베이스 작업의 상태를 반환합니다.
...
### -ElasticPoolName
이 cmdlet이 상태를 얻을 탄력적 데이터베이스 풀의 이름을 지정합니다.
