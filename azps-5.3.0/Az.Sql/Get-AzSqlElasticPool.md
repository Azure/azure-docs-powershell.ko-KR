...
## 설명
**Get-AzSqlElasticPool** cmdlet은 탄력적 풀 및 속성 값을 얻습니다.
기존 탄력적 풀의 이름을 지정하여 해당 풀에 대한 속성 값만 표시합니다.
...
이 명령은 Server01이라는 서버에서 모든 탄력적 풀을 얻습니다.
...
이 명령은 Server01이라는 서버에서 ElasticPool0127이라는 탄력적 풀을 얻습니다.

### 예제 3: Azure SQL Elastic Database 풀에 대한 메트릭을 얻습니다.
```
...
이 명령은 ElasticPool01이라는 탄력적 데이터베이스 SQL Azure에 대한 메트릭을 반환합니다.

### 예제 4: 필터링 -ElasticPoolName "ElasticPool*"을 사용하여 모든 탄력적 풀을 얻습니다.
```
PS C:\>Get-AzSqlElasticPool -ResourceGroupName "ResourceGroup01" -ServerName "Server01"
ResourceId        : /subscriptions/00000000-0000-0000-0000-000000000001/resourceGroups/resourcegroup01/providers/Microsoft.Sql/servers/server01/elasticPools/elasticpool01
ResourceGroupName : resourcegroup01
...
이 명령은 "ElasticPool"으로 시작하는 Server01 서버의 모든 탄력적 풀을 얻습니다.
...
### -ElasticPoolName
이 cmdlet에서 얻을 탄력적 풀의 이름을 지정합니다.
...
### -ResourceGroupName
이 cmdlet에서 얻을 수 있는 탄력적 풀을 포함하는 리소스 그룹의 이름을 지정합니다.
...
### -ServerName
이 cmdlet에서 얻을 수 있는 탄력적 풀을 포함하는 서버의 이름을 지정합니다.
