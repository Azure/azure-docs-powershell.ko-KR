...
## 설명
**Start-AzSqlDatabaseExecuteIndexRecommendation** cmdlet은 Azure SQL Database에 대한 권장 인덱스 작업을 실행하는 워크플로를 시작합니다.
...
### -DatabaseName
이 cmdlet이 워크플로를 시작하는 데이터베이스의 이름을 지정합니다.
...
이 cmdlet이 실행되는 인덱스 권장의 이름을 지정합니다.
...
### -ServerName
이 cmdlet이 워크플로를 시작하는 데이터베이스를 호스트하는 서버를 지정합니다.
