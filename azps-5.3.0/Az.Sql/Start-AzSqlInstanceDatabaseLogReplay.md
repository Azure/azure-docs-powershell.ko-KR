...
## 설명
**Start-AzSqlInstanceDatabaseLogReplay** cmdlet은 로그 재생 서비스의 시작을 시작됩니다.
...
이 명령은 새 관리되는 데이터베이스를 만들고 last_backup.bak가 복원될 때까지 주어진 컨테이너에서 백업을 복원하기 시작할 것입니다.
...
이 명령은 새 관리되는 데이터베이스를 만들고 마지막 백업이 필요한 경우 호출될 때까지 Complete-AzSqlInstanceDatabaseLogReplay 컨테이너에서 백업을 복원하기 시작할 것입니다.

## PARAMETERS

### -AutoCompleteRestore
완료 시 복원을 자동으로 완료할지 여부를 나타내는 표시기입니다.
