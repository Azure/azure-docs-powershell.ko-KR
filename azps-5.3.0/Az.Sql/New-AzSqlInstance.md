...
## 설명
**New-AzSqlInstance** cmdlet은 Azure SQL 데이터베이스 관리되는 인스턴스를 만듭니다.
...
### -AssignIdentity
Azure KeyVault와 같은 키 관리 서비스에서 사용할 이 Managed Instance에 대한 Azure Active Directory ID를 생성하고 할당합니다.
...
Sql Azure Managed Instance에 대한 백업을 저장하는 데 사용되는 Backup 저장소 중복성입니다. 옵션은 로컬, 영역 및 지역입니다.
...
### -Collation
사용할 Azure SQL Managed Instance의 데이터입니다.
...
### -DnsZonePartner
Managed Instance 생성에서 DnsZone 속성을 상속하는 파트너 Managed Server의 리소스 ID입니다.
...
### -LicenseType
사용할 라이선스 유형을 결정 합니다. 가능한 값은
- BasePrice - 기존 라이선스 소유자에 대한 AHB(Azure 하이브리드 혜택) SQL Server 가격이 적용됩니다. Managed Instance 서비스 가격은 기존 라이선스 SQL Server 할인됩니다.
- LicenseIncluded - 기존 라이선스 소유자에 대한 AHB(Azure 하이브리드 혜택) SQL Server 가격은 적용되지 않습니다. Managed Instance 서비스 가격에는 새 라이선스 SQL Server 포함됩니다.
...
인스턴스에 대해 공용 데이터 엔드포인트를 사용하도록 설정되어 있는지 여부입니다.
...
### -SkuName
인스턴스의 SKU 이름(예: 'GP_Gen4', 'BC_Gen4').
