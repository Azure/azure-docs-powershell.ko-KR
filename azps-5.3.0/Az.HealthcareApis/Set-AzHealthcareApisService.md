...
## SYNOPSIS
기존 HealthcareApis fhir 서비스를 업데이트합니다.
...
### 예제 1: 리소스 그룹 MyResourceGroup의 MyService라는 기존 Healthcareapis 서비스를 cosmosdb OfferThroughput = 500으로 업데이트합니다.
...
### 예제 2: Cosmosdb OfferThroughput = 500 및 키 자격 증명 모음 키 URI "https:// \<my-keyvault> .vault.azure.net/keys/" 리소스 그룹 MyResourceGroup의 MyService라는 기존 의료 서비스 업데이트 \<my-key>
...
### -CorsHeader
HealthcareApis FhirService Cors 헤더 목록입니다.
요청 중에 사용할 수 있는 HTTP 헤더를 지정합니다.
모든 헤더에 " * "를 사용 합니다.
...
요청의 결과를 몇 초 만에 캐시할 수 있는 기간을 지정합니다.
예: 600은 10분을 의미합니다.
...
### -CorsMethod
HealthcareApis FhirService Cors 메서드 목록입니다.
...
### -CorsOrigin
HealthcareApis FhirService Cors Origins 목록입니다.
이 API에 액세스할 수 있는 원본 사이트의 URL을 지정하거나 " * "를 사용하여 모든 사이트에서 액세스를 허용합니다.
...
HealthcareApis FhirService CorsCredentials 허용되지 않습니다.
...
HealthcareApis Fhir Service 내보내기 저장소 계정 이름입니다.
...
### -InputObject
Get-AzHealthcareApisFhirService에서 파이프된 HealthcareApis fhir 서비스입니다.
...
### -PublicNetworkAccess
Fhir 서비스에 대한 네트워크 액세스 유형입니다. 일반적으로 `Enabled` 또는 `Disabled` .
...
### -ResourceGroupName
HealthcareApis 서비스 리소스 그룹 이름입니다.
...
### -Tag
HealthcareApis Fhir 서비스 계정 태그입니다.
