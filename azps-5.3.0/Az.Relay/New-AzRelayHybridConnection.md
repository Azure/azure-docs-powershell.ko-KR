...
## 설명
New-AzRelayHybridConnection cmdlet은 지정된 Relay 네임스페이스에 HybridConnection을 만듭니다.
...
지정된 Relay 네임스페이스 \` \` \` TestNameSpace-HybirdConnection에 새 HybirdConnection TestHybirdConnection2를 \` 만듭니다.
...
지정된 Relay 네임스페이스 \` \` \` TestNameSpace-HybirdConnection에 새 HybirdConnection TestHybirdConnection1을 \` 만듭니다.
...
이 HybridConnections에 클라이언트 권한 부여가 필요한 경우 true입니다. 그렇지 않으면 false
