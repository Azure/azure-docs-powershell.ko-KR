...
### 예제 1: 이름으로 PostgreSql Virtual Network 규칙 업데이트
```powershell
...
이 cmdlet은 PostgreSql Virtual Network 규칙을 이름으로 업데이트합니다.

### 예제 2: ID로 PostgreSql Virtual Network 규칙을 업데이트합니다.
```powershell
...
이러한 cmdlet은 ID에 의해 PostgreSql Virtual Network 규칙을 업데이트합니다.
