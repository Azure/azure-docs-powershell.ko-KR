...
## 설명
**New-AzManagementGroup** cmdlet은 관리 그룹을 만듭니다.
...
.로 설정된 새 `DisplayName` `ParentId` 그룹 만들기 `null` 그룹의 부모와 동일하며 `DisplayName` `GroupName` 테넌트가 됩니다.  

### 예제 2: 표시 이름을 사용하여 관리 그룹 만들기
```
...
이 경우 그룹의 부모는 테넌트가 며, 이 값은 주어진 `DisplayName` 값으로 설정됩니다.

### 예제 3: 부모 및 표시 이름을 사용하여 관리 그룹 만들기
```
...
### 예제 4: 부모가 있는 관리 그룹 만들기(부모 개체 사용)
```
