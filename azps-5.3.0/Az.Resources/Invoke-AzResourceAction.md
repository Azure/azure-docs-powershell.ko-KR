...
지원되는 작업 목록을 얻하려면 Azure 리소스 탐색기 도구를 사용합니다.
...
### 예제 2: ResourceName을 통해 VM 전원 공급 호출
...
### 예제 3: ResourceId를 통해 리소스 공급자 등록 호출
...
이 cmdlet이 작업을 호출하는 리소스에 대한 확장 리소스의 이름을 지정합니다.
...
### -ResourceGroupName
이 cmdlet이 작업을 호출하는 리소스 그룹의 이름을 지정합니다.
...
### -ResourceId
이 cmdlet이 작업을 호출하는 리소스의 정식 리소스 ID를 지정합니다.
...
### -ResourceName
이 cmdlet이 작업을 호출하는 리소스의 리소스 이름을 지정합니다.
