...
## 설명
**New-AzServiceBusSubscription** cmdlet은 지정된 Service Bus 토픽에 대한 새 구독을 만듭니다.
...
지정된 `SB-TopicSubscription-Example1` Service Bus 토픽에 대한 구독을 `SB-Topic_exampl1` 만듭니다.

## PARAMETERS

### -AutoDeleteOnIdle
구독이 자동으로 삭제되는 [TimeSpan](https://msdn.microsoft.com/library/system.timespan.aspx) 유휴 간격을 지정합니다. 최소 기간은 5분입니다.
...
구독에 필터 평가 예외에 대한 dead letter 지원이 있는지 여부를 나타내는 값입니다.
...
### -RequiresSession
RequiresSession - 이 큐에 중복 검색이 필요한지 나타내는 값입니다.
