...
## 설명
**Get-AzLogicAppUpgradedDefinition** cmdlet은 리소스 그룹에서 schema 버전 및 논리 앱에 대한 업그레이드된 정의를 얻습니다.
이 cmdlet은 업그레이드된 논리 앱의 정의를 나타내는 개체를 반환합니다.
리소스 그룹 이름, 논리 앱 이름 및 대상 스마마 버전을 지정합니다.
...
첫 번째 명령은 지정된 대상 스마마 버전으로 업그레이드된 논리 앱에 대한 정의를 얻습니다.
이 명령은 정의를 $UpgradedDefinition 변수에 저장합니다.
두 번째 명령은 명령의 $UpgradedDefinition 문자열로 표시됩니다.
