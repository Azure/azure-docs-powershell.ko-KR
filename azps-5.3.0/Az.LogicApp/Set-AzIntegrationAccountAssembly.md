...
"$localAssemblyFilePath"에 포함된 파일 경로에 있는 로컬 파일을 사용하여 "sampleAssembly"라는 어셈블리를 수정합니다.
...
"$assemblyContent"에 포함된 byte 배열을 사용하여 "sampleAssembly"라는 어셈블리를 수정합니다.
...
URL "$assemblyUrl"에 있는 byte 데이터를 사용하여 "sampleAssembly"라는 어셈블리를 수정합니다. 이는 큰 크기의 어셈블리를 만들기 위한 제안된 방법입니다.
