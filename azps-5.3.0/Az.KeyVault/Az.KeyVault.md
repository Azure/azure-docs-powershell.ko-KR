...
지정한 범위에서 관리되는 HSM의 역할 정의를 나열합니다.
...
이전에 내보낼 보안 도메인 데이터를 관리되는 HSM으로 가져올 수 있습니다.
...
백업 파일에서 키 자격 증명 모음의 인증서를 복원합니다.
