...
### 예제 2: 파이프라인을 통해 지정된 확장 매개 변수를 사용하여 새 확장 추가
```powershell
...
파이프라인을 통해 전달된 개체에서 제공하는 확장 매개 변수를 사용하여 새 확장을 만듭니다.
이는 한 컴퓨터의 매개 변수를 잡고 다른 머신에 적용하려는 경우 매우 좋은 것입니다.

### 예제 3: 파이프라인을 통해 지정된 위치가 있는 새 확장 추가
```powershell
...
파이프라인을 통해 제공된 ID를 사용하여 새 컴퓨터 확장을 만듭니다.
이렇게 하지는 않지만 가능할 수 있습니다.

### 예제 4: 업데이트할 위치 및 매개 변수로 확장 개체를 사용하여 새 확장 추가
```powershell
...
파이프라인을 통해 제공된 ID 및 전달된 확장 개체에서 제공하는 확장 세부 정보를 사용하여 새 컴퓨터 확장을 만듭니다.
이렇게 하지는 않지만 가능할 수 있습니다.
