...
## 설명
구성 콘텐츠는 json으로, 디바이스 또는 모듈 의도에 따라 약간의 차이가 있습니다. 디바이스 구성은 {"deviceContent":{...}} 형식입니다.
모듈 구성은 {"moduleContent":{...}} 형식입니다.
사용자 제공 메트릭을 사용하여 구성을 정의하여 필요 시 평가할 수 있습니다.
사용자 메트릭은 json 및 {"queries":{...}} 형식입니다. 또는 {"metrics":{"queries":{...}}}.

참고: 모듈의 대상 조건은 "from devices.modules where"로 시작해야 합니다. 자세한 https://docs.microsoft.com/azure/iot-hub/iot-hub-automatic-device-management 내용은 다음을 참조하세요.
...
디바이스가 건물 9에서 태그가 지정되어 환경이 '테스트'인 경우 조건에 적용되는 우선 순위가 3인 디바이스 구성을 생성합니다.
...
사용자 메트릭을 사용하여 디바이스 구성을 만들 수 있습니다.
...
### -Priority
경쟁 규칙(최고 우승)의 경우 디바이스 구성의 가중치입니다.
