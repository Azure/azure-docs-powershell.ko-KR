...
가상 네트워크 게이트웨이를 만들 때 게이트웨이에서 기본 vpn ipsec 정책 집합을 설정합니다. 지점 및 사이트간 사용자가 특정 사용자 지정 ipsec 정책을 사용하여 VPN Gateway에 연결하려는 경우 사용자는 먼저 VPN Gateway에서 해당 ipsec 정책을 설정해야 합니다. **Set-AzVpnClientIpsecParameter는** 이를 위해 방법을 제공합니다.
