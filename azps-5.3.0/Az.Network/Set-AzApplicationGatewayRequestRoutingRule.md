...
## 설명
**Set-AzApplicationGatewayRequestRoutingRule** cmdlet은 요청 라우팅 규칙을 수정합니다.
...
두 번째 명령은 $Setting 변수에 지정된 백 엔드 HTTP 설정, $Listener 변수에 지정된 HTTP 수신기 및 $Pool 변수에 지정된 백 엔드 주소 풀을 사용하기 위해 애플리케이션 게이트웨이에 대한 요청 라우팅 규칙을 수정합니다.

## PARAMETERS

### -ApplicationGateway
이 cmdlet이 요청 라우팅 규칙을 연결한 애플리케이션 게이트웨이 개체를 지정합니다.
...
애플리케이션 게이트웨이 백 엔드 주소 풀 ID를 지정합니다.
...
### -BackendHttpSettings
애플리케이션 게이트웨이 백드 HTTP 설정을 지정합니다.
...
애플리케이션 게이트웨이 백 엔드 HTTP 설정 ID를 지정합니다.
...
### -HttpListenerId
애플리케이션 게이트웨이 HTTP 수신기 ID를 지정합니다.
...
### -Name
이 cmdlet이 수정하는 요청 라우팅 규칙의 이름을 지정합니다.
