...
## 설명
**Set-AzLoadBalancerInboundNatRuleConfig** cmdlet은 Azure Load Balancer에 대한 인바운드 NAT(네트워크 주소 변환) 규칙 구성을 설정합니다.
...
두 번째 명령은 파이프라인 연산자를 사용하여 $slb Load Balancer를 Add-AzLoadBalancerInboundNatRuleConfig에 전달합니다. 이 명령은 인바운드 NAT 규칙 구성을 추가합니다.
세 번째 명령은 Load **Balancer를 Set-AzLoadBalancerInboundNatRuleConfig에** 전달합니다. 이 명령은 인바운드 NAT 규칙 구성을 저장하고 업데이트합니다.
규칙 구성은 이전 명령에서 사용하도록 설정한 부동 IP를 사용하도록 설정하지 않고 설정되었습니다.
...
대화 상태가 부하 균형 조정기에서 유지되는 시간(분)을 지정합니다.
...
### -LoadBalancer
부하 균형 조정기를 지정합니다.
이 cmdlet은 이 매개 변수가 지정하는 부하 균형 조정에 대한 인바운드 NAT 규칙 구성을 설정합니다.
