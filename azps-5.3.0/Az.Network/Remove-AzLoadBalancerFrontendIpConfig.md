...
## 설명
**Remove-AzLoadBalancerFrontendIpConfig** cmdlet은 Azure Load Balancer에서 프런트 엔드 IP 구성을 제거합니다.

## 예제

### 예제 1: 부하 균형 조정기에서 프런트 엔드 IP 구성 제거
```
PS C:\>$loadbalancer = Get-AzLoadBalancer -Name "MyLoadBalancer" -ResourceGroupName "MyResourceGroup"
PS C:> Remove-AzLoadBalancerFrontendIpConfig -Name "frontendName" -LoadBalancer $loadbalancer
```

첫 번째 명령은 제거하려는 프런트 엔드 IP 구성과 연결된 부하 $loadbalancer 변수에 저장합니다.
두 번째 명령은 연결된 프런트 엔드 IP 구성을 네트워크의 부하 $loadbalancer.
...
### -LoadBalancer
제거할 프런트 엔드 IP 구성을 포함하는 부하 균형 조정기를 지정합니다.
...
### -Name
제거할 프런트 엔드 IP 주소 구성의 이름을 지정합니다.
