...
이 명령은 "westus" 위치에 있는 리소스 그룹 "rg1"에 "nsg1"이라는 새 Azure 네트워크 보안 그룹을 만듭니다.
...
1단계: 인터넷에서 포트 3389로의 액세스를 허용하는 보안 규칙을 작성합니다.
2단계: 인터넷에서 포트 80으로의 액세스를 허용하는 보안 규칙을 작성합니다.
3단계: 위에서 만든 규칙을 NSG-FrontEnd라는 새 NSG에 추가합니다.
...
### -ResourceGroupName
리소스 그룹의 이름을 지정합니다.
이 cmdlet은 이 매개 변수가 지정하는 리소스 그룹에 네트워크 보안 그룹을 만듭니다.
...
### -SecurityRules
네트워크 보안 그룹에 만들 네트워크 보안 규칙 개체의 목록을 지정합니다.
