...
## 설명
**Add-AzApplicationGatewayRequestRoutingRule** cmdlet은 애플리케이션 게이트웨이에 요청 라우팅 규칙을 추가합니다.

## 예제

### 예제 1: 애플리케이션 게이트웨이에 요청 라우팅 규칙 추가
```
...
두 번째 명령은 애플리케이션 게이트웨이에 요청 라우팅 규칙을 추가합니다.

## PARAMETERS

### -ApplicationGateway
이 cmdlet이 요청 라우팅 규칙을 추가하는 애플리케이션 게이트웨이를 지정합니다.
...
### -BackendAddressPool
애플리케이션 게이트웨이 백 엔드 주소 풀 개체를 지정합니다.
...
### -BackendHttpSettings
애플리케이션 게이트웨이에 대한 백 엔드 HTTP 설정 개체를 지정합니다.
...
애플리케이션 게이트웨이에 대한 백end HTTP 설정 ID를 지정합니다.
...
### -HttpListener
애플리케이션 게이트웨이 HTTP 수신기 개체를 지정합니다.
...
### -Name
이 cmdlet이 추가하는 요청 라우팅 규칙의 이름을 지정합니다.
