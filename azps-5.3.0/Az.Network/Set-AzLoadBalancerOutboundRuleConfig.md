...
## 설명
**Set-AzLoadBalancerOutboundRuleConfig** cmdlet은 Azure Load Balancer에 대한 아웃바운드 규칙 구성을 설정합니다.

## 예제

### 예제 1: 부하 균형 조정기에서 아웃바운드 규칙 구성 수정
```powershell
...
두 번째 명령은 파이프라인 연산자를 사용하여 부하 분산 $slb Add-AzLoadBalancerOutboundRuleConfig에 전달합니다. 이 명령은 아웃바운드 규칙 구성을 추가합니다.
세 번째 명령은 부하 분산을 **Set-AzLoadBalancerOutboundRuleConfig에** 전달합니다. 이 명령은 아웃바운드 규칙 구성을 저장하고 업데이트합니다.
