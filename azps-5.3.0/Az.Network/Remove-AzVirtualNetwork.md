...
## 설명
**Remove-AzVirtualNetwork** cmdlet은 Azure 가상 네트워크를 제거합니다.
...
이 예제에서는 리소스 그룹에 가상 네트워크를 만든 다음 즉시 삭제합니다. 가상 네트워크를 삭제하는 경우 프롬프트를 표시하지 않습니다. -Force 플래그를 사용 합니다.
...
### -Name
이 cmdlet이 제거하는 가상 네트워크의 이름을 지정합니다.
...
### -ResourceGroupName
이 cmdlet에서 제거하는 가상 네트워크를 포함하는 리소스 그룹의 이름을 지정합니다.
