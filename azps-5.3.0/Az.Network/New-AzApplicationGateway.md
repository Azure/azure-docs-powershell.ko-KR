...
- 애플리케이션 게이트웨이에서 연 공용 포트인 프런트 엔드 포트입니다. 이러한 포트에 적중하는 트래픽은 백 엔드 서버로 리디렉션됩니다.
...
다음 예제에서는 먼저 다음뿐만 아니라 리소스 그룹 및 가상 네트워크를 만들어 애플리케이션 게이트웨이를 만듭니다.
- 백 엔드 서버 풀
- 백 엔드 서버 풀 설정
- 프런트 엔드 포트
- 프런트 엔드 IP 주소
...
Force firewallPolicy 연결 사용 여부입니다.
...
### -SslCertificates
애플리케이션 게이트웨이에 대한 Secure Sockets Layer(SSL) 인증서 목록을 지정합니다.
