...
## 설명
**New-AzNotificationHubAuthorizationRule** cmdlet은 알림 허브 SAS(공유 액세스 서명) 권한 부여 규칙을 만듭니다.
권한 부여 규칙은 알림 허브에 대한 액세스를 관리하는 데 사용됩니다.
이는 다른 사용 권한 수준에 따라 UR로 링크를 만들어서 수행됩니다.
...
규칙 이름을 포함하여 규칙에 대한 모든 구성 정보는 설정한 입력 파일에서 C:\Configuration\ExternalAccessRule.js.
...
### -InputFile
이 cmdlet에서 만드는 권한 부여 규칙에 대한 입력 파일을 지정합니다.
