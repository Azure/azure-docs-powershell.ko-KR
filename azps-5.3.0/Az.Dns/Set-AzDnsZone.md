...
DNS 영역이 개체로 전달될 때(Zone 개체 사용 또는 파이프라인을 통해) 로컬 DnsZone 개체가 검색된 이후 Azure DNS에서 변경된 경우 업데이트되지 않습니다. 이렇게 하면 동시 변경에 대한 보호를 제공합니다. 동시 변경 내용에 관계없이 영역이 업데이트되는 *덮어* 사용 매개 변수를 사용하면 이 동작을 표시하지 않습니다.
...
이 명령은 먼저 명시적으로 영역이 myzone.com 이름의 영역 태그를 업데이트합니다.
