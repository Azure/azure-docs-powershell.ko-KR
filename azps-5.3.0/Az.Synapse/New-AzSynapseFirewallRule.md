...
## 설명
**New-AzSynapseFirewallRule** cmdlet은 Azure Synapse Analytics 방화벽 규칙을 만듭니다.
...
이 명령은 ContosoFirewallRule이라는 이름의 작업 영역 ContosoWorkspace 아래에 ContosoFirewallRule이라는 방화벽 규칙을 만듭니다.
...
이 명령은 파이프라인을 통해 작업 영역 아래에 ContosoFirewallRule이라는 방화벽 규칙을 만듭니다.
...
이 명령은 작업 영역의 모든 Azure IP를 허용하는 방화벽 규칙을 만듭니다.

## PARAMETERS

### -AllowAllAzureIP
모든 Azure IP가 액세스를 허용하는 특수 방화벽 규칙을 만듭니다.
