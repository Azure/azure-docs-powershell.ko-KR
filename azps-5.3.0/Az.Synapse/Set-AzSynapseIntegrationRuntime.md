...
## 설명
**Set-AzSynapseIntegrationRuntime** cmdlet은 특정 매개 변수를 사용하여 통합 런타임을 업데이트합니다.
...
이 cmdlet은 공유 통합 런타임에 사용할 작업 영역이 추가됩니다. 매개 `-SharedIntegrationRuntimeResourceId` 변수를 사용하는 경우 `-Type` 포함해야 합니다. cmdlet을 실행하기 전에 통합 런타임 사용 권한을 작업 영역에게 부여해야 합니다.
...
스테이징 데이터 저장소의 경로는 Self-Hosted Azure-SSIS Integration Runtime 간에 데이터를 이동하는 경우 사용할 경로로, 지정되지 않은 경우 기본 컨테이너가 사용됩니다.
...
### -ExpressCustomSetup
사용자 지정 설치 스크립트 없이 구성 및 제3자 구성 요소를 설정하는 데 사용할 수 있는 SSIS 통합 런타임에 대한 Express 사용자 지정 설정입니다.
...
AHUB(Azure 하이브리드 사용 혜택) 가격 책정 자격이 있는 경우 BasePrice를 선택하세요.
그렇지 않은 경우 LicenseIncluded를 선택해 주세요.
