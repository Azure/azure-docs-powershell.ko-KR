...
## 설명
**Remove-AzSynapseSqlPoolRestorePoint** cmdlet은 풀 복원 지점에서 Azure Synapse Analytics SQL 영구적으로 삭제합니다.
...
이 명령은 지정된 리소스 ID를 SQL Azure Synapse Analytics 풀 복원 지점을 삭제합니다.
...
### -InputObject
SQL 복원 지점 생성 시간 입력 개체로, 일반적으로 파이프라인을 통해 전달됩니다.
...
### -SqlPoolObject
일반적으로 파이프라인을 통해 전달되는 Sql 풀 입력 개체입니다.
