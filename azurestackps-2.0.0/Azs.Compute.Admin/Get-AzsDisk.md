...
매개 변수를 사용 하지 않으면 `Get-AzsDisk` 모든 디스크가 나열 됩니다.
...
검색할 디스크를 지정 `Name` 합니다.

### 예제 3: 지정 된 수의 디스크 가져오기
```powershell
...
`Count`매개 변수를 사용 하 여 특정 개수의 디스크를 검색 합니다.

### 예제 4: 모든 디스크를 특정 위치에 가져오기
```powershell
...
`ScaleUnit`Or `VolumeLabel` 매개 변수를 사용 하 여 특정 위치에 모든 디스크 나열
...
### Api20180730Preview Eadmin. a t t t t 디스크
