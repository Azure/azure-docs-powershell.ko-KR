...
## 설명은
**AzureStorageFileCopy** cmdlet은 원본 파일을 대상 파일에 복사 하는 데부터 시작 됩니다.

## 예제의

### 예제 1: 공유 이름 및 파일 이름을 사용 하 여 파일에서 파일에 복사 작업 시작
```
...
이 명령은 파일에서 파일에 복사 작업을 시작 합니다.
명령은 공유 이름 및 파일 이름을 지정 합니다.

### 예제 2: 컨테이너 이름과 blob 이름을 사용 하 여 blob에서 파일로 복사 작업 시작
```
...
이 명령은 blob에서 파일로 복사 작업을 시작 합니다.
명령이 컨테이너 이름 및 blob 이름을 지정 합니다.

## 변수

### -AbsoluteUri
원본 파일의 URI를 지정 합니다.
원본 위치에 자격 증명이 필요한 경우이를 제공 해야 합니다.
...
### DestContext
대상의 Azure 저장소 컨텍스트를 지정 합니다.
컨텍스트를 얻으려면 **새-AzureStorageContext** 를 사용 합니다.
...
클라우드 blob을 만들거나 Get-AzureStorageBlob cmdlet을 사용 하 여 가져올 수 있습니다.
...
### -SrcContainer
클라우드 blob 컨테이너 개체를 지정 합니다.
클라우드 blob 컨테이너 개체를 만들거나 Get-AzureStorageContainer cmdlet을 사용할 수 있습니다.
...
### -SrcFile
**Cloudfile** 개체를 지정 합니다.
클라우드 파일을 만들거나 **AzureStorageFile** 를 사용 하 여 가져올 수 있습니다.
...
### -SrcFilePath
원본 디렉터리 또는 원본 공유에 상대적인 원본 파일의 경로를 지정 합니다.
...
### -SrcShare
클라우드 파일 공유 개체를 지정 합니다.
클라우드 파일 공유를 만들거나 Get-AzureStorageShare cmdlet을 사용 하 여 가져올 수 있습니다.
...
' SrcBlob ' 매개 변수는 파이프라인에서 ' CloudBlob ' 형식의 값을 허용 합니다.

### CloudFile

' SrcFile ' 매개 변수는 파이프라인에서 ' CloudFile ' 형식의 값을 허용 합니다.
...
[중지-AzureStorageFileCopy](./Stop-AzureStorageFileCopy.md)
