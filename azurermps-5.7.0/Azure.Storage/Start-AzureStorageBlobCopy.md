...
## 설명은
**시작-AzureStorageBlobCopy** cmdlet이 시작 되어 blob을 복사 합니다.
...
이 명령은 **AzureStorageContainer** cmdlet을 사용 하 여 ContosoUploads 이라는 컨테이너를 가져온 다음 파이프라인 연산자를 사용 하 여 현재 cmdlet에 컨테이너를 전달 합니다.
...
이 명령은 **Get-AzureStorageBlob** cmdlet을 사용 하 여 ContosoUploads 이라는 컨테이너의 blob을 가져온 다음 파이프라인 연산자를 사용 하 여 현재 cmdlet에 결과를 전달 합니다.
...
이 명령은 $Context에 저장 된 컨텍스트에서 복사 작업을 시작 합니다.
...
### -SrcFile
Azure Storage 클라이언트 라이브러리에서 **Cloudfile** 개체를 만듭니다.
이를 만들거나 Get-AzureStorageFile cmdlet을 사용할 수 있습니다.
...
이를 만들거나 Get-AzureStorageShare cmdlet을 사용할 수 있습니다.
...
' CloudBlob ' 매개 변수는 파이프라인에서 ' CloudBlob ' 형식의 값을 허용 합니다.
