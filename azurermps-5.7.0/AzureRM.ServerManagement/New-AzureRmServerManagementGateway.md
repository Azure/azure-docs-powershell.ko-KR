...
## 설명은
**AzureRmServerManagementGateway** Cmdlet은 Azure 서버 관리 게이트웨이를 만듭니다.
...
### -AutoUpgrade
새 버전이 릴리스되면 게이트웨이가 자동으로 업그레이드 됨을 나타냅니다.
...
### --게이트웨이 이름
이 cmdlet이 만드는 게이트웨이의 이름을 지정 합니다.
...
### -ResourceGroupName
이 cmdlet이 게이트웨이를 만드는 리소스 그룹의 이름을 지정 합니다.
