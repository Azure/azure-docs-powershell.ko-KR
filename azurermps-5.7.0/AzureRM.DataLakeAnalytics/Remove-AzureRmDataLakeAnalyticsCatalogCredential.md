...
## 설명은
Remove-AzureRmDataLakeAnalyticsCatalogCredential cmdlet은 Azure Data Lake Analytics 카탈로그 자격 증명을 삭제 합니다.
...
이 명령은 지정 된 Data Lake Analytics 카탈로그 자격 증명을 제거 합니다.
...
### -DatabaseName
자격 증명을 보유 하는 데이터베이스의 이름을 지정 합니다.
...
### -PassThru
이 cmdlet이 작업이 완료 될 때까지 대기 하지 않음을 나타냅니다.
...
### -재귀적으로
이 삭제 작업을 수행 하 고이 자격 증명에 종속 된 모든 리소스를 삭제 및 제거 하는 것을 나타냅니다.
...
### 부울
PassThru가 지정 된 경우 작업이 완료 되 면 true를 반환 합니다.
