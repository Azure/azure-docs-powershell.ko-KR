...
## 설명은
Get-AzureRmNetworkWatcherNextHop cmdlet은 VM에서 다음 홉을 가져옵니다. 다음 홉을 사용 하면 Azure 리소스의 유형, 해당 리소스의 연결 된 IP 주소, 경로를 담당 하는 라우팅 테이블 규칙이 표시 됩니다.

## 예제의

### --예 1: 인터넷 IP와 통신 하는 경우 다음 홉을 확인 하세요.
```
...
지정 된 가상 Vachine (www.bing.com)에서 기본 네트워크 인터페이스의 다음 홉을 기준으로 통신을 시작 합니다.
