...
## 설명은
**Set-AzureKeyVaultCertificateAttribute** cmdlet은 인증서의 편집 가능한 특성을 수정 합니다.
...
마지막 명령은 Get-AzureKeyVaultCertificate cmdlet을 사용 하 여 작업을 확인 하 여 TestCert01 인증서를 표시 합니다.
...
### -사용
인증서를 사용 하거나 사용 하지 않도록 설정할지 여부를 나타냅니다.
사용 $False 하거나 사용 하지 않을 $True를 지정 합니다.
...
### -이름
수정할 인증서의 이름을 지정 합니다. 이 cmdlet은 키 자격 증명 이름, 현재 선택 된 환경, 인증서 이름, 인증서 버전을 기반으로 인증서의 FQDN을 생성 합니다.
...
### -VaultName
이 cmdlet이 인증서를 수정할 키 보관소 이름을 지정 합니다.
이 cmdlet은 이름 및 현재 선택 된 환경에 따라 키 자격 증명의 FQDN을 생성 합니다.
...
### -버전
인증서의 버전을 지정 합니다.
이 cmdlet은 키 자격 증명 이름, 현재 선택 된 환경, 인증서 이름, 인증서 버전을 기반으로 인증서의 FQDN을 생성 합니다.
