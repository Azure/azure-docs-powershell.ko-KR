...
## 설명은
**AzureRmSqlDatabaseBackupLongTermRetentionPolicy** cmdlet은이 데이터베이스에 등록 된 장기 보존 정책을 가져옵니다.
정책은 백업 저장소 정책을 정의 하는 데 사용 되는 Azure Backup 리소스입니다.

## 예제의

### 예제 1: 장기 보존 정책의 현재 버전 가져오기
```powershell
...
이 명령은 database01에 대 한 장기 보존 정책의 현재 버전을 가져옵니다.

### 예제 2: 이전 버전의 장기 보존 정책 가져오기
```powershell
...
이 명령은 database01의 장기 보존 정책의 레거시 버전을 가져옵니다.

## 변수

### -현재
지정 하지 않으면 명령이 레거시 장기 보존 정책 정보를 반환 합니다.
그렇지 않으면 명령이 장기 보존 정책의 현재 버전을 반환 합니다.
