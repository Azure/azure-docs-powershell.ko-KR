...
## 설명은
**Set-AzureRmSqlDatabaseAdvisorAutoExecuteStatus** Cmdlet은 Azure SQL 데이터베이스 관리자에 대 한 자동 실행 속성을 수정 합니다.
현재이 cmdlet은 사용, 사용 안 함 및 기본값 값을 지원 합니다.
...
이 명령은 CreateIndex 라는 관리자의 자동 실행 상태를 사용으로 변경 합니다.

## 변수

### -AdvisorName
이 cmdlet이 상태를 수정 하는 관리자의 이름을 지정 합니다.
...
### -DatabaseName
이 cmdlet에서 상태를 수정 하는 데이터베이스의 이름을 지정 합니다.
...
이 cmdlet은 **AzureSqlDatabaseAdvisorModel** 개체를 반환 합니다.
