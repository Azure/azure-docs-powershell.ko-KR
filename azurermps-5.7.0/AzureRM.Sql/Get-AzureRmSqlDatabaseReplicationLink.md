...
## 설명은
**AzureRMSqlDatabaseReplicationLink** Cmdlet은 **AzureSqlDatabaseCopy** cmdlet을 대신 합니다.
지정 된 Azure SQL 데이터베이스와 리소스 그룹 또는 AzureSQL 서버 간의 모든 지역/복제 링크를 가져옵니다.
...
### -DatabaseName
링크를 검색할 SQL 데이터베이스의 이름을 지정 합니다.
...
### -(-)-(서버 이름)
파트너에 대 한 Azure SQL Server의 이름을 지정 합니다.
...
### -ResourceGroupName
링크를 검색할 데이터베이스에 대 한 Azure 리소스 그룹의 이름을 지정 합니다.
...
### -ServerName
연결을 검색할 데이터베이스에 대 한 SQL Server의 이름을 지정 합니다.
...
###  
이 cmdlet은 주 또는 보조 데이터베이스에 대 한 **ReplicationLink** 또는 **Database** 개체의 인스턴스를 허용 합니다.

## 출력

###  
이 cmdlet은 **ReplicationLink** 개체를 반환 합니다.
