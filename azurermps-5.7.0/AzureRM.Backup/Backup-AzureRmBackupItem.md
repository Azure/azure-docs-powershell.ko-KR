...
## 설명은
**AzureRmBackupItem** cmdlet은 백업 일정에 연결 되지 않은 보호 된 Azure 백업 항목에 대 한 백업을 시작 합니다.
보호를 사용 하도록 설정 하거나 예약 된 백업이 실패 한 후 백업을 시작 하면 즉시 초기 백업을 수행할 수 있습니다.

기존 백업 작업이 실행 되 고 있는 경우이 cmdlet이 실패 합니다.
...
마지막 명령은 Get-AzureRmBackupItem cmdlet을 사용 하 여 $Container의 백업 항목을 가져옵니다.
이 명령은 파이프라인 연산자를 사용 하 여 항목을 현재 cmdlet에 전달 합니다.
현재 cmdlet은 컨테이너의 가상 컴퓨터 백업을 시작 합니다.
