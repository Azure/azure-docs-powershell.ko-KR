...
### 예제 1: 사용 중인 Kusto 클러스터 이름의 가용성 확인
```powershell
...
위의 명령은 "testnewkustocluster"라는 Kusto 클러스터가 "미국 동부" 지역에 있는지 여부를 반환합니다.

### 예제 2: 사용되지 않는 Kusto 클러스터 이름의 가용성 확인
```powershell
...
위의 명령은 "availablekustocluster"라는 Kusto 클러스터가 "미국 동부" 지역에 있는지 여부를 반환합니다.
...
### -Type
리소스 유형, Microsoft.Kusto/clusters입니다.
