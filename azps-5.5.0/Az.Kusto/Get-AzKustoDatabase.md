...
### 예제 1: 이름에 따라 클러스터의 모든 Kusto 데이터베이스 나열
```powershell
...
### 예제 2: 이름으로 특정 Kusto 데이터베이스를 얻습니다.
```powershell
...
위의 명령은 리소스 그룹 "testrg"에 있는 클러스터 "testnewkustocluster"에서 "mykustodatabase"라는 Kusto 데이터베이스를 반환합니다.
