...
### 예제 1: 사용 중 데이터베이스 보안 주체 이름의 가용성 확인
```powershell
...
위의 명령은 "myprincipal"이라는 PrincipalAssignment가 "testnewkustocluster" 클러스터의 데이터베이스 "mykustodatabase"에 있는지 여부를 반환합니다.

### 예제 2: 사용되지 않는 데이터베이스 보안 주체 이름의 가용성 확인
```powershell
...
### -Type
리소스 유형, Microsoft.Kusto/Clusters/Databases/principalAssignments.
