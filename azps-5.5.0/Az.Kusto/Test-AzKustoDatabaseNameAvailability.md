...
### 예제 1: 사용 중인 Kusto 데이터베이스 이름의 가용성 확인
```powershell
...
위의 명령은 "mykustodatabase"라는 Kusto 데이터베이스가 "testnewkustocluster" 클러스터에 있는지 여부를 반환합니다.

### 예제 2: 사용되지 않는 Kusto 데이터베이스 이름의 가용성 확인
```powershell
...
위의 명령은 "mykustodatabase2"라는 Kusto 데이터베이스가 "testnewkustocluster" 클러스터에 있는지 여부를 반환합니다.
...
### -Type
리소스 유형(예: Microsoft.Kusto/Clusters/databases)입니다.
