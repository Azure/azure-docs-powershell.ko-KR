...
### -AcceptEULA
AVS의 EULA에 동의하면 이 매개 변수가 제공되지 않은 법적 용어가 팝업됩니다.
...
### -NetworkBlock
주소 블록은 구독의 VNet과 모든 VNet에서 고유해야 합니다.
CIDR 형식이 (A.B.C.D/X)를 준수하는지 확인
...
### -NsxtPassword
선택적으로 사설 클라우드를 만들 때 NSX-T Manager 암호를 설정
...
### -VcenterPassword
선택적으로 사설 클라우드를 만들 때 vCenter 관리자 암호를 설정
