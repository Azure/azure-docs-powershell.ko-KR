...
## 설명
**New-AzDeploymentManagerServiceUnit** cmdlet은 서비스 토폴로지의 서비스 아래에 서비스를 만들고 해당 서비스 단위를 나타내는 개체를 반환합니다.
이름, 서비스 이름, 서비스 토폴로지 및 리소스 그룹 이름으로 서비스 단위를 지정합니다. 

cmdlet은 ServiceUnit 개체를 반환합니다. 이 개체를 로컬로 수정한 다음, Set-AzDeploymentManagerService cmdlet을 사용하여 서비스에 변경 내용을 적용할 수 있습니다.
...
이 cmdlet은 미국 중부 위치에 있는 토폴로지 ContosoServiceTopology의 ContosoService2 서비스 아래에 ContosoResourceGroup에 ContosoService2Storage 이름이 있는 새 서비스 단위를 만듭니다. 템플릿 및 매개 변수 파일은 서비스 토폴로지 ContosoServiceTopology에서 참조되는 아티팩트 원본 위치에 대한 상대 경로로 정의됩니다. 이 템플릿에 정의된 리소스는 배포 모드가 증분으로 설정된 대상 리소스 그룹 service2ResourceGroup에 배포됩니다.
...
이 cmdlet은 미국 중부 위치에 있는 토폴로지 ContosoServiceTopology의 ContosoService2 서비스 아래에 ContosoResourceGroup에 ContosoService2Storage 이름이 있는 새 서비스 단위를 만듭니다. 템플릿 및 매개 변수 참조는 서비스 토폴로지 ContosoServiceTopology1에서 제공되지 않은 아티팩트 원본 ResourceId로 SAS Uri로 제공됩니다. 이 템플릿에 정의된 리소스는 배포 모드가 완료로 설정된 대상 리소스 그룹 service2ResourceGroup에 배포됩니다.
...
### -DeploymentMode
서비스 단위에서 리소스를 배포할 때 사용할 배포 모드입니다.
...
아티팩트 원본과 상대적인 매개 변수 파일의 경로입니다.
ServiceTopology에서 ArtifactSource를 참조해야 합니다.
...
### -ParametersUri
매개 변수 파일에 대한 SAS Uri입니다.
ArtifactSourceId가 ServiceTopology에서 참조된 경우 ParametersArtifactSourceRelativePath를 사용하여 상대 경로를 지정합니다.
...
### -TemplateUri
템플릿 파일에 대한 SAS URI입니다.
ArtifactSourceId가 ServiceTopology에서 참조된 경우 TemplateArtifactSourceRelativePath를 사용하여 상대 경로를 지정합니다.
