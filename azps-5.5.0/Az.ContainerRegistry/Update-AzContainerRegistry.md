...
## 설명
Update-AzContainerRegistry cmdlet은 컨테이너 레지스트리를 업데이트합니다.

## 예제

### 예제 1: 지정된 컨테이너 레지스트리에 대한 관리 사용자 사용
```powershell
...
이 명령을 사용하면 지정된 컨테이너 레지스트리에 대한 관리 사용자가 사용할 수 있습니다.

### 예제 2: 지정된 컨테이너 레지스트리에서 사용하는 저장소 계정 설정
```powershell
...
이 명령은 지정된 컨테이너 레지스트리를 설정하여 동일한 구독에서 기존 저장소 계정 \` mystorageaccount를 \` 사용하게 합니다.
...
### -DisableAdminUser
컨테이너 레지스트리에 대한 관리자 사용자를 사용하도록 설정합니다.
