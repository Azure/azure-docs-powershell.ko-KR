...
## 설명
Invoke-AzSqlElasticPoolFailover cmdlet은 탄력적 풀을 장애 조치합니다. 이 cmdlet을 실행한 후 탄력적 풀의 모든 데이터베이스에서 장애 조치(failover)가 발생합니다.
...
이 명령은 "Server01"이라는 서버에서 "ElasticPool01"이라는 탄력적 풀을 장애 조치합니다.  즉, "ElasticPool01"이라는 탄력적 풀의 모든 데이터베이스에서 장애 조치(failover)가 발생합니다.
...
### -PassThru
성공한 실행 시 true를 반환합니다.  기본적으로 이 cmdlet은 출력을 생성하지 않습니다.
...
### -ServerName
탄력적 풀이 SQL Server Azure 리소스의 이름입니다.
