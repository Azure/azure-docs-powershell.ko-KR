...
## 설명
**Get-AzSqlServerServiceObjective** cmdlet은 Azure SQL 데이터베이스 서버에 사용 가능한 서비스 목표를 얻습니다.
...
이 명령은 Server01이라는 서버에 대한 서비스 목표를 얻습니다.

### 예제 2: 필터링을 사용하여 서비스 목표 얻기
```
...
이 명령은 "시스템"으로 시작하는 Server01이라는 서버에 대한 서비스 목표를 얻습니다.

### 예제 3: 위치에 대한 서비스 목표 얻기
```
...
이 명령은 지정된 Azure 지역에 대한 서비스 목표를 얻습니다.
...
### -ResourceGroupName
리소스 그룹의 이름을 지정합니다.
이 cmdlet은 이 리소스에 할당된 SQL 데이터베이스 서버에 대한 서비스 목표를 얻습니다.
...
Azure SQL Database 서버에 대한 서비스 목표의 이름을 지정합니다.
이 매개 변수에 허용되는 값은 Basic, S0, S1, S2, P1, P2 및 P3입니다.
