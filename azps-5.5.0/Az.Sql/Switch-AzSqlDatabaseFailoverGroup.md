...
AllowDataLoss 매개 변수를 지정하지 않으면 이 명령은 두 역할이 모두 전환될 때까지 대기합니다. AllowDataLoss 매개 변수가 지정된 경우 명령은 새 주에서 해당 역할을 가정할 때까지만 대기합니다.
...
### -ServerName
장애 조치(failover) 그룹의 SQL Azure 데이터베이스 서버의 이름입니다.
