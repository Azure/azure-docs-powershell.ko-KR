...
## 설명
이 명령은 지정된 이름의 인스턴스 장애 조치(failover) 그룹을 제거하고 모든 데이터베이스는 그대로 유지됩니다. 수신기 엔드포인트는 DNS에서 등록되지 않습니다.
