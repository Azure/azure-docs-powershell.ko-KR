...
## 설명
**Get-AzSqlServerAdvisor** cmdlet은 하나 이상의 Azure SQL Server Azure SQL Server.

## 예제

### 예제 1: 서버에 대한 모든 Advisor 나열
```
...
이 명령은 WIRunnersProd라는 리소스 그룹에 속하는 wi-runner-australia-east 서버의 모든 Advisor 목록을 얻습니다.

### 예제 2: 서버에 대한 단일 Advisor를 얻게 됩니다.
```
...
이 명령은 wi-runner-australia-east라는 서버에 대해 CreateIndex라는 Advisor를 얻습니다.

### 예제 3: 응답에 포함된 권장 작업과 함께 모든 Advisor 나열
```
...
이 명령은 wi-runner-australia-east라는 서버에 대한 모든 Advisor를 얻습니다.
이 명령은 *ExpandRecommendedActions* 매개 변수를 사용하기 때문에 cmdlet은 응답에 포함된 Advisor 권장 작업을 얻습니다.

### 예제 4: 응답에 포함된 권장 작업을 통해 단일 Advisor를 얻습니다.
```
...
이 명령은 응답에 포함된 권장 작업을 사용하여 wi-runner-australia-east라는 서버에서 CreateIndex라는 Advisor를 얻습니다.

### 예제 5: 필터링을 사용하여 서버에 대한 모든 Advisor 나열
```
...
이 명령은 문자 "d"로 시작하는 WIRunnersProd라는 리소스 그룹에 속하는 wi-runner-australia-east이라는 서버에 대한 모든 Advisor 목록을 얻습니다.

## PARAMETERS

### -AdvisorName
이 cmdlet이 얻을 수 있는 Advisor의 이름을 지정합니다.
...
cmdlet에 응답에 포함된 Advisor의 권장 작업이 포함되어 있습니다.
...
### -ServerName
이 cmdlet이 요청하는 Advisor의 서버 이름을 지정합니다.
...
## 참고 사항
* 키워드: azure, azurerm, arm, 리소스, 관리, 관리자, sql, 서버, mssql, 관리자
