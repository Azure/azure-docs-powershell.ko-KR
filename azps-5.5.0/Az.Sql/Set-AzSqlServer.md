...
이 명령은 server01이라는 AzureSQL 서버에서 관리자 암호를 다시 설정합니다.

### 예제 2

SQL 데이터베이스 서버의 속성을 수정합니다. (자동Generated)
...
### -PublicNetworkAccess
서버에 대한 공용 네트워크 액세스가 허용될지 여부를 지정하기 위해 플래그를 사용하도록 설정/비활성화합니다.
사용하지 않도록 설정하면 개인 링크를 통해 만들어진 연결만 이 서버에 도달할 수 있습니다.
...
### -ServerVersion
이 cmdlet이 서버를 변경하는 버전을 지정합니다. 이 매개 변수에 허용되는 값은 2.0 및 12.0입니다.
...
### -SqlAdministratorPassword
데이터베이스 서버 관리자에 대한 새 암호를 **SecureString으로** 지정합니다. **SecureString을 얻습니다.** Get-Credential cmdlet을 사용 합니다. 자세한 내용은 `Get-Help
...
### -Tags
이 cmdlet이 서버와 연결한 태그의 사전을 지정합니다. 서버의 태그로 설정된 해시 테이블 형식의 키-값 쌍입니다. 예: @{key0="value0";key1=$null;key2="value2"}
