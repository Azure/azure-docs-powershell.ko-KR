...
## SYNOPSIS
Azure SQL 서버의 Microsoft 지원 작업 감사 설정을 변경합니다.
...
## 설명
**Set-AzSqlServerMSSupportAudit** cmdlet은 Azure SQL 서버의 Microsoft 지원 작업 감사 설정을 변경합니다.
cmdlet을 사용 설정하기 위해 *ResourceGroupName* 및 *ServerName* 매개 변수를 사용하여 서버를 식별합니다.
Blob Storage가 감사 로그의 대상인 경우 *StorageAccountResourceId* 매개 변수를 지정하여 감사 로그에 대한 스토리지 계정을 판단합니다.

## 예제

### 예제 1: Blob Storage 사용 Microsoft는 Azure SQL 서버의 작업 감사 정책을 지원
```powershell
...
### 예제 2: Blob Storage 사용 안 하여 Microsoft는 Azure SQL 서버의 작업 감사 정책을 지원
```powershell
PS C:\>Set-AzSqlServerMSSupportAudit -ResourceGroupName "ResourceGroup01" -ServerName "Server01" -BlobStorageTargetState Disabled
```

### 예제 3: 이벤트 허브를 사용하도록 설정 Microsoft에서 Azure SQL 서버의 작업 감사 정책을 지원
```powershell
...
### 예제 6: 로그 분석 사용 안 하여 Microsoft는 Azure SQL 서버의 작업 감사 정책을 지원합니다.
```powershell
...
### 예제 7: 파이프라인을 통해 비활성화, 로그 분석 Microsoft는 Azure SQL 서버의 작업 감사 정책을 지원합니다.
```powershell
...
### 예제 8: Azure SQL 서버의 레코드를 Blob Storage에 감사하고 로그 분석에 보낼 수 있도록 Microsoft 지원 작업 보내기 사용 안 하도록 설정
```powershell
...
### 예제 9: Azure SQL 서버의 레코드를 Blob Storage, 이벤트 허브 및 로그 분석에 Microsoft 지원 서비스로 보낼 수 있습니다.
```powershell
...
Blob Storage가 Microsoft 지원 작업 감사 레코드의 대상인지 여부를 나타냅니다.
...
### -EventHubTargetState
이벤트 허브가 Microsoft 지원 작업 감사 레코드의 대상인지 여부를 나타냅니다.
...
로그 분석이 Microsoft 지원 작업 감사 레코드의 대상인지 여부를 나타냅니다.
...
### -PassThru
cmdlet 실행이 끝나면 Microsoft 지원 작업 감사 정책을 출력할지 여부를 지정합니다.
...
### -ServerObject
Microsoft 지원 작업 감사 정책을 관리하는 서버 개체입니다.
...
### -WorkspaceResourceId
Microsoft 지원 작업 감사 로그를 보내고자 하는 Log Analytics 작업 영역의 작업 영역 ID(Log Analytics 작업 영역의 리소스 ID)입니다. 예: /subscriptions/4b9e8510-67ab-4e9a-95a9-e2f1e570ea9c/resourceGroups/insights-integration/providers/Microsoft.OperationalInsights/workspaces/viruela2
