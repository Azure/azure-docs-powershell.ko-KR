...
# Az.Sql 모듈
## 설명
이 항목에서는 Azure SQL Database Cmdlet에 대한 도움말 항목을 제공합니다.
...
제공된 Managed Instance에 키 자격 증명 모음 키를 추가합니다. 
...
데이터베이스의 열에 대한 민감도 권장 사항을 사용하지 않도록 설정(해제)합니다.
...
관리되는 인스턴스에서 고급 데이터 보안을 사용하지 않도록 설정합니다.
...
데이터베이스에 대한 고급 위협 방지 설정을 얻습니다.

### [Get-AzSqlDatabaseAdvisor](Get-AzSqlDatabaseAdvisor.md)
Azure SQL Database에 대한 하나 이상의 Advisor를 얻습니다.

### [Get-AzSqlDatabaseAudit](Get-AzSqlDatabaseAudit.md)
Azure SQL 데이터베이스의 감사 설정을 얻습니다.

### [Get-AzSqlDatabaseBackupLongTermRetentionPolicy](Get-AzSqlDatabaseBackupLongTermRetentionPolicy.md)
데이터베이스 장기 보존 정책을 얻습니다.
...
데이터베이스에 대한 데이터 마스킹 정책을 얻습니다.

### [Get-AzSqlDatabaseDataMaskingRule](Get-AzSqlDatabaseDataMaskingRule.md)
데이터베이스에서 데이터 마스킹 규칙을 얻습니다.

### [Get-AzSqlDatabaseExpanded](Get-AzSqlDatabaseExpanded.md)
데이터베이스 및 확장된 속성 값을 얻습니다.
...
Azure SQL Database 가져오기 또는 내보내기 세부 정보를 얻습니다.
...
인스턴스 장애 조치(failover) 그룹을 얻거나 나열합니다.
...
탄력적 풀 Advisor를 사용하여 Azure SQL 권장되는 작업을 하나 이상 얻습니다.
...
Azure 관리되는 인스턴스 데이터베이스에서 권장되는 정보 유형 및 민감도 레이블을 Azure SQL 수 있습니다.
...
Azure 리소스에 대한 하나 이상의 Advisor를 SQL Server.

### [Get-AzSqlServerAudit](Get-AzSqlServerAudit.md)
Azure SQL 서버의 감사 설정을 얻습니다.
...
Azure SQL Server Advisor에 대해 권장되는 하나 이상의 작업을 얻습니다.

### [Get-AzSqlServerServiceObjective](Get-AzSqlServerServiceObjective.md)
Azure SQL Database 서버에 대한 서비스 목표를 얻습니다.
...
Virtual Network 규칙을 사용하여 Azure SQL Server 나열합니다.
...
Azure SQL 데이터베이스 동기화 그룹에 대한 정보를 반환합니다.

### [Get-AzSqlSyncGroupLog](Get-AzSqlSyncGroupLog.md)
Azure SQL 동기화 그룹의 로그를 반환합니다.

### [Get-AzSqlSyncMember](Get-AzSqlSyncMember.md)
Azure SQL 데이터베이스 동기화 멤버에 대한 정보를 반환합니다.
...
Azure SQL Virtual Cluster에 대한 정보를 반환합니다.
...
기존 데이터베이스에 대한 보조 데이터베이스를 만들고 데이터 복제를 시작합니다.
...
Azure SQL Database Managed Instance를 만듭니다.

### [New-AzSqlInstanceDatabase](New-AzSqlInstanceDatabase.md)
Azure SQL Managed Instance 데이터베이스를 만듭니다.
...
두 데이터베이스 서버 간의 탄력적 데이터베이스 트랜잭션에 대한 SQL 만듭니다.
...
Azure SQL Server Virtual Network 규칙을 만듭니다. 
...
Azure SQL Database Advisor의 자동 실행 상태를 수정합니다.
...
탄력적 풀 권장 SQL Azure의 상태를 업데이트합니다.

### [Set-AzSqlInstance](Set-AzSqlInstance.md)
Azure SQL Database Managed Instance에 대한 속성을 지정합니다.
...
**Set-AzSqlInstanceDatabaseLongTermRetentionBackup** cmdlet은 관리되는 데이터베이스의 장기 보존 정책을 설정합니다.
...
Azure SQL 관리되는 인스턴스 데이터베이스에서 열의 정보 유형 및 민감도 레이블을 지정합니다.
...
Azure SQL Server Advisor의 자동 실행 상태를 업데이트합니다.

### [Set-AzSqlServerAudit](Set-AzSqlServerAudit.md)
Azure SQL 서버의 감사 설정을 변경합니다.
...
서버의 취약성 평가 설정을 업데이트합니다.

### [Update-AzSqlSyncGroup](Update-AzSqlSyncGroup.md)
Azure SQL 데이터베이스 동기화 그룹을 업데이트합니다.

### [Update-AzSqlSyncMember](Update-AzSqlSyncMember.md)
Azure SQL 데이터베이스 동기화 멤버를 업데이트합니다.

### [Update-AzSqlSyncSchema](Update-AzSqlSyncSchema.md)
동기화 멤버 데이터베이스 또는 동기화 허브 데이터베이스에 대한 동기화 SCHEMA를 업데이트합니다.
실제 데이터베이스에서 최신 데이터베이스를 다운로드한 다음 동기화 메타데이터 데이터베이스에서 캐시한 스마마를 새로 고쳐 사용합니다.
"SyncMemberName"이 지정되어 있는 경우 멤버 데이터베이스를 새로 고침합니다. 그렇지 않은 경우 허브 데이터베이스 스마마를 새로 고침합니다.
