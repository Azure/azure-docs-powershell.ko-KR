...
## 설명
**Get-AzSqlDatabaseTransparentDataEncryptionActivity** cmdlet은 Azure SQL 데이터베이스의 TDE(투명한 데이터 암호화) 검사의 진행률을 얻습니다.
암호화 범위가 실행되고 있는 것이 없는 경우 이 cmdlet은 빈 목록을 반환합니다.
...
### -DatabaseName
이 cmdlet이 TDE 암호화 활동을 얻을 데이터베이스의 이름을 지정합니다.
