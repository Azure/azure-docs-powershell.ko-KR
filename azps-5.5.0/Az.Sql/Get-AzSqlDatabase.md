...
## 설명
**Get-AzSqlDatabase** cmdlet은 Azure SQL Database Server에서 하나 이상의 Azure SQL 데이터베이스를 얻습니다.
이 cmdlet은 Azure의 SQL Server Stretch Database 서비스에서도 지원됩니다.

## 예제

### 예제 1: 서버에서 모든 데이터베이스를 얻습니다.
```
...
이 명령은 server01이라는 서버의 모든 데이터베이스를 얻습니다.

### 예제 2: 서버에서 이름으로 데이터베이스를 얻습니다.
```
...
이 명령은 Server01이라는 서버에서 Database02라는 데이터베이스를 얻습니다.

### 예제 3: 필터링을 사용하여 서버의 모든 데이터베이스를 얻습니다.
```
...
이 명령은 "database"로 시작하는 server01이라는 서버의 모든 데이터베이스를 얻습니다.
