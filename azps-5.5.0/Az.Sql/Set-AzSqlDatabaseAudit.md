...
### 예제 3: T-SQL를 사용하여 필터링을 통해 Azure SQL 데이터베이스의 Blob Storage 감사 정책을 사용하도록 SQL 있습니다.
```powershell
...
RECEIVE  
참조  
감사할 작업을 정의하는 일반적인 양식은 [action] ON [object] BY [principal] 위 형식의 [object]는 테이블, 보기 또는 저장 프로시저와 같은 개체 또는 전체 데이터베이스 또는 schema를 참조할 수 있습니다. 후자의 경우 각각 DATABASE::[dbname] 및 SCHEMA::[schemaname] 양식이 사용됩니다.
예를 들어:  
