...
## 설명
**Set-AzSqlDatabaseAdvisorAutoExecuteStatus** cmdlet은 Azure SQL Database Advisor에 대한 자동 실행 속성을 수정합니다.
현재 이 cmdlet은 Enabled, Disabled 및 Default 값을 지원합니다.
...
이 명령은 CreateIndex라는 Advisor의 자동 실행 상태를 Enabled로 변경합니다.

## PARAMETERS

### -AdvisorName
이 cmdlet이 상태를 수정하는 Advisor의 이름을 지정합니다.
...
### -DatabaseName
이 cmdlet이 상태를 수정하는 데이터베이스의 이름을 지정합니다.
