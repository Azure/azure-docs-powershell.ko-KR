...
## 설명
**Get-AzSqlDatabaseDataMaskingRule** cmdlet은 특정 데이터 마스킹 규칙 또는 Azure SQL 데이터베이스에 대한 모든 데이터 마스킹 규칙을 얻습니다.
cmdlet을 사용하려면 *ResourceGroupName,* *ServerName* 및 *DatabaseName* 매개 변수를 사용하여 데이터베이스를 식별하고 *RuleId* 매개 변수를 사용하여 이 cmdlet이 반환하는 규칙을 지정합니다.
*RuleId를* 제공하지 않는 경우 해당 Azure SQL 마스킹 규칙이 반환됩니다.
이 cmdlet은 Azure의 SQL Server Stretch Database 서비스에서도 지원됩니다.

## 예제

### 예제 1: 데이터베이스에서 모든 데이터 마스킹 규칙 사용
```
...
### 예제 2: "dbo", 테이블 "table1" 및 열 "column1"에 정의된 데이터 마스킹 규칙을 얻습니다.
```
