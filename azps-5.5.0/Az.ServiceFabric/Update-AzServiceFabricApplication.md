...
모니터링되는 업그레이드에 사용할 기본 서비스 유형에 대한 상태 정책에서 허용하는 서비스당 최대 미지정 파티션 비율을 지정합니다.
...
모니터링되는 업그레이드에 사용할 기본 서비스 유형에 대한 상태 정책에서 허용하는 서비스당 최대 미지정 복제본의 백분율을 지정합니다.
...
이전 상태 검사가 실패하면 서비스 패브릭이 상태 검사를 다시 검색하는 기간(초)을 지정합니다.
...
서로 다른 서비스 유형에 사용할 상태 정책의 맵을 @"ServiceTypeName" @ {"ServiceTypeName" : "MaxPercentUnhealthyPartitionsPerService,MaxPercentUnhealthyReplicasPerPartition,MaxPercentUnhealthyServices"}}으로 지정합니다.
