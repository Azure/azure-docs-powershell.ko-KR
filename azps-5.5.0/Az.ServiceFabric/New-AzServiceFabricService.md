...
## 설명
이 cmdlet을 사용하면 지정된 애플리케이션에서 상태 비장 또는 상태 비대화 서비스를 만들 수 있습니다. 서비스는 애플리케이션 매니페스트에서 종료해야 합니다. 형식은 매니페스트의 형식과 동일해야 합니다. 애플리케이션 이름은 서비스 이름의 prefix입니다.
...
### -DefaultMoveCost
이동에 대한 기본 비용을 지정합니다.
비용이 높을수록 클러스터의 균형을 조정하려고 할 때 Cluster Resource Manager가 복제본을 이동할 가능성이 줄어 니다.
