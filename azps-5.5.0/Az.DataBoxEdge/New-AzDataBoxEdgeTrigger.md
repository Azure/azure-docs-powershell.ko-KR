...
### -FileEvent
이 스위치 매개 변수를 전달하여 FileEvent 트리거 구성
...
### -Schedule
Timer 이벤트를 발생해야 하는 주기적 빈도입니다. 일(1~365 사이), 시간(1~23 사이) 또는 분(1~59 사이)으로 일정을 지정합니다.
...
### -StartTime
유효한 트리거를 유발하는 하루의 시간입니다. 일정은 최대 초까지 지정된 시간을 참조하여 계산됩니다. timezone이 지정되지 않은 경우 시간은 디바이스 시간제에 있는 것으로 간주됩니다. 값은 항상 UTC 시간으로 반환됩니다.
