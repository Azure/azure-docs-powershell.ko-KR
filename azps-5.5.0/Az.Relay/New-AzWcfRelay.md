...
## 설명
New-AzWcfRelay cmdlet은 지정된 Relay 네임스페이스에 WcfRelay를 만듭니다.
...
지정된 Relay 네임스페이스 \` \` \` TestNameSpace-Relay에 새 WcfRelay TestWCFRelay2를 \` 만듭니다.
...
지정된 Relay 네임스페이스 \` \` \` TestNameSpace-Relay1에 새 WcfRelay TestWCFRelay를 \` 만듭니다.
...
이 릴레이에 클라이언트 권한 부여가 필요한 경우 true입니다. 그렇지 않으면 false
...
이 릴레이에 전송 보안이 필요한 경우 true입니다. 그렇지 않으면 false
