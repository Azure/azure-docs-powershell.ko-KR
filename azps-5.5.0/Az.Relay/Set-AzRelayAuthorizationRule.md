...
## 설명
**Set-AzRelayAuthorizationRule** cmdlet은 지정된 Relay 엔터티(네임스페이스/WcfRelay/HybridConnection)의 지정된 권한 부여 규칙에 대한 설명을 업데이트합니다.

## 예제

### 예제 1.1 - InputObject가 있는 네임스페이스
```
PS C:\>
...
**네임스페이스에서** 권한 부여 규칙의 액세스 권한에서 `AuthoRule1` 보내기 `TestNameSpace-Relay1` 추가.

### 예제 1.2 - Rights 매개 변수가 있는 네임스페이스
```
...
### 예제 2.1 - InputObject를 사용하여 WcfRelay
```
...
WcfRelay의 권한 부여 규칙에 대한 액세스 권한에  `AuthoRule1` Send를 추가합니다. `TestWCFRelay1`

### 예제 2.2 - Rights 매개 변수가 있는 WcfRelay
```
...
### 예제 3.1 - InputObject를 사용하여 HybridConnection
```
...
HybridConnection의 권한 부여 규칙에 대한 액세스 권한에  `AuthoRule1` Send를 추가합니다. `TestHybridConnection`

### 예제 3.2 - Rights 매개 변수를 사용하는 HybridConnection
```
