...
## 설명
**New-AzRecoveryServicesBackupProtectionPolicy** cmdlet은 자격 증명 모음에 Backup 보호 정책을 만듭니다.
...
첫 번째 명령은 **기본 SchedulePolicyObject를** 얻은 다음, $SchPol 변수에 저장합니다.
두 번째 명령은 일정 정책에서 예약된 모든 실행 시간을 $SchPol.
세 번째 명령은 Get-Date cmdlet을 사용하여 현재 날짜 및 시간을 얻습니다.
네 번째 명령은 예약된 실행 시간으로 $Dt 시간의 현재 날짜 및 시간을 일정 정책에 추가합니다.
다섯 번째 명령은 기본 **RetentionPolicy** 개체를 구한 다음, $RetPol 변수에 저장합니다.
여섯 번째 명령은 보존 기간 정책을 365일로 설정합니다.
마지막 명령은 이전 명령에서 만든 일정 및 보존 정책에 따라 **BackupProtectionPolicy** 개체를 만듭니다.

### 예제 2

Backup 보호 정책을 만듭니다. (자동Generated)
