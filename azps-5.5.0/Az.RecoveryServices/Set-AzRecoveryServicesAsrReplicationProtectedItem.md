...
### -LicenseType
Windows Server 가상 머신에 사용할 라이선스 유형 선택을 정합니다. 마이그레이션에 Azure HUB(Hybrid Use Benefit)를 사용할 자격이 있으며 이 보호된 항목을 장애 조치하는 동안 허브 설정을 사용할 수 있도록 지정하려는 경우 라이선스 유형을 WindowsServer로 설정하세요.
...
### -Name
장애 조치 시 만들 복구 가상 머신의 이름을 지정합니다.
...
장애 조치(failover) 후 복제 보호 항목에 대한 가용성 영역 지정
