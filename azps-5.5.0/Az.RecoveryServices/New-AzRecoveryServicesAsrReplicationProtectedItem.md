...
### -AzureToAzure
스위치 매개 변수는 Azure에서 Azure 시나리오로 복제된 항목을 만드는 것이 지정됩니다.
...
Azure에서 Azure로 재해 복구 시나리오에 사용할 디스크 구성을 지정합니다.
...
### -AzureVmId
복구 지역의 재해 복구 보호를 위해 Azure VM ID를 지정합니다.
...
### -UseManagedDisk
장애 조치(failover)에 생성된 Azure 가상 머신이 관리 디스크를 사용할지 지정합니다. True 또는 False를 허용합니다.
...
### -WaitForCompletion
반환하기 전에 cmdlet이 작업 완료를 대기해야 한다고 지정합니다.
