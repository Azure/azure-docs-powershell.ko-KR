...
## 설명
**Export-AzApiManagementApi** cmdlet은 Azure API Management API를 지원되는 형식 중 하나의 파일로 내보낼 수 있습니다.
...
이 명령은 API를 WADL 파일로 내보낼 수 있습니다.

### 예제 2: OpenApi 3.0 사양 형식으로 Json 문서로 API 내보내기
```powershell
PS C:\>$ApiMgmtContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
이 명령은 Api 정의를 Open Api 형식으로 Json 문서로 내보낼 수 있습니다.
...
### -ApiRevision
API 개정의 식별자입니다. 이 매개 변수는 선택 사항입니다. 지정하지 않으면 현재 활성 API 개정에 대해 내보내기 수행됩니다.
...
### -Force
이 작업이 이미 있는 경우 동일한 이름의 파일을 덮어써야 를 나타냅니다.
...
### -PassThru
API를 성공적으로 내보내거나 $True 경우 이 작업이 $False 나타냅니다.
...
### -SpecificationFormat
API 형식을 지정합니다.
psdx_paramvalues Wadl, Wsdl, Swagger, OpenApi 및 OpenApiJson
