...
## 설명
**New-AzApiManagementOperation** cmdlet은 API 작업을 생성합니다.
...
### 예제 2: 요청 및 응답 세부 정보를 사용하여 API 관리 작업 만들기
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
이 예제에서는 요청 및 응답 세부 정보를 통해 API 관리 작업을 만듭니다.

## PARAMETERS

### -ApiId
API 관리 작업의 식별자를 지정합니다.
...
### -ApiRevision
API 개정의 식별자입니다. 이 매개 변수는 선택 사항입니다. 지정하지 않으면 작업이 현재 활성 API 개정에 연결됩니다.
...
### -Responses
가능한 API Management 작업 응답의 배열을 지정합니다.
...
### -TemplateParameters
UrlTemplate 매개 변수에 정의된 매개 변수 *배열을 지정합니다.*
이 매개 변수를 지정하지 않으면 *UrlTemplate에 따라 기본값이 생성됩니다.*
