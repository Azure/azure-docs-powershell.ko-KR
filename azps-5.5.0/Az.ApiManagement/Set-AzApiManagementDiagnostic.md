...
## 설명
**Set-AzApiManagementDiagnostic** cmdlet은 전역 또는 API 범위에 구성된 진단을 업데이트합니다.
...
전역 또는 API 범위에서 API Management 진단을 수정합니다. (자동Generated)
...
### -AlwaysLog
적용하지 말아야 하는 메시지 샘플링 설정 유형을 지정합니다.
이 매개 변수는 선택 사항입니다.
...
### -BackendSetting
백end로 들어오는/발신 Http 메시지에 대한 진단 설정입니다. 이 매개 변수는 선택 사항입니다.
...
### -FrontEndSetting
게이트웨이로 들어오는/보내기 Http 메시지에 대한 진단 설정입니다. 이 매개 변수는 선택 사항입니다.
...
### -PassThru
지정된 경우 진단 집합을 나타내는 Microsoft.Azure.Commands.ApiManagement.ServiceManagement.Models.PsApiManagementDiagnostic 형식의 인스턴스입니다.
