...
## 설명
**Get-AzApiManagementDiagnostic은** 주어진 범위에서 Api Management 서비스에 구성된 진단의 세부 정보를 얻습니다.
...
이 명령은 Api Management 서비스에 구성된 모든 진단을 얻습니다.
...
이 명령은 API 범위에 구성된 모든 `echo-api` 진단을 얻습니다.

### 예제 3: ID로 지정된 API 범위 진단을 얻게 됩니다.
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
이 명령은 `applicationinsights` API에 구성된 진단을 `echo-api` 얻습니다.
...
### -ResourceId
진단 또는 Api 진단의 Arm 리소스 식별자입니다. 지정된 경우 식별자에 의해 진단을 찾으려고 합니다. 이 매개 변수는 필수입니다.
