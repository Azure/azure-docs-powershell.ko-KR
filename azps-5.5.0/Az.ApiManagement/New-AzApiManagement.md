...
이 명령은 유럽 서부에서 사용하도록 설정된 클라이언트 인증서를 사용하여 소비 계층 API Management 서비스를 만듭니다.
...
이 명령은 표준 SKU Api Management 서비스를 만들고, ApiManagement 게이트웨이와 백 엔드 간에 프런트 엔드 클라이언트에서 TLS 1.0을 사용하도록 설정하고 ApiManagement 게이트웨이와 백 엔드 클라이언트를 연결합니다.
...
### -Organization
조직의 이름을 지정합니다.
API Management는 개발자 포털의 전자 메일 알림에서 이 주소를 사용 합니다.
...
### -VirtualNetwork
마스터 Azure API Management 배포 지역의 Virtual Network 구성
