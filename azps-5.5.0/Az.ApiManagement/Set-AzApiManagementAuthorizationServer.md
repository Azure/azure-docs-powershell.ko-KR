...
클라이언트를 권한 부여 서버에 등록하고 클라이언트 자격 증명을 얻기 위한 클라이언트 등록 엔드포인트를 지정합니다.
...
### -TokenEndpointUrl
권한 부여 또는 새로 고침 토큰을 제시하는 대신 액세스 토큰을 얻기 위해 클라이언트에 대한 토큰 엔드포인트를 지정합니다.
