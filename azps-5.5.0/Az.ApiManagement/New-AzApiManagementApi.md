...
### 예제 2: echo-api 및 ApiVersionSet에 모든 작업, 태그, 제품 및 정책을 복사하여 API 만들기
```powershell
...
이 명령은 ApiVersionSet에서 API를 만들고 원본 API에서 모든 `echoapiv3` `xmsVersionSet` 작업, 태그 및 정책을 `echo-api` 복사합니다. SubscriptionRequired, ServiceUrl, Path, Protocols를 오버라이드합니다.

### 예제 3

API를 만듭니다. (자동Generated)
...
### -ApiId
만들 API의 ID를 지정합니다.
이 매개 변수를 지정하지 않으면 이 cmdlet에서 ID를 생성합니다.
...
### -Path
API의 공용 URL의 마지막 부분인 웹 API 경로를 지정하고 관리 포털의 Web API URL 접미사 필드에 해당합니다.
...
### -SourceApiRevision
원본 API의 API 개정입니다. 이 매개 변수는 선택 사항입니다.
