...
## SYNOPSIS
애플리케이션 게이트웨이 SSL 프로필의 SSL 정책을 수정합니다.
...
## 설명
**Set-AzApplicationGatewaySslProfilePolicy** cmdlet은 애플리케이션 게이트웨이 SSL 프로필의 SSL 정책을 수정합니다.
...
첫 번째 명령은 ResourceGroup01이라는 리소스 그룹에 ApplicationGateway01이라는 애플리케이션 게이트웨이를 $AppGw 변수에 저장합니다. 두 번째 명령은 SslProfile01 for $AppGw ssl 프로필을 $profile 변수에 저장합니다. 마지막 명령은 SSL에 저장된 ssl 프로필 개체의 ssl 정책을 $profile.
