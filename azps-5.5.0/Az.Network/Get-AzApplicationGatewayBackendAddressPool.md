...
## 설명
**Get-AzApplicationGatewayBackendAddressPool** cmdlet은 애플리케이션 게이트웨이에서 하나 이상의 백end 주소 풀 구성을 얻습니다.

## 예제

### 예제 1: 지정된 백 엔드 서버 풀을 얻게 됩니다.
```
...
두 번째 명령은 Pool01이라는 $AppGw 연결된 백 엔드 주소 풀을 $BackendPool 변수에 저장합니다.

### 예제 2: 백 엔드 서버 풀 목록 얻기
```
PS C:\>$AppGw = Get-AzApplicationGateway -Name "ApplicationGateway01" -ResourceGroupName "ResourceGroup01"
...
첫 번째 명령은 ResourceGroup01이라는 리소스 그룹에 ApplicationGateway01이라는 애플리케이션 게이트웨이를 $AppGw 변수에 저장합니다.
두 번째 명령은 $AppGw 연결된 백 엔드 주소 풀 목록을 $BackendPools 변수에 저장합니다.

## PARAMETERS

### -ApplicationGateway
**Get-AzApplicationGatewayBackendAddressPool** cmdlet은 애플리케이션 게이트웨이에 대한 백 엔드 주소 풀을 얻습니다.
...
### -Name
이 cmdlet에서 얻을 백 엔드 주소 풀의 이름을 지정합니다.
