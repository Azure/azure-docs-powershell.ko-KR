...
### ByVpnGatewayNameByVpnSiteObject(기본값)
```
...
위에서는 Azure의 "testRG" 리소스 그룹에 미국 서부에 VpnSiteLinks 1개가 있는 리소스 그룹, Virtual WAN, Virtual Network, Virtual Hub 및 VpnSite를 만들 것입니다.
VPN 게이트웨이는 가상 허브에서 이후에 생성됩니다.
게이트웨이가 생성되면 VpnSiteLink에 대한 vpnSiteLink에 New-AzVpnConnection 1개와 함께 New-AzVpnConnection 명령을 사용하여 VpnSite에 연결됩니다.
...
연결을 시작하는 동안 로컬 Azure IP 주소를 원본 주소로 사용하세요.
...
### -VpnSite
이 허브 가상 네트워크 연결이 연결된 원격 VPN 사이트입니다.
...
이 VpnConnection에 있는 VpnSiteLinkConnections 목록입니다.
