...
이 예제에서는 새 가상 네트워크 MyVNET의 서브넷에 속하는 새 네트워크 인터페이스 MyNetworkInterface를 생성합니다. 또한 네트워크 인터페이스의 IP 구성과 연결하기 위해 빈 애플리케이션 보안 그룹 MyASG를 만들 수도 있습니다. 두 개체가 모두 만들어지면 기본 IP 구성을 MyASG 개체에 연결합니다. 마지막으로 애플리케이션 보안 그룹 개체에 연결된 네트워크 인터페이스에서 새 IP 구성을 만듭니다.
...
이 네트워크 인터페이스 IP 구성이 속한 부하 균형 조정기 백드 주소 풀 참조의 컬렉션을 지정합니다.
