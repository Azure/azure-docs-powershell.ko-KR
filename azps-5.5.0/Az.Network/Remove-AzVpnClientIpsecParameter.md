...
**Remove-AzVpnClientIpsecParameter** cmdlet은 Virtual Network 게이트웨이에 설정된 VPN 사용자 지정 ipsec 매개 변수를 제거합니다. 이 매개 변수는 차례로 전달된 VirtualNetworkGateway 이름 및 리소스 그룹 이름을 기반으로 VPN Gateway에서 기본 vpn ipsec 정책을 설정합니다.

## 예제

### 예제 1: Virtual Network 게이트웨이에 설정된 vpn ipsec 매개 변수 집합을 삭제합니다.
```powershell
