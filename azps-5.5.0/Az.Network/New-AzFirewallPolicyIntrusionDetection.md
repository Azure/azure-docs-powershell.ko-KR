...
## SYNOPSIS
방화벽 정책과 연결하기 위해 새 Azure Firewall 정책 침입 검색을 만듭니다.
...
## 설명
**New-AzFirewallPolicyIntrusionDetection** cmdlet은 Azure Firewall 정책 침입 감지 개체를 만듭니다.

## 예제

### 예제 1: 1. 모드로 침입 감지 만들기
```powershell
PS C:\> New-AzFirewallPolicyIntrusionDetection -Mode "Alert"
```

이 예제에서는 경고(검색) 모드로 침입 감지를 만듭니다.

### 예제 2: 2. 서명 오버라이드를 사용하여 침입 감지 만들기
```powershell
PS C:\> $signatureOverride = New-AzFirewallPolicyIntrusionDetectionSignatureOverride -Id "123456798" -Mode "Deny"
PS C:\> New-AzFirewallPolicyIntrusionDetection -Mode "Alert" -SignatureOverride $signatureOverride
```

이 예제에서는 특정 서명을 오버라이드하여 침입 감지를 만듭니다.

### 예제 3: 3. 트래픽 우회 설정으로 구성된 침입 감지를 사용하여 방화벽 정책 만들기
```powershell
...
이 예제에서는 트래픽 우회 설정을 사용하여 침입 감지를 만듭니다.

## PARAMETERS

### -BypassTraffic
우회할 트래픽에 대한 규칙 목록입니다.
