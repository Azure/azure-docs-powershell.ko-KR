...
애플리케이션 게이트웨이에 요청 라우팅 규칙을 추가합니다.

### [Add-AzApplicationGatewayRewriteRuleSet](Add-AzApplicationGatewayRewriteRuleSet.md)
애플리케이션 게이트웨이에 다시 사용 규칙 집합을 추가합니다.
...
애플리케이션 게이트웨이에 신뢰할 수 있는 루트 인증서를 추가합니다.
...
ExpressRoute 회로에 피어링 구성을 추가합니다.
...
가상 머신 명령에 매개 변수로 전달될 수 있는 VirtualHubRoute Add-AzVirtualHubRouteTable 만듭니다. 

### [Add-AzVirtualHubRouteTable](Add-AzVirtualHubRouteTable.md)
VirtualHub의 자식인 Virtual Hub 경로 테이블 리소스를 만듭니다.

### [Add-AzVirtualNetworkGatewayIpConfig](Add-AzVirtualNetworkGatewayIpConfig.md)
가상 네트워크 게이트웨이에 IP 구성을 추가합니다.
...
Application Gateway의 자동 조정 구성을 얻습니다.
...
자동 승인을 통해 개인 끝점에 연결할 수 있는 개인 링크 서비스 ID의 배열을 얻습니다.

### [Get-AzAvailablePrivateEndpointType](Get-AzAvailablePrivateEndpointType.md)
위치에서 사용 가능한 개인 끝점 유형을 반환합니다.
...
지역에서 사용 가능한 서비스 위임을 얻습니다.

### [Get-AzBastion](Get-AzBastion.md)
배스션 리소스 또는 배스션 리소스를 얻습니다.
...
ExpressRoute 회로의 사용량 통계를 얻습니다.

### [Get-AzExpressRouteConnection](Get-AzExpressRouteConnection.md)
이름으로 ExpressRoute 연결을 얻거나 ExpressRouteGateway에 연결된 모든 ExpressRoute 연결을 나열합니다.
...
ResourceGroupName 및 GatewayName을 사용하여 ExpressRouteGateway 리소스를 얻거나 ResourceGroupName 또는 SubscriptionId에 의해 모든 게이트웨이를 나열합니다.
...
ExpressRoutePort에 할당된 ID를 얻습니다.
...
부하 균형 조정에 대한 백end 주소 풀 구성을 얻습니다.
...
부하 균형 조정에 대한 인바운드 NAT 규칙 구성을 얻습니다.

### [Get-AzLoadBalancerOutboundRuleConfig](Get-AzLoadBalancerOutboundRuleConfig.md)
부하 균형 조정기에서 아웃바운드 규칙 구성을 얻습니다.

### [Get-AzLoadBalancerProbeConfig](Get-AzLoadBalancerProbeConfig.md)
부하 균형 조정에 대한 프로브 구성을 얻습니다.

### [Get-AzLoadBalancerRuleConfig](Get-AzLoadBalancerRuleConfig.md)
부하 균형 조정에 대한 규칙 구성을 얻습니다.

### [Get-AzLocalNetworkGateway](Get-AzLocalNetworkGateway.md)
로컬 네트워크 게이트웨이를 얻습니다.

### [Get-AzNatGateway](Get-AzNatGateway.md)
이름 또는 NatGateway ID 또는 리소스 그룹의 모든 Nat Gateway 리소스에 따라 리소스 그룹의 Nat Gateway 리소스를 얻습니다.
...
지정된 이름 또는 연결 모니터 목록이 있는 연결 모니터를 반환합니다.

### [Get-AzNetworkWatcherConnectionMonitorReport](Get-AzNetworkWatcherConnectionMonitorReport.md)
가장 최근 연결 상태의 스냅숏을 쿼리합니다.

### [Get-AzNetworkWatcherFlowLog](Get-AzNetworkWatcherFlowLog.md)
지정된 구독 및 지역의 흐름 로그 리소스 또는 흐름 로그 리소스 목록을 얻습니다.

### [Get-AzNetworkWatcherFlowLogStatus](Get-AzNetworkWatcherFlowLogStatus.md)
리소스에 대한 흐름 로깅 상태를 얻습니다.
...
지정된 Azure 지역에 사용 가능한 모든 인터넷 서비스 공급자를 나열합니다.

### [Get-AzNetworkWatcherReachabilityReport](Get-AzNetworkWatcherReachabilityReport.md)
인터넷 서비스 공급자의 상대적 대기 시간 점수를 지정된 위치에서 Azure 지역으로 얻습니다.

### [Get-AzNetworkWatcherSecurityGroupView](Get-AzNetworkWatcherSecurityGroupView.md)
VM에 적용된 구성된 유효 네트워크 보안 그룹 규칙을 확인합니다.

### [Get-AzNetworkWatcherTopology](Get-AzNetworkWatcherTopology.md)
리소스 그룹에서 리소스 및 리소스의 관계에 대한 네트워크 수준 보기를 얻습니다.

### [Get-AzNetworkWatcherTroubleshootingResult](Get-AzNetworkWatcherTroubleshootingResult.md)
이전에 실행했거나 현재 실행 중인 문제 해결 작업에서 문제 해결 결과를 얻습니다.

### [Get-AzP2sVpnGateway](Get-AzP2sVpnGateway.md)
VirtualHub에서 기존 P2SVpnGateway를 얻습니다.
...
이름 및 ResourceGroupName으로 Azure VirtualHub를 얻거나 ResourceGroupName/Subscription에 의해 모든 Virtual Hubs를 나열합니다.

### [Get-AzVirtualHubRouteTable](Get-AzVirtualHubRouteTable.md)
가상 허브에서 가상 허브 경로 테이블을 얻거나 가상 허브의 모든 경로 테이블을 나열합니다.

### [Get-AzVirtualHubVnetConnection](Get-AzVirtualHubVnetConnection.md)
가상 허브에서 Virtual Network 연결을 얻거나 가상 허브의 모든 가상 네트워크 연결을 나열합니다.

### [Get-AzVirtualNetwork](Get-AzVirtualNetwork.md)
리소스 그룹의 가상 네트워크를 얻습니다.

### [Get-AzVirtualNetworkAvailableEndpointService](Get-AzVirtualNetworkAvailableEndpointService.md)
위치에 사용 가능한 엔드포인트 서비스를 나열합니다.

### [Get-AzVirtualNetworkGateway](Get-AzVirtualNetworkGateway.md)
Virtual Network 게이트웨이를 얻습니다.
...
Azure 가상 네트워크 게이트웨이의 BGP 피어를 나열합니다.
...
ResourceGroupName 및 GatewayName을 사용하여 VpnGateway 리소스를 얻거나 ResourceGroupName 또는 SubscriptionId에 의해 모든 게이트웨이를 나열합니다.
...
대상 리소스에서 지정된 네트워크 프로필에 대한 네트워크 구성 진단 세션을 호출합니다.
...
Application Gateway에 대한 자동 조정 구성을 만듭니다.
...
방화벽 정책에 대한 ManagedRules를 생성합니다.
...
RuleGroupOverrideGroup 항목에 대한 managedRuleOverride 항목을 만듭니다.
...
애플리케이션 게이트웨이에 대한 프런트 엔드 포트를 만듭니다.
...
애플리케이션 게이트웨이에 대한 ID 개체를 만듭니다. 사용자 할당 ID에 대한 참조를 보유합니다.
...
애플리케이션 게이트웨이에 대한 상태 프로브에서 사용되는 상태 프로브 응답 일치를 만듭니다.
...
백end 서버 풀에 대한 URL 경로 매핑 배열을 만듭니다.
...
ExpressRoute 회로에 추가할 새 피어링 구성을 만듭니다.
...
확장 가능한 ExpressRoute 게이트웨이를 만듭니다.
...
새 Azure Firewall 정책 네트워크 규칙 만들기
...
부하 균형 조정에 대한 백end 주소 풀 구성을 만듭니다.
...
부하 균형 조정에 대한 규칙 구성을 만듭니다.

### [New-AzLocalNetworkGateway](New-AzLocalNetworkGateway.md)
로컬 네트워크 게이트웨이를 만듭니다.

### [New-AzNatGateway](New-AzNatGateway.md)
공용 IP 주소/공용 IP Prefix, IdleTimeoutInMinutes 및 SKU 속성을 사용하여 새 Nat Gateway 리소스를 만들 수 있습니다.
...
새 네트워크 구성 진단 프로필 개체를 만듭니다. 이 개체는 지정된 조건을 사용하여 진단 세션 중에 네트워크 구성을 제한하는 데 사용됩니다.
...
사설 DNS 영역 그룹의 DNS 영역 구성을 만듭니다.

### [New-AzPrivateDnsZoneGroup](New-AzPrivateDnsZoneGroup.md)
지정된 개인 엔드포인트에 개인 DNS 영역 그룹을 만듭니다.
...
Virtual Network 게이트웨이를 만듭니다.

### [New-AzVirtualNetworkGatewayConnection](New-AzVirtualNetworkGatewayConnection.md)
가상 네트워크 게이트웨이와 프레미스 VPN 디바이스 간에 사이트 간 VPN 연결을 만듭니다.

### [New-AzVirtualNetworkGatewayIpConfig](New-AzVirtualNetworkGatewayIpConfig.md)
Virtual Network 게이트웨이에 대한 IP 구성을 만듭니다.
...
서비스 엔드포인트 정책 정의를 제거합니다.

### [Remove-AzVirtualHub](Remove-AzVirtualHub.md)
Azure VirtualHub 리소스를 제거합니다.

### [Remove-AzVirtualHubRouteTable](Remove-AzVirtualHubRouteTable.md)
가상 허브와 연결된 가상 허브 경로 테이블 리소스를 삭제합니다.

### [Remove-AzVirtualHubVnetConnection](Remove-AzVirtualHubVnetConnection.md)
이 Remove-AzVirtualHubVnetConnection cmdlet은 허브 VNET에 원격 VNET을 피어링하는 Azure Virtual Network 연결을 제거합니다.

### [Remove-AzVirtualNetwork](Remove-AzVirtualNetwork.md)
가상 네트워크를 제거합니다.

### [Remove-AzVirtualNetworkGateway](Remove-AzVirtualNetworkGateway.md)
Virtual Network 게이트웨이를 삭제합니다.

### [Remove-AzVirtualNetworkGatewayConnection](Remove-AzVirtualNetworkGatewayConnection.md)
Virtual Network 게이트웨이 연결을 삭제합니다.
...
Virtual Network 게이트웨이에서 IP 구성을 제거합니다.
...
Azure VirtualRouter에서 피어를 제거합니다.

### [Remove-AzVirtualWan](Remove-AzVirtualWan.md)
Azure Virtual WAN을 제거합니다.
...
애플리케이션 게이트웨이의 자동 조정 구성을 업데이트합니다.
...
애플리케이션 게이트웨이에 대한 백 엔드 HTTP 설정을 업데이트합니다.
...
애플리케이션 게이트웨이의 신뢰할 수 있는 루트 인증서를 업데이트합니다.

### [Set-AzApplicationGatewayUrlPathMapConfig](Set-AzApplicationGatewayUrlPathMapConfig.md)
백end 서버 풀에 대한 URL 경로 매핑 배열에 대한 구성을 설정합니다.
...
Express 경로 게이트웨이와 프레미스 Express 경로 회로 피어링 간에 생성된 Express 경로 연결을 업데이트합니다.

### [Set-AzExpressRouteCrossConnection](Set-AzExpressRouteCrossConnection.md)
ExpressRoute 교차 연결을 수정합니다.

### [Set-AzExpressRouteGateway](Set-AzExpressRouteGateway.md)
확장 가능한 ExpressRoute 게이트웨이를 업데이트합니다.
...
ExpressRoutePort에 할당된 ID를 업데이트합니다.

### [Set-AzFirewall](Set-AzFirewall.md)
수정된 방화벽을 저장합니다.

### [Set-AzFirewallPolicy](Set-AzFirewallPolicy.md)
수정된 Azure 방화벽 정책을 저장합니다.
...
개인 링크 서비스의 개인 엔드포인트 연결 상태를 업데이트합니다.
...
Virtual Network 게이트웨이 연결에서 패킷 캡처 작업을 시작합니다.
...
**Test-AzPrivateLinkServiceVisibility는** 개인 링크 서비스가 현재 사용할 수 있는지 여부를 검사합니다.

### [Update-AzP2sVpnGateway](Update-AzP2sVpnGateway.md)
지점 및 사이트 연결을 위해 VirtualHub에서 기존 P2SVpnGateway를 업데이트합니다.
