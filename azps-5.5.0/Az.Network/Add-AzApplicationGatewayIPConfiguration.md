...
## 설명
**Add-AzApplicationGatewayIPConfiguration** cmdlet은 애플리케이션 게이트웨이에 IP 구성을 추가합니다.
IP 구성에는 애플리케이션 게이트웨이가 배포된 서브넷이 포함되어 있습니다.

## 예제

### 예제 1: 애플리케이션 게이트웨이에 가상 네트워크 구성 추가
```
...
첫 번째 명령은 가상 네트워크를 얻습니다.
두 번째 명령은 이전에 만든 가상 네트워크를 사용하여 서브넷을 얻습니다.
세 번째 명령은 애플리케이션 게이트웨이를 사용하여 애플리케이션 게이트웨이를 $AppGw 변수에 저장합니다.
네 번째 명령은 애플리케이션 게이트웨이에 저장된 애플리케이션 게이트웨이에 IP 구성을 $AppGw.

## PARAMETERS

### -ApplicationGateway
이 cmdlet이 IP 구성을 추가하는 애플리케이션 게이트웨이를 지정합니다.
