...
## 설명
**Remove-AzApplicationGatewayHttpListener** cmdlet은 Azure 애플리케이션 게이트웨이에서 HTTP 수신기를 제거합니다.

## 예제

### 예제 1: 애플리케이션 게이트웨이 HTTP 수신기 제거
```
...
두 번째 명령은 수신기 이름이 Listener02인 HTTP 수신기를 로컬에 저장된 애플리케이션 게이트웨이에서 $AppGw.

## PARAMETERS

### -ApplicationGateway
HTTP 수신기를 제거할 애플리케이션 게이트웨이를 지정합니다.
...
### -Name
이 cmdlet이 제거하는 HTTP 수신기 이름을 지정합니다.
