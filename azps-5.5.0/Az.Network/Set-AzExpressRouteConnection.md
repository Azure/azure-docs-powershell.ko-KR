...
## 설명
**Set-AzExpressRouteConnection** cmdlet은 Express 경로 게이트웨이와온-프레미스 Express 경로 회로 피어링 간에 생성된 Express 경로 연결을 업데이트합니다.
...
위의 경우 Azure의 "testRG" 리소스 그룹에 미국 서부에 리소스 그룹, Virtual WAN, Virtual Network, Virtual Hub 및 ExpressRouteSite가 생성됩니다. ExpressRoute 게이트웨이는 2개 배율 단위가 있는 가상 허브에서 이후에 만들어집니다.

게이트웨이가 만들어지면 게이트웨이는 New-AzExpressRouteConnection 사용하여 프레미스 ExpressRoute 회로 피어링에 연결됩니다.

그런 다음, Set-AzExpressRouteConnection 명령을 사용하여 다른 RoutingWeight를 가지게 Set-AzExpressRouteConnection 업데이트됩니다.
...
### -AuthorizationKey
ExpressRoute 게이트웨이 연결을 만드는 데 사용할 권한 부여 키입니다.
...
### -RoutingWeight
패킷 라우팅을 위해 이 연결에 할당해야 하는 가중치입니다.
