...
## 설명
**Add-AzKeyVaultNetworkRule** cmdlet은 키 자격 증명 모음에 대한 액세스를 해당 IP 주소 또는 해당 IP 주소가 지정한 호출자 집합 또는 키 자격 증명 모음이 속한 가상 네트워크로 제한합니다. 규칙에는 액세스 정책을 통해 사용 권한이 부여된 다른 사용자, 애플리케이션 또는 보안 그룹에 대한 액세스를 제한할 수 있습니다.

네트워크 규칙을 추가하는 데(개인 `10.0.0.0-10.255.255.255` IP 주소) 내부의 IP 범위를 사용할 수 없습니다.
...
이 명령은 지정된 자격 증명 모음에 네트워크 규칙을 추가하여 지정된 자격 증명 모음으로 식별된 가상 네트워크에서 지정된 IP 주소에 액세스할 $myNetworkResId.
