...
## 설명
**Start-AzBatchPoolResize** cmdlet은 풀에서 Azure Batch 크기 변경 작업을 시작합니다.

## 예제

### 예제 1: 풀을 12개 노드로 다시 설정
```
PS C:\>Start-AzBatchPoolResize -Id "ContosoPool06" -TargetDedicatedComputeNodes 12 -BatchContext $Context
```

이 명령은 Id ContosoPool06이 있는 풀에서 배정 작업의 배정을 시작합니다.
작업에 대한 대상은 12개 전용 계산 노드입니다.
Get-AzBatchAccountKey cmdlet을 사용하여 $Context 변수에 컨텍스트를 할당합니다.

### 예제 2: 재배치 옵션을 사용하여 풀의 양조
```
PS C:\>Get-AzBatchPool -Id "ContosoPool06" -BatchContext $Context | Start-AzBatchPoolResize -TargetDedicatedComputeNodes 5 -ResizeTimeout ([TimeSpan]::FromHours(1)) -ComputeNodeDeallocationOption ([Microsoft.Azure.Batch.Common.ComputeNodeDeallocationOption]::Terminate) -BatchContext $Context
```

이 cmdlet은 풀의 크기는 5개의 전용 계산 노드로 크기입니다.
이 명령은 Get-AzBatchPool cmdlet을 사용하여 ContosoPool06 ID가 있는 풀을 얻습니다.
이 명령은 파이프라인 연산자를 사용하여 해당 풀 개체를 현재 cmdlet에 전달합니다.
이 명령은 풀에 대한 [사소한 작업]을 시작합니다.
대상은 5개의 전용 계산 노드입니다.
이 명령은 1시간의 시간제한 기간을 지정합니다.
이 명령은 Terminate의 재배치 옵션을 지정합니다.
...
이 cmdlet이 시작하는 크기 조정 작업에 대한 재배치 옵션을 지정합니다.
...
이 시간까지 풀이 대상 크기에 도달하지 않는 경우 크기 변경 작업이 중지됩니다.
