...
이 명령은 지정된 이미지 경로의 MSIX 패키지를 HostPool에 추가합니다.

### 예제 2: HostPool에 새 MSIX 패키지를 만듭니다.
```powershell
...
### -FullName
지정된 호스트 풀 내의 MSIX 패키지의 버전별 패키지 전체 이름
...
생성을 위해 PACKAGEAPPLICATION 속성에 대한 NOTES 섹션을 참조하고 해시 테이블을 만드십시오.
...
생성을 위해 PACKAGEDEPENDENCY 속성에 대한 NOTES 섹션을 참조하고 해시 테이블을 만드십시오.
...
### -PackageFamilyName
패키지 패밀리 이름은 appxmanifest.xml.
패키지 이름 및 게시자 이름을 포함 합니다.
...
PACKAGEAPPLICATION <IMsixPackageApplications[]>: 패키지 애플리케이션 목록입니다. 
  - `[AppId <String>]`: 패키지 애플리케이션 ID로, appxmanifest.xml.
  - `[AppUserModelId <String>]`: 패키지 애플리케이션을 활성화하는 데 사용됩니다. 패키지 이름 및 ApplicationID로 구성됩니다. 다음 appxmanifest.xml.
  - `[Description <String>]`: 패키지 애플리케이션에 대한 설명입니다.
  - `[FriendlyName <String>]`: 사용자 친화적인 이름입니다.
  - `[IconImageName <String>]`: 사용자 친화적인 이름입니다.
  - `[RawIcon <Byte[]>]`: 아이콘은 바이트 배열인 64비트 문자열입니다.
  - `[RawPng <Byte[]>]`: 아이콘은 바이트 배열인 64비트 문자열입니다.

PACKAGEDEPENDENCY <IMsixPackageDependencies[]>: 패키지 종속성 목록입니다. 
