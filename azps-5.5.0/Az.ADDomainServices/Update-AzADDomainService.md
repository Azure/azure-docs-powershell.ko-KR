...
업데이트 호출은 PATCH 본문에 나열된 속성만 지원됩니다.
...
### 예제 1: ResourceGroupName 및 Name으로 AzADDomainService 업데이트
```powershell
...
ResourceGroupName 및 Name으로 AzADDomainService 업데이트

### 예제 2: Inputobject로 AzADDomainService 업데이트
```powershell
...
Inputobject로 AzADDomainService 업데이트
