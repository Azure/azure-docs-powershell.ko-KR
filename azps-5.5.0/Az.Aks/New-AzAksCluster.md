...
AKS에서 Windows Server 컨테이너를 만들하려면 AKS를 만들 때 다음 매개 변수를 4개 이상 지정해야 합니다. 값은 각각 및 그 값에 `NetworkPlugin` `NodeVmSetType` `azure` `VirtualMachineScaleSets` 해당해야 합니다.
...
Windows VM에 사용할 관리자 암호의 길이는 12자 이상으로, 소문자(예: 1개와 특수 문자 1개)를 `[a-z]` `[A-Z]` 포함해야 `[!@#$%^&*()]` 합니다.
