...
### -RepairRegistration
클라우드를 사용하여 현재 Azure Stack HCI 등록을 복구합니다. 이 cmdlet은 클러스터형 노드의 로컬 인증서와 클라우드의 Azure AD 애플리케이션에서 원격 인증서를 삭제하고 둘 다에 대한 새 대체 인증서를 생성합니다. 리소스 그룹, 리소스 이름 및 기타 등록 선택이 유지됩니다.
