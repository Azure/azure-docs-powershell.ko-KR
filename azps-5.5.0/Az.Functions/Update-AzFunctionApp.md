...
### 예제 2: 함수 앱에 대한 SystemAssigned 관리 ID를 설정합니다.
```powershell
...
'없음' 형식은 함수 앱에서 ID를 제거합니다.
이 매개 변수에 허용되는 값은 SystemAssigned - UserAssigned - None입니다.
...
INPUTOBJECT: <ISite> 
  ...
  - `[Config <ISiteConfig>]`: 앱의 구성입니다.
    ...
    - `[LimitMaxMemoryInMb <Int64?>]`: 최대 허용되는 메모리 사용량(MB)입니다.
