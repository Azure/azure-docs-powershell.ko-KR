...
## 설명
Azure PowerShell cmdlet에서 대상(또는 계정 또는 테넌트)에 대한 구독을 선택합니다.  이 cmdlet 후 이후 cmdlet은 선택한 컨텍스트를 대상으로 합니다.
...
'작업' 컨텍스트의 계정, 테넌트 및 구독에서 향후 Azure PowerShell cmdlet을 대상으로 합니다.

### 예제 2

이 PowerShell 세션에서 Azure PowerShell cmdlet을 대상으로 하는 구독 및 계정을 선택합니다. (자동Generated)
...
### -Scope
컨텍스트 변경의 범위(예: 변경 내용이 현재 프로세스에만 적용되는지 또는 이 사용자가 시작한 모든 세션에 적용)를 결정
