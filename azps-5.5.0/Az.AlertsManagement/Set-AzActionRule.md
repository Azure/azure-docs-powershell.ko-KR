...
## 설명
**Set-AzActionRule은** 작업 규칙을 생성하거나 업데이트합니다.
...
이 cmdlet은 구독 범위를 통해 압축에 대한 작업 규칙을 만듭니다.
...
이 cmdlet은 리소스 그룹 범위 목록을 통해 작업 그룹에 대한 작업 규칙을 만듭니다.
...
이 cmdlet은 리소스 범위를 사용하여 진단 설정에 대한 작업 규칙을 만듭니다.

## PARAMETERS

### -ActionGroupId
알림을 하게 될 작업 그룹 ID입니다.
...
예상 형식 - { \<operation\> : \<comma separated list of values\> } 예:
...
### -ReccurentValue
해당하는 경우 재시도 값입니다. 매주의 경우 - \[ 1,2 월 - \] \[ 1,3,5,30\]
...
### -SuppressionEndTime
표시 안 압정 종료 시간입니다.
형식 12/09/2018 06:00:00 +재발성 압축 일정의 경우 한 번, 매일, 매주 또는 매월 언급해야 합니다.
...
예상 형식 - { \<operation\> : \<comma separated list of values\> } 예:
포함:Virtual Machines, Storage 계정
