...
## 설명
**Set-AzDataLakeAnalyticsCatalogItemAclEntry** cmdlet은 Data Lake Analytics의 카탈로그 또는 카탈로그 항목의 ACL(액세스 제어 목록)에서 항목(ACE)을 추가하거나 수정합니다.
...
이 명령은 읽기 권한이 있는 Patti Fuller 카탈로그 ACE를 수정합니다.
...
이 명령은 읽기 권한이 있는 Patti Fuller용 데이터베이스 ACE를 수정합니다.
...
이 명령은 다른 사용자에 대해 카탈로그 ACE를 수정하여 읽기 권한을 습니다.

### 예제 4: 데이터베이스에 대한 다른 권한 수정
```powershell
