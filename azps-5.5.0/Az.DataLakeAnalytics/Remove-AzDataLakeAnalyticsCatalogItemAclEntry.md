...
### RemoveCatalogAclEntryForUser(기본값)
```
...
## 설명
**Remove-AzDataLakeAnalyticsCatalogItemAclEntry** cmdlet은 Data Lake Analytics의 카탈로그 또는 카탈로그 항목의 ACL(액세스 제어 목록)에서 ACE(항목)를 제거합니다.

## 예제

### 예제 1: 카탈로그에 대한 사용자 ACL 제거
```powershell
...
이 명령은 contosoadla 계정의 Patti Fuller에 대한 카탈로그 ACL을 제거합니다.

### 예제 2: 데이터베이스에 대한 사용자 ACL 제거
```powershell
...
이 명령은 contosoadla 계정의 Patti Fuller에 대한 데이터베이스 ACL을 제거합니다.
