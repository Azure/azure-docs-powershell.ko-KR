...
이 명령은 NextPageLink를 따라 LogicApp08이라는 논리 앱에서 트리거에 대한 전체 워크플로 트리거 기록을 얻습니다.
...
이 명령은 NextPageLink를 따라 결과 크기를 두 페이지로 제한하여 LogicApp09라는 논리 앱에서 트리거에 대한 워크플로 트리거 기록의 처음 두 페이지를 얻습니다.
...
이 cmdlet이 트리거 기록을 얻을 논리 앱의 이름을 지정합니다.
...
이 cmdlet에서 기록을 얻을 리소스 그룹의 이름을 지정합니다.
