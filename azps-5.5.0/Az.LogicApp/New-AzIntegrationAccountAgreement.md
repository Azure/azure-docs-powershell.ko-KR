...
## 설명
**New-AzIntegrationAccountAgreement** cmdlet은 통합 계정 계약을 만듭니다.
이 cmdlet은 통합 계정 계약을 나타내는 개체를 반환합니다.
통합 계정 이름, 리소스 그룹 이름, 계약 이름, 유형, 파트너 이름, 파트너 지정자 및 계약 콘텐츠를 지정합니다.
...
이 명령은 지정된 Azure 리소스 그룹에 통합 계정 계약을 만듭니다.

### 예제 2

통합 계정 계약을 만듭니다. (자동Generated)
...
### -AgreementContent
계약에 대한 계약 콘텐츠를 JSON(JavaScript Object Notation) 형식으로 지정합니다.
이 매개 변수 또는 *AgreementContentFilePath* 매개 변수를 지정합니다.
...
계약에 대한 협정 콘텐츠의 파일 경로를 지정합니다.
이 매개 변수 또는 *AgreementContent 매개 변수를 지정합니다.*
...
### -AgreementType
통합 계정 계약 유형을 지정합니다. 이 매개 변수에 허용되는 값은
- X12 
- AS2
- Edifact
...
게스트 파트너에 대한 이름 비즈니스 ID 한정자 지정자입니다.
...
호스트 파트너에 대한 이름 비즈니스 ID 한정자 지정자입니다.
