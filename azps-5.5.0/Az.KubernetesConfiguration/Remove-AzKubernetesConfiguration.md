...
## SYNOPSIS
이렇게 하면 원본 제어 구성을 설정하는 데 사용되는 YAML 파일이 삭제되어 원본 리포지터에서 향후 동기화가 중지됩니다.
...
### 예제 1: 이름으로 kubernetes 클러스터의 구성 제거
```powershell
...
이 명령은 kubernetes 클러스터의 구성을 이름으로 제거합니다.

### 예제 2: 개체로 kubernetes 클러스터의 구성 제거
```powershell
...
이 명령은 개체에 의해 kubernetes 클러스터의 구성을 제거합니다.
