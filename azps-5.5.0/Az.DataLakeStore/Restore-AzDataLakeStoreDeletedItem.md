...
## 설명
**Restore-AzDataLakeStoreDeletedItem** cmdlet은 Data Lake Store에서 삭제된 파일 또는 폴더를 복원합니다. Get-AzDataLakeStoreDeletedItem에서 반환된 휴지통에서 삭제된 항목의 경로가 필요합니다.
주의: 파일을 삭제하지 않는 것이 가장 좋은 작업입니다. 파일이 삭제된 후 복원할 수 있는 보장은 없습니다. 이 API는 화이트리스트를 통해 사용할 수 있습니다. ADL 계정이 화이트리스트에 없는 경우 이 API를 사용하여 구현되지 않은 예외를 throw합니다. 자세한 정보 및 지원은 Microsoft 지원에 문의하세요.
