...
## 설명
클라우드 계층화가 서버 엔드포인트(등록된 서버의 특정 위치)에서 사용하도록 설정되면 이 명령을 사용하여 모든 계층화 파일을 로컬 디스크로 회수할 수 있습니다. 회수를 시작하기 전에 이 서버 엔드포인트에서 클라우드 계층을 사용하지 않도록 설정하는 것이 좋습니다. 계층화가 계속 진행 중이면 회수를 통해 다른 파일이 계층화될 수 있습니다. 이 경우 로컬 디스크에 있는 모든 콘텐츠의 원하는 목표를 달성하기 위해 누락된 파일이 발생할 수 있습니다.
