...
## 설명
**Stop-AzDataFactoryV2TriggerRun** cmdlet은 트리거 이름 및 트리거 실행 ID로 지정된 데이터 팩터리에서 트리거 실행을 중지합니다.
현재 WaitingOnDependency 상태의 TumblingWindowTriggers에서만 지원됩니다.
...
이 명령은 팩터리 WikiADF에서 '085860024680058888497807248799CU16'으로 트리거 실행을 중지합니다.
...
### -PassThru
cmdlet을 지정한 경우 작업이 성공하는 경우 true를 작성합니다.
