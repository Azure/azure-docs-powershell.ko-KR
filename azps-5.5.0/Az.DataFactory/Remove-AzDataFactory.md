...
## 설명
**Remove-AzDataFactory** cmdlet은 데이터 팩터리를 제거합니다.
...
이 명령은 ADF라는 리소스 그룹에서 WikiADF라는 데이터 팩터리를 제거합니다.
이 명령은 $True.

## PARAMETERS

### -DataFactory
제거할 **PSDataFactory 개체를** 지정합니다.
...
### -Force
이 cmdlet이 확인을 요청하지 않고 데이터 팩터리를 제거합니다.
...
이 cmdlet은 이 매개 변수가 지정하는 그룹에서 데이터 팩터리를 제거합니다.
