...
## 설명
**Remove-AzDataFactoryHub** cmdlet은 지정된 Azure 리소스 그룹 및 지정된 데이터 팩터리의 Azure Data Factory에서 허브를 제거합니다.
...
이 명령은 ADFResourceGroup이라는 Azure 리소스 그룹 및 ADFDataFactory라는 데이터 팩터리에서 ContosoDataHub라는 허브를 제거합니다.
...
### -Force
이 cmdlet이 확인을 요청하지 않고 허브를 제거합니다.
