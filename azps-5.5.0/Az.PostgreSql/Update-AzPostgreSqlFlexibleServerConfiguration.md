...
### 예제 1: 이름에 따라 Updatae가 지정된 PostgreSql 구성
```powershell
...
이 cmdlet은 지정된 PostgreSql 구성을 이름으로 업데이트합니다.

### 예제 1: ID에 따라 Updatae가 지정된 PostgreSql 구성
```powershell
...
이 cmdlet은 ID에 따라 지정된 PostgreSql 구성을 업데이트합니다.
