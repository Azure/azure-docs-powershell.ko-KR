...
### 예제 2: 기본 설정을 사용하여 유연한 새 PostgreSql 서버 만들기
```powershell
...
이 cmdlet은 기본 매개 변수 값을 사용하여 PostgreSql 유연한 서버를 만들고 새 가상 네트워크 내에서 서버를 프로비전하고 서버에 위임된 서브넷이 있습니다.
위치의 기본값은 미국 서부 2, Sku는 Standard_D2s_v3, Sku 계층은 GeneralPurpose, 저장소 크기는 128GiB입니다.
...
### 예제 3: 가상 네트워크를 사용하여 유연한 새 PostgreSql 서버 만들기
```powershell
PS C:\> $Vnet = 'vnetname'
...
이 cmdlet은 사용자가 제공한 vnet ID 또는 vnet 이름으로 PostgreSql 유연한 서버를 만듭니다.
가상 네트워크가 없는 경우 cmdlet에서 만듭니다.

### 예제 4: 가상 네트워크 및 서브넷 이름을 사용하여 유연한 새 PostgreSql 서버 만들기
```powershell
...
이 cmdlet은 vnet 이름, 서브넷 이름, vnet prefix 및 서브넷 prefix를 통해 PostgreSql 유연한 서버를 만듭니다.
가상 네트워크 및 서브넷이 없는 경우 cmdlet에서 만듭니다.

### 예제 7: 모든 공용 액세스 권한이 있는 새 PostgreSql 유연한 서버 만들기
```powershell
...
### 예제 8: 방화벽이 있는 유연한 새 PostgreSql 서버 만들기
```powershell
...
0.0.0.0을 지정하면 Azure 내에 배포된 모든 리소스에서 공용 액세스를 통해 서버에 액세스할 수 있습니다.
...
서브넷은 Microsoft.DBforPostgreSQL/flexibleServers에 위임됩니다.
