...
대신 Update-AzPostSqlFlexibleServerConfiguration 서버 매개 변수를 업데이트하려는 경우 wait_timeout net_retry_count.
...
기존 서버를 업데이트합니다.
요청 본문은 일반 서버 정의에 있는 속성 중 하나에서 다수 포함할 수 있습니다.
대신 Update-AzPostgreSqlFlexibleServerConfiguration 서버 매개 변수를 업데이트하려는 경우 wait_timeout net_retry_count.
...
이 cmdlet은 리소스 그룹 및 서버 이름으로 PostgreSql 서버를 업데이트합니다.

### 예제 2: ID로 PostgreSql 서버를 업데이트합니다.
```powershell
...
이 cmdlet은 ID에 의해 PostgreSql 서버를 업데이트합니다.
...
### -SslEnforcement
서버에 연결할 때 ssl 적용을 사용하도록 설정하거나 설정하지 않습니다.
...
INPUTOBJECT: <IPostgreSqlIdentity> ID 매개 변수입니다.
