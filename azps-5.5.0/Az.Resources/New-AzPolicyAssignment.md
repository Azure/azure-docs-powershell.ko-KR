...
세 번째 및 네 번째 명령은 이름에 "east"이 있는 모든 Azure 지역을 포함하는 개체를 만듭니다.
명령은 해당 개체를 $AllowedLocations 저장합니다.
마지막 명령은 $Policy 정책 매개 변수 개체를 사용하여 리소스 그룹 수준에서 $AllowedLocations.
리소스 **그룹의 ResourceId** 속성은 $ResourceGroup 식별합니다.

### 예제 4: 정책 매개 변수 파일을 사용하여 리소스 그룹 수준에서 정책 할당
로컬 작업 디렉터리에AllowedLocations.js _다음_ 내용으로 "on"이라는 파일을 만들 수 있습니다.
