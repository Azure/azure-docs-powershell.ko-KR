...
## 설명
**New-AzResourceLock** cmdlet은 리소스 잠금을 만듭니다.

## 예제

### 예제 1: 웹 사이트에 리소스 잠금 만들기
```
...
이 명령은 웹 사이트에 리소스 잠금을 만듭니다.

### 예제 2: 데이터베이스에 리소스 잠금 만들기
```
...
이 명령은 Azure 데이터베이스에 리소스 잠금을 만듭니다.
...
### -LockLevel
잠금 수준을 지정합니다.
현재 유효한 값은 CanNotDelete, ReadOnly입니다.
...
### -ResourceGroupName
잠금이 적용되거나 잠금이 적용되는 리소스 그룹이 포함된 리소스 그룹의 이름을 지정합니다.
...
### -Scope
잠금이 적용되는 범위를 지정합니다.
예를 들어 데이터베이스를 지정하려면 다음 형식을 사용합니다. 구독 ID 리소스 그룹 이름 서버 이름 데이터베이스 이름 리소스 그룹을 지정하려면 다음 형식을 사용합니다. 구독 ID 리소스 그룹 `/subscriptions/` `/resourceGroups/` `/providers/Microsoft.Sql/servers/` `/databases/` `/subscriptions/` `/resourceGroups/` 이름
