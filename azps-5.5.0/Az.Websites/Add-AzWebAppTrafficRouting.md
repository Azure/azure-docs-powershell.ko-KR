...
## 설명
**Add-AzWebAppTrafficRouting** cmdlet은 Azure 웹앱 슬롯에 라우팅 규칙을 추가합니다.

## 예제

### 예제 1 라우팅 규칙을 추가하여 프로덕션 트래픽의 15%를 Stg 슬롯으로 전송
```powershell
PS C:\>Add-AzWebAppTrafficRouting -ResourceGroupName "Default-Web-WestUS" -WebAppName "ContosoSite" 
-RoutingRule @{ActionHostName='XXXX.azurewebsites.net';ReroutePercentage=15;Name='Stg'}
```

이 명령은 프로덕션 트래픽의 15%를 Stg 슬롯으로 전송하는 라우팅 규칙을 추가합니다.

### 예제 2 증분 방식으로 프로덕션 트래픽을 Stg 슬롯 범위로 전송하는 라우팅 규칙을 추가합니다.
```powershell
PS C:\>Add-AzWebAppTrafficRouting -ResourceGroupName "Default-Web-WestUS" -WebAppName "ContosoSite" 
...
이 명령은 프로덕션 트래픽을 50%에서 90% 증분 방식으로 Stg 슬롯 범위로 전송하는 라우팅 규칙을 추가합니다.
