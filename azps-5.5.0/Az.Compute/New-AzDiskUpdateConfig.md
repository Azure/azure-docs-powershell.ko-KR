...
첫 번째 명령은 저장소 계정 유형에서 크기가 10GB인 로컬 빈 Premium_LRS 만듭니다. 또한 Windows OS 유형을 설정하고 암호화 설정을 사용할 수 있습니다. 두 번째 및 세 번째 명령은 디스크 업데이트 개체에 대한 디스크 암호화 키 및 키 암호화 키 설정을 설정합니다.
...
이 명령은 리소스 그룹 'ResourceGroup01'에서 이름이 'Disk01'인 기존 디스크를 10GB 디스크 크기로 업데이트합니다.
...
### -DiskMBpsReadOnly
공유 디스크를 ReadOnly로 탑재하는 모든 VM에서 허용되는 총 데이터량(MBps)입니다. MBps는 초당 수백만의 byte를 의미합니다. 여기서 MB는 10의 거버전의 ISO 의미를 사용 합니다.
...
### -MaxSharesCount
디스크에 동시에 연결할 수 있는 최대 VM 수입니다. 두 개 이상의 값은 여러 VM에 동시에 탑재할 수 있는 디스크를 나타냅니다.
