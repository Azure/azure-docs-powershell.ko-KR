...
디스크 이미지 암호화를 사용하여 갤러리 이미지 버전을 만듭니다.
...
### -DataDiskImage
데이터 디스크 이미지입니다.   예: @{Source = @{Id=<source_id>}; Lun=1; SizeInGB = 100; HostCaching = "ReadOnly" }
...
설정되어 있는 경우 최신 버전의 이미지 정의에서 배포된 Virtual Machines는 이 이미지 버전을 사용하지 않습니다.
...
### -StorageAccountType
이미지를 저장하는 데 사용할 저장소 계정 유형을 지정합니다. 이 속성은 업데이트할 수 없습니다. 사용 가능한 값은 Standard_LRS, Standard_ZRS Premium_LRS.
