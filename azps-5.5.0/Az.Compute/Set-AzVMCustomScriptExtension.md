...
## 설명
**Set-AzVMCustomScriptExtension** cmdlet은 가상 머신에 사용자 지정 스크립트 Virtual Machine 확장을 추가합니다.
이 확장을 사용하면 가상 머신에서 사용자 자신의 스크립트를 실행할 수 있습니다.
...
이 명령은 VirtualMachine07이라는 가상 머신에 사용자 지정 스크립트를 추가합니다.
스크립트 파일이 contososcript.exe.

### 예제 2

가상 머신에 사용자 지정 스크립트 확장을 추가합니다. (자동Generated)
...
### -ContainerName
이 cmdlet이 스크립트를 저장하는 Azure 저장소 컨테이너의 이름을 지정합니다.
...
### -FileName
스크립트 파일의 이름을 지정합니다. 파일이 Azure Blob Storage에 저장되는 경우 파일 이름 값은 대소문자 구분입니다. Azure File Storage에 저장된 파일의 파일 이름은 대소문자 구분이 없습니다.
...
### -SecureExecution
이 cmdlet은 *실행* 매개 변수의 값이 서버에 기록되지 않은지 또는 GET 확장 API를 사용하여 사용자에게 반환되지 않는지 확인하도록 합니다.
실행 *값에는* 스크립트 파일에 안전하게 전달할 비밀 또는 암호가 포함될 수 있습니다.
...
### -StorageAccountName
이 cmdlet이 스크립트를 저장하는 Azure 저장소 계정의 이름을 지정합니다.
...
버전을 얻게 Get-AzVMExtensionImage *Type* 매개 변수에 대한 Microsoft.Compute 매개 변수 및 CustomScriptExtension 값을 사용하여 Get-AzVMExtensionImage cmdlet을 *실행합니다.*
...
### -VMName
가상 머신의 이름을 지정합니다.
이 cmdlet은 이 매개 변수가 지정하는 가상 머신에 대한 사용자 지정 스크립트 확장을 추가합니다.
