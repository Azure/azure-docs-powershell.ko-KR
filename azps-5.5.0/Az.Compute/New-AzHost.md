...
이 명령은 주어진 호스트 그룹 및 주어진 위치에 주어진 SKU의 호스트를 만듭니다.
...
오류가 발생하면 호스트를 자동으로 대체할지 여부를 지정합니다. 값이 제공되지 않은 경우 기본값은 'true'입니다.
...
### -LicenseType
호스트에 배포된 VM에 적용할 소프트웨어 라이선스 유형을 지정합니다. 가능한 값은 None, Windows_Server_Hybrid 및 Windows_Server_Perpetual.  기본값은 None입니다.
...
### -PlatformFaultDomain
호스트의 플랫폼 장애 도메인 수를 지정합니다.  그런 다음, 최소값은 0, 최대값은 2입니다.
