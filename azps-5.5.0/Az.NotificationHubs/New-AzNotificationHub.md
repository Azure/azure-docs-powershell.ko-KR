...
**New-AzNotificationHub** cmdlet과 함께 사용하는 경우 이전 JSON 샘플은 미국 서부 데이터 센터에 있는 ContosoNotificationHub라는 알림 허브를 만듭니다.
...
이 명령은 ContosoNamespace 네임스페이스에 알림 허브를 만듭니다.
새 허브가 ContosoNotificationsGroup에 할당됩니다.
허브에 대한 이름 또는 기타 구성 정보를 지정할 필요가 없습니다. 해당 정보는 입력 파일에서 C:\Configurations\InternalHub.js.
...
### -InputFile
새 알림 허브에 대한 구성 값을 포함하는 JSON 파일의 경로를 지정합니다.
...
### -NotificationHubObj
새 허브에 대한 구성 정보를 포함하는 **NotificationHubAttributes** 개체를 지정합니다.
