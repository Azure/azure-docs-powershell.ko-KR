...
### -ArtifactTag
배포자가 아티팩트를 생성/업데이트한 후 아티팩트에 적용할 태그입니다.
...
### -ExcludeFromLatest
만든 이미지 버전을 최신에서 제외해야 하는지 여부를 나타내는 플래그입니다.
기본값(false)을 사용하기 위해 생략합니다.
...
### -Location
이미지에 대한 Azure 위치는 이미지가 이미 있는 경우 일치해야 합니다.
...
### -StorageAccountType
공유 이미지를 저장하는 데 사용할 저장소 계정 유형입니다.
기본값(Standard_LRS) 사용은 생략합니다.
