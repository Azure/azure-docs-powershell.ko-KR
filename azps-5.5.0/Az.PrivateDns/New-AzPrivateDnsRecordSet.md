...
이 명령은 개인 영역 _sip._tcp라는 RecordSet을 myzone.com. 레코드 집합은 SRV 형식으로, TTL은 1시간(3600초)입니다. IP 주소 2001.2.3.4를 지점으로 하는 단일 개인 DNS 레코드가 포함되어 있습니다. 서비스(sip) 및 프로토콜(tcp)은 레코드 데이터의 일부가 아닌 레코드 집합 이름의 일부로 지정됩니다. 한 줄의 레코드만 사용하여 RecordSet을 만들거나 pn_PowerShell_short 레코드 집합을 만들거나 예제 1을 참조합니다.
