...
## 설명
New-AzPrivateDnsRecordConfig cmdlet은 로컬 PSPrivateDnsRecord 개체를 만듭니다. 이러한 개체의 배열은 PrivateDnsRecord 매개 변수를 사용하여 New-AzPrivateDnsRecordSet cmdlet에 전달되어 레코드 집합에서 만들 레코드를 지정합니다.
...
이 예제에서는 개인 영역 집합에 www라는 RecordSet을 myzone.com. 레코드 집합은 CNAME 형식으로, TTL은 1시간(3600초)입니다. 단일 개인 DNS 레코드를 포함 합니다. 한 줄의 레코드만 사용하여 RecordSet을 만들거나 pn_PowerShell_short 레코드 집합을 만들거나 예제 1을 참조합니다.
