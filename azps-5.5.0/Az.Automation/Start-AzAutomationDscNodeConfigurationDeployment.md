...
## 설명
**Start-AzAutomationDscNodeConfigurationDeployment** cmdlet은 Azure Automation에서 DSC(필요한 상태 구성) 노드 구성을 배포합니다.
...
위의 명령은 "Config01.Node1"이라는 DSC 노드 구성을 주어진 노드 이름의 2차원 배열에 배포합니다. 배포는 단계적 방식으로 발생합니다.
...
위의 명령은 "Config01.Node1"이라는 DSC 노드 구성을 주어진 2차원 노드 배열에 배포하는 작업을 예약합니다. 배포는 준비된 방식으로 수행되고 일정에 따라 실행됩니다.
...
### -Schedule
배포 작업을 예약하는 Automation Schedule 개체입니다.
