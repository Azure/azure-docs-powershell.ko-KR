...
## 설명
**Get-AzAutomationDscNode** cmdlet은 Azure Automation에서 APS DSC(필요한 상태 구성) 노드를 얻습니다.
...
이 명령은 DSC 구성 ContosoConfiguration에서 생성된 DSC 노드 구성에 매핑되는 Contoso17 Automation 계정의 모든 DSC 노드에 대한 메타데이터를 얻습니다.
...
이 명령은 ContosoConfiguration.webserver라는 DSC 노드 구성에 매핑되는 Contoso17 Automation 계정의 모든 DSC 노드에서 메타데이터를 얻습니다.
...
### -Id
이 cmdlet에서 얻을 DSC 노드의 고유 ID를 지정합니다.
...
### -ResourceGroupName
이 cmdlet이 DSC 노드를 얻을 리소스 그룹의 이름을 지정합니다.
...
### -Status
이 cmdlet에서 얻을 DSC 노드의 상태를 지정합니다.
유효한 값은 
- 준수 
- NotCompliant
- 실패
