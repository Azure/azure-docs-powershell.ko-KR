...
## 설명
이 cmdlet은 컴퓨터 실행 목록을 반환합니다. 각 소프트웨어 업데이트 실행은 각 소프트웨어 업데이트 구성 대상 머신에 대해 머신 실행을 트리거합니다. 특정 머신 실행을 얻습니다. ID 매개 변수를 전달합니다. 모든 컴퓨터 실행, 특정 컴퓨터에 대한 모든 실행, 해당 매개 변수를 전달하여 특정 상태의 모든 실행을 나열할 수 있습니다.

## 예제

### 예제 1
이 예제에서는 지정된 Azure 가상 머신에 대해 실패한 모든 컴퓨터 실행을 반환합니다.
...
### -TargetComputer
컴퓨터 실행을 위한 대상 컴퓨터입니다.
az가 아닌 컴퓨터 이름 또는 Azure VM 리소스 ID일 수 있습니다.
