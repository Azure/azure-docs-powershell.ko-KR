...
## 설명
New-AzDataMigrationProject cmdlet은 새 Azure Database Migration Service 프로젝트를 만듭니다. 이 cmdlet은 Azure 리소스 그룹의 이름, 새 프로젝트를 만들 Azure Data Migration Service의 이름, 프로젝트를 만들 지역, 새 프로젝트의 고유한 이름, 원본 및 대상 연결 개체, 마이그레이션할 데이터베이스 목록에 대한 입력으로 대상 유형 개체와 같은 필요한 모든 매개 변수를 사용합니다. New-AzDataMigrationConnectionInfo cmdlet을 사용하여 원본 및 대상 연결 모두에 대한 새 ConnectionInfo 개체를 만듭니다. Microsoft.Azure.Management.DataMigration.Models.DatabaseInfo 목록은 선택한 데이터베이스에 대해 예상됩니다. 이 개체는 cmdlet을 사용하여 New-AzDataMigrationDatabaseInfo 수 있습니다. 
...
위의 예제에서는 TestService라는 Azure Database Migration Service 인스턴스 아래에서 미국 중부 지역에 있는 MyDMSProject라는 새 프로젝트를 만드는 방법을 보여줍니다.
...
### -Location
Azure Database Migration Service 인스턴스의 위치입니다.
...
### -ServiceName
Azure Database Migration Service 인스턴스의 이름입니다.
