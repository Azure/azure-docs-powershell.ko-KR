...
**참고:** 현재 HSM이 Azure에서 완전히 프로비전되기 전에 반환하는 제한 `New-AzDedicatedHsm` 사항이 있습니다.
따라서 새 HSM을 만들고 나면 해당 상태를 쿼리하고 사용 전에 해당 상태를 `Get-AzDedicatedHsm` `Provisioning State` `Succeeded` 확인하시기 바랍니다.
...
생성을 위해 NETWORKINTERFACE 속성에 대한 NOTES 섹션을 참조하고 해시 테이블을 만드십시오.
...
### -SubnetId
리소스 ARM /subscriptions/{SubscriptionId}/resourceGroups/{ResourceGroupName}/...
