...
## 설명
지정된 `Reservation` s를 새 에 `Reservation` 병합합니다. 병합되는 `Reservation` 두 개의 속성은 동일해야 합니다.
...
지정된 두 `Reservation` s를 하나에 병합 `Reservation`
...
### -Reservation
병합할 두 ReservationId의 콤마로 구분된 문자열
...
### -ReservationId
두 s를 포함하는 ReservationOrderId `ReservationOrder` `Reservation`
