...
## 설명
이 Set-AzEventHubNamespace cmdlet은 지정된 Event Hubs 네임스페이스의 속성을 업데이트합니다.
...
\`네임스페이스 MyNamespaceName에 대한 태그를 만든 것으로 \` 업데이트합니다.
...
\` \` AutoInflate = enabled 및 MaximumThroughputUnits = 10으로 MyNamespaceName 네임스페이스의 상태를 업데이트합니다.

### 예제 3

지정된 Event Hubs 네임스페이스를 업데이트합니다. (자동Generated)
