...
## 설명
**Revoke-AzStorageAccountUserDelegationKeys** cmdlet은 Storage 계정의 모든 사용자 위임 키를 해지하기 때문에 Storage 계정의 모든 ID SAS 토큰도 해지됩니다.

## 예제

### 예제 1: Storage 계정의 모든 사용자 위임 키 해지
```powershell
PS C:\>Revoke-AzStorageAccountUserDelegationKeys -ResourceGroupName "myresourcegroup" -AccountName "mystorageaccount"
```

이 예제에서는 Storage 계정의 모든 사용자 위임 키를 해지하여 사용자 위임 키에서 생성된 모든 ID SAS 토큰도 해지됩니다.
...
### -InputObject
New-AzStorageAccount인 Get_AzStorageAccount 스토리지 계정 개체입니다.
...
### -PassThru
일반적으로 이 cmdlet은 성공적인 완료 시 출력을 반환하지 않습니다. 이 매개 변수는 cmdlet이 성공적인 완료 시 값($true)을 반환합니다.
