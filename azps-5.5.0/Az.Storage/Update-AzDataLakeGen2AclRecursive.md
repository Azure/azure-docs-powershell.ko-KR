...
## 설명
**Update-AzDataLakeGen2AclRecursive** cmdlet은 지정된 경로에서 ACL을 재발적으로 업데이트합니다. 입력 ACL은 원래 ACL을 병합합니다. AccessControlType/EntityId/DefaultScope가 동일한 ACL 항목이 있는 경우 권한 업데이트; 또는 새 ACL 항목을 추가합니다.

## 예제

### 예제 1: 파일 파일의 루트 지시문에서 ACL 재발 업데이트
```
...
이 명령은 먼저 ACL 항목이 3개인 ACL 개체를 만든 다음 파일 시스템의 루트 디렉터리에서 ACL을 재발적으로 업데이트합니다.

### 예제 2: 디렉터리에서 ACL을 재발적으로 업데이트하고 ContinuationToken을 통해 실패로부터 다시 시작
```
...
이 명령은 먼저 ACL을 디렉터리에 재발적으로 업데이트하고 실패한 다음, 사용자가 실패한 파일을 수정한 후 ContinuationToken으로 다시 시작됩니다.

### 예제 3: ACL을 재발적으로 청크로 업데이트
```
...
이 스크립트는 디렉터리 청크에서 청크를 BatchSize * MaxBatchCount로 청크 크기로 다시 업데이트합니다. 청크 크기는 이 스크립트에서 5000입니다.

### 예제 4: 디렉터리 및 ContinueOnFailure에서 ACL을 재발적으로 업데이트한 다음 오류에서 하나씩 다시 시작
```
...
이 명령은 먼저 ContinueOnFailure를 사용하여 ACL을 디렉터리에 재발적으로 업데이트하고 일부 항목이 실패한 다음 실패한 항목을 하나씩 다시 시작합니다.

## PARAMETERS

### -Acl
파일 또는 디렉터리에 대해 재발적으로 설정할 POSIX 액세스 제어 목록입니다.
...
### -BatchSize
데이터 집합 크기가 일괄 처리 크기를 초과하면 진행률을 추적할 수 있도록 작업이 여러 요청으로 분할됩니다.
일괄 처리 크기는 1~2000 사이입니다.
기본값은 2000입니다.
...
### -ContinueOnFailure
이 매개 변수를 설정하여 실패를 무시하고 디렉터리의 다른 하위 엔터티에 대한 작업을 계속 진행합니다. 기본적으로 오류가 발생하면 작업이 신속하게 종료됩니다.
...
### -MaxBatchCount
단일 변경 Access Control 작업을 실행할 수 있는 최대 일괄 처리 수입니다. 데이터 집합 크기가 MaxBatchCount BatchSize를 곱하면 연속 토큰이 반환됩니다.
...
### -Path
Acl을 재발적으로 변경할 지정된 FileSystem의 경로입니다.
파일 또는 디렉터리일 수 있습니다.
'directory/file.txt' 또는 'directory1/directory2/' 형식입니다.
이 매개 변수를 건너뛰어 파일 파일의 루트 디렉터리에서 Acl을 재발적으로 변경합니다.
