...
이 명령은 Azure Files Active Directory Domain Service 인증을 사용하도록 설정하여 Storage 계정을 업데이트한 다음 파일 ID 기반 인증 설정을 보여줍니다.
...
### 예제 13: RoutingPreference 설정을 사용하여 Storage 계정 업데이트
```powershell
...
이 명령은 RoutingPreference 설정으로 Storage 계정을 업데이트합니다. PublishMicrosoftEndpoint를 false로, PublishInternetEndpoint를 true로, MicrosoftRouting으로 RoutingChoice를 업데이트합니다.
