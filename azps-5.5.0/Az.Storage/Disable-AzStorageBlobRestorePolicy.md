...
## 설명
**Disable-AzStorageBlobRestorePolicy** cmdlet은 Azure Storage Blob 서비스에 대한 Blob 복원 정책을 사용하지 않도록 설정합니다.

## 예제

### 예제 1: Storage 계정에서 Azure Storage Blob service에 대한 Blob 복원 정책을 사용하지 않도록 설정
```powershell
PS C:\> Disable-AzStorageBlobRestorePolicy -ResourceGroupName "myresourcegoup" -StorageAccountName "mystorageaccount"
```

이 명령은 Storage 계정의 Azure Storage Blob service에 대한 Blob 복원 정책을 사용하지 않도록 설정합니다.
