...
## 설명
**Set-AzStorageContainerStoredAccessPolicy** cmdlet은 Azure 저장소 컨테이너에 대해 저장된 액세스 정책을 설정합니다.

## 예제

### 예제 1: 모든 권한이 있는 저장소 컨테이너에 저장된 액세스 정책 설정
```
PS C:\>Set-AzStorageContainerStoredAccessPolicy -Container "MyContainer" -Policy "Policy06" -Permission rwdl
```

이 명령은 MyContainer라는 저장소 컨테이너에 대해 Policy06이라는 액세스 정책을 설정합니다.
...
### -Permission
저장소 컨테이너에 액세스하기 위한 저장된 액세스 정책의 사용 권한을 지정합니다.
