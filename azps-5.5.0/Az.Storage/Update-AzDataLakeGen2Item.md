...
### 예제 1: ACL 항목이 3개인 ACL 개체를 만들고 파일 파일의 모든 항목으로 ACL을 재발적으로 업데이트합니다.
```
...
이 명령은 먼저 acl 항목이 3개인 ACL 개체를 만든 다음(-InputObject 매개 변수를 사용하여 기존 acl 개체에 acl 항목을 추가) 파일 파일의 모든 항목을 다운로드하고 항목의 acl을 업데이트합니다.
...
이 명령은 파일의 모든 속성(ACL, 사용 권한, 소유자, 그룹, 메타데이터, 속성을 모든 연결로 업데이트할 수 있습니다)을 업데이트하고 Powershell 콘솔에 표시합니다.
...
이 명령은 디렉터리에서 ACL을 다운로드하고, ACL 항목을 업데이트/추가하고, 디렉터리로 다시 설정합니다.
AccessControlType/EntityId/DefaultScope가 동일한 ACL 항목이 없는 경우 새 ACL 항목을 추가하고, 그렇지 않은 경우 기존 ACL 항목의 업데이트 권한을 추가합니다.

## PARAMETERS

### -Acl
파일 및 폴더에 대한 POSIX 액세스 제어 권한을 설정합니다.
...
### -Path
업데이트해야 하는 지정된 파일 파일의 경로입니다.
'directory/file.txt' 또는 'directory1/directory2/' 형식의 파일 또는 디렉터리일 수 있습니다.
이 매개 변수를 지정하지 않으면 파일 파일의 루트 디렉터리가 업데이트됩니다.
...
### -Property
디렉터리 또는 파일의 속성을 지정합니다. 파일에 지원되는 속성은 CacheControl, ContentDisposition, ContentEncoding, ContentLanguage, ContentMD5, ContentType입니다.
