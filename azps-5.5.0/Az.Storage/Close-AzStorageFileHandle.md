...
## 설명
**Close-AzStorageFileHandle cmdlet은** 파일 공유 또는 파일 디렉터리 또는 파일의 파일 핸들을 닫습니다.

## 예제

### 예제 1: 현재 Storage 계정의 모든 파일 공유를 나열하고 파일 공유의 모든 파일 핸들을 재발적으로 닫습니다.
```
PS C:\>Get-AzStorageShare | Close-AzStorageFileHandle -CloseAll -Recursive
```

이 명령은 현재 Storage 계정의 모든 파일 공유를 나열하고 파일 공유의 모든 파일 핸들을 재발적으로 닫습니다.

### 예제 2: 파일 디렉터리의 모든 파일 핸들을 재발적으로 닫고 닫힌 파일 핸들 수 표시
```
PS C:\>Close-AzStorageFileHandle -ShareName "mysharename" -Path 'dir1/dir2' -Recursive -CloseAll -PassThru
10
```

이 명령은 파일 디렉터리의 모든 파일 핸들을 닫고 닫힌 파일 핸들 수를 보여 주며,

### 예제 3: 파일 디렉터리에서 1일 전에 열리며 모든 파일 핸들 닫기
```
...
이 명령은 파일 디렉터리의 모든 파일 핸들을 재발적으로 나열하고, 1일 전에 연 핸들을 필터로 처리한 다음, 닫습니다.
...
### -Directory
CloudFileDirectory 개체는 파일/directories가 나열되는 기본 폴더를 나타냅니다.
...
### -File
CloudFile 개체는 핸들을 닫을 파일을 표시했습니다.
...
### -Share
CloudFileShare 개체는 파일/감독이 나열될 공유를 나타냅니다.
