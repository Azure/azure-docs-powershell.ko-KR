...
## 설명
**Restore-AzStorageBlobRange** cmdlet은 특정 Blob 범위에 대한 Storage 계정의 Blob을 복원합니다.
시작 범위는 포함하며, 끝 범위는 Blob 복원에서 제외됩니다.

## 예제

### 예제 1: 특정 Blob 범위가 있는 Storage 계정의 Blob 복원 시작
```powershell
...
이 명령은 먼저 2개 Blob 범위를 만든 다음, 1일 전의 2개 Blob 범위가 있는 Storage 계정에서 Blob 복원을 시작하세요. 사용자는 나중에 Get-AzStorageAccount 상태를 추적하는 데 사용할 수 있습니다.

### 예제 2: 백end의 Storage 계정에서 모든 Blob 복원
```powershell
...
### 예제 3: 입력 Blob 범위로 Blob을 직접 복원하고 완료될 때까지 기다림
```powershell
...
이 명령은 2개 Blob 범위를 직접 입력하여 1일 전부터 Storage 계정의 blob을 Restore-AzStorageBlobRange cmdlet에 복원합니다. 이 명령은 복원이 완료될 때까지 대기합니다.
