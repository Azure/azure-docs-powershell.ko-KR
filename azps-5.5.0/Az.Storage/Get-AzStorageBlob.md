...
### 예제 1: Blob 이름으로 Blob을 얻게
```
PS C:\>Get-AzStorageBlob -Container "ContainerName" -Blob blob*
```

이 명령은 Blob 이름 및 와일드카드를 사용하여 Blob을 얻습니다.

### 예제 2: 파이프라인을 사용하여 컨테이너에서 Blob을 얻습니다.
```
...
이 명령은 파이프라인을 사용하여 컨테이너의 모든 Blob(삭제된 상태에 Blob 포함)을 얻습니다.

### 예제 3: 이름에 의해 Blob을 얻습니다.
```
PS C:\>Get-AzStorageBlob -Container "ContainerName" -Prefix "blob"
```

이 명령은 이름 prefix를 사용하여 Blob을 얻습니다.
...
이 예제에서는 *MaxCount* 및 *ContinuationToken* 매개 변수를 사용하여 여러 배치에서 Azure Storage Blob을 나열합니다.
처음 4개의 명령은 예제에서 사용할 변수에 값을 할당합니다.
다섯 번째 명령은 **Get-AzStorageBlob** cmdlet을 사용하여 Blob을 얻게 하는 **Do-While** 문을 지정합니다.
이 문에는 다음 변수에 저장된 연속 토큰이 $Token 포함됩니다.
$Token 실행될 때 값을 변경합니다.
자세한 내용은 `Get-Help About_Do` .를 입력합니다.
마지막 명령은 **Echo** 명령을 사용하여 합계를 표시합니다.

### 예제 5: 컨테이너의 모든 Blob에 Blob 버전 포함
```
...
이 명령은 컨테이너의 모든 Blob에 Blob 버전이 포함됩니다.

### 예제 6: 단일 Blob 버전 다운로드
```
...
이 명령은 VersionId를 사용하여 단일 Blob verion을 얻습니다.

### 예제 7: 단일 Blob 스냅숏 만들기
```
...
이 명령은 SnapshotTime을 사용하여 단일 Blob 스냅숏을 얻습니다.

## PARAMETERS

### -Blob
와일드카드 검색에 사용할 수 있는 이름 또는 이름 패턴을 지정합니다.
Blob 이름이 지정되지 않은 경우 cmdlet은 지정된 컨테이너의 모든 Blob을 나열합니다.
이 매개 변수에 값을 지정하면 cmdlet에 이 매개 변수와 일치하는 이름이 있는 모든 Blob이 나열됩니다. 이 매개 변수는 문자열의 어디에서나 와일드카드를 지원합니다.
...
### -Context
Blob 목록을 얻을 Azure Storage 계정을 지정합니다.
New-AzStorageContext cmdlet을 사용하여 저장소 컨텍스트를 만들 수 있습니다.
...
이 매개 변수와 *MaxCount* 매개 변수를 사용하여 여러 배치에 Blob을 나열합니다.
...
### -IncludeDeleted
삭제된 Blob을 포함하면 기본적으로 Blob get은 삭제된 Blob을 포함하지 않습니다.
...
### -IncludeVersion
Blob 버전은 이 매개 변수가 있는 경우만 나열됩니다. 기본적으로 Blob get은 Blob 버전을 포함하지 않습니다.
...
### -MaxCount
이 cmdlet이 반환하는 개체의 최대 수를 지정합니다.
...
### -Prefix
얻게 할 Blob 이름에 대한 prefix를 지정합니다.
이 매개 변수는 정규식 또는 와일드카드 문자를 사용하여 검색할 수 없습니다.
즉, 컨테이너에 "My", "MyBlob1" 및 "MyBlob2"라는 Blob만 있는 경우 "-Prefix My*"를 지정하면 cmdlet은 Blob이 반환하지 않습니다.
그러나 "-Prefix My"를 지정하면 cmdlet은 "My", "MyBlob1" 및 "MyBlob2"를 반환합니다.
