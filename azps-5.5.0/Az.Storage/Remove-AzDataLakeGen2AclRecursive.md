...
## 설명
**Remove-AzDataLakeGen2AclRecursive** cmdlet은 지정된 경로에서 ACL을 재발적으로 제거합니다. 입력 ACL 항목(다른 사용 권한 포함)이 제거된 AccessControlType, DefaultScope 및 EntityId가 동일한 원래 ACL의 ACL 항목입니다.

## 예제

### 예제 1: 파일 파일의 루트 지시문에서 ACL 재발 제거
```
...
이 명령은 먼저 ACL 항목이 2개인 ACL 개체를 만든 다음 파일 시스템의 루트 디렉터리에서 ACL을 재발적으로 제거합니다.

### 예제 2: 디렉터리에서 재발적으로 ACL 제거
```
...
이 명령은 먼저 디렉터리에서 ACL을 재발적으로 제거하고 실패한 다음, 사용자가 실패한 파일을 수정한 후 ContinuationToken으로 다시 시작됩니다.

### 예제 3: ACL 청크를 청크로 재발적으로 제거
```
$token = $null
...
이 스크립트는 BatchSize * MaxBatchCount로 청크 크기를 사용하여 디렉터리 청크에서 재발적으로 청크를 제거합니다. 청크 크기는 이 스크립트에서 50000입니다.

### 예제 4: 디렉터리에서 ACL을 재발적으로 제거하고 ContinueOnFailure를 제거한 다음 오류에서 하나씩 다시 시작
```
...
이 명령은 먼저 ContinueOnFailure를 사용하여 디렉터리에 대한 ACL을 다시 제거하고 일부 항목이 실패한 다음 실패한 항목을 하나씩 다시 시작합니다.
