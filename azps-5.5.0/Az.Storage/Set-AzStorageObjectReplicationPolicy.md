...
## 설명
**Set-AzStorageObjectReplicationPolicy** cmdlet은 Storage 계정에 지정된 개체 복제 정책을 생성하거나 업데이트합니다.

## 예제

### 예제 1: 대상 및 원본 계정 모두에 개체 복제 정책을 설정합니다.
```
...
이 명령은 개체 복제 정책을 대상 및 원본 계정 모두로 설정합니다.
먼저 2개 개체 복제 정책 규칙을 만들고 2개 규칙을 대상 계정에 설정하여 정책을 설정합니다. 그런 다음 대상 계정에서 원본 계정으로 개체 복제 정책을 설정합니다.
...
### -PolicyId
개체 복제 정책 ID입니다. GUID 또는 '기본값'입니다.
PolicyId를 입력하지 않은 경우 'default'를 사용하여 새 정책을 만들고 새 정책의 ID가 생성된 정책에 반환됩니다.
