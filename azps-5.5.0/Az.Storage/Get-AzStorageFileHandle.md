...
### 예제 1: 파일 공유의 모든 파일 핸들을 재발적으로 나열하고 ClientIp 및 OpenTime별로 정렬
```
...
이 명령은 파일 공유에 대한 파일 핸들을 나열하고 ClientIp를 사용하여 출력을 정렬한 다음 OpenTime별로 정렬합니다.

### 예제 2: 파일 디렉터리에 재발적으로 처음 2개 파일 핸들 나열
```
...
이 명령은 파일 디렉터리에 대한 처음 2개 파일 핸들을 재발적으로 나열합니다.

### 예제 3: 파일의 6번째 파일 핸들에 대한 세 번째 항목 나열
```
...
이 명령은 파일의 6번째 파일 핸들에 대한 세 번째 핸들을 나열합니다.
...
### -IncludeTotalCount
데이터 집합(정수)과 개체의 수를 보고합니다. cmdlet에서 총 개수를 확인할 수 없는 경우 '알 수 없는 총 수'가 반환됩니다.
현재 이 매개 변수는 아무 것도 하지 않습니다.
