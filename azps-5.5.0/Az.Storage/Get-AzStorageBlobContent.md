...
## 설명
**Get-AzStorageBlobContent** cmdlet은 지정된 저장소 Blob을 다운로드합니다.
Blob 이름이 로컬 컴퓨터에 유효하지 않은 경우 이 cmdlet은 가능한 경우 자동으로 해결합니다.

## 예제

### 예제 1: 이름으로 Blob 콘텐츠 다운로드
```
PS C:\>Get-AzStorageBlobContent -Container "ContainerName" -Blob "Blob" -Destination "C:\test\"
```

이 명령은 이름으로 Blob을 다운로드합니다.

### 예제 2: 파이프라인을 사용하여 Blob 콘텐츠 다운로드
```
PS C:\>Get-AzStorageBlob -Container containername -Blob blobname | Get-AzStorageBlobContent
```

이 명령은 파이프라인을 사용하여 Blob 콘텐츠를 찾아 다운로드합니다.

### 예제 3: 파이프라인 및 와일드카드 문자를 사용하여 Blob 콘텐츠 다운로드
```
PS C:\>Get-AzStorageContainer container* | Get-AzStorageBlobContent -Blob "cbox.exe" -Destination "C:\test"
```

이 예제에서는 추가 와일드카드 문자와 파이프라인을 사용하여 Blob 콘텐츠를 찾아 다운로드합니다.

### 예제 4: Blob 개체를 다운로드하고 변수에 저장한 다음 Blob 개체가 있는 Blob 콘텐츠를 다운로드합니다.
```
PS C:\>$blob = Get-AzStorageBlob -Container containername -Blob blobname 
PS C:\>Get-AzStorageBlobContent -CloudBlob $blob.ICloudBlob -Destination "C:\test"
```

이 예제에서는 먼저 Blob 개체를 다운로드하고 변수에 저장한 다음 Blob 개체로 Blob 콘텐츠를 다운로드합니다. 
...
### -CloudBlobContainer
Azure 저장소 클라이언트 라이브러리에서 **CloudBlobContainer** 개체를 지정합니다.
cmdlet을 만들거나 Get-AzStorageContainer 수 있습니다.
...
### -Container
다운로드하려는 Blob이 있는 컨테이너의 이름을 지정합니다.
...
### -Context
Blob 콘텐츠를 다운로드하려는 Azure Storage 계정을 지정합니다.
...
## 참고 사항
* 로컬 컴퓨터에 Blob 이름이 올바르지 않은 경우 이 cmdlet은 가능한 경우 자동으로 연결합니다.
