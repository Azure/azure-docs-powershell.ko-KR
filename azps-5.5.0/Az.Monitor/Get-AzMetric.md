...
## 설명
**Get-AzMetric** cmdlet은 지정된 리소스에 대한 메트릭 값을 얻습니다.

## 예제

### 예제 1: 요약된 출력이 있는 메트릭을 얻게 됩니다.
```
...
이 명령은 1분의 시간 단위로 website3에 대한 메트릭 값을 얻습니다.

### 예제 2: 자세한 출력이 있는 메트릭을 얻을 수 있습니다.
```
...
### 예제 3: 지정된 메트릭에 대한 자세한 출력을 얻게 됩니다.
```
...
이 명령은 요청 메트릭에 대한 자세한 출력을 얻습니다.

### 예제 4: 지정된 차원 필터를 사용하여 지정된 메트릭에 대한 요약 출력을 얻게 됩니다.
```
...
이 명령은 지정된 차원 필터 및 집계 형식을 사용하여 PageViews 메트릭에 대한 요약된 출력을 얻습니다.
...
### -OrderBy
결과 정렬 및 정렬 방향(예: sum asc)에 사용할 집계를 지정합니다.
...
### -ResultType
반환할 결과 형식(메타데이터 또는 데이터)을 지정합니다.
...
### -TimeGrain
hh:mm:ss 형식의 **TimeSpan** 개체로 메트릭의 시간 단위를 지정합니다.
...
### -Top
검색할 레코드의 최대 수(기본값:10)를 지정하여 $filter.
...
지원되는 메트릭에 대한 자세한 내용은 다음에서 찾을 수 있습니다. https://docs.microsoft.com/en-us/azure/azure-monitor/platform/metrics-supported
