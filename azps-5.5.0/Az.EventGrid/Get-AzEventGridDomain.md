...
도메인 이름이 제공되지 않은 경우 도메인 목록이 반환됩니다. 이 목록에 반환되는 요소 수는 Top 매개 변수에 의해 제어됩니다. 상위 값이 지정되지 않은 경우 $null 목록에는 반환된 모든 도메인 항목이 포함되어 있습니다. 그렇지 않으면 맨 위가 목록에 반환될 최대 요소 수를 나타냅니다.
더 많은 도메인을 계속 사용할 수 있는 경우 다음 호출에서 NextLink의 값을 사용하여 도메인의 다음 페이지를 얻습니다.
...
리소스 그룹 \` \` \` MyResourceGroupName의 Event Grid 도메인 Domain1에 대한 세부 정보를 \` 얻습니다.
...
ResourceId 옵션을 사용하여 리소스 그룹 \` \` \` MyResourceGroupName에서 Event Grid 도메인 Domain1의 \` 세부 정보를 얻습니다.
...
Pagination 없이 리소스 그룹 \` MyResourceGroupName의 모든 Event Grid 도메인 나열(모든 도메인은 한 샷으로 \` 반환)
...
리소스 그룹 \` MyResourceGroupName에 Event Grid 도메인(있는 경우)을 나열하여 한 $odataFilter \` 10개 도메인을 충족합니다. 더 많은 결과를 사용할 수 있는 경우 $result. NextLink는 $null. 도메인의 다음 페이지를 얻기 위해 사용자는 도메인을 다시 호출하고 결과를 Get-AzEventGridDomain 합니다. 이전 호출에서 얻은 NextLink입니다. 호출자는 결과 시 중지해야 합니다. NextLink가 $null.
...
페이제이션 없이 Azure 구독의 모든 Event Grid 도메인 나열(모든 도메인은 한 번으로 반환)
...
Azure 구독에서 한 $odataFilter 20개 도메인을 충족하는 Event Grid 도메인(있는 경우)을 나열합니다. 더 많은 결과를 사용할 수 있는 경우 $result. NextLink는 $null. 도메인의 다음 페이지를 얻기 위해 사용자는 도메인을 다시 호출하고 결과를 Get-AzEventGridDomain 합니다. 이전 호출에서 얻은 NextLink입니다. 호출자는 결과 시 중지해야 합니다. NextLink가 $null.
...
이 값은 더 많은 리소스를 계속 Get-AzEventGrid 수 있는 경우 첫 번째 Get-AzEventGrid cmdlet 호출을 통해 얻습니다.
...
필터링은 현재 Name 속성에서만 허용됩니다. 지원되는 작업에는 CONTAINS, eq(같음), ne(같지 않은 경우), AND, OR 및 NOT이 포함됩니다.
...
상위 값을 지정하지 않으면 리소스의 전체 목록이 한에 반환됩니다.
