...
## SYNOPSIS
모든 역할 또는 지정 된 배포 슬롯의 명명 된 역할에 적용 되는 AD (클라우드 서비스 Active Directory) 도메인 확장을 가져옵니다.
...
## 설명은
**Get-AzureServiceADDomainExtension** cmdlet은 지정 된 배포 슬롯의 모든 역할 또는 명명 된 역할에 적용 되는 클라우드 서비스 광고 도메인 확장을 가져옵니다.

## 예제의

### 예제 1: 지정 된 서비스에 대 한 AD 도메인 확장명 가져오기
```
PS C:\> Get-AzureServiceADDomainExtension -ServiceName $Svc
```

이 명령은 $Svc에 지정 된 서비스에 대 한 AD 도메인 확장을 가져옵니다.
...
[제거-AzureServiceADDomainExtension](./Remove-AzureServiceADDomainExtension.md)
