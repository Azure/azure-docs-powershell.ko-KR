...
## 설명은
**AzureNetworkSecurityGroupToSubnet** Cmdlet은 Azure 네트워크 보안 그룹을 서브넷에 연결 합니다.
...
### -Force
이 cmdlet이 서브넷에서 이전 네트워크 보안 그룹의 제거를 확인 하 라는 메시지를 표시 하지 않음을 나타냅니다.
...
### -이름
이 cmdlet가 서브넷에 연결 하는 네트워크 보안 그룹의 이름을 지정 합니다.
...
### -VirtualNetworkName
가상 네트워크의 이름을 지정 합니다.
이 cmdlet은이 매개 변수에서 지정 하는 가상 네트워크의 서브넷에 네트워크 보안 그룹을 연결 합니다.
