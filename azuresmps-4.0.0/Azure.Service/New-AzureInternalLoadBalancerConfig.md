...
## 설명은
**AzureInternalLoadBalancerConfig** Cmdlet은 **InternalLoadBalancerConfig** 개체를 만듭니다.
Azure 가상 컴퓨터 배포를 만들 때 내부 부하 분산 장치 구성을 사용할 수 있습니다.

## 예제의

### 예제 1: 내부 부하 분산 장치 구성 만들기
```
PS C:\> $IlbConfig = New-AzureInternalLoadBalancerConfig -InternalLoadBalancerName "Contoso" -SubnetName "FrontEndSubnet"
```

이 명령은 FrontEndSubnet 이라는 서브넷에 대 한 내부 부하 분산 장치 구성을 만듭니다.
명령은 가상 컴퓨터 배포에 사용할 수 있도록 $IlbConfig 변수에 구성을 저장 합니다.
...
### -StaticVNetIPAddress
이 cmdlet가 구성에 포함 하는 내부 부하 분산 장치에 대 한 가상 네트워크 IP 주소를 지정 합니다.
