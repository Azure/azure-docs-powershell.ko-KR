...
## SYNOPSIS
사이트 복구 자격 증명 모음에서 보호 가능한 엔터티를 가져옵니다.
...
## 설명은
**AzureSiteRecoveryProtectionEntity** cmdlet은 현재 Azure Site Recovery 자격 증명 모음에 있는 가상 컴퓨터와 같은 보호 가능한 엔터티나 protected 엔터티를 가져옵니다.

## 예제의

### 예제 1: 컨테이너에 보호 된 가상 컴퓨터 표시
```
...
두 번째 명령은 $Container의 컨테이너에 속하는 보호 된 가상 컴퓨터를 가져온 다음 표시 합니다.

## 변수

### -Id
가져올 보호 엔터티의 ID를 지정 합니다.
