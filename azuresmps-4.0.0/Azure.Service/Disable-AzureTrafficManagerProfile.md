...
## 설명은
**AzureTrafficManagerProfile** Cmdlet은 Microsoft Azure Traffic Manager 프로필을 사용 하지 않도록 설정 합니다.
*PassThru* 매개 변수를 사용 하 여 작업 성공 여부를 표시할 수 있습니다.

## 예제의

### 예제 1: Traffic Manager 프로필을 사용 하지 않도록 설정 하 고 결과 표시
```
PS C:\>Disable-AzureTrafficManagerProfile -Name "MyProfile" -PassThru
True
```

이 명령은 MyProfile 이라는 Traffic Manager 프로필을 사용 하지 않도록 설정 합니다.
명령이 성공 했는지 여부를 표시 하는 *PassThru* 매개 변수를 지정 합니다.

### 예제 2: Traffic Manager 프로필을 비활성화 하 고 결과를 표시 하지 않습니다.
```
PS C:\>Disable-AzureTrafficManagerProfile -Name "MyProfile"
```

이 명령은 MyProfile 이라는 Traffic Manager 프로필을 사용 하지 않도록 설정 하지만 명령의 성공 여부는 표시 하지 않습니다.

## 변수

### -이름
사용 하지 않도록 설정할 Traffic Manager 프로필의 이름을 지정 합니다.
