...
## 설명은
**Set-AzureSubnetRouteTable** cmdlet은 경로 테이블을 서브넷에 연결 합니다.
...
이 명령은 PublicRouteTable 이라는 경로 테이블을 ContosoSubnet 이라는 서브넷에 연결 합니다.
...
### -RouteTableName
이 cmdlet가 서브넷과 연결 하는 경로 테이블의 이름을 지정 합니다.
...
### -SubnetName
이 cmdlet이 경로 테이블을 연결 하는 서브넷을 지정 합니다.
...
### -VirtualNetworkName
이 cmdlet이 경로 테이블을 연결 하는 서브넷을 포함 하는 가상 네트워크의 이름을 지정 합니다.
