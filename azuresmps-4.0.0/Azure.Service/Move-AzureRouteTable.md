...
## SYNOPSIS
Azure Resource Manager 스택으로 경로 테이블을 마이그레이션합니다.
...
## 설명은
**AzureRouteTable** cmdlet은 경로 테이블을 Azure resource Manager 스택의 리소스 그룹으로 마이그레이션합니다.
...
### -Abort
이 cmdlet이 경로 테이블 마이그레이션을 취소 함을 나타냅니다.
...
### -커밋
이 cmdlet이 경로 테이블 마이그레이션을 시작 함을 나타냅니다.
...
### -준비
이 cmdlet이 마이그레이션에 대 한 경로 테이블을 준비 하는 것을 나타냅니다.
...
### -RouteTableName
이 cmdlet이 마이그레이션하는 경로 테이블의 이름을 지정 합니다.
...
### -유효성 검사
이 cmdlet이 마이그레이션에 대 한 경로 테이블의 유효성을 검사 하도록 지정 합니다.
