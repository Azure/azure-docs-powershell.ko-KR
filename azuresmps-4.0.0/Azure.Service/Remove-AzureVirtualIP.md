...
## 설명은
**제거-AzureVirtualIP** Cmdlet은 Azure 서비스에서 VIP (가상 IP)를 삭제 합니다.
가상 IP에 연결 된 끝점이 없는 경우에만 작업이 성공 합니다.

## 예제의

### 예제 1: 서비스에서 가상 IP 주소 제거
```
PS C:\> Remove-AzureVirtualIP -VirtualIPName "Vip01" -ServiceName "ContosoService03"
```

이 명령은 서비스에서 가상 IP 주소를 제거 합니다.
