...
`Microsoft.Azure.Commands.KeyVault.Models.PSKeyVaultSecret` 형식의 속성 `SecretValueText`가 제거되었습니다. 호출에 `-AsPlainText`를 적용하여 일반 텍스트 비밀을 가져오거나 스크립트에 `SecureString` 형식의 `$secret.SecretValue`를 사용합니다.
