...
## 설명은
Disconnect-AzureRmAccount cmdlet은 연결 된 Azure 계정의 연결을 끊고 해당 계정과 연결 된 모든 자격 증명과 컨텍스트 (구독 및 테 넌 트 정보)를 제거 합니다.
이 cmdlet을 실행 한 후에는 Connect-AzureRmAccount를 사용 하 여 다시 로그인 해야 합니다.
