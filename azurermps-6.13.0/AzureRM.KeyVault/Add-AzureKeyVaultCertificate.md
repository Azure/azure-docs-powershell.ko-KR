...
## 설명은
**AzureKeyVaultCertificate** Cmdlet은 Azure key vault의 키 보관소에서 인증서 등록 프로세스를 시작 합니다.
...
첫 번째 명령은 New-AzureKeyVaultCertificatePolicy cmdlet을 사용 하 여 인증서 정책을 만든 다음 $Policy 변수에 저장 합니다.
두 번째 명령은 **Add-AzureKeyVaultCertificate** 를 사용 하 여 프로세스를 시작 하 여 인증서를 만듭니다.
세 번째 명령은 Get-AzureKeyVaultCertificateOperation cmdlet을 사용 하 여 작업을 폴링하여 작업이 완료 되었는지 확인 합니다.
마지막 명령은 Get-AzureKeyVaultCertificate cmdlet을 사용 하 여 인증서를 가져옵니다.

## 변수

### -CertificatePolicy
**KeyVaultCertificatePolicy** 개체를 지정 합니다.
...
매개 변수: CertificatePolicy (ByValue)
