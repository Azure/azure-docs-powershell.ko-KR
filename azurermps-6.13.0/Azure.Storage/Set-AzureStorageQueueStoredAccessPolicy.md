...
## 설명은
**AzureStorageQueueStoredAccessPolicy** Cmdlet은 Azure 저장소 큐에 대해 저장 된 액세스 정책을 설정 합니다.

## 예제의

### 예제 1: 모든 권한을 사용 하 여 큐에 저장 된 액세스 정책 설정
```
PS C:\> Set-AzureStorageQueueStoredAccessPolicy -Queue "MyQueue" -Policy "Policy07" -Permission arup
```

이 명령은 MyQueue 이라는 저장소 큐의 Policy07 라는 액세스 정책을 설정 합니다.
...
### -NoStartTime
이 cmdlet이 시작 시간 $Null 설정 하는 것을 나타냅니다.
