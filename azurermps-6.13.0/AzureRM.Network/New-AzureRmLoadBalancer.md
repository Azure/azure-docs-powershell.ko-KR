...
## 설명은
**AzureRmLoadBalancer** Cmdlet은 Azure 부하 분산 장치를 만듭니다.
...
부하 분산 장치를 배포 하려면 먼저 여러 개체를 만들어야 하 고 처음 일곱 개의 명령은 해당 개체를 만드는 방법을 보여 줍니다.
여덟 번째 명령은 Myloadbalancer 이라는 리소스 그룹에서 MyLoadBalancer 라는 부하 분산 장치를 만듭니다.
아홉 번째 및 마지막 명령은 새 부하 분산을 가져와 성공적으로 생성 되었는지 확인 합니다.
이 예제에서는 부하 분산 장치를 만드는 방법을 보여 줍니다. 또한 Nic를 다른 가상 컴퓨터에 할당 하려면 Add-AzureRmNetworkInterfaceIpConfig cmdlet을 사용 하 여 구성 해야 합니다.
...
### -BackendAddressPool
부하 분산 장치와 연결할 백 엔드 주소 풀을 지정 합니다.
...
### -Force
동일한 이름을 가진 부하 분산이 이미 있는 경우에도이 cmdlet이 부하 분산 장치를 만드는 것을 나타냅니다.
...
부하 분산 장치와 연결할 프런트 엔드 IP 주소 목록을 지정 합니다.
...
### -InboundNatRule
부하 분산 장치에 연결 하는 데 사용 되는 인바운드 NAT (network address translation) 규칙의 목록을 지정 합니다.
...
### -LoadBalancingRule
부하 분산 장치에 연결 하는 부하 분산 규칙의 목록을 지정 합니다.
...
### System.webserver. 목록 1 [[[PSFrontendIPConfiguration, Microsoft azure. 6.4.1.0,. *. *. *. * * \` , Version =, Culture = 중립, PublicKeyToken = null]]

### System.webserver. 목록 1 [[[PSBackendAddressPool, Microsoft azure. 6.4.1.0,. *. *. *. * * \` , Version =, Culture = 중립, PublicKeyToken = null]]

### System.webserver. 목록 1 [[Microsoft Azure. *. *. *. *. *. \` 6.4.1.0, Version =, Culture = 중립, PublicKeyToken = null]]

### System.webserver. 목록 1 [[[PSInboundNatRule, Microsoft azure. 6.4.1.0,. *. *. *. * * \` , Version =, Culture = 중립, PublicKeyToken = null]]

### System.webserver. 목록 1 [[[PSLoadBalancingRule, Microsoft azure. 6.4.1.0,. *. *. *. * * \` , Version =, Culture = 중립, PublicKeyToken = null]]

### System.webserver. 컬렉션 테이블

### System.webserver. 목록 1 [[[PSInboundNatPool, Microsoft azure. 6.4.1.0,. *. *. *. * * \` , Version =, Culture = 중립, PublicKeyToken = null]]
