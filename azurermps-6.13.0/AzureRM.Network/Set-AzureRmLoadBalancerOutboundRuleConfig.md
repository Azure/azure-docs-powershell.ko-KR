...
## 설명은
**AzureRmLoadBalancerOutboundRuleConfig** Cmdlet은 Azure 부하 분산 장치에 대 한 아웃 바운드 규칙 구성을 설정 합니다.

## 예제의

### 예제 1: 부하 분산 장치에서 아웃 바운드 규칙 구성 수정
```powershell
...
두 번째 명령은 파이프라인 연산자를 사용 하 여 $slb의 부하 분산을 추가-AzureRmLoadBalancerOutboundRuleConfig에 전달 하 여 아웃 바운드 규칙 구성을 추가 합니다.
세 번째 명령은 아웃 바운드 규칙 구성을 저장 하 고 업데이트 하는 부하 분산을 **설정-AzureRmLoadBalancerOutboundRuleConfig** 에 전달 합니다.
...
시스템.. i n i a n. 목록 \` 1 [[Microsoft azure. PSBackendAddressPool. 모델. PSResourceId, Microsoft azure. commands. 네트워크, 버전 = 6.5.0.0, Culture = 중립, PublicKeyToken = null]] Microsoft Azure.
