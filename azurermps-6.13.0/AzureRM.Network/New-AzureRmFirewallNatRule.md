...
## 설명은
**AzureRmFirewallNatRule** Cmdlet은 Azure 방화벽에 대 한 NAT 규칙을 만듭니다.

## 예제의

### 1: 대상 10.1.2.3:80에서 대상 10.4.5.6:8080으로 10.0.0.0/24의 모든 TCP 트래픽에 대해 DNAT 대 한 규칙을 만듭니다.
```
...
이 예제에서는 대상 10.1.2.3:80에서 10.4.5.6:8080으로 10.0.0.0/24에서 발생 하는 모든 트래픽에 DNAT에 해당 하는 규칙을 만듭니다.
...
### -프로토콜
이 규칙에 따라 필터링 할 트래픽 유형을 지정 합니다.
지원 되는 프로토콜은 TCP 및 UDP입니다.
특별 한 "모든" 값을 사용할 수 있으며,이는 TCP 및 UDP와 다른 프로토콜은 일치 하지 않는다는 의미입니다.
...
### PSFirewallNatRule에 대 한.

## 상속자

## 관련 링크

[새로운 AzureRmFirewallNatRuleCollection](./New-AzureRmFirewallNatRuleCollection.md)
