...
## 설명은
**AzureRmMediaService** cmdlet은 미디어 서비스를 만듭니다.
미디어 서비스가 이미 있는 경우이 cmdlet은 해당 속성을 업데이트 합니다.

## 예제의

### Example1: 기본 저장소 계정만 사용 하 여 미디어 서비스 만들기
```
PS C:\># Variables
## Global
...
이 예제에서는 기본 저장소 계정만 지정 하 여 미디어 서비스를 만드는 방법을 보여 줍니다.
이 스크립트는 여러 다른 cmdlet을 사용 합니다.

### 예제 2: 저장소를 여러 개 사용 하 여 미디어 서비스 만들기
```
PS C:\># Variables
...
이 예제에서는 여러 저장소 계정을 사용 하 여 미디어 서비스를 만드는 방법을 보여 줍니다.
이 스크립트는 여러 다른 cmdlet을 사용 합니다.

## 변수

### -AccountName
이 cmdlet이 만드는 미디어 서비스의 이름을 지정 합니다.
...
### -위치
이 cmdlet이 미디어 서비스를 만드는 영역을 지정 합니다.
...
### -ResourceGroupName
미디어 서비스가 할당 된 리소스 그룹의 이름을 지정 합니다.
...
### -StorageAccountId
미디어 서비스 계정과 연결 된 저장소 계정을 지정 합니다.
...
### -StorageAccounts
미디어 서비스와 연결할 저장소 계정의 배열을 지정 합니다.
