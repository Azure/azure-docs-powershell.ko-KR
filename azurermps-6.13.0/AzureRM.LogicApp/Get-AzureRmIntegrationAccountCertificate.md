...
## 설명은
**Get-AzureRmIntegrationAccountCertificate** cmdlet은 리소스 그룹에서 통합 계정 인증서를 가져옵니다.
통합 계정 이름, 리소스 그룹 이름 및 인증서 이름을 지정 합니다.
...
이 명령은 IntegrationAccountCertificate01 이라는 통합 계정 인증서를 가져옵니다.

### 예제 2: 통합 계정 이름을 기준으로 통합 계정 인증서 가져오기
```
...
이 명령은 IntegrationAccount31 이라는 통합 계정에 대 한 통합 계정 인증서를 가져옵니다.
