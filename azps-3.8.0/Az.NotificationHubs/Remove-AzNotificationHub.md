...
## 설명은
**AzNotificationHub** cmdlet에서는 기존 알림 허브를 제거 합니다.
알림 허브는 해당 클라이언트에서 사용 하는 플랫폼에 관계 없이 여러 클라이언트로 푸시 알림을 보내는 데 사용 됩니다.
플랫폼에는 iOS, Android, Windows Phone 8, Windows 스토어가 포함 되지만이에 국한 되지 않습니다.
알림 허브는 개별 앱과 거의 동일 합니다. 각 앱에는 일반적으로 자체 알림 허브가 있습니다.
**제거-AzNotificationHub** cmdlet을 사용 하 여 기존 알림 허브를 제거할 수 있습니다.
허브가 제거 된 후에는 더 이상 해당 허브를 사용 하 여 사용자에 게 푸시 알림을 보낼 수 없습니다.
...
이 명령은 ContosoInternalHub 이라는 알림 허브를 제거 합니다.
허브를 제거 하려면 허브가 있는 네임 스페이스와 허브가 할당 된 리소스 그룹을 지정 해야 합니다.
