...
## 설명은
**AzApplicationGatewayUrlPathMapConfig** cmdlet은 백 엔드 서버 풀에 대 한 URL 경로 매핑 배열의 구성을 설정 합니다.
...
두 번째 명령은 응용 프로그램 게이트웨이에서 map01 이라는 URL 경로 매핑을 업데이트 합니다.
세 번째 명령은 응용 프로그램 게이트웨이를 업데이트 합니다.

## 변수

### -ApplicationGateway
이 cmdlet이 URL 경로 맵 구성을 설정 하는 응용 프로그램 게이트웨이를 지정 합니다.
...
### -이름
이 cmdlet이 구성을 설정 하는 URL 경로 맵 이름을 지정 합니다.
