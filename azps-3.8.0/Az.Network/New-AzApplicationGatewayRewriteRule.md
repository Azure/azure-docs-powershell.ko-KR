...
## 설명은
**AzApplicationGatewayRewriteRule Cmdlet은** Azure application gateway에 대 한 재작성 규칙을 만듭니다.

## 예제의

### 예제 1: 응용 프로그램 게이트웨이에 대 한 다시 작성 규칙 만들기
```powershell
PS C:\> $rule = New-AzApplicationGatewayRewriteRule -Name rule1 -ActionSet $action -RuleSequence 101 -Condition $condition
```

이 명령은 rule1 이라는 재작성 규칙을 만들고 그 결과를 $rule 라는 변수에 저장 합니다.
...
### -RuleSequence
다시 작성 규칙 집합에서이 재작성 규칙의 규칙 순서 지정
...
### PSApplicationGatewayRewriteRule에 대 한.
