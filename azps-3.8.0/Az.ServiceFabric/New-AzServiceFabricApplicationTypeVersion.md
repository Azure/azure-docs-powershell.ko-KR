...
## 설명은
이 cmdlet은-PackageUrl에 지정 된 패키지를 사용 하 여 새 응용 프로그램 유형 버전을 만들며, 배포 중에 Azure 리소스 관리자가 사용할 REST 끝점을 통해 액세스할 수 있어야 하며 sfpkg를 사용 하 여 패키지 및 압축 된 응용 프로그램을 포함 합니다. 이 명령은 아직 없는 경우 응용 프로그램 종류를 만듭니다.
...
이 예제에서는 "testAppType" 형식으로 응용 프로그램 유형 버전 "v1"을 만듭니다. 패키지에 포함 된 응용 프로그램 매니페스트의 버전은 버전에 지정 된 것과 동일한 버전을 사용 해야 합니다.
...
### -DefaultParameter
응용 프로그램 매개 변수의 기본 값을 키/값 쌍으로 지정 합니다.
