...
## 설명은
**Get-AzIntegrationAccountAssembly** cmdlet은 통합 계정에서 어셈블리를 가져옵니다.

## 예제의

### 예제 1: 매개 변수를 기준으로 어셈블리 가져오기
```powershell
...
"SampleResourceGroup" 리소스 그룹에 포함 된 통합 계정 "sampleIntegrationAccount"에 있는 "sampleAssembly" 라는 어셈블리를 가져옵니다.

### 예제 2: 통합 계정의 모든 어셈블리를 매개 변수로 나열
```powershell
...
리소스 그룹 "sampleResourceGroup"에 포함 되어 있는 통합 계정 "sampleIntegrationAccount"에 있는 모든 어셈블리를 가져옵니다.
