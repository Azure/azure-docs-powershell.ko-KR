...
## 설명은
**AzVmssDataDisk** cmdlet은 데이터 디스크를 Vmss (가상 컴퓨터 크기 조정 집합) 인스턴스에 추가 합니다.
...
이 명령은 VMSS 개체에 빈 데이터 디스크를 추가 합니다.
...
### -DiskIOPSReadWrite
관리 디스크에 대 한 Read-Write IOPS를 지정 합니다. StorageAccountType이 UltraSSD_LRS 경우에만 사용 해야 합니다. 지정 하지 않으면 diskSizeGB를 기준으로 기본값을 할당 받습니다.
...
### -DiskMBpsReadWrite
관리 디스크의 대역폭 (초당 MB)을 지정 합니다. StorageAccountType이 UltraSSD_LRS 경우에만 사용 해야 합니다. 지정 하지 않으면 diskSizeGB를 기준으로 기본값을 할당 받습니다.
