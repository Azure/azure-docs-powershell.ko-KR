...
## 설명
**Get-AzApiManagementApiRelease** cmdlet은 Azure API Management API의 하나 이상의 릴리스를 얻습니다.

## 예제

### 예제 1: API의 모든 릴리스를 얻습니다.
```powershell
...
이 명령은 지정된 컨텍스트에 대한 `echo-api` API의 모든 릴리스를 얻습니다.

### 예제 2: 특정 API 릴리스의 릴리스 정보 얻기
```powershell
PS C:\>$ApiMgmtContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
이 명령은 지정된 releaseId를 사용하여 특정 API의 릴리스 정보를 얻습니다.
...
### -ResourceId
Api 릴리스의 Arm 리소스 식별자입니다. 지정된 경우 식별자에 의해 API 릴리스를 찾으려고 시도합니다. 이 매개 변수는 필수입니다.
