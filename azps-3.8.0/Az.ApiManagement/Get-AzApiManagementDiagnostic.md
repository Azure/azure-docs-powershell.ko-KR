...
## 설명은
**AzApiManagementDiagnostic** 는 지정 된 범위에서 Api management 서비스에 구성 된 진단 정보를 가져옵니다.

## 예제의

### 예제 1: 테 넌 트 범위에서 구성 된 모든 진단을 가져옵니다.
```powershell
...
이 명령은 Api Management 서비스에 구성 된 모든 진단을 가져옵니다.

### 예제 2: Api 범위에서 구성 된 모든 진단 가져오기
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
이 명령은 Api 범위에서 구성 된 모든 진단을 가져옵니다. `echo-api`

### 예제 3: Id로 지정 된 API 범위 진단 가져오기
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
이 명령은 `applicationinsights` api에 구성 된 진단 정보를 가져옵니다 `echo-api` .

## 변수

### -ApiId
기존 API의 식별자입니다.
지정 된 경우 API 범위 진단을 반환 합니다.
이 매개 변수는 필수 사항입니다.
...
### -ResourceId
진단 또는 Api 진단의 Arm 리소스 식별자입니다. 지정 된 경우 식별자를 사용 하 여 진단 검색을 시도 합니다. 이 매개 변수는 필수입니다.
