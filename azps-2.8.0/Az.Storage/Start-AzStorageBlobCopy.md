...
### 예제 6: 새 blob 이름을 사용 하 여 대상 컨테이너에 블록 blob을 복사 하 고 대상 blob StandardBlobTier을 보관 RehydratePriority로 설정 합니다.
```
...
이 명령은 블록 blob의 복사 작업을 새 blob 이름으로 대상 컨테이너로 시작 하 고, 대상 blob StandardBlobTier을 RehydratePriority로 설정 합니다.
