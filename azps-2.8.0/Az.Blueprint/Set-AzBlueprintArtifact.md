...
## 설명은
아티팩트를 업데이트 합니다. 아티팩트를 입력 파일로 업데이트 하거나 아티팩트에 대 한 인라인 매개 변수를 제공 하는 등 두 가지 방법 중 하나를 사용할 수 있습니다. JSON 메서드에는 인라인 매개 변수 메서드를 제공할 아티팩트의 형식이 필요 하지 않지만 사용자는 아티팩트를 통해 형식의 매개 변수를 제공 해야 합니다.
...
아티팩트 JSON 파일을 통해 아티팩트를 업데이트 합니다.
...
ARM 서식 파일을 통해 아티팩트를 업데이트 합니다.

## 변수

### -ArtifactFile
디스크의 JSON 형식에서 아티팩트 파일의 위치입니다.
...
### -DependsOn
현재 아티팩트를 만들기 전에 만들어야 할 아티팩트의 이름 목록입니다.
...
정책 정의 아티팩트에 전달할 매개 변수의 해시 테이블입니다.
...
### -서식 파일 Parameterfile
디스크에 있는 ARM template 매개 변수 파일의 위치입니다.
...
### -Type
아티팩트의 유형입니다.
지원 되는 형식에는 RoleAssignmentArtifact, PolicyAssignmentArtifact, 서식 가공물 등 3 가지가 있습니다.
...
### Microsoft. i a m. 모델. PSArtifactKind
