...
\' \` 리소스 그룹 MyResourceGroupName에서 이벤트 그리드 도메인 Domain1 키 key1 ' \에 해당 하는 키를 다시 생성 \` \` \` 합니다.
...
\' \` \` \` \` 전체 리소스 Id를 사용 하 여 리소스 그룹 MyResourceGroupName의 이벤트 그리드 도메인 Domain1의 키 key2 ' \ '에 해당 하는 키를 다시 생성 합니다.
...
### Microsoft. Azure. 관리. DomainSharedAccessKeys
