...
## 설명은
**AzDeploymentManagerService** cmdlet은 서비스 토폴로지 아래에 서비스를 만들고 해당 서비스를 나타내는 개체를 반환 합니다.
서비스를 이름, 서비스 토폴로지, 그리고 리소스 그룹 이름으로 지정 합니다. 

Cmdlet은 서비스 개체를 반환 합니다. 이 개체를 로컬로 수정한 다음 Set-AzDeploymentManagerService cmdlet을 사용 하 여 서비스에 변경 내용을 적용할 수 있습니다.
...
중앙 미국에 있는 리소스 그룹 ContosoResourceGroup의 서비스 토폴로지 ContosoServiceTopology에서 이름이 ContosoService1 인 새 서비스를 만듭니다. TargetLocation 속성은 서비스 ContosoService1 지정 된 구독의 동부 US 지역에 배포 되어야 함을 나타냅니다.
