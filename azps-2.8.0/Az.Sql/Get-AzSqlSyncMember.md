...
## 설명은
**AzSqlSyncMember** cmdlet은 하나 이상의 Azure SQL 데이터베이스 동기화 구성원에 대 한 정보를 반환 합니다.
...
### 예제 3: 필터링을 사용 하 여 동기화 그룹에 할당 된 Azure SQL Sync 구성원의 모든 인스턴스 가져오기
```
...
이 명령은 "SyncMember"로 시작 하는 동기화 그룹 SyncGroup01에 할당 된 모든 Azure SQL 데이터베이스 동기화 구성원에 대 한 정보를 가져옵니다.
