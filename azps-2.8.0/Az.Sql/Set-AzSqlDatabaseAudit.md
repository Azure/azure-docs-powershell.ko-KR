...
## 설명은
**AzSqlDatabaseAudit** Cmdlet은 Azure SQL 데이터베이스의 감사 설정을 변경 합니다.
...
### 예제 3.1: T-SQL 조건자를 사용 하 여 고급 필터링이 포함 된 Azure SQL 데이터베이스의 blob 저장소 감사 정책 사용
```
...
### 예제 3.2: Azure SQL 데이터베이스의 감사 정책에서 고급 필터링 설정을 제거 합니다.
```
...
### 예제 4: Azure SQL 데이터베이스의 이벤트 허브 감사 정책 사용
```
...
### 예제 5: Azure SQL 데이터베이스의 이벤트 허브 감사 정책 사용 안 함
```
...
### 예제 6: Azure SQL 데이터베이스의 log analytics 감사 정책 사용
```
...
### 예제 7: Azure SQL 데이터베이스의 log analytics 감사 정책 사용 안 함
```
...
### 예제 8: Azure SQL 데이터베이스의 log analytics 감사 정책을 통해 파이프라인 사용 안 함
```
...
### 예제 9: Azure SQL 데이터베이스의 감사 레코드를 blob 저장소로 보내지 않도록 설정 하 고 로그 분석으로 보내는 기능을 사용 하도록 설정 합니다.
```
...
### 예제 10: Azure SQL 데이터베이스의 감사 레코드를 blob 저장소, 이벤트 허브 및 로그 분석으로 보내는 기능을 사용 하도록 설정 합니다.
```
...
받기  
REFERENCES  
감사할 작업을 정의 하는 일반적인 양식은 [principal]에 따라 [개체]에서 [동작]을 선택 합니다. 위의 형식에서 [개체]는 테이블, 뷰 또는 저장 프로시저 또는 전체 데이터베이스 또는 스키마와 같은 개체를 참조할 수 있다는 점에 유의 하세요. 두 번째 경우에는 forms 데이터베이스:: [dbname] 및 SCHEMA:: [schemaname]이 각각 사용 됩니다.
예를 들어:  
공용에서 dbo myTable 선택  
...
### Microsoft. .Sql. 감사. DatabaseAuditModel
