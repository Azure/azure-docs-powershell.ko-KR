...
### AddAzureRmSqlServerTransparentDataEncryptionCertificateDefaultParameterSet (기본값)
```
...
## 설명은
Add-AzSqlManagedInstanceTransparentDataEncryptionCertificate에서 지정 된 SQL Server 인스턴스에 대 한 투명 데이터 암호화 인증서를 추가 합니다.
...
리소스 그룹 이름 및 SQL Server 이름을 사용 하 여 sql server에 TDE 인증서 추가
...
서버 resourceId를 사용 하 여 서버에 TDE certificate 추가
...
리소스 그룹의 모든 sql server에 TDE 인증서 추가
...
### -PassThru
성공적으로 실행 되 면 추가 된 인증서 개체를 반환 합니다.
