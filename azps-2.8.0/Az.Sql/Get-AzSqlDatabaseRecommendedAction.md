...
## 설명은
**AzSqlDatabaseRecommendedAction** Cmdlet은 Azure SQL Database Advisor에 대해 권장 되는 작업을 하나 이상 가져옵니다.

## 예제의

### 예제 1: 관리자에 게 권장 되는 모든 작업 나열
```
...
이 명령은 wi-러너-오스트레일리아 라는 데이터베이스에 대해 사용할 수 있는 CreateIndex 라는 관리자의 모든 권장 작업 목록을 가져옵니다.

### 예제 2: 관리자를 위한 단일 권장 작업 가져오기
```
...
이 명령은 \[ \] \[ \] createindex 라는 관리자에 대 한 IR_ test_schema _ test_table_0 .0361551 _6C7AE8CC9C87E7FD5893의 권장 작업을 가져옵니다.

## 변수

### -AdvisorName
이 cmdlet가 추천 작업을 요청 하는 관리자의 이름을 지정 합니다.
...
### -DatabaseName
이 cmdlet이 권장 작업을 요청 하는 데이터베이스의 이름을 지정 합니다.
...
이 cmdlet이 가져오는 권장 작업의 이름을 지정 합니다.
...
### RecommendedAction. AzureSqlDatabaseRecommendedActionModel에 대 한

## 상속자
* 키워드: azure, azurerm, arm, resource, 관리, manager, sql, 데이터베이스, mssql, advisor, recommendedaction
