...
새 AKS(Azure Kubernetes Service) 클러스터를 만들 수 있습니다.

## 예제

### 기본 파라메인 AKS를 새로 추가합니다.
...
AKS에서 Windows Server 컨테이너를 만들하려면 AKS를 만들 때 다음 매개 변수를 4개 이상 지정해야 합니다. 이 값은 각각 및 그 값에 `NetworkPlugin` `NodeVmSetType` `azure` `VirtualMachineScaleSets` 해당해야 합니다.
...
### -AcrNameToAttach
지정된 ACR의 'acrpull' 역할을 AKS 서비스 주체(예: myacr)에 부여
...
### -DnsNamePrefix
클러스터에 대한 DNS 이름 연결입니다. 사용자가 windows 컨테이너를 <경우 길이는 9입니다.
...
### -EnableRbac
Kubernetes 및 Access를 Role-Based 여부
...
### -NodeOsDiskSize
노드 풀의 각 노드에 대한 OS 디스크의 크기(GB)입니다. 최소 30GB.
...
우선 순위가 낮은 가상 머신 확장 집합에 대한 지우기 정책을 지정하는 데 사용할 ScaleSetEvictionPolicy입니다. 기본값은 삭제입니다.
...
### -NodeSetPriority
가상 머신 확장 집합 우선 순위를 지정하는 데 사용할 ScaleSetPriority입니다. 기본값은 일반입니다.
...
### -NodeVmSetType
AgentPoolType은 에이전트 풀의 유형을 나타남 가능한 값은 'VirtualMachineScaleSets', 'AvailabilitySet'입니다.
...
Windows VM에 사용할 관리자 암호의 길이는 12자 이상으로, 소문자(예: 1개와 특수 문자 1개)를 `[a-z]` `[A-Z]` 포함해야 `[!@#$%^&_()]` 합니다.
