...
## SYNOPSIS
새 Kubernetes 소스 제어 구성을 만들 수 있습니다.
...
### 예제 1: kubernetes 클러스터에 대한 구성 만들기
```powershell
...
이 명령은 kubernetes 클러스터에 대한 구성을 만듭니다.
...
### -ClusterScoped
전달된 경우 구성 범위를 클러스터로 설정(기본값: nameSpace)합니다.
...
### -ClusterType
Kubernetes 클러스터 리소스 이름 - managedClusters(AKS 클러스터용) 또는 connectedClusters(OnPrem K8S 클러스터의 경우)
...
### -EnableHelmOperator
이 git 구성에 Helm 연산자를 사용하도록 설정하는 옵션입니다.
...
### -OperatorNamespace
이 연산자가 설치된 네임스페이스입니다.
최대 253자 소문자 영속 문자, 하이픈 및 기간만 해당.
...
### -OperatorParameters
문자열 형식의 연산자 인스턴스에 대한 모든 매개 변수입니다.
