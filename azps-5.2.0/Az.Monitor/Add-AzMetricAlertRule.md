...
규칙이 없는 경우 사용하지 않도록 설정됩니다.
규칙이 있으면 해당 규칙만 사용하지 않도록 설정됩니다.
...
규칙을 평가할 때 기간에 적용할 집계 연산자를 지정합니다.
