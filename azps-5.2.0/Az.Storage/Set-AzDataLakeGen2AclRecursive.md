...
## 설명
**Set-AzDataLakeGen2AclRecursive** cmdlet은 지정된 경로에서 ACL을 재발적으로 설정합니다. 입력 ACL은 원래 ACL을 완전히 대체합니다.

## 예제

### 예제 1: directiry에 재발적으로 ACL 설정
```
...
이 명령은 먼저 ACL 항목이 3개인 ACL 개체를 만든 다음, 디렉터리에서 ACL을 재발적으로 설정합니다.

### 예제 2: 파일 파일의 루트 지시문에 ACL 재발 설정
```
...
이 스크립트는 디렉터리 청크에서 ACL을 청크로 재구성하게 설정하고 청크 크기는 BatchSize * MaxBatchCount로 설정됩니다. 청크 크기는 이 스크립트에서 200입니다.

### 예제 4: 디렉터리 및 ContinueOnFailure에서 ACL을 재발적으로 설정한 다음 오류에서 하나씩 다시 시작
```
...
이 명령은 먼저 ACL을 ContinueOnFailure가 있는 디렉터리로 재구성하여 설정하고 일부 항목이 실패한 다음 실패한 항목을 하나씩 다시 시작합니다.
