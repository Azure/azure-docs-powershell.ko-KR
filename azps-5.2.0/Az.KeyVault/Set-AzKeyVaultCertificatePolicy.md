...
## 설명
**Set-AzKeyVaultCertificatePolicy** cmdlet은 키 자격 증명 모음에서 인증서에 대한 정책을 생성하거나 업데이트합니다.
...
이 명령은 ContosoKV01 키 자격 증명 모음에서 TestCert01 인증서에 대한 정책을 설정합니다.

## PARAMETERS

### -CertificateTransparency
이 인증서/발급자에 대해 인증서 투명도를 사용하도록 설정되어 있는지 여부를 나타냅니다. 지정하지 않으면 기본값은 'true'입니다.
`-IssuerName` 이 속성을 설정할 때 지정해야 합니다.
...
### -EmailAtPercentageLifetime
알림에 대한 자동 프로세스가 시작된 후 수명의 백분율을 지정합니다.
...
### -RenewAtNumberOfDaysBeforeExpiry
인증서 갱신에 대한 자동 프로세스가 시작되는 만료 전 일 수를 지정합니다.
...
### -RenewAtPercentageLifetime
인증서 갱신에 대한 자동 프로세스가 시작되는 수명의 백분율을 지정합니다.
