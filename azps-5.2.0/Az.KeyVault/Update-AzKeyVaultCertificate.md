...
## 설명
**Update-AzKeyVaultCertificate** cmdlet은 인증서의 편집 가능한 특성을 수정합니다.
...
첫 번째 명령은 키/값 쌍의 배열을 변수에 $Tags 할당합니다.
두 번째 명령은 TestCert01이라는 인증서의 태그 값을 $Tags.

### 예제 2

인증서의 편집 가능한 특성을 수정합니다. (자동Generated)
...
### -Enable
있는 경우 값이 true이면 인증서를 사용하도록 설정합니다.
값이 false이면 인증서를 사용하지 않도록 설정합니다.
지정하지 않으면 인증서의 사용/사용 안함 상태의 기존 값은 변경되지 않은 상태로 남아 있습니다.
...
### -Tag
인증서 태그를 나타내는 해시테이블입니다.
지정하지 않으면 인증의 기존 태그는 변경되지 않은 상태로 유지됩니다.
...
### -Version
인증서 버전입니다.
Cmdlet은 자격 증명 모음 이름, 현재 선택한 환경, 인증서 이름 및 인증서 버전에서 인증서의 FQDN을 생성합니다.
