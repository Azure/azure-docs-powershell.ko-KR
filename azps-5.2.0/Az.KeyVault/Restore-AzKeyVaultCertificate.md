...
이 인증서는 입력 파일의 백업된 인증서 복제본으로, 원래 인증서와 이름이 동일합니다.
Key Vault에 동일한 이름의 인증서가 이미 포함되어 있는 경우 이 cmdlet은 원래 인증서를 덮어써야 하는 대신 실패합니다.
...
Azure 지역을 지역에 매핑하는 경우 Microsoft Azure 보안 센터를 https://azure.microsoft.com/support/trust-center/) 참조하세요.
...
이 명령은 Backup.blob이라는 백업 파일에서 MyKeyVault라는 키 자격 증명 모음으로 모든 버전을 포함한 인증서를 복원합니다.
