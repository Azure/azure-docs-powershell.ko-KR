...
## 설명
**Set-AzSynapseSqlPoolAuditSetting** cmdlet은 Azure Synapse Analytics 풀의 감사 설정을 SQL 변경합니다.
...
ContosoSqlPool이라는 Azure Synapse Analytics SQL Blob Storage 감사 정책을 사용하도록 설정할 수 있습니다.
...
T-SQL 사용하여 고급 필터링이 있는 ContosoSqlPool이라는 Azure Synapse Analytics SQL 풀의 Blob Storage 감사 정책을 사용하도록 SQL 합니다.
...
ContosoSqlPool이라는 Azure Synapse Analytics SQL 정책에서 고급 필터링 설정을 제거합니다.
...
감사할 작업을 정의하는 일반적인 양식은

\[action \] ON object BY \[ \] \[ principal\]

위의 형식의 개체는 테이블, 뷰 또는 저장 프로시저와 같은 개체 또는 전체 데이터베이스 또는 \[ \] 스마마를 참조할 수 있습니다.
후자의 경우 데이터베이스:: \[ dbname 및 \] SCHEMA:: \[ schemaname 양식이 각각 \] 사용됩니다.
