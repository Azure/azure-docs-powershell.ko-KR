...
## 설명
이 Set-AzContext cmdlet은 현재 세션에서 실행되는 cmdlet에 대한 인증 정보를 설정합니다.
컨텍스트에는 테넌트, 구독 및 환경 정보가 포함됩니다.
...
이 명령은 지정된 구독을 사용할 컨텍스트를 설정합니다.

## PARAMETERS

### -Context
현재 세션에 대한 컨텍스트를 지정합니다.
...
### -Force
기존 컨텍스트를 동일한 이름(있는 경우)으로 덮어 덮어 습니다.
...
### -Subscription
컨텍스트를 설정해야 하는 구독의 이름 또는 ID입니다. 이 매개 변수에는 -SubscriptionName 및 -SubscriptionId에 대한 별칭이 있으므로 명확성을 위해 각각 이름 및 ID를 지정할 때 -Subscription 대신 이러한 매개 변수를 사용할 수 있습니다.
