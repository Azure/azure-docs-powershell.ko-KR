...
## 설명
**Invoke-AzDataBoxEdgeDevice** cmdlet은 Data Box Edge 디바이스에서 업데이트를 검색, 다운로드 및 설치하는 작업을 호출합니다. 자동 검사는 디바이스에서 매일 실행됩니다. 이 cmdlet을 실행하여 명시적으로 검색을 트리거할 수 있습니다.
...
### -FetchUpdate
데이터 상자 에지/게이트웨이 디바이스에서 업데이트를 다운로드합니다.
...
### -InstallUpdate
데이터 상자 에지/게이트웨이 디바이스에 업데이트를 설치합니다.
