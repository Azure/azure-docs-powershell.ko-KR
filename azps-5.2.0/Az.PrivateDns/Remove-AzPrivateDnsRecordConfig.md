...
## 설명
이 Remove-AzPrivateDnsRecordConfig cmdlet은 레코드 집합에서 DNS(개인 도메인 이름 시스템) 레코드를 제거합니다. RecordSet 개체는 오프라인 개체로, Microsoft Azure 개인 DNS 서비스에 대한 변경 내용을 유지하기 위해 Set-AzPrivateDnsRecordSet cmdlet을 실행한 후에만 개인 DNS 응답을 변경하지 않습니다. 레코드를 제거하려면 해당 레코드 형식의 모든 필드가 정확히 일치해야 합니다. SOA 레코드를 추가하거나 제거할 수 없습니다. SOA 레코드는 사설 DNS 영역이 만들어지면 자동으로 생성되고 사설 DNS 영역이 삭제될 때 자동으로 삭제됩니다. RecordSet 개체를 매개 변수로 또는 파이프라인 연산자를 사용하여 이 cmdlet에 전달할 수 있습니다.
