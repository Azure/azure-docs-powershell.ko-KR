...
## 설명
**Remove-AzNotificationHub** cmdlet은 기존 알림 허브를 제거합니다.
알림 허브는 해당 클라이언트에서 사용하는 플랫폼에 관계없이 여러 클라이언트에 푸시 알림을 보내는 데 사용됩니다.
플랫폼에는 iOS, Android, Windows Phone 8 및 Windows 스토어가 포함됩니다.
알림 허브는 개별 앱과 거의 동일합니다. 각 앱에는 일반적으로 자체 알림 허브가 있습니다.
**Remove-AzNotificationHub** cmdlet을 사용하여 기존 알림 허브를 제거할 수 있습니다.
허브가 제거된 후 더 이상 해당 허브를 사용하여 사용자에게 푸시 알림을 보낼 수 없습니다.
...
이 명령은 ContosoInternalHub라는 알림 허브를 제거합니다.
허브를 제거하려면 허브가 있는 네임스페이스와 허브가 할당된 리소스 그룹을 지정해야 합니다.
