...
## 설명
이 Set-AzConsumptionBudget cmdlet은 구독 또는 리소스 그룹의 예산을 업데이트합니다.
...
### 예제 2: 비용 또는 사용량이 구독 수준에서 금액의 90% 임계값에 도달하면 알림으로 예산 업데이트
```powershell
...
### 예제 3: 자원 그룹 수준에서 예산 이름을 새 금액으로 업데이트
```powershell
...
### -ContactEmail
임계값을 초과할 때 예산 알림을 보낼 전자 메일 주소입니다.
...
### -ContactRole
임계값을 초과하는 경우 예산 알림을 보내기 위해 역할에 문의합니다.
...
### -EndDate
예산 기간의 종료 날짜(YYYY-MM-DD(UTC)입니다.
...
### -NotificationKey
예산과 연결된 알림의 키로, 알림 사용 스위치, 알림 임계값, 연락처 전자 메일, 연락처 그룹 또는 연락처 역할로 알림을 만드는 데 필요합니다.
...
알림과 연결된 임계값입니다.
비용 또는 사용량이 임계값을 초과하면 알림이 전송됩니다.
항상 백분율로, 0~1000 사이가 됐습니다.
...
### -StartDate
예산 기간의 시작 날짜(YYYY-MM-DD(UTC)입니다.
월별 시간(월별 시간)에 대해 현재 월 이전이 아 않습니다.
분기별 시간 세분화의 경우 3개월 이전이 아 않습니다.
연별 시간 세분화의 경우 12개월 전이 아니어도 됩니다.
향후 시작 날짜는 3개월을 넘지 않습니다.
...
### -TimeGrain
예산의 시간 세분화는 월별, 분기별 또는 연간일 수 있습니다.
