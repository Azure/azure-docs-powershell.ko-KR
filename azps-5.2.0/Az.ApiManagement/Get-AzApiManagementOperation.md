...
## SYNOPSIS
목록 또는 지정된 API 작업을 선택합니다.

## 구문

### GetAllApiOperations(기본값)
```
...
## 설명
**Get-AzApiManagementOperation은** 목록 또는 지정된 API 작업을 얻습니다.

## 예제

### 예제 1: 모든 API 관리 작업 얻기
```
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
PS C:\>Get-AzApiManagementOperation -Context $apimContext -ApiId $APIId
```

이 명령은 모든 API 관리 작업을 얻습니다.

### 예제 2: 작업 ID로 API Management 작업 얻기
```
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
PS C:\>Get-AzApiManagementOperation -Context $apimContext -ApiId $APIId -OperationId "Operation003"
```

이 명령은 Operation0003이라는 작업 ID로 API 관리 작업을 얻습니다.
...
### -ApiRevision
API 개정의 식별자입니다. 이 매개 변수는 선택 사항입니다. 지정하지 않으면 현재 활성 API 개정에서 작업이 검색됩니다.
