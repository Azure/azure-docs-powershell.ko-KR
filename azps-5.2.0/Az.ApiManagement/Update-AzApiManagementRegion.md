...
이 cmdlet은 메모리 내 **PsApiManagement** 인스턴스를 업데이트하는 것만 배포하지 않습니다.
...
이 명령은 미국 중남부 및 미국 중북부에 지역이 있는 API Management 프리미엄 SKU 서비스를 얻습니다. 그런 다음 **Set-AzApiManagement를** 사용하여 미국 중북부 지역의 용량을 2로 증가합니다. 다음 cmdlet **Set-AzApiManagement는** API Management 서비스에 구성 변경을 적용합니다.

### 예제 2

PsApiManagement 인스턴스의 기존 배포 지역을 업데이트합니다. (자동Generated)
