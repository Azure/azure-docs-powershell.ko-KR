...
**Set-AzApiManagementApiVersionSet** cmdlet은 Azure API Management API 버전 집합을 수정합니다.
...
이 명령은 버전 관리 체계 및 헤더 매개 변수를 사용하여 기존 API 버전 `Header` 집합을 `api-version` 업데이트합니다.

## PARAMETERS

### -ApiVersionSetId
새 API 버전 집합에 대한 식별자입니다.
...
### -HeaderName
버전 정보를 포함할 헤더 값입니다.
버전 지정 체계 헤더를 선택한 경우 이 값을 지정해야 합니다.
...
### -PassThru
지정된 경우 수정된 apiVersionSet을 나타내는 Microsoft.Azure.Commands.ApiManagement.ServiceManagement.Models.PsApiManagementApiVersionSet 형식의 인스턴스가 출력에 기록됩니다.
...
### -QueryName
버전 정보를 포함할 쿼리 값입니다.
버전 구성표 쿼리를 선택한 경우 이 값을 지정해야 합니다.
...
### -Scheme
API 버전 관리 집합에 대해 선택할 버전 관리 체계입니다.
이 매개 변수는 선택 사항입니다.
