...
### 예제 1: EventHub 데이터 연결 매개 변수 유효성 검사
```powershell
...
위의 명령은 "testnewkustocluster" 클러스터의 "mykustodatabase" 데이터베이스에 대해 "myeventhubdc"라는 EventHub 데이터 연결의 유효성을 검사합니다.

### 예제 2: EventGrid 데이터 연결 매개 변수 유효성 검사
```powershell
...
위의 명령은 "testnewkustocluster" 클러스터의 "mykustodatabase" 데이터베이스에 대해 "myeventgriddc"라는 EventGrid 데이터 연결의 유효성을 검사합니다.

### 예제 3: IotHub 데이터 연결 매개 변수 유효성 검사
```powershell
...
위의 명령은 "testnewkustocluster" 클러스터의 "mykustodatabase" 데이터베이스에 대해 "myiothubdc"라는 IotHub 데이터 연결의 유효성을 검사합니다.

### 예제 4: ID를 통해 EventHub 데이터 연결 매개 변수 유효성 검사
```powershell
...
### 예제 5: ID를 통해 EventGrid 데이터 연결 매개 변수 유효성 검사
```powershell
PS C:\> $database = Get-AzKustoDatabase -ResourceGroupName "testrg" -ClusterName "testnewkustocluster" -DatabaseName "mykustodatabase"
...
### 예제 6: ID를 통해 IotHub 데이터 연결 매개 변수 유효성 검사
```powershell
PS C:\> $database = Get-AzKustoDatabase -ResourceGroupName "testrg" -ClusterName "testnewkustocluster" -DatabaseName "mykustodatabase" 
...
처리할 Blob Storage 이벤트 유형의 이름입니다.
...
### -DataFormat
메시지의 데이터 형식입니다.
선택적으로 각 메시지에 데이터 형식을 추가할 수 있습니다.
...
### -EventHubResourceId
데이터 연결을 만드는 데 사용할 이벤트 허브의 리소스 ID/Event Grid가 이벤트를 보내도록 구성됩니다.
...
### -TableName
데이터를 수집해야 하는 테이블입니다.
선택적으로 테이블 정보를 각 메시지에 추가할 수 있습니다.
