...
## 설명
**Set-AzServiceBusAuthorizationRule** cmdlet은 지정된 Service Bus 네임스페이스 또는 큐 또는 토픽에서 지정된 권한 부여 규칙에 대한 설명을 업데이트합니다.
...
**네임스페이스에서** 권한 부여 규칙의 액세스 권한에서 `AuthoRule1` 관리를 `SB-Example1` 제거합니다.
...
큐에서 **권한** 부여 규칙의 액세스 권한에서 `AuthoRule1` 관리 `SBQueue` 제거.
...
**토픽의** 권한 부여 규칙 액세스 권한에서 `AuthoRule1` 관리 `SBTopic` 제거.
