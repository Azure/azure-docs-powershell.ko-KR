...
## 설명
이 명령은 동기화 그룹에서 지정된 클라우드 엔드포인트를 삭제합니다. Azure 파일 공유 클라우드 엔드포인트 참조는 이 프로세스에 의해 그대로 남아 있습니다. 이 명령은 해제 전용입니다. 클라우드 엔드포인트를 제거하는 것은 파괴적인 작업입니다. 하나 이상의 클라우드 엔드포인트가 없는 경우 서버 엔드포인트를 동기화할 수 없습니다. 동기화 문제를 해결하기 위해 이 작업을 수행하면 안 됩니다. 이 Azure 파일 공유가 클라우드 엔드포인트와 동일한 동기화 그룹에 다시 추가되는 경우 충돌 파일 및 기타 의도하지 않은 결과가 발생할 수 있습니다.
...
### -InputObject
일반적으로 파이프라인을 통해 전달되는 CloudEndpoint 입력 개체입니다.
