...
## 설명
서버 엔드포인트를 제거하는 것은 파괴적인 작업입니다. 이 서버 위치는 동기화를 중지합니다. 동기화 문제를 해결하기 위해 이 작업을 수행하면 안 됩니다. 이 서버 위치(파일 포함)가 서버 엔드포인트와 동일한 동기화 그룹에 다시 추가되는 경우 파일 충돌 및 기타 의도하지 않은 결과가 발생할 수 있습니다. 이 명령은 해제 전용입니다.
...
### -InputObject
일반적으로 파이프라인을 통해 전달되는 ServerEndpoint 입력 개체입니다.
