...
## 설명
**Get-AzBatchComputeNode** cmdlet은 풀에서 Azure Batch 계산 노드를 얻습니다.
*PoolID* 또는 풀 매개 *변수를 지정합니다.*
Id 매개 *변수를* 지정하여 단일 계산 노드를 얻습니다.
Open *Data* Protocol(OData) 필터와 일치하는 계산 노드를 얻게 하는 Filter 매개 변수를 지정합니다.
...
이 명령은 ID 풀06이 있는 풀에서 ID tvm-2316545714_1-20150725t213220z가 있는 계산 노드를 얻습니다.
Get-AzBatchAccountKey cmdlet을 사용하여 $Context 변수에 컨텍스트를 할당합니다.

### 예제 2: 풀에서 모든 유휴 계산 노드를 얻습니다.
```
...
이 명령은 ID Pool06이 있는 풀에 포함된 모든 유휴 계산 노드를 얻습니다.
이 명령은 Filter 매개 변수를 사용하여 유휴 상태를 *지정합니다.*

### 예제 3: 지정된 풀의 모든 계산 노드를 얻습니다.
```
...
이 명령은 Get-AzBatchPool cmdlet을 사용하여 ID Pool07이 있는 풀을 얻습니다.
이 명령은 파이프라인 연산자를 사용하여 풀을 현재 cmdlet에 전달합니다.
해당 cmdlet은 해당 풀에서 모든 계산 노드를 얻습니다.
...
### -Filter
OData 필터 절을 지정합니다.
이 cmdlet은 이 매개 변수가 지정하는 필터와 일치하는 계산 노드를 반환합니다.
필터를 지정하지 않으면 이 cmdlet은 풀에 대한 모든 계산 노드를 반환합니다.
...
### -Id
이 cmdlet이 풀에서 얻을 계산 노드의 ID를 지정합니다.
...
### -Pool
계산 노드를 포함하는 **PSCloudPool** 개체로 풀을 지정합니다.
**PSCloudPool 개체를** 얻습니다. Get-AzBatchPool cmdlet을 사용합니다.
