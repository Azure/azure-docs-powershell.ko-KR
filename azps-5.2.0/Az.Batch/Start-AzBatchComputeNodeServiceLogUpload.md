...
## 설명
이 cmdlet은 오류가 발생하고 Azure 지원으로 에스컬레이터하려면 계산 노드에서 Azure Batch 서비스 로그 파일을 수집합니다. Azure Batch 서비스 로그 파일은 Batch 서비스로 문제를 디버깅하는 데 도움을 주기 위해 Azure 지원과 공유해야 합니다. 
...
2018년 1월 1일 자정 또는 이후에 작성된 계산 노드 서비스 로그를 업로드합니다. 이 로그는 계산 노드에서 얻은 것으로, 계산 노드가 상주하는 풀의 풀 ID 및 계산 노드 ID를 제공합니다.
...
계산 노드에서 얻은 2018년 1월 1일 자정 및 2018년 1월 10일 자정 이후에 기록된 계산 노드 서비스 로그, 계산 노드가 상주하는 풀의 풀 ID 및 계산 노드 ID를 업로드합니다.
...
계산 노드 개체에서 얻은 2018년 1월 1일 자정 및 2018년 1월 10일 자정 이전으로 기록된 계산 노드 서비스 로그를 업로드합니다.
