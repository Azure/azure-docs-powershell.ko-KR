...
## 설명
**New-AzOperationalInsightsApplicationInsightsDataSource** cmdlet을 사용하면 주어진 Application-Insights 애플리케이션에서 로그를 수집할 수 있습니다.

## 예제

### 예제 1: 작업 영역의 application-insights 데이터 원본 만들기
```
...
이 명령은 주어진 로그 분석 작업 영역에서 주어진 애플리케이션의 application-insights 데이터 원본을 만듭니다. 이렇게 하면 주어진 애플리케이션에서 로그 분석 작업 영역으로 로그를 수집할 수 있습니다.

### 예제 2: 작업 영역 개체를 만들고 애플리케이션 리소스 ID로 application-insights 데이터 원본 만들기
```
...
이 명령은 로그 분석 작업 영역 개체를 확보한 다음, 출력을 전달하여 애플리케이션 리소스 ID에 의해 연결된 application-insights 데이터 원본을 만드는 것을 보여 합니다. 
