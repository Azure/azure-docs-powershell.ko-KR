...
## 설명
**Remove-AzOperationalInsightsStorageInsight** cmdlet은 작업 영역의 Storage Insight를 삭제합니다.

## 예제

### 예제 1: 이름으로 Storage 인사이트 제거
```
...
이 명령은 지정된 리소스 그룹의 MyWorkspace라는 작업 영역에서 MyStorageInsight라는 Storage Insight를 제거합니다.
이 명령은 Force 매개 변수를 지정하지 않습니다. *따라서* Storage Insight를 제거하기 전에 확인하라는 메시지가 표시됩니다.

### 예제 2: 확인 없이 Storage 인사이트 제거
```
...
첫 번째 명령은 Get-AzOperationalInsightsWorkspace cmdlet을 사용하여 MyWorkspace라는 작업 영역이 있는 다음, $Workspace 변수에 저장합니다. 두 번째 명령은 확인 메시지를 표시하지 않고 $Workspace MyStorageInsight라는 저장소 인사이트를 제거합니다.
...
### -Workspace
Storage 인사이트를 포함하는 작업 공간을 지정합니다.
...
### -WorkspaceName
Storage Insight를 포함하는 작업 영역의 이름을 지정합니다.
