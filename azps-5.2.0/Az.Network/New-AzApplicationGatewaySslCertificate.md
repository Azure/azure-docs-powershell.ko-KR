...
이 명령은 기본 애플리케이션 게이트웨이에 대해 Cert01이라는 SSL 인증서를 만들고 결과를 Cert01이라는 이름의 변수에 $Cert.

### 예제 2: KeyVault Secret(version-less secretId)을 사용하여 SSL 인증서를 만들고 애플리케이션 게이트웨이에 추가합니다.
```
...
비밀을 사용하고 를 사용하여 SSL 인증서를 만들 수 `New-AzApplicationGatewaySslCertificate` 있습니다.
...
### -CertificateFile
이 cmdlet에서 만드는 SSL 인증서의 .pfx 파일의 경로를 지정합니다.
