...
## 설명
**Add-AzApplicationGatewayTrustedRootCertificate** cmdlet은 Azure 애플리케이션 게이트웨이에 신뢰할 수 있는 루트 인증서를 추가합니다.
...
첫 번째 명령은 애플리케이션 게이트웨이를 사용하여 애플리케이션 게이트웨이를 $gw 저장합니다.
두 번째 명령은 루트 인증서의 경로를 입력으로 사용하여 Application Gateway에 신뢰할 수 있는 새 루트 인증서를 추가합니다.
세 번째 명령은 백end 서버 인증서의 유효성을 검사하기 위해 신뢰할 수 있는 루트 인증서를 사용하여 새 백end http 설정을 만듭니다.
네 번째 명령은 Application Gateway를 업데이트합니다.
