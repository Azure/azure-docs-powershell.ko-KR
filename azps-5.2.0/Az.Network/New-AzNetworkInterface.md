...
## 설명
**New-AzNetworkInterface** cmdlet은 Azure 네트워크 인터페이스를 만듭니다.
...
이 명령은 VirtualNetwork1이라는 가상 네트워크의 Subnet1에서 동적으로 할당된 개인 IP 주소를 사용하여 NetworkInterface001이라는 네트워크 인터페이스를 만듭니다. 또한 이 명령은 네트워크 인터페이스에 두 개의 DNS 서버를 할당합니다. IPConfiguration 자식 리소스는 IPConfiguration1 이름을 사용하여 자동으로 만들어집니다.
...
이 예제에서는 IP 구성 개체를 사용하여 새 네트워크 인터페이스를 만듭니다. IP 구성 개체는 정적 개인 IPv4 주소를 지정합니다.
첫 번째 명령은 두 번째 명령에서 서브넷을 할당하는 데 사용되는 기존 지정된 가상 네트워크를 검색합니다.
두 번째 명령은 IPConfig1이라는 네트워크 인터페이스 IP 구성을 만들고 구성을 $IPconfig.
세 번째 명령은 NetworkInterface1이라는 네트워크 인터페이스를 만듭니다. 이 인터페이스는 네트워크 인터페이스 IP 구성을 사용하여 $IPconfig.

### 예제 3

네트워크 인터페이스를 만듭니다. (자동Generated)
...
### -EnableIPForwarding
이 cmdlet을 통해 네트워크 인터페이스에 IP 전달을 사용할 수 있습니다.
IP 전달을 사용하면 가상 머신이 다른 대상에 주소가 주소가 있는 트래픽을 수신할 수 있습니다.
...
### -Force
동일한 이름의 네트워크 인터페이스가 이미 있는 경우에도 네트워크 인터페이스를 강제로 생성합니다.
...
부하 균형 조정에 대한 인바운드 NAT 규칙 구성을 지정합니다.
...
부하 균형 조정에 대한 인바운드 NAT 규칙 구성의 ID를 지정합니다.
...
### -PrivateIpAddress
이 네트워크 인터페이스에 할당할 고정 IPv4 IP 주소를 지정합니다.
...
### -Subnet
서브넷 개체를 **지정합니다.**
이 cmdlet은 이 매개 변수가 지정하는 서브넷에 대한 네트워크 인터페이스를 만듭니다.
