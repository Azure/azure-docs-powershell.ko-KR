...
## 설명
**Add-AzLoadBalancerInboundNatRuleConfig** cmdlet은 Azure Load Balancer에 인바운드 NAT(네트워크 주소 변환) 규칙 구성을 추가합니다.

## 예제

### 예제 1: 부하 균형 조정에 인바운드 NAT 규칙 구성 추가
```
PS C:\>$slb = Get-AzLoadBalancer -Name "MyLoadBalancer" -ResourceGroupName "MyResourceGroup"
PS C:\> $slb | Add-AzLoadBalancerInboundNatRuleConfig -Name "NewNatRule" -FrontendIPConfiguration $slb.FrontendIpConfigurations[0] -Protocol "Tcp" -FrontendPort 3350 -BackendPort 3350 -EnableFloatingIP
```

첫 번째 명령은 MyloadBalancer라는 부하 분산 장치와 해당 부하 분산 $slb.
두 번째 명령은 파이프라인 연산자를 사용하여 $slb Load Balancer에 인바운드 NAT 규칙 구성을 추가하는 **Add-AzLoadBalancerInboundNatRuleConfig에** 부하 분산을 전달합니다.

## PARAMETERS

### -BackendPort
규칙 구성에 의해 일치된 트래픽에 대한 백 끝 포트를 지정합니다.
...
이 cmdlet은 이 매개 변수가 지정하는 부하 균형 조정에 인바운드 NAT 규칙 구성을 추가합니다.
