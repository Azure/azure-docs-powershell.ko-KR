...
## 설명
**Remove-AzApplicationGatewayUrlPathMapConfig** cmdlet은 백end 서버 풀에 대한 URL 경로 매핑을 제거합니다.

## 예제

### 예제 1: 애플리케이션 게이트웨이에서 URL 경로 매핑 제거
```
...
두 번째 명령은 애플리케이션 게이트웨이에서 map01이라는 URL 경로 매핑을 제거합니다.
세 번째 명령은 애플리케이션 게이트웨이를 업데이트합니다.

## PARAMETERS

### -ApplicationGateway
이 cmdlet이 URL 경로 맵 구성을 제거하는 애플리케이션 게이트웨이를 지정합니다.
...
### -Name
이 cmdlet이 백end 서버에서 제거하는 URL 경로 맵 이름을 지정합니다.
