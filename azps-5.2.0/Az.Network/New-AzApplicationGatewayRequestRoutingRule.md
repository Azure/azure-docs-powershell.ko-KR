...
## 설명
**Add-AzApplicationGatewayRequestRoutingRule** cmdlet은 Azure 애플리케이션 게이트웨이에 대한 요청 라우팅 규칙을 만듭니다.

## 예제

### 예제 1: 애플리케이션 게이트웨이에 대한 요청 라우팅 규칙 만들기
```
...
이 명령은 Rule01이라는 기본 요청 라우팅 규칙을 만들고 해당 규칙이라는 변수에 $Rule.

## PARAMETERS

### -BackendAddressPool
만들 요청 라우팅 규칙에 대한 개체로 백 엔드 주소 풀을 지정합니다.
...
만들 요청 라우팅 규칙의 백 엔드 주소 풀 ID를 지정합니다.
...
### -BackendHttpSettings
만들 요청 라우팅 규칙에 대한 개체로 백 엔드 HTTP 설정을 지정합니다.
...
만들 요청 라우팅 규칙의 백 엔드 HTTP 설정 ID를 지정합니다.
...
### -HttpListener
만들 요청 라우팅 규칙에 대한 백 엔드 HTTP 수신기를 지정합니다.
...
### -HttpListenerId
만들 요청 라우팅 규칙에 대한 백end HTTP 수신기 ID를 지정합니다.
...
### -Name
이 cmdlet에서 만드는 요청 라우팅 규칙의 이름을 지정합니다.
