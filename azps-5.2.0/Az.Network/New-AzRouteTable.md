...
## 설명
**New-AzRouteTable** cmdlet은 Azure 경로 테이블을 만듭니다.

## 예제

### 예제 1: 경로가 포함된 경로 테이블 만들기
```
...
첫 번째 명령은 New-AzRouteConfig cmdlet을 사용하여 Route07이라는 경로를 만든 다음 $Route 변수에 저장합니다. 이 경로는 패킷을 로컬 가상 네트워크에 전달합니다.
두 번째 명령은 RouteTable01이라는 경로 테이블을 만들고 새 테이블에 $Route 경로를 추가합니다. 이 명령은 테이블이 속한 리소스 그룹 및 테이블의 위치를 지정합니다.
...
### -Force
동일한 이름의 경로 테이블이 이미 있는 경우에도 이 cmdlet이 경로 테이블을 만듭니다.
...
### -Location
이 cmdlet이 경로 테이블을 만드는 Azure 지역을 지정합니다.
자세한 내용은 [Azure 지역을 참조하세요.](http://azure.microsoft.com/en-us/regions/)
...
### -ResourceGroupName
이 cmdlet이 경로 테이블을 만드는 리소스 그룹의 이름을 지정합니다.
...
### -Route
경로 테이블과 연결하기 위해 **경로** 개체의 배열을 지정합니다.
