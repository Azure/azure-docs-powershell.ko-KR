...
## 설명
**Remove-AzRouteConfig** cmdlet은 Azure 경로 테이블에서 경로를 제거합니다.
...
현재 cmdlet은 Route02라는 경로를 제거하고 변경 내용을 반영하기 위해 테이블을 업데이트하는 **Set-AzRouteTable** cmdlet에 결과를 전달합니다.
테이블에 Route02라는 경로가 더 이상 포함되어 없습니다.
...
### -RouteTable
이 cmdlet에서 삭제하는 경로가 포함된 경로 테이블을 지정합니다.
