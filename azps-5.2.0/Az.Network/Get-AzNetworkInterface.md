...
## 설명
**Get-AzNetworkInterface** cmdlet은 리소스 그룹의 Azure 네트워크 인터페이스 또는 Azure 네트워크 인터페이스 목록을 얻습니다.

## 예제

### 예제 1: 모든 네트워크 인터페이스 얻기
```
...
이 명령은 현재 구독에 대한 모든 네트워크 인터페이스를 얻습니다.

### 예제 2: 특정 프로비전 상태로 모든 네트워크 인터페이스 얻기
```
...
이 명령은 프로비전 상태가 성공한 ResourceGroup1이라는 리소스 그룹의 모든 네트워크 인터페이스를 얻습니다.

### 예제 3: 필터링을 사용하여 네트워크 인터페이스 얻기
```
...
이 명령은 "test"로 시작하는 현재 구독에 대한 모든 네트워크 인터페이스를 얻습니다.
...
### -Name
이 cmdlet이 얻을 네트워크 인터페이스의 이름을 지정합니다.
...
### -ResourceGroupName
이 cmdlet이 네트워크 인터페이스를 얻을 리소스 그룹의 이름을 지정합니다.
...
### -ResourceId
네트워크 인터페이스의 Azure 리소스 관리자 ID입니다.
...
### -VirtualMachineIndex
이 cmdlet이 네트워크 인터페이스를 얻을 가상 머신 확장 집합의 가상 머신 인덱스를 지정합니다.
...
이 cmdlet이 네트워크 인터페이스를 얻을 가상 머신 확장 집합의 이름을 지정합니다.
