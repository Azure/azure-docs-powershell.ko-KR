...
이 명령은 $enabledState 모드, $enabledMode RequestBodyCheck을 false로, FileUploadLimitInMb를 $fileUploadLimitInMb $ $fileUploadLimitInMb MaxRequestBodySizeInKb와 같은 상태로 정책 설정을 $maxRequestBodySizeInKb.
새 policySettings는 새 정책에 $condition.
...
방화벽 정책의 정책 설정에서 requestBodyCheck을 Diables합니다.
...
방화벽 정책의 정책 설정에 있는 MaxRequestBodySizeInKb
