...
## 설명
**Set-AzSqlInstanceActiveDirectoryAdministrator** cmdlet은 현재 구독에서 AzureSQL Managed Instance에 대한 Azure AD(Azure Active Directory) 관리자를 프로비전합니다.
한 번의 관리자만 프로비전할 수 있습니다.
Azure AD의 다음 멤버는 Managed Instance 관리자로 SQL 수 있습니다.
- Azure AD의 네이티브 멤버 
- Azure AD의 페더리된 멤버 
- 다른 Azure AD에서 보안 그룹으로 만든 Azure AD 그룹은 관리자 권한으로 지원되지 않습니다.
microsoft 계정(예: Outlook.com, Hotmail.com 또는 Live.com 도메인의 계정)은 관리자로 지원되지 않습니다.
...
이 명령은 Azure AD 사용자를 관리되는 인스턴스 개체의 관리자로 프로비전합니다.
