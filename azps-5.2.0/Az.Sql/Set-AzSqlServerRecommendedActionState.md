...
## 설명
**Set-AzSqlServerRecommendedActionState** cmdlet은 권장되는 작업으로 Azure SQL Server 업데이트합니다.
이 cmdlet은 새 상태를 기반으로 권장되는 작업을 적용, 되회 또는 삭제합니다.

## 예제

### 예제1: 지정된 권장 작업의 상태를 보류 중으로 업데이트합니다.
```
...
"IR_ test_schema \[ \] \[ _test_table_0.0361551 "보류 \] 중"으로 _6C7AE8CC9C87E7FD5893 서버 권장 작업의 상태를 업데이트합니다.

## PARAMETERS

### -AdvisorName
권장 작업을 포함하는 Advisor의 이름을 지정합니다.
...
이 cmdlet이 상태를 업데이트하는 권장 작업의 이름을 지정합니다.
...
### -State
이 cmdlet이 권장 작업 상태를 업데이트하는 새 값을 지정합니다.
이 매개 변수에 허용되는 값은
- 활성
- 보류 중
- PendingRevert
