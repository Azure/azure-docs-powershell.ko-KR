...
## 설명
이 명령은 별칭이 포인터가 있는 서버를 업데이트합니다. 별칭이 지점이 될 새 서버가 있는 구독에 로그인하는 동안 이 명령을 실행해야 합니다.
...
이 명령은 이전에 oldServer를 통해 서버 newServer를 지점으로 하는 별칭을 업데이트하고 있습니다.
...
### -SourceServerName
현재 별칭이 지정되는 Azure Sql Server의 이름입니다.
...
### -TargetServerName
별칭이 지정해야 하는 Azure Sql Server의 이름입니다.
