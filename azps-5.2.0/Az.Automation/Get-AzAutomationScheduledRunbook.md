...
특정 Runbook 일정을 표시하도록 Runbook의 이름 또는 일정 또는 둘 다를 지정합니다.

## 예제

### 예제 1: 예약된 모든 Runbook을 얻습니다.
```
...
이 명령은 Contoso17이라는 Azure Automation 계정에서 Runbk01 Runbook에 대해 예약된 모든 Runbook을 얻습니다.

### 예제 3: 일정과 연결된 모든 Runbook을 얻습니다.
```
...
### -JobScheduleId
이 cmdlet이 얻을 예약된 작업의 ID를 지정합니다.
...
### -ResourceGroupName
이 cmdlet에서 얻을 예약된 Runbook에 대한 리소스 그룹의 이름을 지정합니다.
...
### -RunbookName
이 cmdlet이 예약된 Runbook을 얻을 Runbook의 이름을 지정합니다.
...
### -ScheduleName
이 cmdlet이 예약된 Runbook을 받을 일정의 이름을 지정합니다.
