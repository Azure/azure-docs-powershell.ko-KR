...
## 설명
**Set-AzPolicySetDefinition** cmdlet은 정책 정의를 수정합니다.

## 예제

### 예제 1: 정책 집합 정의에 대한 설명 업데이트
```
PS C:\> $PolicySetDefinition = Get-AzPolicySetDefinition -ResourceId '/subscriptions/mySub/Microsoft.Authorization/policySetDefinitions/myPSSetDefinition'
PS C:\> Set-AzPolicySetDefinition -Id $PolicySetDefinition.ResourceId -Description 'Updated policy to not allow virtual machine creation'
```

첫 번째 명령은 Get-AzPolicySetDefinition cmdlet을 사용하여 정책 집합 정의를 얻습니다.
이 명령은 해당 개체를 $PolicySetDefinition 저장합니다.
두 번째 명령은 리소스의 **ResourceId** 속성으로 식별되는 정책 집합 정의에 대한 설명을 $PolicySetDefinition.

### 예제 2: 정책 집합 정의의 메타데이터 업데이트
```
...
이 명령은 해당 범주가 "Virtual Machine"이라는 것을 나타내기 위해 VMPolicySetDefinition이라는 정책 집합 정의의 메타데이터를 업데이트합니다.

### 예제 3: 정책 집합 정의의 그룹 업데이트
```
...
이 명령은 VMPolicySetDefinition이라는 정책 집합 정의 그룹을 업데이트합니다.

### 예제 4: 해시 테이블을 사용하여 정책 집합 정의 그룹 업데이트
```
$groupsJson = ConvertTo-Json @{ name = "group1", displayName = "Virtual Machine Security" }, @{ name = "group2" }
PS C:\> Set-AzPolicySetDefinition -Name 'VMPolicySetDefinition' -GroupDefinition $groupsJson
```

이 명령은 해시 테이블을 사용하여 VMPolicySetDefinition이라는 정책 집합 정의 그룹을 업데이트하여 그룹을 구성합니다.
...
### -GroupDefinition
업데이트된 정책 집합 정의의 정책 정의 그룹입니다. 그룹이 포함된 파일의 경로 또는 JSON 문자열로 그룹을 사용할 수 있습니다.
...
### -InputObject
다른 cmdlet에서 출력된 정의 개체를 업데이트할 정책 설정 정의 개체입니다.
...
### -Metadata
업데이트된 정책 집합 정의의 메타데이터입니다. 메타데이터를 포함하는 파일 이름의 경로 또는 메타데이터를 JSON 문자열로 사용할 수 있습니다.
...
### -Parameter
업데이트된 정책 집합 정의의 매개 변수 선언입니다. 매개 변수 선언을 포함하는 파일 이름 또는 URI에 대한 경로 또는 매개 변수 선언을 JSON 문자열로 사용할 수 있습니다.
