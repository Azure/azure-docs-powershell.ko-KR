...
## 설명
이 Set-AzAttestationPolicy cmdlet은 Azure Attestation의 테넌트에서 정책을 설정합니다.
...
텍스트 정책 형식(기본값)을 사용하여 TEE 형식 *SgxEnclave* for Attest Provider *pshtest에* 대해 사용자 정의 정책을 설정합니다.
...
JWT 정책 형식을 사용하여 TEE 형식 *SgxEnclave* for Attest Provider *pshtest에* 대해 사용자 정의 정책을 설정합니다.
...
### -Name
테넌트의 이름을 지정합니다.
이 cmdlet은 이 매개 변수가 지정하는 테넌트에 대한 설명 정책을 설정합니다.
...
### -PolicyFormat
정책의 형식(텍스트 또는 JWT(JSON 웹 토큰)을 지정합니다.  기본 정책 형식은 Text입니다.
...
SgxEnclave, OpenEnclave, CyResComponent 및 VBSEnclave의 네 가지 유형의 환경이 지원됩니다.
