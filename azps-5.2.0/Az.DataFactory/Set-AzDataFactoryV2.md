...
### 예제 2: 기존 팩터리 개체를 사용하여 리포지토리 구성 세부 정보가 있는 데이터 팩터리 만들기
```
...
이 명령은 Azure DevOps 소스 제어 구성을 사용하여 EastUS 위치에 있는 ADF라는 리소스 그룹에 WikiADF라는 데이터 팩터리를 만듭니다.

### 예제 3: 새 팩터리 개체를 사용하여 GitHub 리포지토리 구성 세부 정보가 있는 데이터 팩터리 만들기
```
...
이 명령은 GitHub 소스 제어 구성을 사용하여 EastUS 위치에 있는 ADF라는 리소스 그룹에 WikiADF라는 데이터 팩터리를 만듭니다.

## PARAMETERS

### -AccountName
리포지타이어 구성에 대한 계정 이름입니다.
...
### -ProjectName
프로젝트 이름은 리포지터 구성을 위한 Azure DevOps입니다.
...
### -TenantId
Azure DevOps 리포지터 구성에 대한 테넌트 ID입니다.
