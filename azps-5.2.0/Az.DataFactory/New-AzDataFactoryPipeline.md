...
이미 있는 파이프라인의 이름을 지정하는 경우 cmdlet은 파이프라인을 바꾸기 전에 확인을 요청하는 메시지를 표시합니다.
Force 매개 *변수를 지정하면* cmdlet이 확인 없이 기존 파이프라인을 대체합니다.
다음 순서로 이러한 작업을 수행할 수 있습니다. 
- 데이터 팩터리 만들기 
- 연결된 서비스를 만들 수 있습니다. 
- 데이터 세트를 만들 수 있습니다. 
- 파이프라인을 만들 수 있습니다.
동일한 이름의 파이프라인이 데이터 팩터리에 이미 있는 경우 이 cmdlet은 새 파이프라인으로 기존 파이프라인을 덮어써야 하는지 여부를 확인하라는 메시지를 제공합니다.
기존 파이프라인을 덮어들이는 것이 확인되면 파이프라인 정의도 대체됩니다.
...
이 명령은 ADF라는 데이터 팩터리에 DPWikisample이라는 파이프라인을 만듭니다.
이 명령은 파이프라인의 파일 DPWikisample.js기반입니다.
...
### -File
파이프라인에 대한 설명이 포함된 JSON(JavaScript Object Notation) 파일의 전체 경로를 지정합니다.
...
### -Force
확인 메시지를 표시하지 않고 이 cmdlet이 기존 파이프라인을 대체하는지 나타냅니다.
