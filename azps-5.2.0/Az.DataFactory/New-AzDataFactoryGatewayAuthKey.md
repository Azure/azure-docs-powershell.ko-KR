...
## 설명
**New-AzDataFactoryGatewayAuthKey** cmdlet은 지정된 Azure Data Factory 게이트웨이에 대한 게이트웨이 auth 키를 만듭니다.
이 키를 사용하여 클라우드 서비스에 게이트웨이를 등록합니다.

## 예제

### 예제 1: Key1에 대한 게이트웨이 Auth 키 생성
```
...
이 명령은 MyGateway라는 데이터 팩터리 게이트웨이에 대한 Key1의 게이트웨이 Auth 키를 만듭니다.
...
### -KeyName
다시 생성될 게이트웨이인 'key1' 또는 'key2'의 이름입니다.
