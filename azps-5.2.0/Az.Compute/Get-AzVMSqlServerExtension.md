...
## 설명
**Get-AzVMSqlServerExtension** cmdlet은 가상 머신에서 SQL Server IaaS(Infrastructure as a Service) 에이전트의 설정을 얻습니다.

## 예제

### 예제 1: 가상 머신에서 SQL Server 확장의 설정을 확인합니다.
```
...
이 명령은 ContosoVM07이라는 가상 SQL Server 확장의 설정을 얻습니다.

### 예제 2: 파이프라인을 사용하여 설정 얻기
```
...
이 명령은 Get-AzVM cmdlet을 사용하여 Service08에서 ContosoVM22라는 가상 머신을 얻습니다.
이 명령은 파이프라인 연산자를 사용하여 결과를 현재 cmdlet에 전달합니다.
현재 명령은 가상 머신에서 SQL Server IaaS 에이전트의 설정을 얻습니다.

### 예제 3: 특정 버전에 대한 SQL Server
```
...
이 명령은 ContosoVM07이라는 가상 머신에서 SQL Server 버전 1.0의 설정을 얻습니다.
