...
## 설명
**Stop-AzVmss** cmdlet은 VMSS(Virtual Machine Scale Set) 또는 가상 머신 집합 내의 모든 가상 머신을 중지합니다.
...
이 명령은 ContosoVMSS라는 VMSS에 속하는 인스턴스 ID 문자열 배열로 지정된 특정 가상 머신 집합을 중지합니다.
...
### -StayProvisioned
지정된 경우 가상 머신이 중지된 상태가 됩니다. 지정하지 않으면 가상 머신이 중지된-위치가 지정되지 않은 상태가 됩니다. 사용자는 중지된 상태의 VM에 대한 요금이 계속 청구되지만 중지된-재배치된 상태의 VM에 대한 요금은 청구되지 않습니다.
