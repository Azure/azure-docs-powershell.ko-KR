...
보호해야 하는 단일 디스크가 하나만 있는 시나리오에 해당합니다.
...
보호해야 하는 디스크가 여러 개 있는 시나리오에 해당합니다.
...
### -DiskToInclude
복제에 포함될 원본 서버의 디스크를 지정합니다.
생성을 위해 DISKTOINCLUDE 속성에 대한 NOTES 섹션을 참조하고 해시 테이블을 만드십시오.
...
### -InputObject
마이그레이션할 검색된 서버를 지정합니다.
Get-AzMigrateServer cmdlet을 사용하여 서버 개체를 검색할 수 있습니다.
...
### -LicenseType
Azure 하이브리드 혜택이 마이그레이션할 원본 서버에 적용할 수 있는지 지정합니다.
...
### -OSDiskID
마이그레이션할 원본 서버에 대한 운영 체제 디스크를 지정합니다.
...
### -PerformAutoResync
복제 중 원본 서버에 대한 변경 내용 추적이 손실되는 경우 복제를 자동으로 복구할지 지정합니다.
...
VM 생성에 사용할 가용성 집합을 지정합니다. VM 생성에 사용할 가용성 집합을 지정합니다.
...
### -TargetNetworkId
서버를 마이그레이션해야 하는 대상 Azure 구독 내의 Virtual Network ID를 지정합니다.
...
서버를 마이그레이션해야 하는 대상 Azure 구독 내의 리소스 그룹 ID를 지정합니다.
...
### -TargetSubnetName
서버를 마이그레이션해야 하는 대상 Virtual Netowk 내의 서브넷 이름을 지정합니다.
...
DISKTOINCLUDE <IVMwareCbtDiskInput[]>: 복제에 포함될 원본 서버의 디스크를 지정합니다.
  - `DiskId <String>`: 디스크 ID입니다.
  - `IsOSDisk <String>`: 디스크가 OS 디스크인지 여부를 나타내는 값입니다.
  - `LogStorageAccountId <String>`: 로그 저장소 계정 ARM ID입니다.
  - `LogStorageAccountSasSecretName <String>`: 로그 저장소 계정의 키 자격 증명 모음 비밀 이름입니다.
  - `[DiskEncryptionSetId <String>]`: DiskEncryptionSet ARM ID입니다.
  - `[DiskType <DiskAccountType?>]`: 디스크 유형입니다.

INPUTOBJECT: 마이그레이션할 검색된 <IVMwareMachine> 서버를 지정합니다. Get-AzMigrateServer cmdlet을 사용하여 서버 개체를 검색할 수 있습니다.
  - `[GuestOSDetailOstype <String>]`: 운영 체제의 유형입니다.
