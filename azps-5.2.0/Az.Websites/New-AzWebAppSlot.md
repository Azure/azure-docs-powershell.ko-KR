...
## 설명
**New-AzWebAppSlot** cmdlet은 지정된 App Service 계획 및 데이터 센터를 사용하는 지정된 리소스 그룹에 Azure 웹앱 슬롯을 만듭니다.
...
이 명령은 미국 서부 데이터 센터의 Default-Web-WestUS라는 기존 리소스 그룹에 있는 기존 웹앱 이름 ContosoSite 아래에 Slot001이라는 슬롯을 만듭니다.
이 명령은 ContosoServicePlan이라는 기존 App Service 계획을 사용 합니다.

## PARAMETERS

### -AppServicePlan
App Service 계획 이름 또는 App Service 계획 ID입니다. 슬롯 및 App Service 계획이 다른 리소스 그룹에 있는 경우 이름 대신 ID를 사용합니다. App Service 계획 ID는 $asp = Get-AzAppServicePlan -ResourceGroup myRG -Name MyWebapp $asp.id App Service 계획 ID를 반환하여 검색할 수 있습니다.
