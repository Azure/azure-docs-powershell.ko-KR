...
이 명령은 Windows 업데이트 사용자 지정 프로그램을 만듭니다.
...
이 명령은 PowerShell 사용자 지정 프로그램을 만듭니다.
...
### -Destination
파일(sourceUri에서)이 VM에 업로드될 파일의 절대 경로(이미 생성된 중첩된 디렉터리 구조)입니다.
...
### -RestartCustomizer
VM을 다시 부팅하고 다시 온라인(Windows)으로 돌아올 때까지 기다릴 수 있습니다.
Packer windows-restart 프로비전에 해당합니다.
...
Github 링크, Azure Storage에 대한 SAS URI 등이 될 수 있습니다.
...
### -SearchCriterion
업데이트를 검색하는 조건입니다.
기본값을 사용하기 위해 빈 문자열을 생략하거나 지정합니다(모두 검색).
이 필드의 예제 및 자세한 설명은 위의 링크를 참조하세요.
...
### -Sha256Checksum
scriptUri 필드에 제공된 셸 스크립트의 SHA256 체크um입니다.
...
### -ShellCustomizer
사용자 지정 단계(Linux) 중에 셸 스크립트를 실행합니다.
Packer 셸 프로비전에 해당합니다.
정확히 'scriptUri' 또는 'inline' 중 하나를 지정할 수 있습니다.
...
기본값을 사용하기 위해 0을 생략하거나 지정합니다(1000).
...
### -WindowsUpdateCustomizer
Windows 업데이트를 설치합니다.
Packer Windows Update https://github.com/rgl/packer-provisioner-windows-update) Provisioner(.
