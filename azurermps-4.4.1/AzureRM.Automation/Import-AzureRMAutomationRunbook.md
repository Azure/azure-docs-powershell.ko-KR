...
## 설명은
**AzureRmAutomationRunbook** Cmdlet은 Azure Automation runbook을 가져옵니다. Wps_2 및 wps_2 워크플로 runbook에 대해 가져올 wps_2 스크립트 (ps1) 파일의 경로를 지정 하거나 그래픽 runbook에 대 한 그래픽 runbook (graphrunbook) 파일을 만듭니다. 파일 이름이 runbook의 이름이 됩니다. Wps_2 워크플로 runbook의 경우 스크립트에는 파일 이름과 일치 하는 단일 wps_2 워크플로 정의가 포함 되어 있어야 합니다.
