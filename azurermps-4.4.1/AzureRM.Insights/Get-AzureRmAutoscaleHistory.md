...
## 설명은
**AzureRmAutoscaleHistory** Cmdlet은 자동 크기 조정 설정과 관련 된 이벤트 기록을 가져옵니다.

## 예제의

### 예제 1: 구독과 연결 된 모든 이벤트 가져오기
```
PS C:\>Get-AzureRmAutoscaleHistory -StartTime 2015-02-09T18:35:00 -EndTime 2015-02-09T18:40:00 -DetailedOutput
```

이 명령은 현재 구독과 연결 된 자동 크기 조정 관련 이벤트를 모두 가져옵니다.

### 예제 2: 특정 리소스에 대 한 GetAutoscaleHistory
```
...
이 명령은 리소스 ID (본질적으로 ResourceUri)로 식별 되는 특정 리소스와 관련 된 자동 크기 조정 관련 이벤트를 모두 가져옵니다.
...
### -ResourceId
자동 크기 조정 설정이 연결 되는 리소스 ID를 지정 합니다.
...
### IPSEventData의 목록 ' 1 [Microsoft Azure. t e m]
