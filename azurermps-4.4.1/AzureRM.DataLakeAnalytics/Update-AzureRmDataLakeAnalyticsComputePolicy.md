...
이 명령은 사용자가 5 개 이상의 병렬 처리를 사용 하 여 작업을 제출할 수 없도록 "contosoadla" 계정의 "myPolicy" 라는 정책을 업데이트 합니다.
...
이 명령은 사용자가 5 개 이상의 병렬 처리를 사용 하거나 우선 순위가 100 보다 낮은 작업을 제출할 수 없도록 "contosoadla" 계정에 "myPolicy" 이라는 정책을 만듭니다.
...
이 정책에 대해 지원 되는 작업당 병렬 처리의 최대 수준입니다. This, MinPriorityPerJob 또는 both 매개 변수 중 하나를 지정 해야 합니다.
...
### -MinPriorityPerJob
이 정책에 대해 지원 되는 작업당 최소 우선 순위입니다. This, MaxDegreeOfParallelismPerJob 또는 both 매개 변수 중 하나를 지정 해야 합니다.
