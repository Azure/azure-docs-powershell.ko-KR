...
## 설명은
Get-AzureRmNetworkWatcherTopology cmdlet은 리소스 그룹의 리소스 및 자원의 관계에 대 한 네트워크 수준 보기를 표시 합니다. 참고: 여러 영역의 리소스가 리소스 그룹에 있는 경우 네트워크 감시자와 동일한 지역에 있는 리소스만 JSON 출력에 포함 됩니다.

## 예제의

### --------------------------예제 1: Azure 토폴로지 가져오기--------------------------
```
...
이 예제에서는 VM, Nic, NSG 및 공용 IP가 포함 된 리소스 그룹에서 Get-AzureRmNetworkWatcherTopology cmdlet을 실행 합니다.
