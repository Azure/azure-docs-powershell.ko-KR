...
## 설명은
**Set-AzureRmServiceBusTopicAuthorizationRule** cmdlet은 지정 된 Service Bus 항목에 대 한 설명을 업데이트 합니다.
...
항목에 대 한 권한 부여 규칙의 액세스 권한에 대 한 **관리** 를 추가 `SBTopicAuthoRule1` `SB-Topic_exampl1` 합니다.
...
### -권한
권리가 예를 들어 @ ("수신", "보내기", "관리"). **-Authruleobj** 가 지정 되지 않은 경우 필요 합니다.
...
### -InputObj
ServiceBus Topic AuthorizationRule 개체입니다.
...
### -이름
AuthorizationRule Name-' AuthruleObj '가 지정 되지 않은 경우 필수입니다.
