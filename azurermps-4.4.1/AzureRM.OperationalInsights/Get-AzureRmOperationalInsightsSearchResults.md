...
## 설명은
**AzureRmOperationalInsightsSearchResults** cmdlet은 지정 된 매개 변수를 기준으로 검색 결과를 반환 합니다.
...
이 명령은 쿼리를 사용 하 여 모든 검색 결과를 가져옵니다.

### 예제 2: ID를 사용 하 여 검색 결과 가져오기
```
...
이 명령은 ID를 사용 하 여 검색 결과를 가져옵니다.

### 예제 3: 검색을 완료 한 후 결과 표시
```
PS C:\>$error.clear()
$response = @{}
$StartTime = Get-Date
...
이 스크립트는 검색을 시작 하 고 결과를 표시 하기 전에 완료 될 때까지 기다립니다.
...
### -Id
Id를 지정 하면 해당 id에 대 한 검색 결과가 원본 쿼리 매개 변수를 사용 하 여 검색 됩니다.
...
**PSSearchGetSearchResultsResponse** 개체에는 JSON 형식의 검색에서 반환 된 레코드와 검색 결과에 대 한 정보를 포함 하는 메타 데이터 개체를 포함 하는 Value 속성이 포함 됩니다.
