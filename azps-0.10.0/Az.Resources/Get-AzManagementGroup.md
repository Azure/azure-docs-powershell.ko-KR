...
## 설명은
Get-AzManagementGroup cmdlet은 모든 또는 특정 관리 그룹을 가져옵니다.
...
### 예제 3: 특정 관리 그룹 및 계층의 첫 번째 수준 가져오기
```
...
플래그를 사용 하 여 `Expand` `Children` 배열을 탐색 하 고 각 자식에 대 한 세부 정보를 가져올 수 있습니다. 예를 들어 `Children[0]` 표시 이름을 사용 하 여 그룹에 대 한 세부 정보를 제공 `TestGroup1DisplayName` 합니다.

### 예제 4: 특정 관리 그룹 및 모든 수준의 hiearchy 가져오기
```
...
### PSManagementGroupInfo-.Resources. 모델. 관리 그룹.
