...
## 설명은
Import-AzContext cmdlet은 파일에서 인증 정보를 로드 하 여 Azure 환경 및 컨텍스트를 설정 합니다.
...
이 예제에서는 cmdlet으로 전달 되는 JSON 파일에서 컨텍스트를 선택 합니다. 이 JSON 파일은 Save-AzContext에서 만들 수 있습니다.

## 변수

### -AzureContext
{{Fill의 AzureContext Description}}
...
### -Path
Save-AzContext를 사용 하 여 저장 된 컨텍스트 정보의 경로를 지정 합니다.
