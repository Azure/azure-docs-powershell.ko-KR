...
## 설명은
**AzureRmDeploymentManagerRollout** cmdlet은 터미널 상태에서 롤아웃을 삭제 합니다.
...
이 명령은 ContosoResourceGroup에서 ContosoRollout 이라는 롤아웃을 삭제 합니다.

### 예제 2: 리소스 식별자를 사용 하 여 롤아웃 삭제
```powershell
...
### 예제 3: 롤아웃 개체를 사용 하 여 출시를 삭제 합니다.
```powershell
PS C:\> Remove-AzureRmDeploymentManagerRollout -Rollout $rolloutObject
```

이 명령은 이름 및 ResourceGroup이 $rolloutObject의 Name 및 ResourceGroupName 속성과 각각 일치 하는 롤아웃을 삭제 합니다.
