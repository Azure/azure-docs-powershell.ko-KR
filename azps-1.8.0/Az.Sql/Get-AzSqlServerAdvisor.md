...
## 설명은
**AzSqlServerAdvisor** Cmdlet은 azure sql server에 대 한 하나 이상의 Azure sql server 관리자를 가져옵니다.

## 예제의

### 예제 1: 서버의 모든 관리자 목록 표시
```
...
이 명령은 WIRunnersProd 이라는 리소스 그룹에 속하는 wi-러너-오스트레일리아-동부 서버에 대 한 모든 관리자의 목록을 가져옵니다.

### 예제 2: 서버에 대 한 단일 관리자 가져오기
```
...
이 명령은 wi-러너-동부 라는 서버의 CreateIndex 라는 관리자를 가져옵니다.
...
이 명령은 wi-러너-오스트레일리아 라는 서버에 대 한 모든 관리자를 가져옵니다.
명령이 *ExpandRecommendedActions* 매개 변수를 사용 하기 때문에 cmdlet은 응답에 포함 된 관리자 권장 작업을 가져옵니다.
...
이 명령은 이름에 wi-러너-호주 서버 로부터 CreateIndex 라는 이름을 제공 하며 응답에는 권장 작업이 포함 되어 있습니다.

### 예제 5: 필터링을 사용 하 여 서버의 모든 관리자 목록 표시
```
...
이 명령은 "d" 문자로 시작 하는 WIRunnersProd 이라는 리소스 그룹에 속하는 wi-러너-오스트레일리아-동부 서버의 모든 관리자 목록을 가져옵니다.
...
Cmdlet에 응답에 포함 된 관리자의 권장 작업이 포함 되어 있음을 나타냅니다.
...
### -ServerName
이 cmdlet이 요청 하는 관리자에 대 한 서버의 이름을 지정 합니다.
...
### AzureSqlServerAdvisorModel을 (를).

## 상속자
* 키워드: azure, azurerm, arm, resource, 관리, manager, sql, server, mssql, advisor
