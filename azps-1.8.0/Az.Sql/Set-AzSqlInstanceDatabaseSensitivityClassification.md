...
### ClassificationObjectParameterSet (기본값)
```
Set-AzSqlInstanceDatabaseSensitivityClassification
...
## 설명은
Set-AzSqlInstanceDatabaseSensitivityClassification cmdlet은 Azure SQL 관리 인스턴스 데이터베이스의 열에 대 한 정보 유형 및 민감도 레이블을 설정 합니다.

## 예제의

### 예제 1: Azure SQL 관리 인스턴스 데이터베이스에서 열의 정보 유형 및 민감도 레이블을 설정 합니다.
```powershell
...
### 예제 2: Azure SQL 관리 인스턴스 데이터베이스에서 열에 대 한 권장 정보 유형 및 민감도 레이블을 설정 합니다.
```powershell
...
### 예제 3: 파이프를 사용 하 여 Azure SQL 관리 인스턴스 데이터베이스의 열에 대 한 정보 유형 및 민감도 레이블을 설정 합니다.
```powershell
...
SQL 관리 되는 인스턴스 데이터베이스 민감도 분류를 나타내는 개체입니다.
...
### -InformationType
열에 저장 된 데이터의 정보 유형을 설명 하는 이름입니다.
...
### -PassThru
Cmdlet 실행 종료 시 민감도 분류 모델을 출력할지 여부를 지정 합니다.
