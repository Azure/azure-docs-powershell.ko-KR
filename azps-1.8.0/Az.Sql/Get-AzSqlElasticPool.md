...
## 설명은
**Get-AzSqlElasticPool** cmdlet은 탄력적 풀 및 해당 속성 값을 가져옵니다.
기존 탄력적 풀의 이름을 지정 하 여 해당 풀에 대 한 속성 값을 표시 합니다.
...
이 명령은 Server01 이라는 서버의 모든 탄력적 풀을 가져옵니다.
...
이 명령은 Server01 라는 서버에서 ElasticPool0127 이라는 탄력적 풀을 가져옵니다.

### 예제 3: Azure SQL 탄력적 데이터베이스 풀에 대 한 메트릭 가져오기
```
...
이 명령은 ElasticPool01 이라는 Azure SQL 탄력적 데이터베이스 풀에 대 한 메트릭을 반환 합니다.

### 예제 4: 필터링을 사용 하 여 모든 탄력적인 풀 가져오기-ElasticPoolName "ElasticPool *"
```
PS C:\>Get-AzSqlElasticPool -ResourceGroupName "ResourceGroup01" -ServerName "Server01"
ResourceId        : /subscriptions/00000000-0000-0000-0000-000000000001/resourceGroups/resourcegroup01/providers/Microsoft.Sql/servers/server01/elasticPools/elasticpool01
ResourceGroupName : resourcegroup01
...
이 명령은 Server01 이라는 서버에서 "ElasticPool"로 시작 하는 모든 탄력적 풀을 가져옵니다.
...
### -ElasticPoolName
이 cmdlet이 가져오는 탄력적 풀의 이름을 지정 합니다.
...
### -ResourceGroupName
이 cmdlet이 가져오는 탄력적인 풀을 포함 하는 리소스 그룹의 이름을 지정 합니다.
...
### -ServerName
이 cmdlet이 가져오는 탄력적 풀을 포함 하는 서버의 이름을 지정 합니다.
