...
이 예제에서는 버전 2 로그가 지정 된 네트워크 보안 그룹 (NSG)에서 흐름 로깅을 구성 합니다. 응답에서 지정 된 NSG에 흐름 로깅이 사용 하도록 설정 되 고, 형식이 설정 되 고, 구성 된 보존 정책이 없음이 표시 됩니다. 지역에서 지정 버전을 지원 하지 않는 경우 네트워크 감시자는 해당 지역에서 지원 되는 기본 버전을 작성 합니다.
