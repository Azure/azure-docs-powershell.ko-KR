...
## 설명은
**Set-AzRouteTable** cmdlet은 경로 테이블을 업데이트 합니다.

## 예제의

### 예제 1: 경로 구성을 추가 하 여 라우트 테이블 업데이트
```
...
이 명령은 파이프라인 연산자를 사용 하 여 Add-AzRouteConfig cmdlet에 해당 테이블을 전달 합니다.
**Add-AzRouteConfig** 는 Route07 이라는 경로를 추가한 다음 결과를 현재 cmdlet에 전달 하 여 변경 내용을 반영 하도록 표를 업데이트 합니다.
...
첫 번째 명령은 rtName 이라는 경로 테이블을 가져와 $rt 변수에 저장 합니다.
두 번째 명령은 DisableBgpRoutePropagation의 값을 표시 합니다.
세 번째 명령은 DisableBgpRoutePropagation의 값을 업데이트 합니다.
네 번째 명령은 서버에서 경로 테이블을 업데이트 합니다.
다섯 번째 명령은 업데이트 된 경로 테이블을 가져와 $rt 변수에 저장 합니다.
여섯째 명령은 DisableBgpRoutePropagation의 값을 표시 합니다.
...
### -RouteTable
경로 테이블을 설정 해야 하는 상태를 나타내는 경로 테이블 개체를 지정 합니다.
