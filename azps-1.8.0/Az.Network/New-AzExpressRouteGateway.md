...
New-AzExpressRouteGateway는 확장 가능 하는 Express 경로 게이트웨이를 만듭니다. 이는 VirtualHub 내에서 온-프레미스에서 Azure로의 소프트웨어 정의 된 연결입니다. 

이 게이트웨이에는이 게이트웨이 또는 Set-AzExpressRouteGateway cmdlet에 지정 된 배율 단위를 기준으로 크기를 조정할 수 있습니다. 

온-프레미스 Express 경로 회로에서 확장 가능한 게이트웨이로의 연결이 설정 됩니다.

ExpressRouteGateway는 참조 되는 VirtualHub와 같은 위치에 있게 됩니다.
...
위의 방법을 통해 Azure의 "testRG" 리소스 그룹에 미국 내에서 리소스 그룹, 가상 WAN, 가상 네트워크, 가상 허브를 만듭니다. 1 개의 배율 단위를 사용 하 여 가상 허브에서 Express 경로 게이트웨이를 만듭니다.
...
### -VirtualHub
이 VpnGateway 연결 해야 하는 VirtualHub입니다.
...
### -VirtualHubId
이 VpnGateway와 연결 되어야 하는 VirtualHub의 Id입니다.
