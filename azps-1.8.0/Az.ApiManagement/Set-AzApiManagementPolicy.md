...
### -형식
정책의 형식을 지정 합니다. 사용 하 `application/vnd.ms-az-apim.policy+xml` 는 경우 정책에 포함 된 식은 XML 이스케이프 되어야 합니다. 사용 하는 경우 `application/vnd.ms-az-apim.policy.raw+xml` 정책이 XML로 이스케이프 될 필요는 **없습니다** .
기본값은 `application/vnd.ms-az-apim.policy+xml` 입니다.
이 매개 변수는 선택 사항입니다.
