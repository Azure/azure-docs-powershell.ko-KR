...
위 명령은 리소스 그룹 "testrg"에 있는 클러스터 "mykustocluster"의 Kusto 삭제 기간을 "mykustodatabase" 데이터베이스에 업데이트 합니다.

### 예제 2-파이핑을 통해 기존 데이터베이스 업데이트
...
위 명령에서는 cmdlet을 사용 하 여 리소스 그룹 "testrg"에 있는 "mykustocluster" 클러스터의 Kusto database "mykustodatabase"를 가져온 `Get-AzKustoDatabase` 다음 결과를 파이프 하 여 `Update-AzKustoDatabase` 데이터베이스의 소프트 삭제 기간을 5 일로 업데이트 합니다.
...
빠른 쿼리를 위해 캐시에 데이터를 유지 해야 하는 날짜 수입니다.
...
데이터에 대 한 액세스를 중지 하기 전에 유지 해야 하는 날짜 수입니다.
