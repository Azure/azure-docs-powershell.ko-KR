...
## 설명은
**새 AzIntegrationAccountMap** cmdlet은 통합 계정 맵을 만듭니다.
이 cmdlet은 통합 계정 맵을 나타내는 개체를 반환 합니다.
통합 계정 이름, 리소스 그룹 이름, 맵 이름 및 맵 정의를 지정 합니다.
...
이 명령은 지정 된 리소스 그룹에 통합 계정 맵을 만듭니다.
명령은 이전 명령에 의해 $MapContent 변수에 저장 된 지도 정의를 지정 합니다.

## 변수

### -ContentType
통합 계정 맵의 콘텐츠 형식을 지정 합니다.
...
### -MapDefinition
통합 계정 맵에 대 한 정의 개체를 지정 합니다.
이 매개 변수 또는 *Mapfilepath* 매개 변수 중 하나를 지정 합니다.
...
### -MapFilePath
통합 계정 맵에 대 한 정의의 파일 경로를 지정 합니다. 이 매개 변수 또는 *Mapdefinition* 매개 변수 중 하나를 지정 합니다.
...
### -MapType
통합 계정 맵의 유형을 지정 합니다.
이 cmdlet은 지도 형식으로 Xslt를 지원 합니다.
...
### IntegrationAccountMap를 통해 관리 되는 모델
