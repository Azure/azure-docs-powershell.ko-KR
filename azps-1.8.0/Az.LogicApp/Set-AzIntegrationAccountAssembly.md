...
## 설명은
**Set-AzIntegrationAccountAssembly** cmdlet은 통합 계정 어셈블리를 수정 합니다.

## 예제의

### 예제 1: 로컬 파일을 사용 하 여 어셈블리 수정
```powershell
...
"$LocalAssemblyFilePath"에 포함 된 파일 경로에 있는 로컬 파일을 사용 하 여 "sampleAssembly" 라는 어셈블리를 수정 합니다.

### 예제 2: byte 데이터를 사용 하 여 어셈블리 수정
```powershell
...
"$AssemblyContent"에 포함 된 a 바이트 배열을 사용 하 여 "sampleAssembly" 라는 어셈블리를 수정 합니다.

### 예제 3: 콘텐츠 링크를 사용 하 여 어셈블리 수정
```powershell
...
URL "$assemblyUrl"에 있는 a 바이트 데이터를 사용 하 여 "sampleAssembly" 라는 어셈블리를 수정 합니다. 이 방법은 크기가 큰 어셈블리를 만들 때 제안 되는 방법입니다.
