...
### -ComputerName
Azure에 등록되는 클러스터 이름 또는 프레미스 클러스터의 클러스터 노드 중 하나를 지정합니다.
...
### -RepairRegistration
클라우드로 현재 Azure Stack HCI 등록을 복구합니다. 이 cmdlet은 클러스터된 노드 및 클라우드의 Azure AD 애플리케이션의 원격 인증서에서 로컬 인증서를 삭제하고 둘 다에 대한 새 대체 인증서를 생성합니다. 리소스 그룹, 리소스 이름 및 기타 등록 선택이 유지됩니다.
...
### 결과: 성공 또는 실패 또는 보류 중ForAdminConsent 또는 Cancelled.
### ResourceId: Azure에서 만든 리소스의 리소스 ID입니다.
### PortalResourceURL: Azure Portal Resource URL입니다.
### PortalAADAppPermissionsURL: AAD 앱 사용 권한에 대한 Azure Portal URL입니다.
## 참고 사항
