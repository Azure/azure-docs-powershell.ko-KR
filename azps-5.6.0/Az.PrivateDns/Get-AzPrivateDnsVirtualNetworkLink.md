...
## 설명
**Get-AzPrivateDnsVirtualNetworkLink** cmdlet은 지정된 리소스 그룹에서 특정 개인 DNS 영역과 연결된 가상 네트워크 링크를 얻습니다.
이름 매개 *변수를 지정하면* **단일 PSPrivateDnsVirtualNetworkLink 개체가** 반환됩니다.
이름 매개 변수를  지정하지 않으면 지정된 리소스 그룹의 영역과 연결된 모든 링크를 포함하는 배열이 반환됩니다.
**PSPrivateDnsVirtualNetworkLink** 개체를 사용하여 링크를 업데이트할 수 있습니다.

## 예제

### 예제 1: 가상 네트워크 링크를 얻습니다.
```
...
이 예제에서는 지정된 리소스 그룹에서 myzone.com 라는 개인 DNS 영역과 연결된 가상 네트워크 링크 mylink를 $Link 변수에 저장합니다.

### 예제 2: 리소스 그룹의 영역과 연결된 모든 링크를 얻습니다.
```
...
이 예제에서는 지정된 리소스 그룹의 개인 DNS 영역 "myzone.com"와 연결된 모든 가상 네트워크 링크를 구한 다음, 해당 가상 $Links 저장합니다.
...
### -Name
얻을 가상 네트워크 링크의 이름을 지정합니다.
이름 매개 변수에 대한  값을 지정하지 않으면 이 cmdlet은 지정된 리소스 그룹의 지정된 개인 DNS 영역과 연결된 모든 링크를 얻습니다.
...
### -ResourceGroupName
얻을 가상 네트워크 링크를 포함하는 리소스 그룹의 이름을 지정합니다.
...
### -ZoneName
가상 네트워크 링크가 연결된 개인 DNS 영역의 이름을 지정합니다.
