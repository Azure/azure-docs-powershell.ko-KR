...
## 설명
New-AzPrivateDnsRecordSet cmdlet은 지정된 이름이 지정된 새 DNS(개인 도메인 이름 시스템) 레코드 집합을 만들고 지정된 개인 영역의 입력을 만듭니다. RecordSet 개체는 이름과 형식이 동일한 개인 DNS 레코드 집합입니다. 이름은 완전하게 자격을 갖춘 이름이 아니라 개인 영역과 상대적입니다. PrivateDnsRecord 매개 변수는 레코드 집합의 레코드를 지정합니다. 이 매개 변수는 New-AzPrivateDnsRecordConfig를 사용하여 생성된 개인 DNS 레코드의 배열을 사용합니다. 파이프라인 연산자를 사용하여 PSPrivateDnsZone 개체를 이 cmdlet에 전달하거나, PSPrivateDnsZone 개체를 영역 매개 변수로 전달하거나 해당 ResourceId를 사용하여 영역 지정하거나 이름으로 영역 지정할 수 있습니다. 확인 매개 변수 및 $ConfirmPreference Windows PowerShell 변수를 사용하여 cmdlet에서 확인을 묻는 메시지를 표시하는지 여부를 제어할 수 있습니다. 일치하는 RecordSet이 이미 있는 경우(동일한 이름 및 레코드 형식) 덮어 작성 매개 변수를 지정해야 합니다. 그렇지 않으면 cmdlet이 새 RecordSet 을 만들지 않습니다.
...
이 명령은 개인 영역의 정수(또는 루트)에 RecordSet을 myzone.com. 이렇게 하여 레코드 집합 이름은 "@"(두 번 따옴표 포함)로 지정됩니다. 영역의 정수에 CNAME 레코드를 만들 수 없습니다. DNS 표준의 제약 조건입니다. Azure Private DNS의 제한 사항이 아 않습니다. 한 줄의 레코드만 사용하여 RecordSet을 만들거나 pn_PowerShell_short 레코드 집합을 만들 경우 예제 1을 참조합니다.
...
이 명령은 개인 영역의 *라는 RecordSet을 myzone.com. 와일드카드 레코드 집합입니다. 한 줄의 레코드만 사용하여 RecordSet을 만들거나 pn_PowerShell_short 레코드 집합을 만들 경우 예제 1을 참조합니다.
...
이 명령은 개인 영역의 *라는 RecordSet을 myzone.com. 레코드 집합은 A 형식의 TTL이 1시간(3600초)입니다. 빈 레코드 집합으로, 나중에 레코드를 추가할 수 있는 자리 1개 역할을 합니다.

### 예제 11: 레코드 집합 만들기 및 확인 표시 안 하여
...
이 명령은 RecordSet을 만듭니다. 덮어 덮어 덮어 사용 매개 변수는 이 레코드 집합이 동일한 이름과 형식을 사용하여 기존 레코드 집합을 덮어 덮습니다(해당 레코드 집합의 기존 레코드가 손실됩니다). 값의 확인 $False 확인 프롬프트를 표시하지 않습니다.
