...
## 설명
**New-AzMediaService** cmdlet은 미디어 서비스를 만듭니다.
미디어 서비스가 이미 있는 경우 이 cmdlet은 해당 속성을 업데이트합니다.

## 예제

### 예제1: 기본 저장소 계정만 사용하여 미디어 서비스 만들기
```
PS C:\># Variables
## Global
...
이 예제에서는 기본 저장소 계정만 지정하는 미디어 서비스를 만드는 방법을 보여줍니다.
이 스크립트는 다른 여러 cmdlet을 사용 합니다.

### 예제 2: 여러 저장소를 사용하여 미디어 서비스 만들기
```
PS C:\># Variables
...
이 예제에서는 여러 저장소 계정으로 미디어 서비스를 만드는 방법을 보여줍니다.
이 스크립트는 다른 여러 cmdlet을 사용 합니다.

## 매개 변수

### -AccountName
이 cmdlet에서 만드는 미디어 서비스의 이름을 지정합니다.
...
### -Location
이 cmdlet에서 미디어 서비스를 만드는 지역을 지정합니다.
...
### -ResourceGroupName
미디어 서비스가 할당된 리소스 그룹의 이름을 지정합니다.
...
### -StorageAccounts
미디어 서비스에 연결하기 위한 스토리지 계정 배열을 지정합니다.
