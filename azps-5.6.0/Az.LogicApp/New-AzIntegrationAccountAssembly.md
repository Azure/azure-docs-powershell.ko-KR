...
"$localAssemblyFilePath"에 포함된 파일 경로에 있는 로컬 파일을 사용하여 새 어셈블리를 $localAssemblyFilePath.
...
"$assemblyContent"에 포함된 $assemblyContent.
...
"$assemblyUrl"에 있는 byte 데이터를 사용하여 새 어셈블리를 $assemblyUrl. 크기가 큰 어셈블리를 만드는 데 제안된 방법입니다.
