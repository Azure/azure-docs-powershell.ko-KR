...
이 명령은 LogicApp03이라는 논리 앱에 대해 특정 논리 앱을 실행합니다.
...
이 명령은 NextPageLink를 따라 결과 크기를 두 페이지로 제한하여 LogicApp03이라는 논리 앱의 처음 두 페이지 실행 기록을 얻습니다.
...
이 cmdlet에서 실행 기록을 얻을 논리 앱의 이름을 지정합니다.
...
논리 앱의 실행 이름을 지정합니다.
이 cmdlet은 이 cmdlet이 지정하는 워크플로 실행을 제공합니다.
