...
## 설명
**New-AzLogicApp** cmdlet은 Logic Apps 기능을 사용하여 논리 앱을 만듭니다.
논리 앱은 Logic App 정의에 정의된 작업 또는 트리거의 컬렉션입니다.
이 cmdlet은 **워크플로** 개체를 반환합니다.
이름, 위치, 논리 앱 정의, 리소스 그룹 및 계획을 지정하여 논리 앱을 만들 수 있습니다.
논리 앱 정의 및 매개 변수는 JSON(JavaScript 개체 표시)에 서식이 지정됩니다.
논리 앱을 정의 및 매개 변수에 대한 템플릿으로 사용할 수 있습니다.
...
### 예제 1: 정의 및 매개 변수 파일 경로를 사용하여 논리 앱 만들기
```
...
이 명령은 지정된 리소스 그룹에 논리 앱을 만듭니다.
논리 앱에는 파일 경로에 의해 지정된 정의 및 매개 변수가 포함됩니다.

### 예제 2: 정의 및 매개 변수 개체를 사용하여 논리 앱 만들기
```
...
이 명령은 지정된 리소스 그룹 리소스 그룹에 논리 앱을 만듭니다.

### 예제 3: 파이프라인을 사용하여 리소스 그룹을 지정하여 논리 앱 만들기
```
...
이 명령은 Get-AzResourceGroup cmdlet을 사용하여 ResourceGroup11이라는 리소스 그룹을 얻습니다.
명령은 파이프라인 연산자를 사용하여 현재 cmdlet에 리소스 그룹을 전달합니다.
현재 cmdlet은 리소스 그룹에 논리 앱을 만듭니다.
논리 앱에는 파일 경로에 의해 지정된 정의 및 매개 변수가 포함됩니다.

### 예제 4: 기존 논리 앱을 기반으로 논리 앱 만들기
```
...
첫 번째 명령은 Get-AzLogicApp cmdlet을 사용하여 LogicApp03이라는 논리 앱을 얻습니다.
명령은 논리 앱을 $Workflow 저장합니다.
두 번째 명령은 에 저장된 논리 앱의 정의 및 매개 변수를 사용하는 새 논리 앱을 $Workflow.
...
### -Definition
논리 앱에 대한 정의를 개체 또는 JSON(JavaScript 개체 표기)형식으로 문자열로 지정합니다.
...
### -DefinitionFilePath
논리 앱의 정의를 JSON 형식의 정의 파일의 경로로 지정합니다.
...
### -Location
논리 앱의 위치를 지정합니다.
미국 서부 또는 동남 아시아와 같은 Azure 데이터 센터 위치를 입력합니다.
논리 앱을 모든 위치에 위치할 수 있습니다.
...
### -매개 변수
Logic App에 대한 매개 변수 컬렉션 개체를 지정합니다.
해시 테이블, 사전 \<string\> 또는 \<string, WorkflowParameter\> 사전을 지정합니다.
...
### -State
논리 앱의 상태를 지정합니다.
이 매개 변수에 대해 허용되는 값은 사용 및 사용 안 하다입니다.
