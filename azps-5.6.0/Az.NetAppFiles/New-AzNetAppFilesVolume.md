...
이 명령은 풀 "MyAnfPool"에 새 ANF 볼륨 "MyAnfVolume"을 만듭니다.
...
### -BackupId
백업 ID입니다. 백업을 식별하는 데 사용되는 UUID v4 또는 리소스 식별자
...
### -KerberosEnabled
볼륨이 Kerberos를 사용하도록 설정되어 있는 경우 설명
...
### -SecurityStyle
볼륨의 보안 스타일입니다. 가능한 값은 다음과 같습니다. 'ntfs', 'unix'
...
활성화된 경우(true) 볼륨에는 각 볼륨의 스냅숏에 대한 액세스를 제공하는 읽기 전용 .스냅숏 디렉터리가 포함됩니다(기본값: true).
...
### -SubnetId
위임된 서브넷에 대한 Azure Resource URI
