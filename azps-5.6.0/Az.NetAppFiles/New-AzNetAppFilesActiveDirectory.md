...
## 설명
**New-AzNetAppFilesActiveDirectory** cmdlet은 ANF 계정에 대한 새 활성 디렉터리 구성을 만듭니다.
...
이 명령은 PROmt에서 ANF 계정 "MyAnfAccount"에 대한 새 Active Directory 구성을 secreates로 하여 PROmt에서 AD 암호를 얻습니다.
...
### -BackupOperator
기본 제공 Backup 연산자 활성 디렉터리 그룹에 추가할 사용자입니다.
도메인 지정자 없이 고유한 사용자 이름 목록
...
### -Dns
Active Directory 도메인에 대한 DNS 서버 IP 주소 목록(IPv4만 해당)
...
### -KdcIP
활성 디렉터리 머신에 대한 kdc 서버 IP 주소입니다.
이 선택적 매개 변수는 kerberos 볼륨을 만드는 동안에만 사용됩니다.
...
### -OrganizationalUnit
Windows Active Directory 내의 OU(조직 단위)
...
### -Password
Active Directory 도메인 관리자의 일반 텍스트 암호, 응답에서 값이 마스킹됩니다.
...
SSL/TLS를 통해 LDAP를 사용하도록 설정하면 LDAP 클라이언트에 base64 인코딩된 Active Directory 인증서 서비스의 자체 서명 루트 CA 인증서가 필요합니다. 이 선택적 매개 변수는 LDAP 사용자 매핑 볼륨이 있는 이중 프로토콜에만 사용됩니다.
...
이 이름은 AD에 컴퓨터 계정으로 등록되고 볼륨을 탑재하는 데 사용됩니다.
