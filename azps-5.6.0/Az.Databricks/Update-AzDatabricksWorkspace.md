...
Databricks 작업 영역에서 암호화를 사용하도록 설정하는 데는 다음 세 단계가 필요합니다.
1.
작업 영역이 만들어지지 않은 `-PrepareEncryption` 경우)로 업데이트합니다.
1.
마지막 `StorageAccountIdentityPrincipalId` 단계의 출력에서 찾을 수 있습니다.
주체에 키 사용 권한을 부여합니다.
1.
작업 영역 다시 업데이트하여 암호화 키에 대한 정보를 입력합니다.
    - `-EncryptionKeySource`
    - `-EncryptionKeyVaultUri`
    - `-EncryptionKeyName`
    - `-EncryptionKeyVersion`

### 예제 3: Databricks 작업 영역의 암호화 사용 안 하세요.
```powershell
...
### -EncryptionKeySource
암호화 키Source(공급자).
가능한 값(대/소문자 감지): 기본값, Microsoft.Keyvault
...
### -PrepareEncryption
암호화를 위해 작업 영역 준비.
관리되는 저장소 계정에 대해 관리 ID를 사용할 수 있습니다.
