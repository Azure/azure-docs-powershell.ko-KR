...
## 설명
Set-AzEventHubNamespace cmdlet은 지정된 Event Hubs 네임스페이스의 속성을 업데이트합니다.
...
네임스페이스 \` MyNamespaceName의 태그를 만든 으로 \` 업데이트합니다.
...
AutoInflate = 사용 및 \` \` MaximumThroughputUnits = 10으로 네임스페이스 MyNamespaceName의 상태를 업데이트합니다.

### 예제 3

지정된 Event Hubs 네임스페이스를 업데이트합니다. (자동 재생)

```powershell <!-- Aladdin Generated Example --> 
Set-AzEventHubNamespace -Location 'WestUS' -Name MyNamespaceName -ResourceGroupName MyResourceGroupName -SkuName Basic
```

### 예제 5: 클러스터에서 ID 관리로 네임스페이스 만들기 및 업데이트 
```powershell
...
### -EnableAutoInflate
자동 인플레이트를 사용하도록 설정되어 있는지 여부를 나타냅니다.
...
### -EnableKafka
네임스페이스에 대해 Kafka를 사용하도록 설정하거나 사용 안 하게 설정
...
### -KeyProperty
키 속성 목록, @(@(KeyName, KeyVaultUri, Keyversion),@(KeyName, KeyVaultUri, Keyversion))
...
자동 인플레이트를 사용하도록 설정하면 0에서 20개까지의 값으로 제한됩니다.
...
### -State
네임스페이스를 사용하지 않도록 설정/사용하도록 설정합니다.
