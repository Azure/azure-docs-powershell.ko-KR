...
## 설명
Remove-AzEventHubConsumerGroup cmdlet은 지정된 Event Hub에서 지정된 소비자 그룹을 제거하고 삭제합니다.
...
MyNamespaceName 네임스페이스로 범위가 지정되는 \` \` Event Hub \` MyEventHubName에서 소비자 그룹 MyConsumerGroupName을 \` \` \` 삭제합니다.
