...
## 설명
New-AzEventHubNamespace cmdlet은 Event Hubs 형식의 새 네임스페이스를 만듭니다.
...
리소스 그룹 \` \` \` \` \` MyResourceGroupName에서 지정된 지리적 위치 MyLocation에서 Event Hubs 네임스페이스 MyNamespaceName을 \` 만듭니다.
...
지정된 지리적 위치 MyLocation에서 Event Hubs 네임스페이스 \` \` \` MyNamespaceName을 만듭니다. 리소스 그룹 \` \` MyResourceGroupName 및 \` AutoInflate에서 MaximumThroughputUnits 10을 사용할 수 있습니다.

### 예제 3: Kafka 사용 네임스페이스
```powershell
...
Kafka 및 AutoInflate를 사용하도록 설정된 리소스 그룹 \` \` \` \` \` MyResourceGroupName에서 지정된 지리적 위치 MyLocation에서 Event Hubs 네임스페이스 MyNamespaceName을 \` 만듭니다.
...
### -ZoneRedundant
네임스페이스에 대해 영역 중복을 사용하도록 설정하거나 사용 안 하게 설정
