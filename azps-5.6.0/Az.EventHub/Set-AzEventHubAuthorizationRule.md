...
## 설명
Set-AzEventHubAuthorizationRule cmdlet은 지정된 Event Hub에서 지정된 권한 부여 규칙을 업데이트합니다.
...
MyAuthRuleName 권한 부여 규칙을 업데이트하여 \` \` 네임스페이스 \` \` \` MyNamespaceName에서 범위가 지정한 Event Hub MyEventHubName에 \` 대한 관리 권한을 부여합니다.
