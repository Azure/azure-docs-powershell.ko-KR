...
리소스 그룹 \` \` \` \` \` MyResourceGroupName이 있는 네임스페이스 MyNamespaceName에 수신 및 보내기 권한을 부여하는 권한 부여 규칙 MyAuthRuleName을 \` 만듭니다.
...
리소스 그룹 \` \` \` \` \` \` \` MyResourceGroupName이 있는 네임스페이스 MyNamespaceName에서 Event Hub MyEventHubName에 수신 및 보내기 권한을 부여하는 권한 부여 규칙 MyAuthRuleName을 \` 만듭니다.
