...
## 설명
New-AzEventHub cmdlet은 새 Azure Event Hub를 만듭니다.
Capture 설명 속성을 사용하여 Eventhub를 만들 경우 아래 단계(예제 2)를 따르세요. 
...
리소스 그룹 \` \` \` \` \` MyResourceGroupName이 있는 WestUS 위치에 3일 메시지 보존 기간과 파티션 2개가 있는 MyEventHubName이라는 Event Hub를 \` 만듭니다.
...
리소스 그룹 \` \` \` \` \` MyResourceGroupName을 사용하여 3일 메시지 보존 기간, 2개 파티션 및 CaptureDescription 속성을 사용하여 MyEventHubName이라는 Event Hub를 \` 만듭니다.
