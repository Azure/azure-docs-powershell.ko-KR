...
## 설명
**Get-AzDataShare** cmdlet은 Azure 데이터 공유 accoount의 데이터 공유에 대한 정보를 얻습니다.
데이터 공유의 이름을 지정하는 경우 이 cmdlet은 해당 데이터 공유에 대한 정보를 얻습니다.
이름을 지정하지 않으면 이 cmdlet은 Azure 데이터 공유 계정의 모든 데이터 공유에 대한 정보를 제공합니다.

## 예제

### 예제 1 : 특정 데이터 공유를 얻게 됩니다.
```
...
이 명령은 Azure 데이터 공유 계정 WikiAdsAccount 및 리소스 그룹 ADS에서 데이터 공유 AdsShare에 대한 정보를 표시합니다.
