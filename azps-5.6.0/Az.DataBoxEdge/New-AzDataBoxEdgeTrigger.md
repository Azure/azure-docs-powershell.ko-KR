...
## 설명
**New-AzDataBoxEdgeTrigger** cmdlet은 Data Box Edge 디바이스에서 트리거를 구성합니다. 
...
### -FileEvent
이 스위치 매개 변수를 전달하여 FileEvent Trigger를 구성합니다.
...
### -PeriodicTimerEvent
이 스위치 매개 변수를 전달하여 PeriodicTimerEvent 트리거를 구성합니다.
...
### -Schedule
시간 이벤트가 발생해야 하는 주기적 빈도입니다. 일정을 일(1~365 사이) 또는 시간(1~23 사이) 또는 분(1~59 사이)으로 지정합니다.
...
### -ShareId
FileEvent Trigger에서 전달할 파일 공유 ID
...
### -StartTime
유효한 트리거가 발생되는 하루의 시간입니다. 일정은 최대 초까지 지정된 시간을 참조하여 계산됩니다. 타임존이 지정되지 않은 경우 디바이스 타임존에 있는 것으로 간주됩니다. 값은 항상 UTC 시간으로 반환됩니다.
...
### -Topic
주기적인 이벤트가 IoT 디바이스에 게시되는 토픽입니다.
