...
U-SQL 스크립트 매개 변수는 기본 스크립트 내용 위에 미리 입력됩니다(예: DECLARE @Department 문자열 = "Sales"). DECLARE @NumRecords int = 1000; DECLARE @StartDateTime DateTime = 새 DateTime(2017, 12, 6, 0, 0, 0);
...
### -AnalyticsUnits
이 작업에서 사용할 분석 단위입니다. 일반적으로 스크립트 전용 분석 단위가 1개 더 수록 스크립트 실행 시간이 빨라집니다.
...
### -CompileMode
이 작업에서 수행될 컴파일 유형입니다. 유효한 값: 
- 시맨틱(시맨틱 검사 및 필요한 위생 검사만 수행)
- 전체(전체 컴파일)
- SingleBox(로컬에서 수행된 전체 컴파일)
...
### -PipelineId
이 작업의 제출을 나타내는 ID는 재발 작업 집합의 일부이자 작업 파이프라인과도 연결됩니다.
...
### -PipelineName
이 작업과 연결된 파이프라인에 대한 선택적 친숙한 이름입니다.
...
### -Priority
작업의 우선 순위입니다. 지정하지 않으면 우선 순위가 1000입니다. 숫자가 낮을수록 작업 우선 순위가 높을 수 있습니다.
...
### -RecurrenceId
이 작업의 제출을 나타내는 ID는 동일한 재발 ID가 있는 재발 작업 집합의 일부입니다.
...
### -RecurrenceName
작업 간의 재발 상관 관계에 대한 선택적 친숙한 이름입니다.
...
### -RunId
파이프라인의 이 특정 실행 이터러 를 식별하는 ID입니다.
...
### -ScriptParameter
이 작업의 스크립트 매개 변수는 매개 변수 이름(string)의 사전으로 값(바이트, sbyte, int, uint(또는 uint32), long, ulong(또는 uint64), float, double, decimal, short(또는 int16), char, string, DateTime, bool, Guid 또는 byte[]입니다.
