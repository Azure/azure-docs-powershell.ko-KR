...
## 설명
**New-AzDataLakeAnalyticsAccount** cmdlet은 Azure Data Lake Analytics 계정을 만듭니다.
...
이 명령은 ContosoOrg라는 리소스 그룹에 ContosoAdlStore Data Store를 사용하는 ContosoAdlAccount라는 Data Lake Analytics 계정을 만듭니다.

## 매개 변수

### -DefaultDataLakeStore
기본 데이터 원본으로 설정할 Data Lake Store 계정의 이름을 지정합니다.
...
### -Location
Data Lake Analytics 계정을 만들 위치를 지정합니다.
현재 미국 동부 2만 지원됩니다.
...
### -MaxJobCount
계정에서 동시에 실행되는 선택적 최대 지원되는 작업입니다. 지정되지 않은 경우 기본값은 3으로 설정됩니다.
...
### -QueryStoreRetention
작업 메타데이터가 보존되는 선택적 일 수입니다. 지정하지 않은 경우 기본값은 30일입니다.
...
리소스 그룹을 만들 경우 New-AzResourceGroup cmdlet을 사용 합니다.
