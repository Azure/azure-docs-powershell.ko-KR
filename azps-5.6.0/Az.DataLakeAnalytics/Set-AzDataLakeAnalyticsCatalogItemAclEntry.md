...
## 설명
**Set-AzDataLakeAnalyticsCatalogItemAclEntry** cmdlet은 Data Lake Analytics의 카탈로그 또는 카탈로그 항목의 액세스 제어 목록(ACL)에 ACE(항목)를 추가하거나 수정합니다.
...
이 명령은 Patti Fuller용 카탈로그 ACE를 수정하여 읽기 권한을 습니다.
...
이 명령은 Patti Fuller에 대한 데이터베이스 ACE를 수정하여 읽기 권한을 습니다.

### 예제 3: 카탈로그에 대한 다른 사용 권한 수정
```powershell
...
이 명령은 다른 사용자에 대한 카탈로그 ACE를 수정하여 읽기 권한을 습니다.
...
### 예제 5: 카탈로그에 대한 사용자 소유자 사용 권한 수정
```powershell
...
### -기타
다른 카탈로그에 대한 카탈로그의 ACL 항목을 설정합니다.
