...
## 설명
**Get-AzNotificationHubListKey** cmdlet은 SAS(공유 액세스 서명) 권한 부여 규칙의 기본 및 보조 연결 문자열을 반환합니다.
권한 부여 규칙은 허브에 대한 사용자 권한을 관리합니다.
각 규칙에는 기본 및 보조 연결 문자열이 포함됩니다.
이러한 연결 문자열(URIS)은 다음을 수행합니다.
- 사용자를 리소스로 지 습니다.
- 쿼리 매개 변수를 포함하는 토큰을 포함합니다.
이러한 매개 변수 중 하나인 서명은 사용자를 인증하고 지정된 액세스 수준을 제공하는 데 사용됩니다.
