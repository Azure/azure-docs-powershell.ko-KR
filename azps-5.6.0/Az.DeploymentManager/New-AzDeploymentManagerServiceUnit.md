...
## 설명
**New-AzDeploymentManagerServiceUnit** cmdlet은 서비스 토폴로지에서 서비스 아래에 서비스를 만들고 해당 서비스 단위를 나타내는 개체를 반환합니다.
서비스 단위를 이름, 서비스 이름, 있는 서비스 토폴로지 및 리소스 그룹 이름으로 지정합니다. 

cmdlet은 ServiceUnit 개체를 반환합니다. 이 개체를 로컬로 수정한 다음, cmdlet을 사용하여 서비스에 Set-AzDeploymentManagerService 수 있습니다.
...
이 cmdlet은 미국 중부의 토폴로지 ContosoServiceTopology의 ContosoServiceTopology의 ContosoService2 서비스 아래에 ContosoResourceGroup에 ContosoService2Storage 이름이 있는 새 서비스 단위를 만듭니다. 템플릿 및 매개 변수 파일은 서비스 토폴로지 ContosoServiceTopology에서 참조하는 아티팩트 원본 위치에 대한 상대 경로로 정의됩니다. 이 템플릿에 정의된 리소스는 대상 리소스 그룹 service2ResourceGroup에 배포 모드가 증분으로 설정되어 있습니다.
...
이 cmdlet은 미국 중부의 토폴로지 ContosoServiceTopology의 ContosoServiceTopology의 ContosoService2 서비스 아래에 ContosoResourceGroup에 ContosoService2Storage 이름이 있는 새 서비스 단위를 만듭니다. 템플릿 및 매개 변수 참조는 SAS Uri의 아티팩트 원본 ResourceId가 Service 토폴로지 ContosoServiceTopology1에 제공되지 않은 것으로 제공됩니다. 이 템플릿에 정의된 리소스는 배포 모드가 완료로 설정된 대상 리소스 그룹 service2ResourceGroup에 배포해야 합니다.
...
아티팩트 원본을 상대로 하는 매개 변수 파일의 경로입니다.
ServiceTopology에서 아티팩트Source를 참조해야 합니다.
...
아티팩트SourceId가 ServiceTopology에서 참조된 경우 ParametersArtifactSourceRelativePath를 사용하여 상대 경로를 지정합니다.
...
아티팩트SourceId가 ServiceTopology에서 참조된 경우 TemplateArtifactSourceRelativePath를 사용하여 상대 경로를 지정합니다.
