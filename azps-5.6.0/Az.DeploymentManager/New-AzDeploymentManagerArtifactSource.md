...
반환된 개체를 로컬로 수정한 다음, cmdlet을 사용하여 아티팩트 원본에 Set-AzDeploymentManagerArtifactSource 수 있습니다.

cmdlet은 ServiceUnit 리소스, 템플릿 및 매개 변수 파일에 필요한 아티팩트를 이 위치에서 참조할 수 있도록 New-AzDeploymentManagerServiceTopology cmdlet에서 참조할 수 있는 ResourceId가 있는 ArtifactSource 개체를 반환합니다.
...
ContosoResourceGroup에서 리소스의 위치로 미국 중부와 함께 ContosoArtifactSource 이름이 있는 아티팩트 원본을 만듭니다. SasUri 속성은 아티팩트가 저장되는 저장소 컨테이너에 Azure Storage SAS Uri를 제공합니다.
...
### -SasUri
아티팩트가 저장되는 Azure Storage 컨테이너에 대한 SAS Uri입니다.
