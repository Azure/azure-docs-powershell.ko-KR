...
## 설명
New-AzRelayHybridConnection cmdlet은 지정된 릴레이 네임스페이스에 HybridConnection을 만듭니다.
...
지정된 릴레이 네임스페이스 \` \` \` TestNameSpace-HybirdConnection에서 새 HybirdConnection TestHybirdConnection2를 \` 만듭니다.
...
지정된 릴레이 네임스페이스 \` \` \` TestNameSpace-HybirdConnection 에서 새 HybirdConnection TestHybirdConnection1을 \` 만듭니다.
...
### -UserMetadata
사용자 메타데이터를 얻거나 설정하는 것은 HybridConnection 엔드포인트에 대한 사용자 정의 문자열 데이터를 저장하는 자리 표시자입니다. 팀 목록 및 해당 연락처 정보와 같은 설명 데이터를 저장하는 데 사용할 수 있으며 사용자 정의 구성 설정도 저장할 수 있습니다.
