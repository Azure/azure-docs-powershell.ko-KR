...
## 설명
New-AzWcfRelay cmdlet은 지정된 릴레이 네임스페이스에 WcfRelay를 만듭니다.
...
지정된 릴레이 네임스페이스 \` \` \` TestNameSpace-Relay에서 새 WcfRelay TestWCFRelay2를 \` 만듭니다.
...
지정된 릴레이 네임스페이스 \` \` \` TestNameSpace-Relay1에서 새 WcfRelay TestWCFRelay를 \` 만듭니다.
...
### -WcfRelayType
WcfRelay 형식입니다.
가능한 값은 다음과 같습니다. 'NetTcp' 또는 'Http'
