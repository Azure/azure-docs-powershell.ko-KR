...
## 설명
Get-AzSubscription cmdlet은 현재 계정이 액세스할 수 있는 구독에 대한 구독 ID, 구독 이름 및 홈 테넌트를 얻습니다.

## 예제

### 예제 1: 모든 테넌트에서 모든 구독을 얻습니다.
```
...
이 명령은 현재 계정에 대해 권한이 부여된 모든 테넌트의 모든 구독을 얻습니다.

### 예제 2: 특정 테넌트에 대한 모든 구독을 얻습니다.
```
...
현재 계정에 대해 권한이 부여된 해당 테넌트의 모든 구독을 나열합니다.

### 예제 3: 현재 테넌트의 모든 구독을 얻습니다.
```
PS C:\>Get-AzSubscription
...
이 명령은 현재 사용자에 대해 권한이 부여된 현재 테넌트의 모든 구독을 얻습니다.

### 예제 4: 특정 구독을 사용하도록 현재 컨텍스트 변경
```
...
이 명령은 지정된 구독을 얻은 다음, 현재 컨텍스트를 사용하여 설정합니다. 이 세션의 모든 후속 cmdlet은 기본적으로 새 구독(Contoso 구독 1)을 사용 합니다.
