...
## 설명
스크립트, 스택 추적 및 모든 내부 및 집계 예외에서 오류가 발생한 위치를 포함하여 현재 PowerShell 세션의 오류에 대한 자세한 정보를 확인 및 표시합니다. Azure PowerShell 오류는 오류의 원인인 요청 및 서버 응답에 대한 전체 세부 정보를 포함하여 서비스 문제 디버깅에 대한 추가 세부 정보를 제공합니다.
