...
## 설명
Azure PowerShell cmdlet에서 대상(또는 계정 또는 테넌트)에 대한 구독을 선택합니다.  이 cmdlet 후 향후 cmdlet은 선택한 컨텍스트를 대상으로 합니다.
...
'Work' 컨텍스트에서 계정, 테넌트 및 구독에서 향후 Azure PowerShell cmdlet을 대상으로 합니다.

### 예제 2

이 powershell 세션에서 Azure PowerShell cmdlet을 대상으로 하는 구독 및 계정을 선택합니다. (자동 재생)
