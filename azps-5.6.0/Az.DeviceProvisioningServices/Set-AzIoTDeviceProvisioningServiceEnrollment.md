...
## 설명
Azure IoT Hub Device Provisioning Service에서 디바이스 등록을 업데이트합니다.
...
등록 레코드에 대한 할당 정책 및 허브를 업데이트합니다.
...
### -ApiVersion
사용자 지정 할당 요청의 프로비전 서비스의 API 버전입니다.
...
여러 IoT Hubs에 대해 공백으로 구분된 목록을 사용하세요.
...
### -PrimaryCAName
기본 루트 CA 인증서의 이름입니다.
루트 CA 인증서를 증명하는 것이 필요한 경우 루트 ca 이름을 제공해야 합니다.
...
### -PrimaryCertificate
기본 인증서를 포함하는 파일의 경로입니다.
X509 인증서 .cer 파일 또는 .pem 파일 경로의 Base-64 표현입니다.
...
### -ProvisioningStatus
등록 항목을 사용하도록 설정하거나 사용하지 않도록 설정합니다.
...
### -RootCertificate
루트 인증서를 사용하여 X509attestation을 업데이트로 전환합니다.
