...
### 예제 1: 지정된 PostgreSql 서버에 있는 모든 가상 네트워크 규칙을 나열합니다.
```powershell
...
이 cmdlet은 지정된 PostgreSql 서버에 있는 모든 가상 네트워크 규칙을 나열합니다.

### 예제 2: 이름에 의해 가상 네트워크 규칙 얻기
```powershell
...
이 cmdlet은 ID에 의해 Virtual Network Rule을 얻습니다.
