...
INPUTOBJECT <IServerAutoGenerated> : 연결 문자열의 서버
  ...
  - `[AdministratorLoginPassword <SecureString>]`: 관리자 로그인 암호(서버 만들기에 필요).
  ...
  - `[PointInTimeUtc <DateTime?>]`: 복원할 시간을 지정하여 지점 만들기 시간(ISO8601 형식)을 복원합니다.
  - `[PropertiesTag <IServerPropertiesTags>]`: 키-값 쌍의 형태로 애플리케이션별 메타데이터입니다.
    - `[(Any) <String>]`: 이 개체에 속성을 추가할 수 있는 경우를 나타냅니다.
  - `[SkuName <String>]`: sku의 이름, 일반적으로 계층 + 패밀리 + 코어(예: Standard_D4s_v3.
  - `[SkuTier <SkuTier?>]`: 특정 SKU의 계층(예: Burstable)입니다.
