...
## 설명
이 cmdlet은 -PackageUrl에 지정된 패키지를 사용하여 새 애플리케이션 형식 버전을 만듭니다. 이 버전은 Azure Resource Manager의 REST 엔드포인트를 통해 배포 중에 사용할 수 있으며 확장 .sfpkg로 애플리케이션 패키지 및 압축을 포함해야 합니다. 이 명령은 애플리케이션 형식이 아직 없는 경우 생성됩니다.
...
이 예제에서는 "testAppType"을 입력하여 애플리케이션 형식 버전 "v1"을 생성합니다. 패키지에 포함된 애플리케이션 매니페스트의 버전은 -Version에 지정된 버전과 동일해야 합니다.
