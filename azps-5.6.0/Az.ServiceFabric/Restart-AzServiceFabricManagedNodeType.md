...
## 설명
노드 형식에서 특정 노드를 다시 시작합니다. vm을 다시 시작하기 전에 서비스 패브릭 노드를 사용하지 않도록 설정하고 다시 돌아오면 다시 사용하도록 설정합니다. 주 노드 형식에서 이 작업을 수행하면 모든 노드를 동시에 다시 시작하지 않을 수 있습니다. -ForceRestart를 사용하여 서비스 패브릭이 노드를 사용하지 않도록 설정할 수 없지만 노드에서 상태 저장 워크로드가 실행되는 경우 데이터 손실이 발생할 수 있습니다.
...
### -ForceRestart
이 플래그를 사용하는 경우 서비스 패브릭에서 노드를 사용하지 않도록 설정할 수 없는 경우에도 노드가 다시 시작됩니다.
