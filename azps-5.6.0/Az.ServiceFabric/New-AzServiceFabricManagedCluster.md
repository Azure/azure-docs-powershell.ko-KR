...
## 설명
이 cmdlet은 노드 형식 없이 관리되는 클러스터 리소스를 생성합니다. 클러스터를 부트스트레이프하기 위해 [New-AzServiceFabricManagedNodeType을](./New-AzServiceFabricManagedNodeType.md)사용하여 기본 노드 유형을 추가해야 합니다.
...
이 명령은 기본 기본 sku를 사용하여 클러스터 리소스를 만듭니다.
...
이 명령은 초기 관리자 클라이언트 인증서 및 표준 sku를 사용하여 centraluseuap에 클러스터 리소스를 만듭니다.
...
### -ClientCertIsAdmin
클라이언트 인증서에 관리자 수준이 있는지 지정하는 데 사용할 수 있습니다.
...
클라이언트 인증서에 대한 발급자 지문 목록입니다.
ClientCertCommonName과 함께만 사용.
...
### -CodeVersion
클러스터 서비스 패브릭 코드 버전입니다.
업그레이드 모드가 수동인 경우만 사용할 수 있습니다.
...
### -Sku
클러스터의 Sku, 옵션은 기본입니다. 최소 3개 시드 노드가 있으며 1개 노드 유형 및 표준만 허용됩니다. 최소 5개 시드 노드가 있으며 여러 노드 형식을 허용합니다.
...
### -UpgradeMode
클러스터 서비스 패브릭 코드 버전 업그레이드 모드입니다.
자동 또는 수동.
...
### -UseTestExtension
클러스터를 지정하는 경우 서비스 테스트 vmss 확장으로 crated됩니다.
