...
## 설명
이 cmdlet을 사용하면 지정된 애플리케이션에서 상태 비장 또는 상태 비우기 서비스를 만들 수 있습니다. 서비스는 애플리케이션 매니페스트에서 종료해야 합니다. 형식은 매니페스트의 형식과 동일해야 합니다. 애플리케이션 이름은 서비스 이름의 프레픽스가
...
이 예제에서는 인스턴스 수 -1(모든 노드에서)을 사용하여 새 상태 비어 있는 서비스 "testApp~testService1"을 생성합니다.
...
이 예제에서는 5개 노드의 대상이 있는 새 상태 보존 서비스 "testApp~testService2"를 생성합니다.
...
### -DefaultMoveCost
이동에 대한 기본 비용을 지정합니다.
비용이 높을수록 클러스터의 균형을 조정하려고 할 때 클러스터 리소스 관리자가 복제본을 이동할 가능성이 줄어 니다.
...
서비스가 명명된 파티션 구성표를 사용하는지 나타냅니다.
이 모델을 사용하는 서비스에는 일반적으로 바인딩된 집합 내에서 버킷할 수 있는 데이터가 있습니다.
명명된 파티션 키로 사용되는 데이터 필드의 일반적인 예로는 지역, 우편 번호, 고객 그룹 또는 기타 비즈니스 경계가 있습니다.
...
서비스가 Singleton 파티션 구성표를 사용하는지 나타냅니다.
Singleton 파티션은 일반적으로 서비스에 추가 라우팅이 필요하지 않은 경우 사용됩니다.
...
서비스가 UniformInt64 파티션 구성표를 사용하는지 나타냅니다.
즉, 각 파티션은 int64 키 범위를 소유합니다.
...
### -Type
애플리케이션 매니페스트에 존재해야 하는 애플리케이션의 서비스 형식 이름을 지정합니다.
