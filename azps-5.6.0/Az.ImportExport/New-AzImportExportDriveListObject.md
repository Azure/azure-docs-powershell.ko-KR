...
### 예제 1: ImportExport 작업용 새 DriveList 만들기
```powershell
...
이 필드는 드라이브가 전송 상태일 때까지 응답에서 반환되지 않습니다.
...
### -ErrorLogUri
데이터 전송 작업에 대한 오류 로그가 포함된 Blob을 지적하는 URI입니다.
...
### -ManifestHash
드라이브의 매니페스트 파일의 Base16 인코딩 MD5 해시입니다.
...
### -ManifestUri
드라이브 매니페스트 파일이 포함된 Blob을 지적하는 URI입니다.
...
### -VerboseLogUri
데이터 전송 작업에 대한 자세한 로그가 포함된 Blob을 지적하는 URI입니다.
