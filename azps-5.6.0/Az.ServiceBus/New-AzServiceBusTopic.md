...
지정된 Service Bus 네임스페이스에서 `SB-Topic_exampl1` 새 Service Bus 토픽을 `SB-Example1` 만듭니다.

### 예제 2

지정된 Service Bus 네임스페이스에서 새 Service Bus 토픽을 만듭니다. (자동 재생)
...
서버 쪽 일괄 처리 작업이 사용하도록 설정되어 있는지 여부를 나타냅니다.
...
### -EnableExpress
Express 엔터티를 사용하도록 설정하는지 여부를 나타냅니다. Express 큐는 영구 저장소에 쓰기 전에 일시적으로 메모리에 메시지를 저장합니다.
...
### -enablePartitioning
토픽을 여러 메시지 브로커에서 분할할지 여부를 지정합니다. 
...
### -MaxSizeInMegabytes
토픽에 할당된 메모리 크기인 토픽의 최대 크기는 메가바이트입니다.
