...
Cmdlet은 표준에서 프리미엄 네임스페이스로 마이그레이션을 완료로 설정하고 표준 네임스페이스의 연결 문자열은 이제 Premium 네임스페이스를 지정합니다.
...
네임스페이스에 대한 MigrationConfiguration 검색
...
지정된 Service Bus 토픽에 대한 설명을 반환합니다.
...
네임스페이스/큐/토픽의 Azure serviucebus 권한 부여 규칙에 대한 SAS tolen을 생성합니다. 
...
지정된 Service Bus 네임스페이스에서 새 Service Bus 토픽을 만듭니다.
...
Cmdlet은 표준에서 프리미엄 네임스페이스에 대한 마이그레이션 구성을 삭제합니다.
...
새 마이그레이션 구성을 만들고 표준에서 Premium 네임스페이스로 엔터티 마이그레이션을 시작합니다.
