...
이 기간은 메시지가 Service Bus로 전송될 때부터 메시지가 만료되는 기간입니다.
TimeToLive가 메시지 자체에서 설정되지 않은 경우 사용되는 기본값입니다.
...
Batched Operations 사용 - 서버 쪽 일괄 처리 작업이 사용하도록 설정되어 있는지 여부를 나타내는 값
...
### -RequiresSession
필요Session - 이 큐에 중복 검색이 필요한지 나타내는 값입니다.
