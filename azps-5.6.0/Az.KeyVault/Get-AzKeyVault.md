...
## 설명
**Get-AzKeyVault** cmdlet은 구독의 키 자격 증명 모음에 대한 정보를 얻습니다. 구독에서 모든 키 자격 증명 모음 인스턴스를 보거나 리소스 그룹 또는 특정 키 자격 증명 모음을 통해 결과를 필터링할 수 있습니다.
단일 키 자격 증명 모음을 얻을 때 리소스 그룹을 지정하는 것은 이 cmdlet에 선택 사항이지만 성능을 향상하기 위해 이 작업을 해야 합니다.

## 예제

### 예제 1: 현재 구독의 모든 주요 자격 증명 모음을 얻습니다.
```powershell
PS C:\> Get-AzKeyVault
...
이 명령은 현재 구독의 모든 주요 자격 증명 모음을 얻습니다.

### 예제 2: 특정 키 자격 증명 모음을 구합니다.
```powershell
...
이 명령은 ContosoPayRollResourceGroup이라는 리소스 그룹의 모든 주요 자격 증명 모음을 얻습니다.
...
이 명령은 현재 구독 및 서부 지역에서 myvault4라는 삭제된 키 자격 증명 모음 정보를 얻습니다.

### 예제 6: 필터링을 사용하여 주요 자격 증명 모음을 얻습니다.
```powershell
PS C:\> Get-AzKeyVault -VaultName 'myvault*'
...
### -InRemovedState
출력에 이전에 삭제된 자격 증명 모음을 표시할지 여부를 지정합니다.
...
### Microsoft.Azure.Commands.KeyVault.Models.PSDKeyVault
