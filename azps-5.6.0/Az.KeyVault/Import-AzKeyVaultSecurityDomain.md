...
## 설명
이 cmdlet은 이전에 내보낼 보안 도메인 데이터를 관리되는 HSM으로 가져온다.
...
먼저 보안 도메인 데이터를 해독하려면 키를 제공해야 합니다.
그런 다음 **Import-AzKeyVaultSecurityDomain** 명령은 이러한 키를 사용하여 관리되는 HSM에 이전 백업된 보안 도메인 데이터를 복원합니다.
...
### -키
보안 도메인 데이터를 해독하는 데 사용되는 키에 대한 정보입니다.
구성 방법에 대한 예제를 참조합니다.
