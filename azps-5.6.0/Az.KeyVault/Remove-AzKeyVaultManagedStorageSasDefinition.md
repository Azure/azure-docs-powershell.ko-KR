...
## 설명
Key Vault 관리 Azure Storage SAS 정의를 제거합니다. 또한 이 SAS 정의당 SAS 토큰을 사용하는 데 사용되는 비밀도 제거됩니다.

## 예제

### 예제 1: Key Vault 관리 Azure Storage SAS 정의를 제거합니다.
```powershell
...
자격 증명 모음 'myvault'의 계정 'mystorageaccount'와 연결된 Key Vault 관리 저장소 SAS 정의 'mysasdef'를 제거합니다.

### 예제 2: 사용자 확인 없이 Key Vault 관리 Azure Storage SAS 정의를 제거합니다.
```powershell
...
### -AccountName
저장소 계정 이름입니다.
Cmdlet은 자격 증명 모음 이름, 현재 선택한 환경 및 저장소 계정 이름에서 관리되는 저장소 계정 이름의 FQDN을 생성합니다.
...
### -InputObject
ManagedStorageSasDefinition 개체입니다.
