...
## 설명
**Undo-AzKeyVaultManagedStorageSasDefinitionRemoval** 명령은 이전에 삭제된 관리 저장소 SAS 정의를 복구합니다. 이 자격 증명 모음에 대해 소프트 삭제가 활성화되고 복구 간격 동안 복구 시도가 발생합니다.
...
이 명령 순서는 지정된 저장소 SAS 정의가 삭제된 상태의 자격 증명 모음에 있는지 여부를 결정합니다. 후속 명령은 삭제된 sas 정의를 복구하여 활성 상태로 되돌려 넣습니다.
...
Cmdlet은 자격 증명 모음 이름, 현재 선택한 환경, 관리되는 저장소 계정 이름 및 SAS 정의의 이름에서 대상의 FQDN을 생성합니다.
