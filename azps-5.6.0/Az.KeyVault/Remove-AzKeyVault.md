...
## 설명
**Remove-AzKeyVault** cmdlet은 지정된 키 자격 증명 모음을 삭제합니다.
또한 해당 인스턴스에 포함된 모든 키와 비밀을 삭제합니다.
리소스 그룹을 지정하는 것은 이 cmdlet에 대한 선택 사항이지만 성능을 향상하기 위해 그렇게 해야 합니다.
...
이 명령은 현재 구독에서 Contoso03Vault라는 키 자격 증명 모음을 제거합니다.

### 예제 2: 지정된 리소스 그룹에서 키 자격 증명 모음 제거
```powershell
...
이 명령은 명명된 리소스 그룹에서 Contoso03Vault라는 키 자격 증명 모음을 제거합니다.
리소스 그룹 이름을 지정하지 않으면 cmdlet은 현재 구독에서 삭제하기 위해 명명된 키 자격 증명 모음을 검색합니다.
...
이 명령은 현재 구독에서 testManagedHsm이라는 관리되는 hsm을 제거합니다.
...
### -Force
cmdlet에서 확인을 묻는 메시지가 표시되지 않는다는 의미입니다.
기본적으로 이 cmdlet은 키 자격 증명 모음을 삭제하려는지 확인하라는 메시지를 표시합니다.
