...
## 설명
Key Vault에서 관리할 Key Vault for Storage 계정 키를 사용하여 기존 Azure Storage 계정을 설정합니다. Storage 계정이 이미 있어야 합니다. 저장소 키는 호출자에 노출되지 않습니다.
Key Vault는 재생성 기간에 따라 활성 키를 다시 생성하고 전환합니다. 이 기능에 대한 개요는 [Azure Key Vault 관리 저장소 계정 - PowerShell을](https://docs.microsoft.com/azure/key-vault/key-vault-overview-storage-keys-powershell) 참조하세요.

## 예제

### 예제 1: Key Vault를 사용하여 Azure Storage 계정 설정 키를 관리합니다.
```powershell
...
Key Vault에서 관리할 키에 대해 Key Vault를 사용하여 Storage 계정을 설정합니다. 활성 키 집합은 'key1'입니다. 이 키는 sas 토큰을 생성하는 데 사용됩니다. Key Vault는 이 명령의 재생성 기간 후에 'key2' 키를 다시 생성하고 활성 키로 설정합니다. 이 자동 재생성 프로세스는 90일 간격으로 'key1'과 'key2' 간에 계속됩니다.

### 예제 2: Key Vault를 사용하여 클래식 Azure Storage 계정 설정하여 해당 키를 관리합니다.
```powershell
PS C:\> $regenerationPeriod = [System.Timespan]::FromDays(90)
...
Key Vault에서 관리할 키에 대해 Key Vault를 사용하여 클래식 저장소 계정을 설정합니다. 활성 키 집합은 '기본'입니다. 이 키는 sas 토큰을 생성하는 데 사용됩니다. Key Vault는 이 명령의 재생성 기간 이후 '보조' 키를 다시 생성하고 활성 키로 설정합니다. 이 자동 재생성 프로세스는 90일 간격으로 '기본'과 '보조' 간에 계속됩니다.
...
### -사용 안
sas 토큰 생성에 대해 관리 저장소 계정의 키를 사용하지 않도록 설정합니다.
...
키를 자동으로 다시 생성합니다. true이면 관리 저장소 계정의 비활성 키가 자동으로 다시 생성되고 재생성 기간 후에 새 활성 키가 됩니다. false이면 관리되는 저장소 계정의 키가 자동으로 다시 생성되지 않습니다.
...
### -RegenerationPeriod
재생성 기간입니다. 자동 다시 생성 키를 사용하도록 설정하면 이 값은 관리 저장소 계정의 비활성 키가 자동으로 다시 생성되고 새 활성 키가 되는 시간(timespan)을 지정합니다.
