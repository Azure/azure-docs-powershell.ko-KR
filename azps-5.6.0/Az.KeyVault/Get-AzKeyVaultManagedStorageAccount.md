...
자격 증명 모음 'myvault'에서 키가 관리되는 모든 계정을 나열합니다.
...
키가 자격 증명 모음 'myvault'에서 관리되는 경우 'mystorageaccount'의 Key Vault 관리 저장소 계정의 세부 정보를 얻습니다.

### 예제 3: 필터링을 사용하여 모든 Key Vault 관리 저장소 계정 나열
```powershell
...
"test"로 시작하는 자격 증명 모음 'myvault'에서 키가 관리되는 모든 계정을 나열합니다.
...
### -InRemovedState
출력에 이전에 삭제된 저장소 계정을 표시할지 여부를 지정합니다.
