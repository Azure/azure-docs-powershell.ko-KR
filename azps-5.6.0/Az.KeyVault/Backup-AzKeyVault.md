...
## 설명
관리되는 HSM을 저장소 계정에 완전히 백업합니다.
백업을 `Restore-AzKeyVault` 복원하는 데 사용할 수 있습니다.
...
cmdlet은 저장소 컨테이너에 폴더(일반적으로 이름)를 만들고 해당 폴더에 백업을 저장하고 `mhsm-{name}-{timestamp}` 폴더 URI를 출력합니다.
