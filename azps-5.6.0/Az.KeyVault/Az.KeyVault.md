...
# Az.KeyVault 모듈
## 설명
이 항목에서는 Azure Key Vault Cmdlet에 대한 도움말 항목을 제공합니다.

## Az.KeyVault Cmdlet
### [Add-AzKeyVaultCertificate](Add-AzKeyVaultCertificate.md)
키 자격 증명 모음에 인증서를 추가합니다.
...
클라이언트의 인터넷 주소를 기반으로 키 자격 증명 모음에 대한 액세스를 제한하는 규칙을 추가합니다.
...
관리되는 HSM의 보안 도메인 데이터를 내보낼 수 있습니다.
...
키 자격 증명 모음에 대한 인증서 알림에 등록된 연락처를 수신합니다.

### [Get-AzKeyVaultCertificateIssuer](Get-AzKeyVaultCertificateIssuer.md)
키 자격 증명 모음에 대한 인증서 발급자 를 얻습니다.
...
Key Vault 관리 스토리지 SAS 정의를 얻습니다.
...
Key Vault 관리 Azure Storage SAS 정의를 제거합니다.

### [Remove-AzKeyVaultNetworkRule](Remove-AzKeyVaultNetworkRule.md)
키 자격 증명 모음에서 네트워크 규칙을 제거합니다.
...
키 자격 증명 모음에서 인증서에 대한 정책을 생성하거나 업데이트합니다.
...
Key Vault 관리 Azure Storage 계정의 지정된 키를 다시 생성합니다.
