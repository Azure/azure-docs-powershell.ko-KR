...
## 설명
정의 이름이 지정되어 있는 경우 Key Vault 관리 저장소 SAS 정의를 얻습니다. 정의 이름을 지정하지 않으면 자격 증명 모음에 지정된 Key Vault 관리 저장소 계정과 연결된 모든 SAS 정의가 나열됩니다.

## 예제

### 예제 1: 모든 Key Vault 관리 스토리지 SAS 정의 나열
```powershell
...
자격 증명 모음 'myvault'에서 관리되는 Key Vault 관리 저장소 계정 'mystorageaccount'와 연결된 모든 SAS 정의를 나열합니다.

### 예제 2: Key Vault 관리 저장소 계정 사용
```powershell
...
자격 증명 모음 'myvault'에서 관리되는 Key Vault 관리 저장소 계정 'mystorageaccount'와 연결된 SAS 정의 'accountas'의 세부 정보를 얻습니다.

### 예제 3: 필터링을 사용하여 모든 Key Vault 관리 저장소 SAS 정의 나열
```powershell
...
"계정"으로 시작하는 자격 증명 모음 'myvault'에서 관리되는 Key Vault 관리 저장소 계정 'mystorageaccount'와 연결된 모든 SAS 정의를 나열합니다.
...
### -InRemovedState
출력에 이전에 삭제된 저장소 sas 정의를 표시할지 여부를 지정합니다.
...
### -Name
Storage sas 정의 이름입니다.
Cmdlet은 자격 증명 모음 이름, 현재 선택한 환경, 저장소 계정 이름 및 sas 정의 이름에서 저장소 sas 정의의 FQDN을 생성합니다.
