...
## 설명
**Undo-AzKeyVaultRemoval** cmdlet은 이전에 삭제된 키 자격 증명 모음을 복구합니다. 복구된 자격 증명 모음이 복구 후 활성화됩니다.
...
이 명령은 eastus2 지역 및 'MyResourceGroup' 리소스 그룹에서 이전에 삭제된 키 자격 증명 모음 'MyKeyVault'를 활성 상태 및 사용 가능한 상태로 복구합니다. 또한 태그를 새 태그로 바 대체합니다.
...
### -Location
삭제된 자격 증명 모음 원래 Azure 지역을 지정합니다.
