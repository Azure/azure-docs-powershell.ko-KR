...
## 설명
**Undo-AzKeyVaultManagedStorageAccountRemoval** 명령은 이전에 삭제된 관리 저장소 계정을 복구합니다. 이 자격 증명 모음에 대해 소프트 삭제가 활성화되고 복구 간격 동안 복구 시도가 발생합니다.
...
이 명령 순서는 지정된 저장소 계정이 삭제된 상태의 자격 증명 모음에 있는지 여부를 결정합니다. 후속 명령은 삭제된 저장소 계정을 복구하여 활성 상태로 되돌려 넣습니다.
