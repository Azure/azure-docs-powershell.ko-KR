...
ACR을 대상으로 하는 원본 ACR에서 busybox를 가져와야 합니다. 참고: 
* 원본 레지스트리에 대한 관리자 사용자를 사용하지 않도록 설정한 경우 자격 증명이 필요합니다.
...
### -Mode
Force를 사용할 때 기존 대상 태그는 덮어 덮어 덮어 놨습니다.
NoForce가면 복사가 시작되기 전에 기존 대상 태그가 작업을 실패합니다.
...
리포지토리로 이미지를 지정합니다('hello-world').
이 태그는 '최신' 태그를 사용하게 됩니다.
...
### -TargetTag
폼 리포지토의 문자열 \[ 목록 \] :tag.
태그가 생략된 경우 원본이 사용됩니다(또는 원본 태그도 생략된 경우 '최신').
...
매니페스트 전용 복사를 할 리포지토리 이름의 문자열 목록입니다.
태그가 생성되지 않습니다.
