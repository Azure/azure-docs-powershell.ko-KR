...
이 작업 영역이 생성 시 *CustomerId* 매개 변수를 통해 기존 계정에 연결된 경우 원래 계정은 Operational Insights 포털에서 삭제되지 않습니다.

## 예제

### 예제 1: 이름에 따라 작업 영역 제거
```
...
Force *매개* 변수가 지정되기 때문에 작업 영역 제거 전에 명령에 메시지가 표시되지 않습니다.
