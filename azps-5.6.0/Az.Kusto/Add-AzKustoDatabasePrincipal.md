...
### -Value
Kusto 데이터베이스 주체 목록입니다.
생성을 위해 VALUE 속성에 대한 NOTES 섹션을 참조하고 해시 테이블을 만드십시오.
...
VALUE <IDatabasePrincipal[]>: Kusto 데이터베이스 주체 목록입니다.
  - `Name <String>`: 데이터베이스 주체 이름입니다.
  - `Role <DatabasePrincipalRole>`: 데이터베이스 주체 역할입니다.
  - `Type <DatabasePrincipalType>`: 데이터베이스 주체 유형입니다.
  - `[AppId <String>]`: 애플리케이션 ID - 애플리케이션 주체 유형에만 관련됩니다.
  - `[Email <String>]`: 데이터베이스 주체 전자 메일이 있는 경우.
  - `[Fqn <String>]`: 데이터베이스 주체가 완전히 자격을 갖춘 이름입니다.
