...
## SYNOPSIS
Azure 데이터베이스 데이터베이스에 대한 하나 이상의 SQL 합니다.
...
## 설명
**Get-AzSqlDatabaseAdvisor** cmdlet은 Azure SQL 데이터베이스용 하나 이상의 Azure SQL 제공합니다.

## 예제

### 예제 1: 지정된 데이터베이스에 대한 모든 어드바이저 나열
```
...
이 명령은 wi-runner-australia-east라는 서버에 속하는 WIRunner라는 데이터베이스에 대한 모든 어드바이저를 나열합니다.

### 예제 2: 지정된 데이터베이스에 대한 단일 고문을 얻습니다.
```
...
### 예제 3: 응답에 포함된 권장 작업과 함께 모든 고문 나열
```
...
이 명령은 응답에 포함된 권장 작업과 함께 'WIRunner'라는 데이터베이스에 대한 모든 고문을 얻습니다.
명령은 *ExpandRecommendedActions* 매개 변수를 사용하기 때문에 cmdlet은 응답을 사용하여 권장되는 작업을 얻습니다.

### 예제 4: 응답에 포함된 권장 작업으로 단일 고문을 얻습니다.
```
...
### 예제 5: 필터링을 사용하여 지정된 데이터베이스에 대한 모든 어드바이저 나열
```
...
이 명령은 wi-runner-australia-east이라는 서버에 속하는 WIRunner라는 데이터베이스에 대한 모든 어드바이저를 나열하고 "d"로 시작합니다.

## 매개 변수

### -AdvisorName
이 cmdlet이 얻게 하는 어드바이저의 이름을 지정합니다.
...
### -DatabaseName
이 cmdlet에서 Advisor를 요청하는 데이터베이스의 이름을 지정합니다.
...
## 참고 사항
* 키워드: azure, azurerm, arm, 리소스, 관리, 관리자, sql, Database, mssql, Advisor
