...
## 설명
**Get-AzSqlDatabaseTransparentDataEncryption** cmdlet은 Azure 데이터베이스 데이터베이스에 대한 TDE(투명한 데이터 암호화) SQL 합니다.
...
암호화 검사 진행률을 표시하기 위해 Get-AzSqlDatabaseTransparentDataEncryptionActivity cmdlet을 실행합니다.
...
이 명령은 server01이라는 서버에서 Database01이라는 데이터베이스의 TDE 상태를 얻습니다.

## 매개 변수

### -DatabaseName
이 cmdlet에서 TDE 상태를 얻을 데이터베이스의 이름을 지정합니다.
