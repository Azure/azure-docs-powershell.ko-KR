...
## SYNOPSIS
Azure 데이터베이스 권장 작업의 SQL 업데이트합니다.
...
## 설명
**Set-AzSqlDatabaseRecommendedActionState** cmdlet은 Azure 데이터베이스 권장 작업의 SQL 업데이트합니다.
이렇게 하면 새 상태를 기반으로 권장되는 작업을 적용, 되버리거나, 버려야 합니다.

## 예제

### 예제 1: 보류 중인 권장 작업 상태 적용
```
...
이 명령은 wiRunner라는 데이터베이스에 속한 IR_ test_schema \[ \] \[ _test_table_0.0361551 _6C7AE8CC9C87E7FD5893 라는 권장 작업의 상태를 보류 중으로 \] 업데이트합니다.

## 매개 변수

### -AdvisorName
이 권장 작업의 속하는 고문의 이름을 지정합니다.
...
### -DatabaseName
이 cmdlet에서 권장 작업 상태를 설정하는 데이터베이스 이름을 지정합니다.
...
### -State
이 cmdlet이 권장되는 작업 상태를 업데이트하는 새 값을 지정합니다.
...
## 참고 사항
* 키워드: azure, azurerm, arm, 리소스, 관리, 관리자, sql, 데이터베이스, mssql, 고문, 권장 작업
