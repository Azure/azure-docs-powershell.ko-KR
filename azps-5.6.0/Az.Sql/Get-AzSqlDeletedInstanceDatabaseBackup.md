...
## 설명
**Get-AzSqlDeletedInstanceDatabaseBackup** cmdlet은 복원할 수 SQL 인스턴스 데이터베이스 백업 또는 복원할 수 있는 모든 삭제된 백업을 지정한 삭제된 백업을 얻습니다.
이 cmdlet은 Azure의 인스턴스 SQL 스트레치 데이터베이스 서비스에서 지원됩니다.

## 예제

### 예제 1: 서버에서 삭제된 모든 데이터베이스 백업을 얻습니다.
```powershell
...
### -DatabaseName
Azure 인스턴스 데이터베이스의 SQL 백업을 검색합니다.
...
### -DeletionDate
밀리초 정밀도(예: 2016-02-23T00:21:22.847Z)에 대한 백업을 검색하는 Azure 인스턴스 데이터베이스의 SQL 인스턴스 데이터베이스의 지우기 날짜
...
### -InstanceName
데이터베이스가 SQL Managed Instance의 이름입니다.
