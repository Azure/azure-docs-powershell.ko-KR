...
## SYNOPSIS
Azure 데이터베이스 데이터베이스에서 탄력적 데이터베이스 풀의 SQL 수정합니다.
...
## 설명
**Set-AzSqlElasticPool** cmdlet은 Azure SQL 데이터베이스에서 탄력적 풀에 대한 속성을 설정합니다. 이 cmdlet은 풀당 *eDTU(Dtu),* 풀당 저장소 최대 *크기(StorageMB),* 데이터베이스당 최대 eDTU(DatabaseDtuMax), 데이터베이스당 최소 eDTU(DatabaseDtuMin)를 수정할 수 *있습니다.*
...
이 명령은 Elasticpool01이라는 탄력적 풀에 대한 속성을 수정합니다. 이 명령은 탄력적 풀의 DUS 수를 1000으로 설정하고 최소 및 최대 DUS를 설정합니다.
...
이 명령은 Elasticpool01이라는 탄력적 풀에 대한 속성을 수정합니다. 이 명령은 탄력적 풀의 최대 저장소를 2B로 설정합니다.

### 예제 3

Azure 데이터베이스 데이터베이스에서 탄력적 데이터베이스 풀의 SQL 수정합니다. (자동 재생)
...
### -DatabaseDtuMax
풀의 단일 데이터베이스가 사용할 수 있는 최대 DTUS 수를 지정합니다.
...
### -Edition
탄력적 풀에 대한 Azure SQL 데이터베이스의 에디션을 지정합니다. 에디션은 변경할 수 없습니다. 이 매개 변수에 대해 허용되는 값은 다음입니다.
- 없음
- 기본
- 표준
- 프리미엄
...
### -StorageMB
탄력적 풀에 대한 저장소 제한(메가바이트)을 지정합니다. 자세한 내용은 New-AzSqlElasticPool cmdlet을 참조하세요.
...
### -Tags
이 cmdlet이 해시 테이블의 형태로 탄력적 풀과 연결되는 Key-value 쌍의 사전을 지정합니다. 예를 들어: `@{key0="value0";"key 1"=$null;key2="value2"}`
...
### -VCore
Sql Azure Elastic Pool에 대한 총 공유 Vcore 수입니다.
