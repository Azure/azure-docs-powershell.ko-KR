...
### PolicyByResourceServerDatabaseSet(기본값)
```
...
## 설명
**Set-AzSqlDatabaseBackupShortTermRetentionPolicy** cmdlet은 이 데이터베이스에 등록된 단기 보존 정책을 제공합니다.
이 정책은 지점 시간 복원 백업의 보존 기간(일)입니다.
...
이 명령은 데이터베이스 개체의 배관을 통해 Database01에 대한 단기 보존 정책을 35일로 설정합니다.

## 매개 변수

### -AzureSqlDatabaseObject
에 대한 정책을 얻을 데이터베이스 개체입니다.
