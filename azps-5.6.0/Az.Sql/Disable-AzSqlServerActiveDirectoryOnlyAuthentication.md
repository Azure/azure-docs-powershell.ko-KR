...
## SYNOPSIS
특정 사용자에 대한 Azure AD만 인증을 SQL Server.
...
## 설명
**Disable-AzSqlServerActiveDirectoryOnlyAuthentication** cmdlet은 현재 구독에서 AzureSQL Server에 대한 Azure Active Directory(Azure Active Directory) 인증 요구 사항만 사용하지 않도록 설정합니다.
...
이 명령은 ResourceGroup01이라는 리소스 그룹과 연결된 Server01이라는 AzureSQL 서버에 대한 Azure AD(Azure Active Directory) 인증 요구 사항을 사용하지 않도록 설정합니다.
...
### -ServerName
Azure Active Directory SQL Server 인증에 있는 Azure 이름입니다.
