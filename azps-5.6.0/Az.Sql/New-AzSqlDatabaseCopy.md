...
## 설명
**New-AzSqlDatabaseCopy** cmdlet은 현재 SQL 데이터 스냅숏을 사용하는 Azure SQL 데이터베이스의 복사본을 만듭니다. 이 cmdlet을 Start-AzSqlDatabaseCopy cmdlet을 사용하여 일회성 데이터베이스 복사본을 만듭니다. 이 cmdlet은 **복사본의 데이터베이스** 개체를 반환합니다.
...
현재 스냅숏을 SQL 데이터베이스의 복사본을 만듭니다. (자동 재생)
...
### -ServerName
복사할 데이터베이스를 포함하는 SQL Server 이름을 지정합니다.
...
### -Tags
Azure 데이터베이스 복사본과 연결하기 위해 해시 테이블의 형태로 키 값 쌍을 SQL 지정합니다. 예: @{key0="value0";key1=$null;key2="value2"}
