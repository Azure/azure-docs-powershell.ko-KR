...
## 설명
**New-AzSqlInstance** cmdlet은 Azure SQL 데이터베이스 관리 인스턴스를 만듭니다.
...
### 예제 3: 인스턴스 풀 개체를 사용하여 인스턴스 풀에서 새 인스턴스 만들기
```powershell
...
### 예제 4: 인스턴스 풀 리소스 식별자를 사용하여 인스턴스 풀에서 새 인스턴스 만들기
```powershell
...
이 명령은 name instancePool0을 사용하여 인스턴스 풀에 새 인스턴스를 만듭니다.

### 예제 6: 유지 관리 구성을 사용하여 새 인스턴스 만들기
```powershell
...
이 명령은 유지 관리 구성이 있는 새 인스턴스를 MI_2
...
### -AssignIdentity
Azure KeyVault와 같은 주요 관리 서비스와 함께 사용할 이 Managed 인스턴스에 대한 Azure Active Directory ID를 생성하고 할당합니다.
...
### -DnsZonePartner
Managed Instance 만들기에서 DnsZone 속성을 상속하는 파트너 Managed Server의 리소스 ID
