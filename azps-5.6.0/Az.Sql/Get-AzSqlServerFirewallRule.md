...
## 설명
**Get-AzSqlServerFirewallRule** cmdlet은 Azure SQL 데이터베이스 서버에 대한 방화벽 규칙을 제공합니다.
방화벽 규칙의 이름을 지정하는 경우 이 cmdlet은 해당 특정 방화벽 규칙에 대한 정보를 얻습니다.

## 예제

### 예제 1: 서버에 대한 모든 규칙 사용
```
...
### 예제 2: 필터링을 사용하여 서버에 대한 모든 규칙 사용
```
...
이 명령은 "규칙"으로 시작하는 Server01이라는 서버에 대한 모든 방화벽 규칙을 얻습니다.
...
### -ResourceGroupName
리소스가 할당된 리소스 그룹의 SQL Server 지정합니다.
