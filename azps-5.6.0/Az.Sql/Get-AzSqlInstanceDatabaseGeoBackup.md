...
## SYNOPSIS
Managed Instance 데이터베이스 중복 SQL Azure에 대한 정보를 반환합니다.
...
## 설명
**Get-AzSqlInstanceDatabaseGeoBackup** cmdlet은 Azure SQL 데이터베이스 관리 인스턴스에서 하나 이상의 Azure SQL 백업을 제공합니다.
...
이 명령은 managedInstance1이라는 인스턴스의 모든 데이터베이스 중복 백업을 제공합니다.

### 예제 2: 관리되는 인스턴스의 이름으로 데이터베이스 중복 백업을 얻습니다.
```
...
이 명령은 "managedDatabase"로 시작하는 managedInstance1이라는 인스턴스의 모든 데이터베이스 중복 백업을 얻습니다.
