...
## SYNOPSIS
Azure SQL 탄력적 풀에 대한 하나 이상의 SQL 합니다.
...
## 설명
**Get-AzSqlElasticPoolAdvisor** cmdlet은 Azure SQL 탄력적 풀에 대한 하나 이상의 Azure SQL 제공합니다.

## 예제

### 예제 1: 지정된 탄력적 풀에 대한 모든 어드바이저 나열
```
...
명령은 WIRunnerPool이라는 탄력적 풀에 대한 모든 어드바이저를 나열합니다.

### 예제 2: 지정된 탄력적 풀에 대한 단일 고문을 얻다
```
...
이 명령은 응답에 포함된 권장 작업을 사용하여 탄력적 풀에 대한 모든 어드바이저를 얻습니다.
...
### 예제 5: 필터링을 사용하여 지정된 탄력적 풀에 대한 모든 어드바이저 나열
```
...
명령은 "d"로 시작하는 WIRunnerPool이라는 탄력적 풀에 대한 모든 어드바이저를 나열합니다.

## 매개 변수

### -AdvisorName
이 cmdlet이 도착하는 Advisor의 이름을 지정합니다.
...
### -ElasticPoolName
이 cmdlet에서 Advisor를 요청하는 탄력적 풀의 이름을 지정합니다.
...
cmdlet에 응답에서 Advisor의 권장된 작업이 포함되어 있습니다.
...
## 참고 사항
* 키워드: azure, azurerm, arm, 리소스, 관리, 관리자, sql, elasticpool, mssql, 고문
