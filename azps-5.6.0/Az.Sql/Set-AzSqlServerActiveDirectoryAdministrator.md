...
## 설명
**Set-AzSqlServerActiveDirectoryAdministrator** cmdlet은 현재 구독에서 AzureSQL Server에 대한 Azure AD(Azure Active Directory) 관리자를 프로비전합니다.
한 번씩 하나의 관리자만 프로비전할 수 있습니다.
Azure AD의 다음 구성원을 관리자 권한으로 프로비전할 SQL Server 있습니다.
- Azure AD의 네이티브 멤버 
- Azure AD의 페더리드 멤버 
...
이 명령은 Server01이라는 서버에 대한 관리자로 Azure AD 사용자를 프로비전합니다.
...
이 명령은 Server01이라는 서버에 대해 DBAs라는 Azure AD 관리자 그룹을 프로비전합니다.
명령은 ObjectId 매개 변수에 대한 *ID를 지정합니다.*
...
표시 이름이 고유하지 않은 경우 이 매개 변수에 대한 값을 지정해야 합니다.
