...
동기화 에이전트의 이름을 지정하여 해당 동기화 에이전트에 대한 정보를 표시합니다.

## 예제

### 예제 1: Azure SQL 할당된 Azure SQL 동기화 에이전트의 모든 인스턴스를 SQL Server
```
...
이 명령은 Azure SQL 할당된 모든 Azure SQL Server 정보를 SQL Server.
...
이 명령은 "SyncAgent01"을 SQL Azure 데이터베이스 동기화 에이전트에 대한 정보를 얻습니다.

### 예제 3: 필터링을 사용하여 Azure SQL Azure SQL Server 동기화 에이전트의 모든 인스턴스를 얻습니다.
```
