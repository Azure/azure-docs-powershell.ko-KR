...
## 설명
Invoke-AzSqlElasticPoolFailover cmdlet 장애 조치(failover)는 탄력적 풀입니다. 장애 조치(Failover)는 이 cmdlet을 실행한 후 탄력적 풀의 모든 데이터베이스에 발생합니다.
...
이 명령은 "Server01"이라는 서버에서 "ElasticPool01"이라는 탄력적 풀을 장애 조치합니다.  즉, "ElasticPool01"이라는 탄력적 풀의 모든 데이터베이스에 장애 조치(failover)가 발생합니다.
