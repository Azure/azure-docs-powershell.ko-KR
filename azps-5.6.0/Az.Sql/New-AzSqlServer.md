...
### -Location
이 cmdlet에서 서버를 만드는 데이터 센터의 위치를 지정합니다.
...
### -ServerVersion
새 서버의 버전을 지정합니다. 이 매개 변수에 대해 허용되는 값은 2.0 및 12.0입니다.
버전 11 서버를 만들거나 12.0을 만들어 버전 12 서버를 만들 수 있는 2.0을 지정합니다.
...
새 서버에 SQL 데이터베이스 서버 관리자 자격 증명을 지정합니다. **PSCredential** 개체를 얻은 경우 Get-Credential cmdlet을 사용합니다. 자세한 내용은 `Get-Help
