...
## SYNOPSIS
Azure SQL 데이터베이스를 .bacpac 파일로 저장소 계정에 내보낼 수 있습니다.
...
## 설명
**New-AzSqlDatabaseExport** cmdlet은 Azure SQL 데이터베이스를 .bacpac 파일로 저장소 계정에 내보낼 수 있습니다.
이 요청에 대한 상태 정보를 검색하기 위해 내보내기 데이터베이스 상태 요청이 전송될 수 있습니다.
이 cmdlet은 Azure의 SQL Server Stretch Database 서비스에서 지원됩니다.

> [!IMPORTANT]
> 이 cmdlet을 사용하려면 Azure SQL Server "Azure 서비스 및 리소스가 이 서버에 액세스할 수 있도록 허용"으로 구성해야 합니다. 구성되지 않은 경우 GatewayTimeout 오류가 발생합니다.
...
인증 형식이 SQL 설정되어 있는 경우 기본값이 설정됩니다.
이 매개 변수에 대해 허용되는 값은 다음입니다.
- Sql.
SQL 인증.
*AdministratorLogin* 및 *AdministratorLoginPassword를* 관리자 사용자 SQL 암호로 설정합니다. 
- ADPassword.
Azure Active Directory 인증.
*AdministratorLogin* 및 *AdministratorLoginPassword를* Azure AD 관리자 사용자 이름 및 암호로 설정합니다.
...
### -StorageUri
BLOB 링크를 URL로 .bacpac 파일에 지정합니다.
