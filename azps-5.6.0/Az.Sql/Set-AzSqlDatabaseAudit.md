...
## SYNOPSIS
Azure 데이터베이스 데이터베이스의 감사 SQL 변경합니다.
...
## 설명
**Set-AzSqlDatabaseAudit cmdlet은** Azure SQL 변경합니다.
...
### 예제 3: T SQL-SQL 사용하여 필터링을 사용하여 Azure SQL Blob Storage 감사 정책 사용
```powershell
...
### 예제 4: Azure 데이터베이스의 감사 정책에서 필터링 SQL 제거
```powershell
...
### 예제 5: Azure 데이터베이스의 이벤트 허브 감사 정책 사용 SQL
```powershell
...
### 예제 6: Azure 데이터베이스의 이벤트 허브 감사 정책을 사용하지 SQL
```powershell
...
### 예제 8: Azure SQL 로그 분석 감사 정책을 사용하지 않도록 설정
```powershell
...
### 예제 10: Azure SQL 데이터베이스의 감사 레코드를 Blob Storage에 보내지 않도록 설정하고 로그 분석에 보낼 수 있습니다.
```powershell
...
### 예제 11: Azure SQL 데이터베이스의 감사 레코드를 Blob Storage, 이벤트 허브 및 로그 분석에 전송하도록 설정합니다.
```powershell
...
수신  
참조  
감사할 작업을 정의하기 위한 일반적인 양식은 [작업] ON [object] BY [principal] 위 형식의 [개체]는 표, 보기 또는 저장 프로시저 또는 전체 데이터베이스 또는 계획과 같은 개체를 참조할 수 있습니다. 후자의 경우 DATABASE::[dbname] 및 SCHEMA::[schemaname]을 각각 사용합니다.
예를 들어:  
