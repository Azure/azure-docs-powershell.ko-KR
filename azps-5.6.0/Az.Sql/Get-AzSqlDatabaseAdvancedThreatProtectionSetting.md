...
## 설명
**Get-AzSqlDatabaseAdvancedThreatProtectionSetting** cmdlet은 Azure SQL 보호 설정을 제공합니다.
이 cmdlet을 사용하려면 *ResourceGroupName,* *ServerName* 및 *DatabaseName* 매개 변수를 지정하여 이 cmdlet에서 설정을 얻을 데이터베이스를 식별합니다.

## 예제

### 예제 1: 데이터베이스에 대한 고급 위협 보호 설정 사용
```
...
이 명령은 Database01이라는 데이터베이스에 대한 고급 위협 보호 설정을 제공합니다.
데이터베이스는 ResourceGroup11 리소스 그룹에 할당되는 Server01이라는 서버에 있습니다.
