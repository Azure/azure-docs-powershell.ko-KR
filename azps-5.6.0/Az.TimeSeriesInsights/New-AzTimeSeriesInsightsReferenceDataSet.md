...
기본적으로 값은 'Ordinal'으로, 이벤트와 참조 데이터를 조인하거나 새 참조 데이터를 추가하는 동안 대소문자 구분 키 비교가 수행됩니다.
'OrdinalIgnoreCase'가 설정되어 있는 경우 대소문자 무관정 비교가 사용됩니다.
...
### -KeyProperty
참조 데이터 집합의 키 속성 목록입니다.
생성을 위해 KEYPROPERTY 속성에 대한 노트 섹션을 참조하고 해시 테이블을 만드십시오.
...
KEYPROPERTY <IReferenceDataSetKeyProperty[]>: 참조 데이터 집합의 키 속성 목록입니다.
