...
### 예제 1: 지정된 환경 아래에서 eventhub 이벤트 원본 만들기
```powershell
...
### 예제 2: 지정된 환경 아래에서 iothub 이벤트 원본 만들기
```powershell
...
### -ConsumerGroupName
이벤트가 읽을 파티션을 보유하는 event/iot Hub의 소비자 그룹의 이름입니다.
...
Azure Resource Manager의 이벤트 원본의 리소스 ID입니다.
...
### -KeyName
Event/iot Hub에 대한 Time Series Insights 서비스 액세스 권한을 부여하는 SAS 키의 이름입니다.
...
### -SharedAccessKey
Time Series Insights 서비스가 이벤트/iot 허브에 대한 액세스 권한을 부여하는 공유 액세스 키의 값입니다.
