...
이 명령은 타임 시리즈 인사이트 환경을 제공합니다.

### 예제 2: 모든 시간 계열 인사이트 환경 나열
```powershell
...
이 명령은 리소스 그룹의 모든 시간 계열 인사이트 환경을 나열합니다.

### 예제 3: 개체에 따라 타임 시리즈 인사이트 환경 제공
```powershell
...
### -확장
$expand=status를 설정하면 Time Series Insights 서비스의 환경 내부 서비스의 상태가 포함됩니다.
