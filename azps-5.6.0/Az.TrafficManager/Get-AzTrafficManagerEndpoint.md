...
이 개체를 로컬로 수정한 다음, cmdlet을 사용하여 프로필에 Set-AzTrafficManagerEndpoint 수 있습니다.
...
이 명령은 ResourceGroup11이라는 리소스 그룹의 ContosoProfile이라는 프로필에서 contoso라는 Azure 엔드포인트를 얻은 다음 해당 개체를 $TrafficManagerEndpoint 저장합니다.
...
### -Name
이 cmdlet이 얻을 Traffic Manager 엔드포인트의 이름을 지정합니다.
...
### -ResourceGroupName
리소스 그룹의 이름을 지정합니다.
이 cmdlet은 이 매개 변수가 지정하는 그룹에 Traffic Manager 엔드포인트를 제공합니다.
