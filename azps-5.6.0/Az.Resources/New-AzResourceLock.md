...
### -ResourceGroupName
잠금이 적용되거나 잠금이 적용되는 리소스 그룹을 포함하는 리소스 그룹의 이름을 지정합니다.
...
### -Scope
잠금이 적용되는 범위를 지정합니다.
예를 들어 데이터베이스를 지정하려면 구독 ID 리소스 그룹 이름 서버 이름 데이터베이스 이름: 리소스 그룹을 지정하려면 다음 형식을 `/subscriptions/` `/resourceGroups/` `/providers/Microsoft.Sql/servers/` `/databases/` 사용합니다. `/subscriptions/` 구독 ID `/resourceGroups/` 리소스 그룹 이름
