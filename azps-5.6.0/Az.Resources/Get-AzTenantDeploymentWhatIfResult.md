...
## SYNOPSIS
테넌트 What-If 배포에 대한 템플릿 결과를 얻습니다. 
...
## 설명
**Get-AzTenantDeploymentWhatIfResult** cmdlet은 지정된 테넌트 범위에서 ARM What-If 템플릿 배포에 대한 What-If 템플릿을 제공합니다. 실제 리소스를 변경하지 않고 배포가 적용될 경우 업데이트될 리소스를 나타내는 변경 내용 목록을 반환합니다. 반환 결과에 대한 형식을 지정하기 위해 *ResultFormat* 매개 변수를 사용합니다.

## 예제

### 예제 1: What-If 범위에서 결과 얻음
```powershell
...
이 명령은 What-If 템플릿 파일 및 디스크의 매개 변수 파일을 사용하여 테넌트 범위에서 2018년 결과를 얻습니다.
...
### 예제 2: ResourceIdOnly를 What-If 범위에서 What-If 결과 얻음
```powershell
PS C:\> Get-AzTenantDeploymentWhatIfResult `
    -DeploymentName "deploy-01" `
    -Location "West US" `
