...
매개 변수가 없는 경우 이 명령은 구독에서 수행한 모든 거부 할당을 반환합니다.
이 목록은 주체, 거부 할당 이름 및 범위에 대한 필터링 매개 변수를 사용하여 필터링할 수 있습니다.
...
기본값은 선택한 구독입니다.
거부 할당의 범위를 다음 매개 변수 조합 a 중 하나를 사용하여 지정할 수 있습니다.
범위 - /subscriptions/로 시작하는 완전히 자격이 있는 \<subscriptionId\> 범위입니다.
이렇게 하면 해당 특정 범위(예: 해당 범위 이상에서 모든 거부 할당)에서 효과가 있는 거부 할당을 필터링합니다.
b.
ResourceGroupName - 구독 아래 리소스 그룹의 이름입니다.
이렇게 하면 지정된 리소스 그룹(예: 해당 범위 이상에서 모든 거부 할당)에서 효과적으로 할당을 필터링합니다.
c.
ResourceName, ResourceType, ResourceGroupName 및 (선택 사항) ParentResource - 구독에서 특정 리소스를 식별하고 해당 리소스 범위에서 유효하게 거부 할당을 필터링합니다.
구독에서 특정 사용자에 대해 거부되는 액세스 권한을 확인하려면 ExpandPrincipalGroups 스위치를 사용하세요.
...
범위 테스트RG 이상에서 사용자에게 수행된 모든 거부 john.doe@contoso.com 할당을 얻습니다.
...
지정한 경우 사용자와 사용자가 멤버인 그룹에 직접 할당된 거부 할당을 반환합니다(전이적으로).
사용자 주체에만 지원됩니다.
...
지정된 주체에 대해 수행된 모든 거부 할당을 필터합니다.
...
### -ParentResource
ResourceName 매개 변수를 사용하여 지정된 리소스의 계층 구조의 상위 리소스입니다.
...
ResourceName, ResourceType 및 ParentResource 매개 변수와 함께 사용하는 경우 명령은 리소스 그룹 내의 리소스에서 효과적인 거부 할당을 나열합니다.
...
이 명령은 해당 범위에서 효과적인 모든 거부 할당을 필터합니다.
