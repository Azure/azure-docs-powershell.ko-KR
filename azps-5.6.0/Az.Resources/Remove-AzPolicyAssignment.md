...
첫 번째 명령은 ResourceGroup11이라는 리소스 그룹을 구한 다음 해당 개체를 $ResourceGroup 저장합니다.
두 번째 명령은 리소스 그룹 수준에서 정책 할당을 수행한 다음, 정책 할당을 $PolicyAssignment 변수에 저장합니다.
$ResourceGroup **ResourceId** 속성은 리소스 그룹을 식별합니다.
마지막 명령은 해당 리소스의 **ResourceId** 속성이 식별하는 정책 $PolicyAssignment 제거합니다.
...
### -Id
이 cmdlet이 제거한 정책 할당에 대해 완전히 자격을 갖춘 리소스 ID를 지정합니다.
...
### -Name
이 cmdlet이 제거하는 정책 할당의 이름을 지정합니다.
