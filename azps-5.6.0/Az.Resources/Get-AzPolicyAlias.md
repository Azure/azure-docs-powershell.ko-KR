...
## SYNOPSIS
Get-AzPolicyAlias 정의되고 지정한 매개 변수 값과 일치하는 Azure 공급자 리소스 형식을 검색하고 출력합니다. 매개 변수가 제공되지 않는 경우 별칭을 포함하는 모든 공급자 리소스 형식이 출력됩니다.
...
정책 별칭은 Azure Policy에서 리소스 형식 속성을 참조하는 데 사용됩니다.
리소스 유형 또는 별칭의 다양한 속성과 일치하여 목록의 항목을 제한하는 매개 변수가 제공됩니다.
대상 문자열에 대소문자 무차분 비교를 사용하여 포함된 경우 지정한 일치 값이 일치합니다.
...
별칭이 없는 경우를 포함하여 모든 공급자 리소스 유형을 나열합니다.
...
리소스 유형이 'virtual'과 일치하고 별칭을 포함하는 모든 공급자 리소스 유형을 나열합니다.
...
별칭이 없는 경우를 포함하여 리소스 유형이 '가상'에 일치하는 모든 공급자 리소스 유형을 나열합니다.
...
네임스페이스가 'compute' 및 리소스 유형과 일치하는 모든 공급자 리소스 유형을 나열하고 별칭을 포함합니다.
참고: -NamespaceMatch 및 -ResourceTypeMatch는 배타적 일치를 제공하는 반면, 다른 매치는 포괄적입니다.
...
'virtual'에 일치하는 별칭을 포함하는 모든 공급자 리소스 형식을 나열합니다.
...
'virtual'와 일치하는 별칭 또는 '네트워크'와 일치하는 경로가 있는 별칭을 포함하는 모든 공급자 리소스 형식을 나열합니다.
...
알파 api 버전 또는 알파 api 버전이 있는 별칭을 포함하는 모든 공급자 리소스 형식을 나열합니다.
...
### -ApiVersion
설정하면 사용할 리소스 공급자 API의 버전을 나타냅니다. 지정하지 않으면 API 버전이 사용 가능한 최신 버전으로 자동으로 결정됩니다.
...
### -ApiVersionMatch
리소스 유형 또는 별칭에 일치하는 API 버전이 있는 출력 항목에 포함됩니다.
...
### -LocationMatch
리소스 유형에 일치하는 위치가 있는 출력 항목에 포함됩니다.
...
### -ResourceTypeMatch
리소스 유형이 이 값과 일치하는 항목으로 출력을 제한합니다.
...
* 추가 속성은 출력에서 사용할 수 있으며 에 출력을 파이핑하여 표시할 수 `Format-List` 있습니다. 예를 들어: `Get-AzPolicyAlias -NamespaceMatch 'Web' -ResourceTypeMatch site -PathMatch cert | Format-List`
