...
## 설명
**Invoke-AzResourceAction** cmdlet은 지정된 Azure 리소스에 대한 작업을 호출합니다.
지원되는 작업 목록을 얻하려면 Azure Resource Explorer 도구를 사용합니다.

## 예제

### 예제 1: ResourceId를 통해 VM 시작 호출
...
이 명령은 {ResourceId}로 Virtual Machine을 시작합니다.

### 예제 2: ResourceName을 통해 VM을 파워오프 호출
...
이 명령은 {ResourceId}를 사용하여 Virtual Machine을 중지합니다.
명령은 *Force* 매개 변수를 지정하므로 확인을 묻는 메시지가 표시되지 않습니다.

### 예제 3: ResourceId에 리소스 공급자 등록 호출
...
이 명령은 리소스 공급자 "Microsoft.Network"를 등록합니다.
명령은 *Force* 매개 변수를 지정하므로 확인을 묻는 메시지가 표시되지 않습니다.
...
이 cmdlet에서 작업을 호출하는 리소스에 대한 확장 리소스의 이름을 지정합니다.
예를 들어 데이터베이스를 지정하려면 서버 이름 데이터베이스 이름 형식을 `/` 사용합니다.
...
확장 리소스의 유형을 지정합니다.
예를 들어: `Microsoft.Sql/Servers/Databases`
...
### -매개 변수
이 cmdlet이 호출하는 작업의 매개 변수를 해시 테이블로 지정합니다.
...
### -ResourceGroupName
이 cmdlet에서 작업을 호출하는 리소스 그룹의 이름을 지정합니다.
...
### -ResourceId
이 cmdlet에서 작업을 호출하는 리소스의 완전히 자격을 갖춘 리소스 ID를 지정합니다.
ID에는 다음 예제와 같이 `/subscriptions/` 구독이 포함됩니다.`/providers/Microsoft.Sql/servers/ContosoServer/databases/ContosoDatabase`
...
### -ResourceName
이 cmdlet에서 작업을 호출하는 리소스의 이름을 지정합니다.
...
### -ResourceType
리소스 유형을 지정합니다.
예를 들어 데이터베이스의 경우 리소스 유형은 다음과 같습니다. `Microsoft.Sql/Servers/Databases`
