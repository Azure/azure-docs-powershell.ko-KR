...
이름 매개 *변수를* 사용하여 특정 배포를 중지합니다.
이름 매개 변수를 생략하면 **Stop-AzResourceGroupDeployment에서** 실행 중인 배포를 검색하고 중지합니다. 
cmdlet에서 실행 중인 배포를 두 개 이상 찾은 경우 명령이 실패합니다.
...
실행 중인 배포가 두 개 이상 발견된 경우 명령이 실패합니다.
배포 이름을 얻으면 Get-AzResourceGroupDeployment cmdlet을 사용 합니다.
