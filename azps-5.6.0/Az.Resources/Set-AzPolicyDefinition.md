...
명령은 해당 개체를 $PolicyDefinition 저장합니다.
두 번째 명령은 $PolicyDefinition. 
...
이 명령은 Set-AzPolicyDefinition cmdlet을 사용하여 모드 속성을 '모두'로 설정하여 VMPolicyDefinition이라는 정책 정의를 업데이트합니다.
...
이 명령은 VMPolicyDefinition이라는 정책 정의의 메타데이터를 업데이트하여 해당 범주가 "Virtual Machine"이라는 것을 나타냅니다.
...
### -Id
이 cmdlet이 수정하는 정책 정의에 대해 완전히 자격을 갖춘 리소스 ID를 지정합니다.
...
### -매개 변수
정책 정의에 대한 매개 변수 선언입니다. 매개 변수 선언을 포함하는 파일 이름 또는 uri 또는 매개 변수 선언을 문자열로 포함하는 경로일 수 있습니다.
...
.json 파일 또는 JSON(JavaScript Object Notation) 형식의 정책을 포함하는 문자열의 경로를 지정할 수 있습니다.
