...
### 예제 3: 특정 관리 그룹 및 계층 구조의 첫 번째 수준 사용
```
...
플래그를 사용하여 배열을 탐색하고 각 자식에 대한 `Expand` 세부 정보를 얻을 수 `Children` 있습니다. 예를 들어 표시 이름이 있는 그룹에 대한 `Children[0]` 세부 정보를 `TestGroup1DisplayName` 제공합니다.
