...
### 예제 1: 정책 집합 파일을 사용하여 메타데이터를 사용하여 정책 집합 정의 만들기
```
[
   {
...
이 명령은 해당 범주가 "Virtual Machine"임을 나타내는 메타데이터가 있는 VMPolicySetDefinition이라는 정책 집합 정의를 C:\VMPolicy.js. 위의 VMPolicy.js예제 콘텐츠가 제공됩니다.

### 예제 2: 매개 변수화된 정책 집합 정의 만들기
```
[
   {
      "policyDefinitionId": "/providers/Microsoft.Authorization/policyDefinitions/2a0e14a6-b0a6-4fab-991a-187a4f81c498",
...
이 명령은 VMPolicySetDefin C:\VMPolicy.jsition이라는 매개 변수화된 정책 집합 정의를 만듭니다. 위의 VMPolicy.js예제 콘텐츠가 제공됩니다.
...
이 명령은 VMPolicySetDefinition이라는 정책 집합 정의를 만들고, 설정에 지정된 정책 정의의 그룹화가 C:\VMPolicy.js. 위의 VMPolicy.js예제 콘텐츠가 제공됩니다.
...
### -GroupDefinition
새 정책 집합 정의에 대한 정책 정의 그룹입니다. 그룹이 포함된 파일에 대한 경로 또는 JSON 문자열로 그룹을 사용할 수 있습니다.
...
### -메타데이터
정책 집합 정의에 대한 메타데이터입니다. 메타데이터를 포함하는 파일 이름에 대한 경로 또는 JSON 문자열로 메타데이터가 될 수 있습니다.
...
매개 변수 선언을 포함하는 파일 이름의 경로 또는 매개 변수 선언을 JSON 문자열로 설정할 수 있습니다.
