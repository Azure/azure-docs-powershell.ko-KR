...
리소스 그룹 \` \` MyResourceGroupName의 Azure Event Grid 토픽1에 대한 이벤트 구독 \` \` \` EventSubscription1을 \` 제거합니다.
...
리소스 그룹 \` \` \` MyResourceGroupName에 이벤트 구독 EventSubscription1을 \` 제거합니다.
...
Event Hub 네임스페이스에 이벤트 구독 \` EventSubscription1을 \` 제거합니다.
...
Event Grid 토픽에 이벤트 구독 \` EventSubscription1을 \` 제거합니다.
