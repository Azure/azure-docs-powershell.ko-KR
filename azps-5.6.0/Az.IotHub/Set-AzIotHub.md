...
SKU를 S1로 업데이트하고 "myiothub"라는 IotHub의 단위를 5로 업데이트합니다.
...
"myiothub"라는 IotHub의 원격 분석 보존 시간을 4일로 업데이트합니다.

## 매개 변수

### -CloudToDevice
클라우드에서 디바이스 명령 큐에 대한 속성입니다. 
...
파일 업로드 알림 큐의 메시지에 대한 라이브 값에 대한 시간입니다. 
...
### -FileUploadSasUriTtl
파일 업로드를 위해 생성된 SAS Uri에 대한 라이브 시간입니다. 
...
### -RoutingProperties
외부 엔드포인트에 메시지를 라우팅하기 위한 라우팅 속성 
