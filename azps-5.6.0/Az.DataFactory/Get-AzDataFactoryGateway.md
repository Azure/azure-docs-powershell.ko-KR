...
게이트웨이 이름을 지정하는 경우 이 cmdlet은 해당 게이트웨이에 대한 정보를 얻습니다.
이름을 지정하지 않으면 이 cmdlet은 데이터 팩터리에 대한 모든 게이트웨이에 대한 정보를 제공합니다.
데이터 팩터리에 연결된 서비스로 Microsoft SQL Server 프레미스를 추가하려면 게이트웨이를 On-프레미스 컴퓨터에 설치해야 합니다.

## 예제

### 예제 1: 데이터 팩터리에서 모든 논리 게이트웨이를 얻습니다.
```
...
이 명령은 ADF라는 리소스 그룹의 WikiADF라는 데이터 팩터리의 모든 논리 게이트웨이에 대한 정보를 얻습니다.

### 예제 2: 데이터 팩터리에서 특정 논리 게이트웨이를 얻다
```
...
이 cmdlet은 이 매개 변수가 지정하는 데이터 팩터리의 논리 게이트웨이에 대한 정보를 제공합니다.
