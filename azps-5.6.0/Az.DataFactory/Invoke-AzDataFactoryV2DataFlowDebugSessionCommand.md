...
데이터 흐름 디버그 워크플로에 대한 PowerShell 명령 시퀀스는 다음을 수행해야 합니다.
...
### -Command
데이터 흐름 디버그 명령입니다. 선택 사항은PreviewQuery를 실행하고StatisticsQuery를 실행하고ExpressionQuery를 실행합니다.
...
## 참고 사항
키워드: azure, azurerm, arm, 리소스, 관리, 관리자, 데이터, 팩터리Keywords: azure, azurerm, arm, 리소스, 관리, 관리자, 데이터, 팩터리
