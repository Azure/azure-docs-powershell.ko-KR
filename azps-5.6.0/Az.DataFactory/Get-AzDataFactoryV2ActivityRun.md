...
## 설명
**Get-AzDataFactoryV2ActivityRun** cmdlet은 지정된 기간에 발생한 지정된 파이프라인 실행에 대한 Azure Data Factory에서 실행에 대한 정보를 제공합니다. 또한 활동 이름에 대한 필터, 실행을 실행한 연결된 서비스 이름 및 실행 상태를 지정할 수 있습니다.

## 예제

### 예제 1: 파이프라인 실행에 대한 모든 활동 실행을 얻습니다.
```
...
이 명령은 "2017-09-01"과 "2017-09-30"에서 발생한 ID "f288712d-4cb8-96ef-82d3b9b30621"을 사용하여 파이프라인 실행의 모든 활동 실행에 대한 세부 정보를 제공합니다.
...
### -RunStartedAfter
파이프라인 실행이 시작된 시간 또는 그 이후에 실행하기 시작한 시간입니다.
...
### -RunStartedBefore
파이프라인이 실행되기 시작한 시간 또는 그 이전의 시간입니다.
