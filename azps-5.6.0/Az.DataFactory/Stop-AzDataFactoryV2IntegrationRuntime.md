...
## 설명
**Stop-AzDataFactoryV2IntegrationRuntime** cmdlet은 관리되는 전용 통합 런타임이 '시작' 상태로 중지됩니다. 이 런타임은 Start-AzDataFactoryV2IntegrationRuntime cmdlet에서 시작했습니다. 리소스가 릴리스된 후 상태가 '중지됨'으로 전송됩니다.
...
관리되는 통합 런타임 'test-reserlved-ir'은 '시작' 상태입니다. Stop-AzDataFactoryV2IntegrationRuntime cmdlet을 실행한 후 리소스가 릴리스되고 상태가 '중지됨'으로 전송됩니다.
