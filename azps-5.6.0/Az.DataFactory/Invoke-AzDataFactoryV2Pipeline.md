...
## 설명
**Invoke-AzDataFactoryV2Pipeline** 명령은 지정된 파이프라인에서 실행을 시작하고 이 실행에 대한 ID를 반환합니다. 이 GUID는 **Get-AzDataFactoryV2PipelineRun** 또는 **Get-AzDataFactoryV2ActivityRun에** 전달하여 이 실행에 대한 자세한 내용을 얻을 수 있습니다.

## 예제

### 예제 1: 실행을 시작하기 위해 파이프라인을 호출합니다.
```powershell
...
### -IsRecovery
복구 모드 플래그입니다. 복구 모드가 true로 설정되어 있는 경우 지정된 참조 파이프라인 실행 및 새 실행은 동일한 그룹Id 아래에 그룹화됩니다.
...
파이프라인 실행 ID를 다시 실행합니다. 실행 ID가 지정되어 있는 경우 지정된 실행의 매개 변수가 새 실행을 만드는 데 사용됩니다.
...
### -StartActivityName
복구 모드에서 다시 시작은 이 활동에서 시작됩니다. 지정하지 않으면 모든 활동이 실행됩니다.
...
### -StartFromFailure
실패한 활동 플래그에서 다시 시작합니다. 복구 모드에서 지정한 경우 다시 실행이 실패한 작업에서 시작됩니다.
