...
## 설명
**Get-AzDataFactoryV2PipelineRun** 명령은 지정된 파이프라인에 대한 실행에 대한 정보를 반환합니다. PipelineRunId가 지정되어 있는 경우 해당 ID를 통해 실행에 대한 세부 정보를 보여줍니다. PipelineRunId를 지정하지 않은 경우 LastUpdatedAfter 및 LastUpdatedBefore 값 사이에 발생한 파이프라인에 대한 모든 실행에 대한 정보를 보여줍니다.

## 예제

### 예제 1: 파이프라인 실행에 대한 정보 보기
```
...
### -LastUpdatedAfter
파이프라인 실행이 ISO8601 형식으로 업데이트된 시간 또는 그 후의 시간입니다.
...
### -LastUpdatedBefore
파이프라인 실행이 ISO8601 형식으로 업데이트된 시간 또는 이전의 시간입니다.
