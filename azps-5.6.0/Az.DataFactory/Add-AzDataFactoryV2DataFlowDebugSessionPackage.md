...
## 설명
이 명령은 데이터 흐름 리소스 및 해당 종속성은 데이터 흐름 디버그 워크플로에 대한 PowerShell 명령 시퀀스를 특정 디버그 세션에 연결합니다.
...
Pakcage 파일에는 데이터 흐름 디버그 리소스, 데이터 세트 디버그 다시 사용 목록, 연결된 서비스 디버그 리소스 목록, 디버그 설정 및 세션 ID가 포함되어 있습니다. 예를 들어:
...
SessionID 매개 변수는 패키지 파일의 기존 sessionId 속성을 대체하는 데 사용됩니다.
...
### -PassThru
지정한 경우 작업이 성공할 경우 true를 기록합니다. 이 매개 변수는 선택 사항입니다.
