...
## 설명
**Start-AzDataFactoryV2Trigger** cmdlet은 데이터 팩터리에서 트리거를 시작합니다. 트리거가 '중지됨' 상태인 경우 cmdlet이 트리거를 시작하고 결국 정의에 따라 파이프라인을 호출합니다. 트리거가 이미 '시작' 상태인 경우 이 cmdlet은 영향을주지 않습니다. Force 매개 변수를 지정하면 cmdlet이 트리거를 시작하기 전에 프롬프트하지 않습니다.
