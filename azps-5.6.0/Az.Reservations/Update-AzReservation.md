...
지정된의 AppliedScopeType을 Single 및 `Reservation` InstanceFlexibility에서 On으로 업데이트합니다.
...
지정된의 AppliedScopeType을 공유 및 `Reservation` InstanceFlexibility에서 해제로 업데이트합니다.

## 매개 변수

### -AppliedScope
적용될 SubscriptionId `Reservation`
...
### -InstanceFlexibility
있는 경우 의 InstanceFlexibility 값을 `Reservation` 업데이트합니다. 지정하지 않으면 기존 값은 변경되지 않습니다.
