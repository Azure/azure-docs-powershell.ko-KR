...
## 설명
지정된 `Reservation` s를 새 `Reservation` 로 병합합니다. 병합되는 두 `Reservation` s에는 동일한 속성이 있어야 합니다.
...
지정된 두 `Reservation` s를 하나씩 병합합니다. `Reservation`
...
### -예약
병합할 두 ReservationIds의 콤마로 구분된 문자열
...
### -ReservationId
두 의를 포함하는 ReservationOrderId `ReservationOrder` `Reservation`
