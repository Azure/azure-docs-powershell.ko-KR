...
## 설명
지정된 `Reservation` 수량 분포를 통해 a를 `Reservation` 2개로 분할합니다.
...
지정된 수량을 `Reservation` `Reservation` 2개로 분할합니다.
...
### -수량
두 의 수량 필드에 대한 콤마 구분 `Reservation` 정수
...
### -ReservationOrderId
사용자가 `ReservationOrder` 분할하려는 `Reservation` ID를 포함하는 ID
