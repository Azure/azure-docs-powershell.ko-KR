...
## SYNOPSIS
Key Vault에서 웹앱으로 SSL 인증서를 가져온다.
...
## 설명
**Import-AzWebAppKeyVaultCertificate** cmdlet은 Key Vault에서 웹앱으로 SSL 인증서를 가져온다.
...
이 명령은 Key Vault에서 웹앱에 SSL 인증서를 가져온다.
...
이 명령은 리소스 ID를 사용하여 Key Vault에서 웹앱으로 SSL 인증서 가져오기(일반적으로 Key Vault가 다른 구독에 있는 경우).
