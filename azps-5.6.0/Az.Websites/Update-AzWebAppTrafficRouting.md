...
## 설명
**Update-AzWebAppTrafficRouting** cmdlet은 Azure Web App Slot에 대한 라우팅 규칙 구성을 업데이트합니다.

## 예제

### 예제 1 프로덕션 트래픽의 15%를 Stg 슬롯으로 전송하기 위해 라우팅 규칙 업데이트
```powershell
PS C:\>Update-AzWebAppTrafficRouting -ResourceGroupName "Default-Web-WestUS" -WebAppName "ContosoSite" 
- RoutingRule @{AtionHostName='XXXX.azurewebsites.net';ReroutePercentage=15;Name='Stg'}
```

이 명령은 라우팅 규칙을 업데이트하여 프로덕션 트래픽의 15%를 Stg 슬롯으로 전송합니다.

### 예제 2 라우팅 규칙을 업데이트하여 프로덕션 트래픽을 Stg 슬롯 범위의 50%에서 90%까지 증분 방식으로 전송합니다.
```powershell
PS C:\>Update-AzWebAppTrafficRouting -ResourceGroupName "Default-Web-WestUS" -WebAppName "ContosoSite" 
...
이 명령은 라우팅 규칙을 업데이트하여 프로덕션 트래픽을 Stg 슬롯 범위의 50%에서 90%까지 증분 방식으로 전송합니다.
