...
보호 컨테이너 매핑 및 캐시 저장소(VM과 동일한 지역에서)를 사용하여 정의된 대상 Azure 지역에서 지정된 복제 보호 항목에 대한 업데이트 방향 작업을 시작합니다.
...
보호 컨테이너 매핑 및 캐시 저장소(VM과 동일한 지역에서) 및 근접 배치 그룹을 사용하여 정의된 대상 Azure 지역에서 지정된 복제 보호 항목에 대한 업데이트 방향 작업을 시작합니다.
...
### -방향
장애 조치(failover)를 게시하는 업데이트 작업에 사용할 방향을 지정합니다.
...
### -HyperVToAzure
장애 조치(failback) Hyper-V 가상 머신을 다시 처리합니다.
...
장애 조치(failover)시 가상 머신을 만들어야 하는 가용성 집합
...
이 가상 머신에 장애 조치(failover)할 복구 근접 배치 그룹의 리소스 ID입니다.
...
### -VmmToVmm
두 VMM 관리되는 Hyper-V 사이트 간에 보호되는 가상 머신에 대한 Hyper-V 업데이트합니다.
