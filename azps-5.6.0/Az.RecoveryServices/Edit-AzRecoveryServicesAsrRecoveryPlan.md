...
그룹을 기존 Azure Site Recovery 계획에 추가하고 메모리 내 업데이트된 복구 계획을 반환합니다. 

### 예제 2

Site Recovery 계획을 편집합니다. (자동 재생)
...
### -AppendGroup
매개 변수를 전환하여 복구 계획 그룹을 복구 계획 개체에 추가합니다.
...
### -InputObject
편집할 ASR 복구 계획 개체(메모리 작업에서). 복구 계획 실행을 Update-AzASRRecoveryPlan 복구 계획 개체로 업데이트합니다.)
