...
이 cmdlet을 사용하여 ASR 보호 가능한 항목에 대해 복제를 사용하도록 설정합니다.
...
지정된 ASR 보호 가능한 항목에 대한 복제 보호 항목 만들기 작업을 시작하고 작업을 추적하는 데 사용되는 ASR 작업을 반환합니다.
...
지정된 ASR 보호 가능한 항목에 대한 복제 보호 항목 만들기 작업을 시작하고 작업을 추적하는 데 사용되는 ASR 작업(vmWare에서 Azure 시나리오)을 반환합니다.
...
지정된 ASR 보호 가능한 항목에 대한 복제 보호 항목 만들기 작업을 시작하고 작업을 추적하는 데 사용되는 ASR 작업(Azure에서 Azure 시나리오)을 반환합니다.
...
지정된 VmId에 대한 복제 보호된 항목 만들기 작업을 시작하고 작업을 추적하는 데 사용되는 ASR 작업(Azure에서 Azure 시나리오)을 반환합니다.
...
선택적 디스크를 포함하여 지정된 ASR 보호 항목에 대한 복제 보호 항목 만들기 작업을 시작하고 선택한 디스크를 사용하여 작업을 추적하는 데 사용되는 ASR 작업(vmWare에서 Azure 시나리오)을 반환합니다.
...
지정된 VmId에 대한 복제 보호된 항목 만들기 작업을 시작하고 작업을 추적하는 데 사용되는 ASR 작업(Azure에서 Azure 시나리오)을 반환합니다. 암호화를 위해 cmdlet에 전달된 장애 조치(failover) VM 세부 정보가 사용됩니다.
...
근접 배치 그룹 내에서 Virtual Machine에 대한 복제 보호 항목 만들기 작업을 시작하고 작업을 추적하는 데 사용되는 ASR 작업(Azure에서 Azure 시나리오)을 반환합니다.

## 매개 변수

### -Account
필요한 경우 모바일 서비스 설치를 푸시하는 데 사용할 계정으로 실행합니다. ASR 패브릭의 계정으로 실행 목록에서 하나만 있어야 합니다.
...
### -AzureToAzure
스위치 매개 변수는 Azure에서 복제된 항목을 Azure 시나리오로 만드는 것이 지정됩니다.
...
Azure에서 Azure로의 Vm을 사용하는 디스크 구성을 지정합니다.
...
### -AzureVmId
복구 지역에서 재해 복구 보호를 위해 Azure VM ID를 지정합니다.
...
장애 조치(failover) 후 복구 VM으로 사용할 디스크 암호화 비밀 자격 증명 모음 ID(Azure Disk Encryption)를 지정합니다.
...
### -HyperVToAzure
복제된 항목을 지정하는 스위치 매개 변수는 Azure에 Hyper-V 가상 머신입니다.
...
### -Name
ASR 복제 보호 항목에 대한 이름을 지정합니다. 이름은 자격 증명 모음 내에서 고유해야 합니다.
...
### -OS
운영 체제 패밀리를 지정합니다.
이 매개 변수에 대해 허용되는 값은 Windows 또는 Linux입니다.
...
장애 조치(failover)가 발생하면 머신을 복구하는 AvailabilitySet의 ID입니다.
...
장애 조치(failover)가 발생하면 머신을 복구하는 Azure 가상 네트워크의 ID입니다.
...
장애 조치(failover)의 경우 장애 조치(failover)를 통해 가상 머신을 연결해야 하는 복구 Azure 가상 네트워크 내의 서브넷입니다.
...
이 가상 머신에 장애 조치(failover)를 위해 복구 클라우드 서비스의 리소스 ID를 지정합니다.
...
대상 복구 지역의 장애 조치 Vm에서 사용할 근접 배치 그룹 ID를 지정합니다.
...
장애 조치(failover)ARM 가상 머신을 만들 리소스 그룹의 ARM 식별자를 지정합니다.
...
### -RecoveryVmName
장애 조치(failover) 후에 만든 복구 Vm의 이름입니다.
...
다중 VM 일관성 복구 지점을 만드는 데 사용할 복제 그룹 이름을 지정합니다. 기본적으로 각 복제 보호 항목은 자체 그룹에서 생성되고 다중 VM 일관성 복구 지점은 생성되지 않습니다. 모든 컴퓨터를 동일한 복제 그룹으로 보호하여 컴퓨터 그룹 전체에서 다중 VM 일관된 복구 지점을 만들어야 하는 경우만 이 옵션을 사용합니다. 
...
### -ManagedDisk 사용
장애 조치(failover)에서 만든 Azure 가상 머신이 관리 디스크를 사용해야 하는지 지정합니다. True 또는 False를 허용합니다.
...
### -VmmToVmm
복제된 항목을 지정하는 스위치 매개 변수는 VMM 관리되는 Hyper-V 사이트 간에 복제되는 Hyper-V 가상 머신입니다.
...
### -VMwareToAzure
복제된 항목을 지정하는 스위치 매개 변수는 Azure에 복제될 VMware 가상 머신 또는 물리적 서버입니다.
...
### -WaitForCompletion
반환하기 전에 cmdlet이 작업이 완료될 때까지 기다려야 한다고 지정합니다.
