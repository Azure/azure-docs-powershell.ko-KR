...
**Get-AzRecoveryServicesBackupJob** cmdlet은 특정 자격 증명 모음에 대한 Azure Backup 작업을 제공합니다.
...
첫 번째 명령은 진행 중 작업의 상태를 배열로 하여 해당 작업을 $Joblist 변수에 저장합니다.
두 번째 명령은 $Joblist 표시됩니다.

### 예제 2: 지난 7일 동안 실패한 모든 작업 얻습니다.
...
From  매개 변수는 UTC에 지정된 지난 7일의 시간을 지정합니다.
명령은 To 매개 변수에 대한 *값을 지정하지* 않습니다.
...
### 예제 4: 성공적으로 완료된 지난 2일 동안 모든 AzureVM 작업 사용

```powershell
$vault = Get-AzRecoveryServicesVault -ResourceGroupName "resourceGroup" -Name "vaultName"
$Jobs = Get-AzRecoveryServicesBackupJob  -VaultId $vault.ID  -Status Completed -From (Get-Date).AddDays(-2).ToUniversalTime() -BackupManagementType AzureVM
```
첫 번째 cmdlet은 자격 증명 모음 개체를 페치합니다. 두 번째 cmdlet은 지난 2일 동안 완료된 해당 자격 증명 모음에 모든 AzureVM 작업을 $jobs. MAB 에이전트 작업을 인출하기 위해 BackupManagementType 매개 변수 값을 MAB로 변경합니다.

## 매개 변수

### -BackupManagementType

보호되는 리소스의 클래스입니다. 현재 이 cmdlet에 대해 지원되는 값은 AzureVM, AzureStorage, AzureWorkload, MAB입니다.
...
이 cmdlet이 얻을 작업의 시간 범위의 **시작을 DateTime** 개체로 지정합니다.
...
이 cmdlet이 도착하는 작업의 시간 범위의 끝을 **DateTime** 개체로 지정합니다.
기본값은 현재 시스템 시간입니다.
