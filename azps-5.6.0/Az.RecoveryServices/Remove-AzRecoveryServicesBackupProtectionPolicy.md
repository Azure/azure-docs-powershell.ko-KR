...
백업 보호 정책을 삭제하려면 먼저 정책에 연결된 Backup 항목이 있어야 합니다.
정책을 삭제하기 전에 연결된 각 항목이 다른 정책과 연결되어 있는지 확인해야 합니다.
...
첫 번째 명령은 NewPolicy라는 Backup 보호 정책을 구한 다음, 새 변수에 $Pol 저장합니다.
두 번째 명령은 두 번째 명령에서 정책 개체를 $Pol.

### 예제 2

자격 증명 모음에서 백업 보호 정책을 삭제합니다. (자동 재생)
...
### -Policy
제거할 백업 보호 정책을 지정합니다.
**BackupPolicy 개체를 얻은** 경우 Get-AzRecoveryServicesBackupProtectionPolicy cmdlet을 사용합니다.
