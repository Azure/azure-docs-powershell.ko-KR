...
## 설명
**Start-AzRecoveryServicesAsrTestFailoverJob** cmdlet은 Azure Site Recovery 복제 보호 항목 또는 복구 계획의 테스트 장애 조치(failover)를 시작합니다.
...
테스트 장애 조치(failover) 작업을 시작합니다. (자동 재생)
...
새 클라우드 서비스를 만들지 아니면 VM에 대해 구성된 복구 클라우드 서비스를 테스트 장애 조치(failover)에 사용할지 여부를 지정합니다.
...
### -RecoveryPoint
보호된 컴퓨터의 장애 조치(failover)를 테스트하기 위해 사용자 지정 복구 지점을 지정합니다.
...
### -RecoveryTag
장애 조치(failover)를 테스트하기 위해 복구 태그를 지정합니다.
...
### -VMNetwork
테스트 장애 조치(failover) 가상 머신을 연결하기 위해 Site Recovery 가상 머신 네트워크를 지정합니다.
