...
## 설명
이 명령을 사용하면 Azure Backup에서 리소스를 백업 컨테이너로 변환한 다음, 해당 Recovery 서비스 자격 증명 모음에 등록됩니다. 그런 다음 Azure Backup 서비스는 나중에 보호할 이 컨테이너 내에서 주어진 워크로드 유형의 워크로드를 검색할 수 있습니다.
...
보호되는 리소스의 클래스입니다. 현재 이 cmdlet에 대해 지원되는 값은 AzureWorkload입니다.
...
### -Force
강제 등록 컨테이너(확인 대화 상자 방지) 이 매개 변수는 선택 사항입니다.
