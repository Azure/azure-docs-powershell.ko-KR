...
### -AzureToAzure
스위치 매개 변수는 azure 정책 만들기에 대한 시나리오를 지정합니다.
...
### -AzureToVMware
스위치 매개 변수는 azure에서 vMWare 정책 만들기에 대한 시나리오를 지정합니다.
...
### -HyperVToAzure
정책을 지정하기 위해 매개 변수를 전환하여 Azure에 가상 머신을 Hyper-V 사용할 수 있습니다.
...
주어진 시간 동안 복구 지점을 몇 시간 동안 유지합니다.
...
### -VmmToVmm
정책을 지정하기 위한 스위치 매개 변수는 VMM 서버에서 관리되는 Hyper-V 복제하는 데 사용됩니다.
...
### -VMwareToAzure
생성되는 복제 정책이 VMware 가상 머신 및/또는 물리적 서버를 Azure로 복제하는 데 사용될 것을 지정하는 스위치 매개 변수입니다.
