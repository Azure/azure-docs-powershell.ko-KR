...
명령은 파이프라인을 사용하여 해당 노드를 현재 cmdlet에 전달합니다.
...
### -ComputeNode
이 cmdlet이 제거한 계산 노드를 나타내는 **PSComputeNode** 개체를 지정합니다.
...
### -DeallocationOption
이 cmdlet이 시작하는 제거 작업에 대한 대리점 지정 옵션을 지정합니다.
기본값은 다시 큐어입니다.
...
### -PoolId
이 cmdlet이 제거하는 계산 노드가 포함된 풀의 ID를 지정합니다.
