...
첫 번째 명령은 MyCert.pfx라는 파일의 데이터를 $RawData 변수로 읽습니다.
두 번째 명령은 에 저장된 원시 데이터를 사용하여 지정된 Batch 계정에 인증서를 $RawData.
...
### -FilePath
인증서 파일의 경로를 지정합니다.
인증서 파일은 .cer 또는 .pfx 형식으로 있어야 합니다.
...
### -Kind
만들 인증서의 종류입니다. 이 지정되지 않은 경우 암호가 없는 모든 인증서는 CER이고 암호가 있는 모든 인증서는 PFX인 것으로 가정합니다.
...
### -RawData
.cer 또는 .pfx 형식으로 원시 인증서 데이터를 지정합니다.
