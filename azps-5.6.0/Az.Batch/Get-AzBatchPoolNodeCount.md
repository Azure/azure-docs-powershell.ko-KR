...
## 설명
Get-AzBatchPoolNodeCount cmdlet을 사용하면 고객이 풀로 그룹화된 노드 상태당 노드 수를 되돌려 받을 수 있습니다. 가능한 노드 상태는 만들기, 유휴 상태, 오프라인, 선점, 재부팅, 다시 시작, 실행, startTaskFailed, 알 수 없음, 사용할 수 없음 및 대기ForStartTask를 만듭니다. cmdlet은 PoolId 또는 Pool 매개 변수를 사용하여 풀 ID가 지정된 풀만 필터링합니다. 
...
현재 일괄 처리 계정 컨텍스트에서 풀에 대한 노드 상태당 노드 수를 나열합니다.
...
주어진 풀 ID에 대한 노드 상태당 노드 수를 표시합니다.
...
### Microsoft.Azure.Commands.Bat. Models.PSPoolNodeCounts
