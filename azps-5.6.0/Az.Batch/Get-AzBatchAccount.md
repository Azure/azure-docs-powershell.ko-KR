...
## 설명
**Get-AzBatchAccount** cmdlet은 현재 구독에서 Azure Batch 계정을 얻습니다. *AccountName* 매개 변수를 사용하여 단일 계정을 얻거나 *ResourceGroupName* 매개 변수를 사용하여 해당 리소스 그룹에서 계정을 얻을 수 있습니다.

## 예제

### 예제 1: 이름에 따라 일괄 처리 계정 얻기
```
...
이 명령은 pfuller라는 일괄 처리 계정을 얻습니다.

### 예제 2: 리소스 그룹과 연결된 일괄 처리 계정 얻기
```
...
이 명령은 CmdletExampleRG 리소스 그룹과 연결된 일괄 처리 계정을 얻습니다.

## 매개 변수

### -AccountName
계정 이름을 지정합니다.
계정 이름을 지정하는 경우 이 cmdlet은 해당 계정만 반환합니다.
...
### -ResourceGroupName
리소스 그룹의 이름을 지정합니다.
리소스 그룹을 지정하는 경우 이 cmdlet은 지정된 리소스 그룹 아래에서 계정을 얻습니다.
...
### -Tag
해시 테이블의 형태로 키 값 쌍입니다. 예를 들어 @{key0="value0";key1=$null;key2="value2"} 이 cmdlet은 이 매개 변수가 지정하는 태그를 포함하는 계정을 얻습니다.
