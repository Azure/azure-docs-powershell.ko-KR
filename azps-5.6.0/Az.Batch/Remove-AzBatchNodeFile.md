...
## 설명
**Remove-AzBatchNodeFile** cmdlet은 작업 또는 계산 노드에 대한 Azure Batch 노드 파일을 삭제합니다.
...
이 명령은 이름의 노드 파일을 wd\testFile.txt.
이 파일은 Job-000001 아래에 ID Task26이 있는 작업과 연결됩니다.
...
이 명령은 ID pool07이 startup\testFile.txt 풀의 지정된 계산 노드에서 이름의 노드 파일을 삭제합니다.
...
이 명령은 **Get-AzBatchNodeFile** 을 사용하여 노드 파일을 얻습니다.
이 파일은 Job-000001 아래에 ID Task26이 있는 작업과 연결됩니다.
명령은 파이프라인을 사용하여 해당 파일을 현재 cmdlet에 전달합니다.
...
### -ComputeNodeId
이 cmdlet이 삭제하는 Batch 노드 파일이 포함된 계산 노드의 ID를 지정합니다.
...
**PSNodeFile을 얻은** 경우 Get-AzBatchNodeFile cmdlet을 사용 합니다.
...
### -PoolId
이 cmdlet에서 파일을 제거하는 계산 노드가 포함된 풀의 ID를 지정합니다.
...
### -Recursive
이 cmdlet은 지정된 경로 아래에 있는 폴더 및 모든 하위 폴더 및 파일을 삭제합니다.
이 cmdlet은 경로가 폴더인 경우와 관련이 있습니다.
