...
## 설명
**Get-AzBatchTaskCount** cmdlet은 Batch 작업에 대한 Azure Batch 작업 수를 얻습니다.
*JobId* 매개 변수 또는 작업 매개 변수를 사용하여 *작업을 지정합니다.*
작업 수는 활성, 실행 또는 완료된 작업 상태 및 성공 또는 실패한 작업 수를 통해 작업 수를 제공합니다. 준비 상태의 작업은 실행 중으로 계산됩니다. 유효성 검사Status가 평가되지 않은 경우 Batch 서비스는 작업 목록 API에 보고된 작업 상태와의 상태 수를 확인할 수 없습니다. 작업에 200,000개 이상의 작업이 포함된 경우 유효성 검사 통계가 평가되지 않을 수 있습니다.
...
이 명령은 job Job01에 대한 작업 수를 얻습니다.
...
### -Job
이 cmdlet에서 얻을 작업을 포함하는 작업을 지정합니다.
