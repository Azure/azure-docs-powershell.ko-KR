...
## 설명
**Get-AzBatchTask** cmdlet은 Batch 작업에 대한 Azure Batch 작업을 제공합니다.
*JobId* 매개 변수 또는 작업 매개 변수를 사용하여 *작업을 지정합니다.*
단일 작업을 수행하기 위해 Id 매개 *변수를 지정합니다.*
...
이 명령은 job Job01에서 ID Task03을 사용하여 작업을 얻습니다.
Get-AzBatchAccountKey cmdlet을 사용하여 변수 변수에 컨텍스트를 $Context 합니다.

### 예제 2: 지정된 작업에서 완료된 모든 작업 얻기
```
...
이 명령은 ID Job02가 있는 작업에서 완료된 작업을 얻습니다.
...
### -확장
OData 확장 절을 지정합니다.
이 매개 변수에 대한 값을 지정하여 주 엔터티의 관련 엔터티를 얻을 수 있습니다.
...
필터를 지정하지 않으면 이 cmdlet은 Batch 계정 또는 작업에 대한 모든 작업을 반환합니다.
...
### -JobId
이 cmdlet에서 얻을 작업을 포함하는 작업의 ID를 지정합니다.
