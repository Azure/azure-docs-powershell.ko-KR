...
이 cmdlet은 Force 매개 변수를 지정하지 않는 한 작업을 제거하기 전에 확인을 *요청합니다.*
...
이 명령은 ID Job-0000001이 있는 작업을 삭제합니다.
이 명령은 작업을 삭제하기 전에 확인을 묻는 메시지를 표시합니다.
...
이 명령은 Get-AzBatchJob cmdlet을 사용하여 ID Job-000002를 Get-AzBatchJob 합니다.
명령은 파이프라인 연산자를 사용하여 현재 cmdlet에 이 작업을 전달합니다.
명령은 해당 작업을 삭제합니다.
명령에 Force 매개 *변수가* 포함되어 있기 때문에 확인을 묻는 메시지가 표시되지 않습니다.
