...
*PoolID* 또는 Pool 매개 변수를 *지정합니다.*
Id *매개 변수를* 지정하여 단일 계산 노드를 얻습니다.
필터 *매개* 변수를 지정하여 OData(Open Data Protocol) 필터와 일치하는 계산 노드를 얻습니다.
...
이 명령은 ID Pool06이 있는 풀에서 ID tvm-2316545714_1-20150725t213220z가 있는 계산 노드를 얻습니다.
...
이 명령은 필터 매개 변수를 사용하여 유휴 상태를 *지정합니다.*

### 예제 3: 지정된 풀에서 모든 계산 노드를 얻습니다.
```
...
이 명령은 Get-AzBatchPool cmdlet을 사용하여 ID Pool07을 Get-AzBatchPool 합니다.
명령은 파이프라인 연산자를 사용하여 현재 cmdlet에 풀을 전달합니다.
...
**PSCloudPool** 개체를 얻은 경우 Get-AzBatchPool cmdlet을 사용합니다.
