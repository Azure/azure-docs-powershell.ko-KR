...
### 예제 1: 일괄 처리 계정 키를 구하고 나중에 사용할 $Context 변수에 저장합니다.
```
PS C:\>$Context = Get-AzBatchAccountKey -AccountName myaccount
```

이 명령은 계정 세부 정보를 얻게 하여 나중에 사용할 `$Context` 개체에 저장합니다.
...
### -AccountName
이 cmdlet에서 키를 받을 계정의 이름을 지정합니다.
...
### -ResourceGroupName
이 cmdlet에서 키를 얻을 수 있는 계정을 포함하는 리소스 그룹의 이름을 지정합니다.
