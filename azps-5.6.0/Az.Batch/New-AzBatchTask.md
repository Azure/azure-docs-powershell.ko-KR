...
## 설명
**New-AzBatchTask** cmdlet은 *JobId* 매개 변수 또는 작업 매개 변수에 의해 지정된 작업에서 Azure Batch *작업을 만듭니다.*
...
이 명령은 ID 작업-0000001이 있는 작업 아래에 ID Task23이 있는 작업을 만듭니다.
태스크는 지정된 명령을 실행합니다.
...
명령은 이 작업 아래에 ID Task26이 있는 작업을 만듭니다.
태스크는 권한 상승된 권한을 사용하여 지정된 명령을 실행합니다.
...
마지막 명령은 **Get-AzBatchJob** 을 사용하여 ID Job-000001이 있는 Batch 작업을 얻습니다.
그런 다음, 명령은 파이프라인 연산자를 사용하여 해당 작업을 현재 cmdlet에 전달합니다.
명령은 해당 작업의 작업 컬렉션을 추가합니다.
명령은 에 저장된 컨텍스트를 $Context.
...
다음 두 명령은 New-Object cmdlet을 사용하여 **PSCloudTask** 개체를 생성합니다.
명령은 $Task 01 및 $Task 02 변수에 작업을 저장합니다.
최종 명령은 ID Job-000001이 있는 작업 아래에 $Task 01 및 $Task 02에 저장된 작업을 추가합니다.
...
### -AffinityInformation
Batch 서비스가 태스크를 실행할 노드를 선택하는 데 사용하는 지역성 힌트를 지정합니다.
...
이 설정이 설정된 경우 Batch 서비스는 계정 액세스 키 없이 Batch 서비스 작업을 인증하는 데 사용할 수 있는 인증 토큰을 태스크에 제공합니다. 토큰은 AZ_BATCH_AUTHENTICATION_TOKEN 변수를 통해 제공됩니다. 토큰을 사용하여 태스크가 수행될 수 있는 작업은 설정에 따라 달라 니다. 예를 들어 태스크는 작업에 다른 작업을 추가하거나 작업 또는 다른 작업의 상태를 확인하기 위해 작업 권한을 요청할 수 있습니다.
...
### -ContainerSettings
작업이 실행되는 컨테이너에 대한 설정입니다.
이 작업을 실행할 풀에 containerConfiguration 집합이 있는 경우 이 설정도 설정해야 합니다. 이 작업을 실행할 풀에 containerConfiguration 집합이 없는 경우 이 집합을 설정하지 말아야 합니다. 이 지정이 지정되면 모든 AZ_BATCH_NODE_ROOT_DIR(노드의 Azure Batch AZ_BATCH_NODE_ROOT_DIR 루트)는 컨테이너에 매핑되고, 모든 작업 환경 변수가 컨테이너에 매핑되고 작업 명령줄이 컨테이너에서 실행됩니다.
...
### -JobId
이 cmdlet에서 작업을 만드는 작업의 ID를 지정합니다.
...
다중 인스턴스 작업의 경우 파일은 기본 작업이 실행되는 계산 노드에서만 업로드됩니다.
