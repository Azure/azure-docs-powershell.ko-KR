...
## 설명
이 cmdlet은 청구, 구독 관리, 할당량 또는 기술 문제에 대한 지원 티켓을 만드는 데 사용할 수 있습니다. azure Get-AzSupportService 및 Get-AzSupportProblemClassification cmdlet을 사용하여 Azure 서비스를 식별하고 지원을 요청하려는 해당 문제 분류를 각각 식별합니다. 다음 매개 변수를 지정해야 합니다. 
...
도우미 New-AzSupportContactProfileObject cmdlet을 사용하여 CustomerContactDetail 개체를 만들 수 있습니다.

클라우드 솔루션 공급자는 고객의 테넌트에 로그인하고 *CSPHomeTenantId* 매개 변수를 사용하여 홈 테넌트 ID를 지정하여 고객의 구독에 대한 지원 티켓을 만들 수 있습니다.

__기술 티켓의 경우:__

리소스 이름을 지정하기 위해 *TechnicalTicketResourceId* 매개 변수를 ARM 리소스의 리소스 ID를 지정합니다. 아래 예제를 참조하세요. 

__할당량 티켓의 경우:__

**Compute VM Cores,** **Batch, SQL** **데이터베이스** 및 데이터 SQL 할당량 증가를 요청하려면 *QuotaTicketDetail* 개체에서 추가 **세부** 정보를 제공합니다. QuotaTicketDetail 개체는 아래에 설명된 3개의 속성으로 구성됩니다. 자세한 설명서는 여기를 [클릭하세요.](https://aka.ms/supportrpquotarequestpayload)
...
다양한 할당량 유형에 대한 페이로드를 구성하는 방법에 대한 자세한 설명서는 여기를 [클릭하세요.](https://aka.ms/supportrpquotarequestpayload)

## 예제

### 예제 1: 청구 또는 구독 관리 지원 티켓 만들기 지원 Get-AzSupportService Get-AzSupportProblemClassification 청구 또는 구독 관리 문제 분류에 대한 올바른 GUID를 검색하려면 지원 및 지원 
```powershell
...
### 예제 2: Windows용 Virtual Machine 리소스에 대한 기술 지원 티켓 만들기 지원 Get-AzSupportService 및 Get-AzSupportProblemClassification Windows용 Virtual Machine 문제 분류에 대한 올바른 GUID를 검색하려면 지원을 요청합니다. 
```powershell
...
### 예제 3: 특정 VM 패밀리에 대한 Virtual Machine Cores에 대한 할당량 증가를 위한 할당량 지원 티켓을 생성합니다. 할당 Get-AzSupportService 및 Get-AzSupportProblemClassification 할당량 계산 VM Cores 문제 분류에 대한 올바른 GUID를 검색합니다.
```powershell
...
### 예제 4: 할당량 지원 티켓을 만들어 Batch 계정에 대한 우선 순위가 낮은 코어에 대한 할당량 증가 할당량 Get-AzSupportService 및 Get-AzSupportProblemClassification 할당량 배치 문제 분류에 대한 올바른 GUID를 검색합니다.
```powershell
...
### 예제 5: Batch 계정에 대한 특정 VM Family에 대한 VM 코어 할당량 증가를 위한 할당량 지원 티켓을 생성합니다. 할당량 Get-AzSupportService 및 Get-AzSupportProblemClassification 할당량 배치 문제 분류에 대한 올바른 GUID를 검색합니다.
```powershell
...
### 예제 6: Batch 계정에 대한 풀 할당량 증가를 위해 할당량 지원 티켓을 생성합니다. 할당량 Get-AzSupportService 및 Get-AzSupportProblemClassification 할당량 배치 문제 분류에 대한 올바른 GUID를 검색합니다.
```powershell
...
### 예제 7: 할당량 지원 티켓을 만들어 Batch 계정에 대한 활성 작업 및 작업 일정 할당량 증가 할당량 Get-AzSupportService 및 Get-AzSupportProblemClassification 할당량 배치 문제 분류에 대한 올바른 GUID를 검색합니다.
```powershell
...
### 예제 8: 할당량 지원 티켓을 만들어 구독에 대한 Batch 계정 수를 늘입니다. 할당량 Get-AzSupportService 및 Get-AzSupportProblemClassification 할당량 배치 문제 분류에 대한 올바른 GUID를 검색합니다.
```powershell
...
### 예제 9: 할당량 지원 티켓을 만들어 데이터베이스의 DTUS에 대한 할당 SQL 합니다. 데이터베이스 Get-AzSupportService 및 Get-AzSupportProblemClassification 데이터베이스 문제 분류에 대한 올바른 GUID를 SQL 사용합니다.
```powershell
...
### 예제 10: 데이터베이스의 서버 할당량 증가를 위해 할당량 지원 티켓을 SQL 있습니다. 데이터베이스 Get-AzSupportService 및 Get-AzSupportProblemClassification 데이터베이스 문제 분류에 대한 올바른 GUID를 SQL 사용합니다.
```powershell
...
### 예제 11: 데이터 웨어하우스에 대한 DTUS에 대한 할당량 증가를 위해 할당량 지원 티켓을 SQL 있습니다. 할당량 Get-AzSupportService 및 Get-AzSupportProblemClassification 웨어하우스 문제 분류에 대한 올바른 GUID를 검색하려면 SQL 수 있습니다.
```powershell
...
### 예제 12: 데이터 웨어하우스에 대한 서버 할당량 증가를 위해 할당량 지원 티켓을 SQL 있습니다. 데이터 Get-AzSupportService 및 Get-AzSupportProblemClassification 데이터 웨어하우스 문제 분류에 대한 올바른 GUID를 SQL 수 있습니다.
```powershell
PS C:\> New-AzSupportTicket -Name "test1" -Title "Test" -Description "Test" -Severity "minimal" -ProblemClassificationId "/providers/Microsoft.Support/services/{quota_service_guid}/problemClassifications/{sql_datawarehouse_problemClassification_guid}" -QuotaTicketDetail @{QuotaChangeRequestVersion = "1.0" ; QuotaChangeRequestSubType = "Servers" ; QuotaChangeRequests = (@{Region = "westus"; Payload = "{`"NewLimit`":200}"})} -CustomerContactDetail @{FirstName = "first" ; LastName = "last" ; PreferredTimeZone = "pacific standard time" ; PreferredSupportLanguage = "en-us" ; Country = "USA" ; PreferredContactMethod = "Email" ; PrimaryEmailAddress = "user@contoso.com"}
...
### 예제 13: Machine Learning 서비스에 대한 우선 순위가 낮은 코어에 대한 할당량 증가를 위한 할당량 지원 티켓을 생성합니다. 할당 Get-AzSupportService 및 Get-AzSupportProblemClassification 할당량 기계 학습 서비스 문제 분류에 대한 올바른 GUID를 검색합니다.
```powershell
...
### 예제 14: 특정 VM Family for Machine Learning 서비스에 대한 VM 코어 할당량 증가를 위한 할당량 지원 티켓 만들기 할당 Get-AzSupportService 및 Get-AzSupportProblemClassification 할당량 기계 학습 서비스 문제 분류에 대한 올바른 GUID를 검색합니다.
```powershell
...
### 예제 15: Managed Instance에 대한 Azure 할당량 증가를 위해 할당량 지원 티켓을 SQL 있습니다. 할당량 Get-AzSupportService 및 Get-AzSupportProblemClassification Managed Instance 서비스 문제 분류에 대한 올바른 GUID를 SQL 수 있습니다.
```powershell
...
### 예제 17: Azure에서 24 x 7 응답에 대한 요청이 있는 지원 티켓을 생성합니다. 
```powershell
...
### 예제 18: CSP(클라우드 솔루션 공급자)인 경우 고객을 대신하여 지원 티켓을 만드십시오. CSP는 먼저 테넌트에 로그인한 다음 아래 예제와 같이 고객의 테넌트에 로그인해야 합니다. 그런 다음 -CSPHomeTenantId 매개 변수를 사용하여 지원 티켓을 만들 때 홈 테넌트 ID를 지정해야 합니다.  
```powershell
...
추가 전자 메일 주소입니다.
여기에 나열된 전자 메일 주소는 지원 티켓에 대한 모든 통신에 복사됩니다.
...
사용자 지정의 페퍼러된 언어입니다.
이 코드는 여기에 나열된 지원되는 언어 중 하나에 대한 유효한 언어 contry 코드 되어야 https://azure.microsoft.com/en-us/support/faq/ 합니다.
...
고객이 선호하는 표준 시간대입니다.
이 값은 유효한 System.TimeZoneInfo.Id 있어야 합니다.
...
### -심각도
지원 티켓의 심각도입니다.
이는 서비스 수준 계약에 따라 Azure와 함께 하는 기술 지원 계획에 따라 응답 시간을 결정하는 사례의 긴급을 나타냅니다.
