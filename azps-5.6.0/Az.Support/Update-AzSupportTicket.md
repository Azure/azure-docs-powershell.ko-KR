...
### UpdateByNameWithContactDetailParameterSet(기본값)
```
...
## 설명
이 cmdlet을 사용하여 지원 티켓의 심각도 수준, 상태 또는 고객 연락처 세부 정보를 업데이트합니다. 지원 엔지니어에게 티켓을 할당할 때 지원 티켓의 심각도 수준 또는 상태를 업데이트하는 것은 허용되지 않습니다. 티켓 할당 후 심각도 수준 또는 상태를 업데이트할 경우 티켓에 대한 통신을 보내 지원 엔지니어에게 문의하세요.

## 예제

### 예제 1: 지원 티켓의 심각도 업데이트
```powershell
...
### 예제 3: 연락처 개체를 지정하여 지원 티켓의 연락처 세부 정보를 업데이트합니다.
```powershell
...
### 예제 4: 지원 티켓 개체를 파이핑하여 지원 티켓의 심각성을 업데이트합니다.
```powershell
...
### 예제 5: 개별 연락처 매개 변수를 지정하여 지원 티켓의 연락처 세부 정보를 업데이트합니다.
```powershell
...
### 예제 6: 지원 티켓 개체를 파이핑하여 지원 티켓의 상태를 업데이트합니다.
```powershell
...
SupportTicket에서 연락처 세부 정보를 업데이트합니다.
...
고객이 선호하는 지원 언어입니다.
이 코드는 여기에 나열된 지원되는 언어 중 하나에 대한 유효한 언어 contry 코드 되어야 https://azure.microsoft.com/support/faq/ 합니다.
...
### -InputObject
이 cmdlet이 업데이트하는 SupportTicket 리소스 개체입니다.
...
### -Name
이 cmdlet이 업데이트하는 SupportTicket 리소스의 이름입니다.
