...
이 예제에서는 Storage 계정의 모든 사용자 위임 키를 해지하기 때문에 사용자 위임 키에서 생성된 모든 ID SAS 토큰도 해지됩니다.
...
### -InputObject
New-AzStorageAccount의 Get_AzStorageAccount 저장소 계정 개체입니다.
...
### -PassThru
일반적으로 이 cmdlet은 성공적으로 완료할 때 출력을 반환하지 않습니다. 이 매개 변수는 cmdlet이 성공적으로 완료할 때 값($true)을 반환합니다.
