...
### 예제 3: Storage 암호화를 사용할 암호화 범위 업데이트
```
...
이 명령은 저장소 암호화를 사용하기 위해 암호화 범위를 업데이트합니다.

### 예제 4: Keyvault 암호화를 사용할 암호화 범위 업데이트
```
...
이 명령은 Keyvault 암호화를 사용할 암호화 범위를 updtaeses합니다.
...
### -State
업데이트 암호화 범위 상태, 가능한 값에는 '사용 가능', '사용 안 하세요'가 포함됩니다.
...
### -StorageEncryption
keySource를 Microsoft.Storage로 사용하여 암호화 범위를 생성합니다.
