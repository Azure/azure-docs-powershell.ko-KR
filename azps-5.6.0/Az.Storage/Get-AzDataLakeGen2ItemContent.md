...
## 설명
**Get-AzDataLakeGen2ItemContent** cmdlet은 Azure Storage 계정의 Filesystem에서 파일을 다운로드합니다.
...
이 명령은 프롬프트 없이 파일을 로컬 파일에 다운로드합니다.

### 예제 2: 파일, 파이프라인을 통해 파일을 로컬 파일에 다운로드합니다.
```
...
이 명령은 먼저 파일을 얻은 다음 파이프라인을 사용하여 파일을 로컬 파일에 다운로드합니다.
...
### -Force
기존 Blob 또는 파일을 강제로 덮어 덮어 덮어 던지기
...
### -InputObject
다운로드할 Azure Datalake Gen2 항목 개체입니다.
