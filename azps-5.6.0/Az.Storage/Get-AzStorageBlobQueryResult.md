...
## 설명
**Get-AzStorageBlobQueryResult** cmdlet은 Blob의 콘텐츠에 구조적 쿼리 언어(SQL) 명령문을 적용하고 데이터의 검색된 하위 집합을 로컬 파일에 저장합니다.
...
이 명령은 csv로 입력 구성을 사용하여 Blob succsfully를 쿼리하고 출력 구성을 json으로 쿼리하고 출력을 "c:\resultfile.js설정"에 저장합니다.
...
이 명령은 먼저 Blob 스냅숏에 대한 Blob 개체를 만든 다음 Blob 스냅숏을 쿼리하고 결과에 쿼리 오류가 포함됩니다.
...
클라이언트 쪽에서 각 요청에 대한 최대 실행 시간(초)입니다.
...
### -PassThru
지정된 Blob을 성공적으로 퀘이크할지 여부를 반환합니다.
...
### -QueryString
쿼리 문자열은 다음에서 자세한 내용을 참조합니다. https://docs.microsoft.com/azure/storage/blobs/query-acceleration-sql-reference
