...
# Az.Storage 모듈
## 설명
이 항목에서는 Azure Storage Management Cmdlet에 대한 도움말 항목을 제공합니다.

## Az.Storage Cmdlet
### [Add-AzRmStorageContainerLegalHold](Add-AzRmStorageContainerLegalHold.md)
Storage Blob 컨테이너에 법적 보류 태그 추가
...
Azure Storage Blob 서비스에 대한 삭제 보존 정책을 사용하지 않도록 설정합니다.

### [Disable-AzStorageBlobRestorePolicy](Disable-AzStorageBlobRestorePolicy.md)
Storage 계정에서 Blob 복원 정책을 사용하지 않도록 설정합니다.
...
Azure Storage 계정에 대한 정적 웹 사이트를 사용하지 않도록 설정합니다.
...
Azure Storage 계정에 대한 정적 웹 사이트를 사용하도록 설정합니다.
...
파일 시스템에서 파일 또는 디렉터리의 세부 정보를 얻습니다.
...
Azure Storage 서비스에 대한 로깅 속성을 얻습니다.
...
Storage Blob 컨테이너의 ImmutabilityPolicy 잠금

### [Move-AzDataLakeGen2Item](Move-AzDataLakeGen2Item.md)
파일 또는 디렉터리를 동일한 Storage 계정의 다른 파일 또는 디렉터리로 이동합니다.
...
Azure Storage 계정에 대한 저장소 키를 다시 생성합니다.
...
Get-AzStorageBlobQueryResult에서 사용할 수 있는 Blob 쿼리 구성 개체를 만듭니다.
...
Azure 저장소 컨테이너에 대한 저장된 액세스 정책을 만듭니다.
...
Azure Storage 컨테이너에서 저장된 액세스 정책을 제거합니다.

### [Remove-AzStorageCORSRule](Remove-AzStorageCORSRule.md)
Storage 서비스에 대한 CORS를 제거합니다.
...
Azure 저장소 큐에서 저장된 액세스 정책을 제거합니다.
...
Azure Storage 테이블에서 저장된 액세스 정책을 제거합니다.
...
Storage 계정의 모든 사용자 위임 키를 해지합니다.
...
Azure Storage 테이블에 대한 저장된 액세스 정책을 설정합니다.
...
페이지 Blob 스냅숏에서 지정된 대상 페이지 Blob으로 증분 복사 작업을 시작합니다.
...
Azure Storage 파일 서비스에 대한 서비스 속성을 수정합니다.
