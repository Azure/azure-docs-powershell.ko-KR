...
Blob 이름이 로컬 컴퓨터에 유효하지 않은 경우 이 cmdlet은 가능한 경우 자동으로 해결됩니다.
...
이 명령은 파이프라인을 사용하여 Blob 콘텐츠를 찾고 다운로드합니다.
...
이 예제에서는 Asterisk 와일드카드 문자 및 파이프라인을 사용하여 Blob 콘텐츠를 찾고 다운로드합니다.

### 예제 4: Blob 개체를 다운로드하고 변수에 저장한 다음 Blob 개체로 Blob 콘텐츠를 다운로드합니다.
```
PS C:\>$blob = Get-AzStorageBlob -Container containername -Blob blobname 
PS C:\>Get-AzStorageBlobContent -CloudBlob $blob.ICloudBlob -Destination "C:\test"
```

이 예제에서는 먼저 Blob 개체를 구하고 변수에 저장한 다음 Blob 개체로 Blob 콘텐츠를 다운로드합니다. 
...
### -Container
다운로드할 Blob이 있는 컨테이너의 이름을 지정합니다.
...
### -Context
Blob 콘텐츠를 다운로드할 Azure Storage 계정을 지정합니다.
...
## 참고 사항
* 로컬 컴퓨터에 Blob 이름이 올바르지 않은 경우 이 cmdlet은 가능한 경우 자동으로 실행됩니다.
