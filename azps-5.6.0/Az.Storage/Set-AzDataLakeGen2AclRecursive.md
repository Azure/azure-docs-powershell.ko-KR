...
## 설명
**Set-AzDataLakeGen2AclRecursive** cmdlet은 지정된 경로에서 ACL을 재구성적으로 설정합니다. 입력 ACL은 원래 ACL을 완전히 대체합니다.

## 예제

### 예제 1: 디렉터리에서 ACL 재구성 설정
```
...
이 명령은 먼저 ACL 항목이 3개인 ACL 개체를 만든 다음 디렉터리에서 ACL을 다시 설정합니다.

### 예제 2: 파일 구조의 루트 디렉터리에서 ACL 재구성 설정
```
...
이 명령은 먼저 ACL을 루트 디렉터리로 다시 설정하고 실패한 다음 사용자가 실패한 파일을 수정한 후 ContinuationToken으로 다시 시작됩니다.

### 예제 3: ACL 재구성 청크를 청크로 설정
```
$token = $null
...
이 스크립트는 BatchSize * MaxBatchCount로 청크 크기를 사용하여 디렉터리 청크를 청크로 재구성하여 설정합니다. 청크 크기는 이 스크립트에서 200입니다.

### 예제 4: 디렉터리 및 ContinueOnFailure에서 ACL을 다시 설정한 다음 하나씩 실패에서 다시 시작
```
...
먼저 이 명령은 ContinueOnFailure를 사용하여 디렉터리에 ACL을 다시 설정하고 일부 항목이 실패한 다음 실패한 항목을 하나씩 다시 시작합니다.

## 매개 변수

### -Acl
파일 또는 디렉터리에 대해 재구성적으로 설정하는 POSIX 액세스 제어 목록입니다.
...
### -ContinueOnFailure
이 매개 변수를 설정하여 실패를 무시하고 디렉터리의 다른 하위 엔터티에서 작업을 계속 진행합니다. 기본적으로 오류가 발생하면 작업이 빠르게 종료됩니다.
...
### -MaxBatchCount
단일 변경 액세스 제어 작업을 실행할 수 있는 최대 일괄 처리 수입니다. 데이터 집합 크기가 MaxBatchCount를 초과하는 경우 BatchSize를 곱하면 연속 토큰이 반환됩니다.
...
### -Path
Acl을 다시 변경하는 지정된 FileSystem의 경로입니다.
파일 또는 디렉터리일 수 있습니다.
'directory/file.txt' 또는 'directory1/directory2/'의 형식입니다.
이 매개 변수를 건너뛰어 Filesystem의 루트 디렉터리에서 Acl을 다시 변경합니다.
