...
## 설명
**Start-AzStorageBlobCopy** cmdlet은 Blob을 복사하기 시작합니다.
...
이 명령은 **Get-AzStorageBlob** cmdlet을 사용하여 ContosoUploads라는 컨테이너의 Blob을 얻은 다음 파이프라인 연산자를 사용하여 현재 cmdlet에 결과를 전달합니다.
해당 cmdlet은 ContosoArchives라는 컨테이너에 Blob의 복사 작업을 시작합니다.
...
이 명령은 지정된 키를 사용하는 ContosoGeneral이라는 계정에 대한 컨텍스트를 만든 다음 해당 $Context 저장합니다.
두 번째 명령은 ContosoArchive라는 컨테이너에서 ContosoPlanning이라는 Blob에 지정된 URI에서 파일을 복사합니다.
명령은 에 저장된 대상 컨텍스트로 복사 작업을 $Context.
원본 저장소 컨텍스트가 없습니다. 따라서 원본 Uri는 원본 개체에 대한 액세스 권한이 있어야 합니다. 예: 원본이 공용 Azure Blob이 없는 경우 Uri에는 Blob에 대한 읽기 액세스 권한이 있는 SAS 토큰이 포함되어야 합니다.

### 예제 6: 새 Blob 이름으로 대상 컨테이너에 블록 Blob을 복사하고 대상 Blob StandardBlobTier를 핫으로 설정하고 높음으로 다시 하이하이드레이트Priority
```
...
이 명령은 새 Blob 이름으로 대상 컨테이너에 대한 블록 Blob의 복사 작업을 시작하고 대상 Blob StandardBlobTier를 Hot, RehydratePriority로 높음으로 설정합니다.
...
### -Force
이 cmdlet이 확인을 요청하지 않고 대상 Blob을 덮어 묻는 메시지를 나타냅니다.
...
### -RehydratePriority
Blob RehydratePriority를 차단합니다. 보관된 Blob을 다시하이드레이션할 우선 순위를 나타냅니다. 유효한 값은 높음/표준입니다.
...
### -SrcFile
Azure Storage 클라이언트 라이브러리에서 **CloudFile** 개체를 지정합니다.
cmdlet을 만들거나 Get-AzStorageFile 수 있습니다.
...
cmdlet을 만들거나 Get-AzStorageShare 수 있습니다.
