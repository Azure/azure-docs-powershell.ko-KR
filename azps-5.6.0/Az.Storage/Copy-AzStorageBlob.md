...
## 설명
**Copy-AzStorageBlob** cmdlet은 Blob을 동기적으로 복사하고 현재 블록 Blob만 지원합니다.

## 예제

### 예제 1: 명명된 Blob을 다른 Blob에 복사합니다.
```
...
이 명령은 새 Blob 이름으로 Blob을 원본 컨테이너에서 대상 컨테이너로 복사합니다. 
...
이 명령은 블록 Blob 암호화 범위를 새 암호화 범위로 자체로 복사하여 업데이트합니다.
...
### -EncryptionScope
dest Blob에 요청을 할 때 사용할 암호화 범위입니다.
...
### -RehydratePriority
Blob RehydratePriority를 차단합니다.
보관된 Blob을 다시하이드레이션할 우선 순위를 나타냅니다.
유효한 값은 높음/표준입니다.
...
자세한 정보 보기 https://docs.microsoft.com/en-us/azure/storage/blobs/storage-blob-storage-tiers
