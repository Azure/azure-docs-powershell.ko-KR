...
이 명령은 Asterisk(*) 와일드카드 문자 및 파이프라인을 사용하여 Blob 또는 Blob을 검색한 다음 제거합니다.
...
### -DeleteSnapshot
모든 스냅숏이 삭제되지만 기본 Blob은 지정하지 않습니다.
이 매개 변수를 지정하지 않으면 기본 Blob 및 해당 스냅숏이 함께 삭제됩니다.
삭제 작업을 확인하라는 메시지가 사용자에게 표시됩니다.
...
### -Force
이 cmdlet은 확인 없이 Blob 및 해당 스냅숏을 제거합니다.
...
읽을 cmdlet에 대한 Azure 프로필을 지정합니다.
지정하지 않으면 cmdlet은 기본 프로필에서 읽습니다.
