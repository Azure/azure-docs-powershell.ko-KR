...
## 설명
**Get-AzStorageFile** cmdlet에는 지정한 공유 또는 디렉터리에 대한 디렉터리 및 파일이 나열됩니다.
...
이 명령은 ContosoShare06 공유의렉터리만 나열합니다.
먼저 파일과 디렉터를 모두 검색하고  파이프라인 연산자를 사용하여 연산자에 전달한 다음, 형식이 "AzureStorageFileDirectory"가 아닌 개체를 삭제합니다.
...
이 명령은 ContosoShare06 공유 아래에 있는 디렉터리 ContosoWorkingFolder의 파일 및 폴더를 나열합니다.
먼저 디렉터리 인스턴스를 얻은 다음 **Get-AzStorageFile** cmdlet에 파이프라인을 추가하여 디렉터리를 나열합니다.

## 매개 변수

### -ClientTimeoutPerRequest
하나의 서비스 요청에 대해 클라이언트 쪽 시간 시간 간격(초)을 지정합니다.
이전 호출이 지정된 간격 내에 실패하면 이 cmdlet에서 요청을 다시 검색합니다.
...
Storage 컨텍스트를 구하기 위해 New-AzStorageContext cmdlet을 사용 합니다.
...
### -Path
폴더의 경로를 지정합니다.
경로 매개 변수를 생략하면 **Get-AzStorageFile은** 지정된 파일 공유 또는 디렉터리에 디렉터리 및 파일을 나열합니다. 
경로 매개 *변수를 포함하면* **Get-AzStorageFile은** 지정된 경로에 디렉터리 또는 파일의 인스턴스를 반환합니다.
...
요청에 대해 서비스 쪽 시간 제한 간격(초)을 지정합니다.
서비스가 요청을 처리하기 전에 지정된 간격이 경과하면 Storage 서비스가 오류를 반환합니다.
