...
## 설명
**Set-AzStorageCORSRule** cmdlet은 Azure Storage 서비스의 유형에 대한 CORS(원본 간 리소스 공유) 규칙을 설정합니다.
이 cmdlet에 대한 저장소 서비스의 유형은 Blob, Table, Queue 및 File입니다.
이 cmdlet은 기존 규칙을 덮어 덮어 덮습니다.
...
이 명령은 이 코드 블록의 여러 줄에 표준 확장을 사용 합니다.
두 번째 명령은 Blob 서비스 $CorsRules 규칙에 할당합니다.

### 예제 2: Blob 서비스에 대한 CORS 규칙의 속성 변경
```
...
명령은 배열 변수에 $CorsRules 저장합니다.
두 번째 및 세 번째 명령은 $CorsRules.
마지막 명령은 Blob 서비스 $CorsRules 규칙에 할당합니다.
수정된 규칙은 현재 CORS 규칙을 덮어 덮어 덮습니다.
