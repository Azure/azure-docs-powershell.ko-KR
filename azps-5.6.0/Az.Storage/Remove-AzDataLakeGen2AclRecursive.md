...
## 설명
**Remove-AzDataLakeGen2AclRecursive** cmdlet은 지정된 경로에서 ACL을 재구성적으로 제거합니다. 입력 ACL 항목과 동일한 AccessControlType, DefaultScope 및 EntityId가 있는 원래 ACL의 ACL 항목(다른 권한으로도) wil lbe가 제거되었습니다.

## 예제

### 예제 1: 파일계의 루트 지시문에서 ACL 재구성 제거
```
...
이 명령은 먼저 acl 항목이 2개인 ACL 개체를 만든 다음 파일 시스템의 루트 디렉터리에서 ACL을 다시 제거합니다.

### 예제 2: 디렉터리에서 ACL 재구성 제거
```
...
이 명령은 먼저 디렉터리에서 ACL을 다시 제거하고 실패한 다음 사용자가 실패한 파일을 수정한 후 ContinuationToken으로 다시 시작됩니다.

### 예제 3: 청크로 ACL 재구성 청크 제거
```
$token = $null
...
이 스크립트는 BatchSize * MaxBatchCount로 청크 크기가 있는 디렉터리 청크의 청크를 청크로 재구성하여 제거합니다. 청크 크기는 이 스크립트에서 50000입니다.

### 예제 4: 디렉터리 및 ContinueOnFailure에서 ACL을 다시 제거한 다음 하나씩 실패에서 다시 시작
```
