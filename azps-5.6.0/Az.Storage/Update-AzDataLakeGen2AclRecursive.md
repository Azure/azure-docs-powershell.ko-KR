...
## 설명
**Update-AzDataLakeGen2AclRecursive** cmdlet은 지정된 경로에서 ACL을 재구성적으로 업데이트합니다. 입력 ACL은 원래 ACL을 병합합니다. 동일한 AccessControlType/EntityId/DefaultScope가 있는 ACL 항목이 있는 경우 권한 업데이트; 다른 새 ACL 항목을 추가합니다.

## 예제

### 예제 1: 파일계의 루트 지시문에서 ACL 재구성 업데이트
```
...
이 명령은 먼저 3개 acl 항목이 있는 ACL 개체를 만든 다음 파일 시스템의 루트 디렉터리에서 ACL을 다시 업데이트합니다.

### 예제 2: 디렉터리에서 ACL을 다시 업데이트하고 ContinuationToken을 통해 실패에서 다시 시작
```
...
이 명령은 먼저 ACL을 디렉터리에 다시 업데이트하고 실패한 다음 사용자가 실패한 파일을 수정한 후 ContinuationToken으로 다시 시작됩니다.

### 예제 3: 청크로 ACL 재구성 청크 업데이트
```
...
이 스크립트는 BatchSize * MaxBatchCount로 청크 크기로 디렉터리 청크에 대한 ACL을 청크로 다시 업데이트합니다. 청크 크기는 이 스크립트에서 5000입니다.

### 예제 4: 디렉터리 및 ContinueOnFailure에서 ACL을 다시 업데이트한 다음 하나씩 실패에서 다시 시작
```
...
이 명령은 먼저 ContinueOnFailure를 사용하여 디렉터리에 ACL을 다시 업데이트하고 일부 항목이 실패한 다음 실패한 항목을 하나씩 다시 시작합니다.
