...
## 설명
**Update-AzStorageBlobServiceProperty** cmdlet은 Azure Storage Blob 서비스에 대한 서비스 속성을 수정합니다.
...
### 예제 2: Storage 계정의 Blob 서비스에서 Changefeed 사용
```
...
이 명령은 Azure Blob Storage의 Storage 계정 변경 피드 지원의 Blob 서비스에서 Blob 수준 만들기, 수정 또는 삭제 이벤트에 대해 GPv2 또는 Blob Storage 계정을 수신하여 작동합니다. 그런 다음 저장소 계정 내의 컨테이너에 저장된 Blob에 대한 $blobchangefeed 로그를 출력합니다. 직렬화된 변경 내용은 Apache Avro 파일로 유지되고 비동기 및 증분으로 처리될 수 있습니다.

### 예제 3: Storage 계정의 Blob 서비스에서 버전 관리 사용
```
...
이 명령은 Storage 계정의 Blob 서비스에서 버전 관리가 가능하게 합니다.
...
### -EnableChangeFeed
저장소 계정에 대해 변경 피드 로깅을 사용하도록 설정하고 $true 로깅을 설정하여 변경 피드 로깅을 사용하지 않도록 $false.
