...
## 설명
**Get-AzStorageFileHandle cmdlet에는** 파일 공유 또는 파일 디렉터리 또는 파일의 파일 핸들이 나열됩니다.

## 예제

### 예제 1: 파일 공유에 대한 모든 파일 핸들을 다시 나열하고 ClientIp 및 OpenTime별로 정렬
```
...
이 명령은 파일 공유에 대한 파일 핸들을 나열하고 ClientIp로 출력을 정렬한 다음 OpenTime으로 정렬합니다.

### 예제 2: 파일 디렉터리에 처음 2개 파일 핸들을 다시 나열합니다.
```
...
이 명령은 파일 디렉터리에 처음 2개 파일 핸들을 재구성적으로 나열합니다.

### 예제 3: 파일에서 3~6번째 파일 핸들 나열
```
...
### -IncludeTotalCount
데이터 집합의 개체 수(정수)와 개체 수를 보고합니다. cmdlet에서 총 수를 확인할 수 없는 경우 '알 수 없음 총 수'가 반환됩니다.
