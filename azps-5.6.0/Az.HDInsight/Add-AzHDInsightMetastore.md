...
메타스토어는 Hive SQL Oozie 또는 둘 다에 대한 메타데이터를 저장하는 데 사용할 수 있는 데이터베이스입니다.

## 예제

### 예제 1: SQL 구성 개체에 데이터베이스 metastore 추가
```
...
이 명령은 SQL 데이터베이스 metastore를 your-hadoop-001이라는 클러스터에 추가합니다.
...
### -자격 증명
AzureSQL Server 데이터베이스에 사용할 자격 증명을 지정합니다.
...
### -DatabaseName
이 metastore에 사용할 AzureSQL Server 인스턴스의 데이터베이스를 지정합니다.
...
### -SqlAzureServerName
이 metastore에 사용할 AzureSQL Server 인스턴스를 지정합니다.
