...
## 설명
이 New-AzHDInsightCluster 지정된 매개 변수를 사용하거나 cmdlet을 사용하여 만든 구성 개체를 사용하여 Azure HDInsight 클러스터를 New-AzHDInsightClusterConfig 만듭니다.
...
### 예제 6: 자동 조정을 가능하게 하는 Azure HDInsight 클러스터를 생성합니다.
```
PS C:\&gt; # Primary storage account info
        $storageAccountResourceGroupName = "Group"
        $storageAccountResourceId = "yourstorageaccountresourceid"
...
### 예제 10: 컴퓨팅을 고리로 사용할 수 있는 Azure HDInsight 클러스터를 생성합니다.
```
PS C:\&gt; # Primary storage account info
        $storageAccountResourceGroupName = "Group"
        $storageAccountResourceId = "yourstorageaccountresourceid"
...
또는 cmdlet을 Add-AzHDInsightStorage 수 있습니다.
...
### -ApplicationId
Azure Data Lake에 액세스하기 위해 서비스 주체 애플리케이션 ID를 얻거나 설정합니다.
...
### -CertificateFilePath
서비스 주체로 인증하는 데 사용할 인증서에 대한 파일 경로를 지정합니다.
클러스터는 Azure Data Lake Store에 액세스할 때 이 기능을 사용됩니다.
...
### -CertificatePassword
서비스 주체로 인증하는 데 사용할 인증서에 대한 암호를 지정합니다.
클러스터는 Azure Data Lake Store에 액세스할 때 이 기능을 사용됩니다.
...
기본적으로 표준입니다.
Premium 계층은 Linux 클러스터에서만 사용할 수 있으며, 일부 새 기능을 사용할 수 있습니다.
...
또는 cmdlet을 Add-AzHDInsightConfigValues 수 있습니다.
...
### -EdgeNodeSize
에지 노드에 대한 가상 머신의 크기를 지정합니다. 허용 Get-AzVMSize VM 크기에 대한 사용 및 HDInsight의 가격 책정 페이지를 참조하세요. 이 매개 변수는 RServer 클러스터에만 유효합니다.
...
### -EnableIDBroker
HDInsight Identity Broker 기능을 사용 가능하게 합니다.
...
### -EncryptionAtHost
호스트에서 암호화를 사용하도록 설정하는지 여부를 나타내는 플래그를 얻거나 설정합니다.
...
### -EncryptionInTransit
전송 중 암호화를 사용하도록 설정하는지 여부를 나타내는 플래그를 얻거나 설정합니다.
...
허용 Get-AzVMSize VM 크기에 대한 사용 및 HDInsight의 가격 책정 페이지를 참조하세요.
...
또는 cmdlet을 Add-AzHDInsightMetastore 수 있습니다.
...
### -ScriptActions
클러스터 생성이 끝날 때 클러스터에서 실행할 스크립트 작업을 지정합니다.
...
또는 cmdlet을 Add-AzHDInsightSecurityProfile 수 있습니다.
...
Storage 계정에 대한 Storage 리소스 ID를 얻거나 설정합니다.
...
### -StorageContainer
기본 Azure Storage 계정에 대한 StorageContainer 이름을 얻거나 설정합니다.
...
### -StorageRootPath
기본 Data Lake Store 계정에서 클러스터의 루트로 경로를 얻거나 설정합니다.
...
### -ZookeeperNodeSize
Zookeeper 노드에 대한 가상 머신 크기를 지정합니다.
허용 Get-AzVMSize VM 크기에 대한 사용 및 HDInsight의 가격 책정 페이지를 참조하세요.
