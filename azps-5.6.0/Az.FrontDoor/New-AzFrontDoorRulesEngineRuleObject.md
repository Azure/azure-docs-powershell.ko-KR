...
cmdlet "New-AzFrontDoorRulesEngineActionObject"를 사용하여 PSRulesEngineAction 개체를 만들어 "-Action" 매개 변수에 전달합니다.
...
새 PSRulesEngineRule 개체를 만들고 하위 필드를 보는 방법을 보여 옵니다.
...
### -Action
모든 일치 조건이 충족된 경우 요청 및 응답에서 수행할 작업입니다.
...
이 규칙이 일치하면 규칙 엔진이 나머지 규칙을 계속 실행하거나 중지해야 합니다.
가능한 값은 계속 및 중지입니다.
없는 경우 기본값은 계속으로 설정됩니다.
