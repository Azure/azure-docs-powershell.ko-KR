...
## 설명
**Set-AzFrontDoor** cmdlet은 Front Door 부하 밸런스를 업데이트합니다. 입력 매개 변수가 제공되지 않은 경우 기존 Front Door의 이전 매개 변수가 사용됩니다.
...
### 예제 2: PSFrontDoor 개체로 기존 Front Door를 업데이트합니다.
```powershell
...
업데이트할 Front Door는 FrontoorName 및 ResourceGroupName, PSFrontDoor 개체 또는 ResourceId를 사용하여 식별할 수 있습니다. (예를 들어 위의 3개 예제를 참조) 아래 예제에서는 PSFrontDoor 개체를 사용합니다.
...
모든 백end 풀에 대한 HTTPS 요청에서 인증서 이름 확인을 사용하지 않도록 설정할지 여부입니다. HTTPS가 아닌 요청에는 영향을주지 않습니다.
...
이 Front Door 인스턴스와 연결된 부하 분산 설정.
