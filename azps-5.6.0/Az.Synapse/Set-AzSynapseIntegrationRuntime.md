...
cmdlet은 공유 통합 런타임에 사용할 작업 영역이 추가됩니다. 매개 `-SharedIntegrationRuntimeResourceId` 변수를 사용하는 경우 `-Type` 포함해야 합니다. cmdlet을 실행하기 전에 통합 런타임을 사용할 수 있는 권한을 작업 영역에게 부여해야 합니다.
...
데이터 이동 시 사용할 스테이징 데이터 저장소를 참조하는 Azure Blob Storage 연결된 서비스 Self-Hosted Azure-SSIS 통합 런타임 간에 데이터를 이동합니다.
...
### -ExpressCustomSetup
사용자 지정 설정 스크립트 없이 구성 및 제3자 구성 요소를 설정하는 데 사용할 수 있는 SSIS 통합 런타임에 대한 Express 사용자 지정 설정입니다.
...
라이선스인클로드 또는 BasePrice에는 두 가지 유형이 있습니다.
AHUB(Azure Hybrid Use Benefit) 가격 책정 자격이 있는 경우 BasePrice를 선택하세요.
