...
## SYNOPSIS
Synapse Analytics Spark 작업이 완료될 때까지 기다렸다.
...
## 설명
**Wait-AzSynapseSparkJob** cmdlet은 Azure Synapse Analytics 작업이 완료될 때까지 기다렸다.
...
이 명령은 지정된 ID가 있는 작업이 완료될 때까지 기다렸다.
...
### -SparkJobObject
일반적으로 파이프라인을 통해 전달되는 Spark 작업 입력 개체입니다.
...
### -TimeoutInSeconds
오류가 발생하기 전에 대기하는 최대 시간입니다. 기본값은 시간 제한을 절대로 설정하지 않는 것입니다.
