...
## 설명
**Restore-AzSynapseSqlPool** cmdlet은 모든 SQL 백업 또는 복원 지점에서 Azure Synapse Analytics SQL 복원합니다.
...
이 명령은 리소스 ID를 SQL 구독의 모든 SQL 백업에서 복원하여 Azure Synapse Analytics SQL 풀을 만듭니다.
...
이 명령은 리소스 ID를 SQL 이전 상태의 복구 또는 복사를 위해 이 구독의 모든 SQL 풀에서 복원 지점을 활용하여 Azure Synapse Analytics SQL 풀을 만듭니다.
...
### -FromRestorePoint
이 구독의 모든 SQL 풀에서 복원 지점을 활용하여 이전 상태의 복구 또는 복사를 나타냅니다.
