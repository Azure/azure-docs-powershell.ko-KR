...
## 설명
**Set-AzSynapsePipeline cmdlet은** 작업 영역의 파이프라인을 만듭니다.
...
이 명령은 ContosoWorkspace라는 작업 영역에서 ContosoPipeline이라는 파이프라인을 만듭니다.
명령은 파일에서 파이프라인을 pipeline.js기반입니다.
이 파일에는 활동에 대한 정보가 포함되어 있습니다.
...
이 명령은 파이프라인을 통해 ContosoWorkspace라는 작업 영역의 ContosoPipeline이라는 파이프라인을 만듭니다.
명령은 파일에서 파이프라인을 pipeline.js기반입니다.
이 파일에는 활동에 대한 정보가 포함되어 있습니다.
