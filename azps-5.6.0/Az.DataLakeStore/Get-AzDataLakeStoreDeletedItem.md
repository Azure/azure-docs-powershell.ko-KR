...
OriginalPath, TrashDirPath, Type, CreationTime 등 삭제된 항목의 특성이 표시됩니다.
휴지통의 수백만 파일을 검색해야 할 수 있으며 작업으로 실행할 수 있는 장기 실행 작업이 될 수 있습니다.
주의: 파일 삭제를 선택하지 않는 것이 가장 좋은 작업입니다. 파일을 삭제한 후 복원할 수 있는 보장은 없습니다. 이 API는 화이트리스트를 통해 사용할 수 있습니다. ADL 계정이 화이트리스트에 없는 경우 이 API를 사용하여 구현되지 않은 예외를 throw합니다. 자세한 정보 및 지원은 Microsoft 지원에 문의하세요.

## 예제

### 예: Data Lake Store에서 파일의 세부 정보 확인
```
...
### -Count
사용자가 찾을 결과의 수를 지정합니다. 쿼리는 Count 결과를 찾거나 전체 휴지통을 검색할 때까지 실행됩니다.
