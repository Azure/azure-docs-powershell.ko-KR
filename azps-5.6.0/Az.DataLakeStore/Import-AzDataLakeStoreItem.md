...
이 명령은 파일을 SrcFile.csv 4의 동시성으로 데이터 레이크 저장소의 MyFiles 폴더에 File.csv 추가합니다.
...
### -DiagnosticLogLevel
선택적으로 파일 또는 폴더 가져오기 중에 이벤트를 기록하는 데 사용할 진단 로그 수준을 나타냅니다. 기본값은 오류입니다.
...
### -DiagnosticLogPath
파일 또는 폴더 가져오기 중에 이벤트를 기록하는 진단 로그의 경로를 지정합니다.
...
### -ForceBinary
복사되는 파일이 추가 전반에 걸쳐 새 줄 보존에 대한 염려가 없는 복사해야 한다고 나타냅니다.
...
### -Recurse
이 작업은 모든 하위폴더에 모든 항목을 업로드해야 한다고 나타냅니다.
...
### -다시 시작
복사되는 파일이 이전 업로드의 연속성을 나타냅니다. 이렇게 하면 시스템이 완전히 업로드되지 않은 마지막 파일에서 다시 시작하려고 합니다.
