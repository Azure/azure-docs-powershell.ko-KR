...
## 설명
**Set-AzDataLakeStoreAccount** cmdlet은 Data Lake Store 계정을 수정합니다.
...
이 명령은 ContosoADL이라는 Data Lake Store 계정에 지정된 태그를 추가합니다.

### 예제 2

Data Lake Store 계정을 수정합니다. (자동 재생)
...
### -AllowAzureIpState
방화벽을 통해 Azure에서 시작되는 IPS를 허용/차단할 수 있습니다.
...
### -DefaultGroup
AzureActive Directory 그룹의 ID를 지정합니다.
이 그룹은 만든 파일 및 폴더의 기본 그룹입니다.
...
### -FirewallState
선택적으로 기존 방화벽 규칙을 사용하도록 설정하거나 사용하지 않도록 설정합니다.
...
### -KeyVersion
암호화 형식이 사용자 할당된 경우 사용자는 이 매개 변수를 사용하여 키 버전을 회전할 수 있습니다.
...
### -ResourceGroupName
수정할 Data Lake Store 계정이 포함된 리소스 그룹의 이름을 지정합니다.
...
### -Tag
태그를 키 값 쌍으로 지정합니다.
태그를 사용하여 다른 Azure 리소스에서 Data Lake Store 계정을 식별할 수 있습니다.
...
선택적으로 기존 신뢰할 수 있는 ID 공급자를 사용하도록 설정하거나 사용하지 않도록 설정합니다.
