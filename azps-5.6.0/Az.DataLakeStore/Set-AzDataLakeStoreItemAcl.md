...
첫 번째 명령은 ContosoADL 계정의 루트 디렉터리에 대한 ACL을 구한 다음, 해당 ACL을 $ACL 변수에 저장합니다.
두 번째 명령은 파일 Test.txt ACL을 $ACL.

### 예제 2: 폴더에 대한 ACL을 재구성적으로 설정
```
...
첫 번째 명령은 ContosoADL 계정의 디렉터리 폴더1에 대한 ACL을 구한 다음, $ACL 변수에 저장합니다.
두 번째 명령은 ACL을 폴더2로 다시 설정하고 해당 하위 $ACL.
...
### -Recurse
자식 하위디렉터리 및 파일로 다시 설정될 ACL을 나타냅니다.
...
### -ShowProgress
통과하면 진행 상태가 표시됩니다. 재구성 Acl 집합이 완료된 경우만 해당됩니다.
