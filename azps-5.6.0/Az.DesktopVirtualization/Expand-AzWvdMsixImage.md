...
### 예제 1: 지정된 이미지 경로를 확장하고 패키지 메타데이터를 검색합니다AppxManifest.xml
```powershell
...
### -MsixImageUri
생성하는 MSIX 이미지를 참조하는 URI를 나타내고 MSIXIMAGEURI 속성에 대한 노트 섹션을 참조하고 해시 테이블을 만듭니다.
...
MSIXIMAGEURI : MSIX 이미지를 <IMsixImageUri> 참조하는 URI를 나타내고
  - `[Uri <String>]`: URI에서 이미지로
