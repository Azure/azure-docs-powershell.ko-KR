...
이 명령은 해당 애플리케이션 그룹의 Windows Virtual Desktop 애플리케이션을 업데이트합니다.
...
### -CommandLineSetting
이 게시된 애플리케이션을 클라이언트에서 제공하는 명령줄 인수, 게시 시 지정된 명령줄 인수 또는 명령줄 인수가 없는지 여부를 지정합니다.
...
### -ShowInPortal
RD Web Access 서버에 RemoteApp 프로그램을 표시할지 여부를 지정합니다.
