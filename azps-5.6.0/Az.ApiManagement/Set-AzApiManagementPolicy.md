...
### -ApiRevision
API 개정의 식별자입니다. 이 매개 변수는 선택 사항입니다. 지정하지 않으면 현재 활성 API 개정에서 정책이 업데이트됩니다.
...
### -Format
정책의 형식을 지정합니다. 을 사용하는 경우 정책에 포함된 식은 `application/vnd.ms-azure-apim.policy+xml` XML 이스케이프되어야 합니다. 이 정책을 사용하는 경우 정책을 XML 이스케이프할 `application/vnd.ms-azure-apim.policy.raw+xml` 필요가 없습니다. 
...
### -PolicyFilePath
정책 문서 파일 경로를 지정합니다.
정책 매개 변수를 *지정하지* 않은 경우 이 매개 변수가 필요합니다.
...
### -ProductId
기존 제품의 식별자를 지정합니다.
이 매개 변수를 지정하면 cmdlet은 제품 범위 정책을 설정합니다.
