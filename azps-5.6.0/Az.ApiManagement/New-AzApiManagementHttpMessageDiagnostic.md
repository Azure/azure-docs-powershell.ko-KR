...
## 설명
cmdlet **New-AzApiManagementHttpMessageDiagnostic는** Http 메시지 진단 설정을 만듭니다.
...
100비트와 함께 로그 및 헤더에 http 메시지 진단 `Content-Type` `User-Agent` 설정 만들기 `body`

## 매개 변수

### -BodyBytesToLog
로그할 요청 본문의 수입니다. 이 매개 변수는 선택 사항입니다.
...
### -HeadersToLog
로그할 헤더의 배열입니다. 이 매개 변수는 선택 사항입니다.
