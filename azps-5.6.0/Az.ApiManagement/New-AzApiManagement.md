...
명령은 조직 및 관리자 주소를 지정합니다.
명령은 SKU 매개 *변수를 지정하지* 않습니다.
따라서 cmdlet은 개발자의 기본값을 사용 합니다.
...
이 명령은 미국 서부의 마스터 지역이 있는 외부 연결 게이트웨이 엔드포인트를 갖는 Azure 가상 네트워크 서브넷에 Premium-tier API Management 서비스를 만듭니다.
...
이 명령은 Azure 가상 네트워크 서브넷에 미국 서부의 마스터 지역이 있는 내부 게이트웨이 엔드포인트를 갖는 프리미엄 계층 API Management 서비스를 만듭니다.
...
이 명령은 표준 SKU Api Management 서비스를 만들고 Frontend 클라이언트에서 ApiManagement Gateway 및 백 엔드 클라이언트에서 ApiManagement Gateway와 백 엔드 클라이언트에 TLS 1.0을 사용하도록 설정합니다.

## 매개 변수

### -AdditionalRegions
Azure API Management의 추가 배포 지역.
...
### -AdminEmail
API Management 시스템이 보내는 모든 알림에 대해 원래 전자 메일 주소를 지정합니다.
...
사용자 지정 호스트 이름 구성입니다. 기본값은 $null. 전달 $null 기본 호스트 이름을 설정합니다.
...
### -EnableClientCertificate
소비 SKU ApiManagement 서비스에만 사용할 플래그입니다. 그러면 게이트웨이에 대한 각 요청에 대해 클라이언트 인증서가 제공될 수 있습니다. 또한 게이트웨이의 정책에서 인증서를 인증할 수 있습니다.
...
### -Organization
조직의 이름을 지정합니다.
API Management는 전자 메일 알림의 개발자 포털에서 이 주소를 사용 합니다.
...
서비스에 설치될 내부 CA에서 발급한 인증서입니다. 기본값은 $null.
...
Azure KeyVault와 같은 주요 관리 서비스와 함께 사용할 사용자 ID를 이 서버에 할당합니다.
...
### -VirtualNetwork
마스터 Azure API Management 배포 지역의 가상 네트워크 구성입니다.
...
- "None"(기본값). ApiManagement는 어떤 Virtual Network의 일부도 아니며")
- "External"(ApiManagement 배포는 인터넷 연결 엔드포인트가 있는 가상 네트워크 내에서 설정됩니다.
...
### System.Collections.Generic.Dictionary'2[[System.String, System.Private.CoreLib, Version=4.0.0.0, Culture=neutral, PublicKeyToken=7cec85d7bea7798e],[System.String, System.Private.CoreLib, Version=4.0.0.0, Culture=neutral, PublicKeyToken=7cec85d7bea798e]]
