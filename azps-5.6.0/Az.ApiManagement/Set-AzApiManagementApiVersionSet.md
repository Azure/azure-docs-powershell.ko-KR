...
## SYNOPSIS
API 관리 컨텍스트에서 API 버전 집합을 업데이트합니다.
...
이 명령은 버전 관리 체계 및 헤더 매개 변수를 사용하여 기존 API `Header` 버전 집합을 업데이트합니다. `api-version`
...
### -HeaderName
버전 정보를 포함하는 헤더 값입니다.
구성표 HEADER 버전 지정을 선택한 경우 이 값을 지정해야 합니다.
...
### -PassThru
지정된 경우 Microsoft.Azure.Commands.ApiManagement.ServiceManagement.Models.PsApiManagementApiVersionSet 형식의 인스턴스가 출력에 기록됩니다.
...
### -QueryName
버전 정보를 포함하는 쿼리 값입니다.
구성표 쿼리 버전 지정을 선택한 경우 이 값을 지정해야 합니다.
