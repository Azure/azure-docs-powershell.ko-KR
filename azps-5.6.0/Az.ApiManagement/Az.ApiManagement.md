...
구성 데이터베이스와 Git 리포지토리 간에 가장 최근 동기화의 상태를 얻습니다.
...
ApiManagement 서비스에 새 API Schema를 만듭니다.
...
기존 게이트웨이에 대한 호스트 이름 구성atin을 만듭니다.
...
진단의 Http 메시지 진단 설정인 **PsApiManagementHttpMessageDiagnostic의** 인스턴스를 만듭니다.
...
게이트웨이에 들어오는/들어오는 HTTP 메시지에 대한 진단 설정을 생성합니다.
...
새 리소스 위치 계약을 만들 수 있습니다(게이트웨이에서 사용).
...
지정된 Azure Storage Blob에서 API Management Service를 복원합니다.
...
테넌트 액세스를 활성화하거나 사용하지 않도록 설정합니다.
...
Api Management 서비스의 캐시를 업데이트합니다.
