...
## 설명
Api Management 서비스에 구성된 캐시에 대한 세부 정보를 얻습니다.
...
Api Management 서비스에 구성된 모든 캐시 목록을 제공합니다.

### 예제 2: 식별자 westus에서 지정한 캐시를 얻습니다.
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
westus에 대해 구성된 지정된 캐시의 세부 정보 확인

## 매개 변수

### -CacheId
캐시의 식별자입니다.
지정한 경우 식별자에 의해 캐시를 찾으려고 합니다.
이 매개 변수는 선택 사항입니다.
...
### -ResourceId
캐시의 리소스 식별자 팔입니다. 지정한 경우 식별자에 의해 캐시를 찾으려고 합니다. 이 매개 변수는 필수입니다.
