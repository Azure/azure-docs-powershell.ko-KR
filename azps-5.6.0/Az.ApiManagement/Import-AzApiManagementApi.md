...
## 설명
**Import-AzApiManagementApi** cmdlet은 WADL(웹 애플리케이션 설명 언어), 웹 서비스 설명 언어(WSDL) 또는 Swagger 형식의 파일 또는 URL에서 Azure API Management API를 가져온다.
...
이 명령은 지정된 Swagger 파일에서 API를 가져온다.
...
이 명령은 지정된 WADL 링크에서 API를 가져온다.

### 예제 4: 오픈 API 링크에서 API 가져오기
```powershell
...
이 명령은 지정된 Open 3.0 사양 링크에서 API를 가져온다.
...
### -ApiRevision
API 개정의 식별자입니다. 이 매개 변수는 선택 사항입니다. 지정하지 않으면 가져오기가 현재 활성 개정 또는 새 API로 수행됩니다.
...
### -ApiType
이 매개 변수는 Http의 기본값을 사용하는 선택 사항입니다. Soap 옵션은 WSDL을 가져올 때만 적용될 수 있으며 SOAP Passthrough API를 생성합니다.
...
### -Path
API의 공용 URL의 마지막 부분으로 웹 API 경로를 지정합니다.
...
### -Protocol
웹 API 프로토콜(http, https)입니다. 사용할 수 있는 API를 사용하는 프로토콜입니다. 이 매개 변수는 선택 사항입니다. 제공된 경우 사양 문서에 지정된 프로토콜을 다시 지정합니다.
...
### -SpecificationFormat
사양 형식을 지정합니다.
psdx_paramvalues Wadl, Wsdl 및 Swagger를 제공합니다.
...
### -WsdlEndpointName
가져올 WSDL 엔드포인트(포트)의 로컬 이름입니다. 1~400자 이상이 되어야 합니다. 이 매개 변수는 선택 사항이며 Wsdl을 가져오는 데만 필요합니다. 기본값은 $null.
...
### -WsdlServiceName
가져올 WSDL 서비스의 로컬 이름입니다. 1~400자 이상이 되어야 합니다. 이 매개 변수는 선택 사항이며 Wsdl 을 가져오는 데만 필요합니다. 기본값은 $null.
