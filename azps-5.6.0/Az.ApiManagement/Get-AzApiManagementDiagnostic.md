...
## 설명
**Get-AzApiManagementDiagnostic는** 특정 범위의 Api 관리 서비스에 구성된 진단에 대한 세부 정보를 제공합니다.

## 예제

### 예제 1: 테넌트 범위에서 구성된 모든 진단을 얻습니다.
```powershell
...
이 명령은 Api Management 서비스에 구성된 모든 진단을 제공합니다.

### 예제 2: Api 범위에서 구성된 모든 진단을 얻습니다.
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
이 명령은 Api 범위에서 구성된 모든 `echo-api` 진단을 얻습니다.

### 예제 3: ID에 의해 지정된 API 범위 진단을 얻게 됩니다.
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
이 명령은 api `applicationinsights` 에서 구성된 진단을 얻습니다. `echo-api`

## 매개 변수

### -ApiId
기존 API의 식별자입니다.
지정된 경우 API 범위 진단을 반환합니다.
이 매개 변수는 필수입니다.
...
### -ResourceId
진단 또는 Api 진단의 Arm 리소스 식별자입니다. 지정한 경우 식별자에 의해 진단을 찾으려고 합니다. 이 매개 변수는 필수입니다.
