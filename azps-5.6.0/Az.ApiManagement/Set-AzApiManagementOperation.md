...
### -ApiRevision
API 개정의 식별자입니다. 이 매개 변수는 선택 사항입니다. 지정하지 않으면 작업이 현재 활성 API 개정에서 업데이트됩니다.
...
### -TemplateParameters
매개 변수 *UrlTemplate에* 정의된 배열 또는 매개 변수를 지정합니다.
값을 지정하지 않으면 UrlTemplate를 기반으로 기본값이 생성됩니다.
