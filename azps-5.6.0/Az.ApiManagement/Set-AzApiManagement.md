...
### 예제 1: API Management 서비스를 시작하고 Premium으로 확장하고 지역 추가
```powershell
...
이 예제에서는 Api Management 인스턴스를 5개 프리미엄 단위로 확장한 다음 프리미엄 지역에 3개 단위를 추가합니다.
...
### -PassThru
작업이 성공하면 파이프라인에 업데이트된 PsApiManagement를 전송합니다.
