...
## 설명
**Get-AzApiManagementPolicy** cmdlet은 지정된 범위 정책을 얻습니다.
...
이 명령은 테넌트 수준 정책을 얻게 하여 테넌트 수준 정책을 tenantpolicy.xml.
...
이 명령은 작업 범위 정책을 얻습니다.

### 예제 5: RawXml 형식으로 테넌트 범위 정책 사용
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
이 명령은 Xml 이스케이프되지 않은 형식으로 테넌트 범위 정책을 얻습니다.

## 매개 변수

### -ApiId
기존 API의 식별자를 지정합니다.
이 매개 변수를 지정하면 cmdlet이 API 범위 정책을 반환합니다.
...
### -ApiRevision
API 개정의 식별자입니다. 이 매개 변수는 선택 사항입니다. 지정하지 않으면 현재 활성 API 개정에서 정책이 검색됩니다.
...
### -Format
API 관리 정책의 형식을 지정합니다.
이 매개 변수의 기본값은 "xml"입니다.
...
### -OperationId
기존 API 작업의 식별자를 지정합니다.
*ApiId를* 사용하여 이 매개 변수를 지정하는 경우 cmdlet은 작업 범위 정책을 반환합니다.
...
### -ProductId
기존 제품의 식별자를 지정합니다.
이 매개 변수를 지정하면 cmdlet이 제품 범위 정책을 반환합니다.
...
### -SaveAs
결과를 저장하기 위해 파일 경로를 지정합니다.
이 매개 변수를 지정하지 않으면 결과는 스티로로 파이프라인됩니다.
