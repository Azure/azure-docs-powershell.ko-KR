...
## 설명
**Set-AzApiManagementLogger** cmdlet은 Azure API Management **Logger의 설정을 수정합니다.**
...
이 명령은 ID Logger123이 있는 로거를 수정합니다.

### 예제 2

API Management 로거를 수정합니다. (자동 재생)
...
### -ConnectionString
정책 보내기 권한을 포함하는 Azure Event Hubs 연결 문자열을 지정합니다.
...
### -IsBuffered
로거의 레코드가 게시하기 전에 버퍼링되는지 지정합니다.
레코드가 버퍼링될 때 15초마다 또는 버퍼가 256 KB의 메시지를 수신할 때마다 Event Hubs로 전송됩니다.
...
### -PassThru
이 cmdlet은 이 cmdlet이 수정하는  **PsApiManagementLogger를** 반환합니다.
