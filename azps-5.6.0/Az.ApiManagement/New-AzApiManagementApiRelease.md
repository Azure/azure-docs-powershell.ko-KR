...
**New-AzApiManagementApiRelease** cmdlet은 API Management 컨텍스트에서 API 개정에 대한 API 릴리스를 만듭니다. 릴리스는 Api 개정을 현재 개정으로 만드는 데 사용됩니다.

## 예제

### 예제 1: API 개정에 대한 API 릴리스 만들기
```powershell
...
이 명령은 의 개정의 API `2` 릴리스를 `echo-api` 만듭니다.
...
### -Note
Api 릴리스 정보입니다. 이 매개 변수는 선택 사항입니다.
