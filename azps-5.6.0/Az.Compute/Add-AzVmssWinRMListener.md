...
## 설명
**Add-AzVmssWinRMListener** cmdlet은 VMSS(Virtual Machine Scale Set)에 Windows 원격 관리(WinRM) 수신기가 추가됩니다.
...
이 예제에서는 VMSS에 WinRM 수신기 를 추가합니다.
첫 번째 명령은 **New-AzVmssConfig** cmdlet을 사용하여 VMSS 구성 개체를 만들고 결과를 새 이름의 변수에 $VMSS.
두 번째 명령은 지정된 URL에서 인증서가 있는 HTTP 프로토콜 WinRM 수신기를 VMSS에 추가합니다.

## 매개 변수

### -CertificateUrl
새 가상 머신이 프로비전되는 인증서의 URL로 링크를 지정합니다.
