...
## 설명
**Set-AzVmss** cmdlet은 VMSS(Virtual Machine Scale Set)에서 특정 작업을 설정합니다.
이 cmdlet에서 지원하는 유일한 작업은 Reimage입니다.
...
이 명령은 ContosoGroup이라는 리소스 그룹에 속하는 ContosoVMSS라는 VMSS를 다시 지정합니다.

### 예제 2

지정된 VMSS에 대한 특정 작업을 설정합니다. (자동 재생)
...
### -PerformMaintenance
이 cmdlet은 VMSS에서 하나 이상의 가상 머신 유지 관리 작업을 수행하고 있습니다.
...
### -재배포
cmdlet이 VMSS에서 하나 이상의 가상 머신을 다시 재배포합니다.
...
### -ReimageAll
cmdlet이 VMSS의 모든 디스크를 다시 표시하는지 나타냅니다.
...
### -VMScaleSetName
이 cmdlet에서 작업을 설정하는 VMSS의 이름을 지정합니다.
