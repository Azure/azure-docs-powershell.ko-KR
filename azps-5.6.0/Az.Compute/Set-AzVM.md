...
이 cmdlet을 실행하기 전에 가상 머신에 로그온하고 Sysprep를 사용하여 하드 디스크를 준비합니다.
...
### -일반화
이 cmdlet은 가상 머신을 일반화된 것으로 표시하는지 나타냅니다.
...
### -재배포
이 cmdlet은 가상 머신을 다른 Azure 호스트로 수동으로 다시 재배포하여 문제를 해결했다고 나타냅니다.
가상 머신을 다시 재배포하면 가상 머신이 다시 시작됩니다. 이로 인하여 에메랄드 드라이브 데이터가 손실됩니다.
...
### -SimulateEviction
이 cmdlet은 스폿 가상 머신의 터전을 시뮬레이션합니다.
API를 호출한 후 30분 이내에 은닉이 발생합니다.
