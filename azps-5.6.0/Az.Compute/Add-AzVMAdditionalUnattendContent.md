...
이 cmdlet이 추가하는 추가 기본 64 인코딩 .xml 서식 정보를 unattend.xml 지정합니다.
...
세 번째 명령은 Get-Credential cmdlet을 사용하여 자격 증명 개체를 만든 다음 결과를 $Credential 변수에 저장합니다.
...
콘텐츠에는 암호가 포함됩니다.
마지막 명령은 에 저장된 콘텐츠를 $AucContent 파일에 unattend.xml 추가합니다.

## 매개 변수

### -Content
기본 64 인코딩된 XML 형식의 콘텐츠를 지정합니다.
이 cmdlet은 콘텐츠를 unattend.xml 추가합니다.
