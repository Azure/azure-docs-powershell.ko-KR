...
오류가 발생하면 호스트를 자동으로 교체해야 하는지 여부를 지정합니다. 이 값은 제공되지 않을 때 'true'로 기본값이 지정됩니다.
...
### -LicenseType
호스트에 배포된 VM에 적용할 소프트웨어 라이선스 유형을 지정합니다. 가능한 값은 없음, Windows_Server_Hybrid 및 Windows_Server_Perpetual.  기본값은 없음입니다.
...
### -PlatformFaultDomain
호스트의 플랫폼 장애 도메인 수를 지정합니다.  그런 다음 최소 값은 0이고 최대 값은 2입니다.
