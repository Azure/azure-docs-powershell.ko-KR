...
## 설명
**Set-AzVmssStorageProfile** cmdlet은 VMSS(Virtual Machine Scale Set)에 대한 저장소 프로필 속성을 설정합니다.

## 예제

### 예제 1: VMSS에 대한 저장소 프로필 속성 설정
```
...
이 명령은 ContosoVMSS라는 VMSS에 대한 저장소 프로필 속성을 설정합니다.
...
### -OsDiskCreateOption
이 cmdlet에서 VMSS 가상 머신을 만드는 방법을 지정합니다.
이 매개 변수에 대해 허용되는 값은 다음입니다.
- 연결 : 이 값은 특수 디스크를 사용하여 VMSS 가상 머신을 만들 때 사용됩니다. 
- FromImage : 이 값은 이미지를 사용하여 VMSS 가상 머신을 만들 때 사용됩니다.
플랫폼 이미지를 사용하는 경우 *imageReference 매개 변수도 사용합니다.*
...
### -OsDiskOsType
디스크의 운영 체제 유형을 지정합니다.
이 시나리오는 플랫폼 이미지가 아니라 사용자 이미지 시나리오에만 필요합니다.
...
VMSS 개체를 지정합니다.
개체를 구하기 위해 개체 New-AzVmssConfig 사용합니다.
...
### System.Nullable'1[[Microsoft.Azure.Management.Compute.Models.CachingTypes, Microsoft.Azure.Management.Compute, Version=23.0.0, Culture=neutral, PublicKeyToken=31bf3856ad364e35]]
