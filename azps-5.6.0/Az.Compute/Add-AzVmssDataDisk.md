...
## 설명
**Add-AzVmssDataDisk** cmdlet은 VMSS(Virtual Machine Scale Set) 인스턴스에 데이터 디스크를 추가합니다.
...
이 명령은 VMSS 개체에 빈 데이터 디스크를 추가합니다.
...
### -DiskIOPSReadWrite
관리 디스크에 Read-Write IOPS를 지정합니다. StorageAccountType이 있는 UltraSSD_LRS. 지정하지 않으면 diskSizeGB에 따라 기본값이 할당됩니다.
...
### -DiskMBpsReadWrite
관리 디스크에 대한 초당 MB의 대역폭을 지정합니다. StorageAccountType이 있는 UltraSSD_LRS. 지정하지 않으면 diskSizeGB에 따라 기본값이 할당됩니다.
...
### -WriteAccelerator
데이터 디스크에서 WriteAccelerator를 사용하도록 설정하거나 사용하지 않도록 설정해야 하는지 여부를 지정합니다.
