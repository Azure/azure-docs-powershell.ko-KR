...
## 설명
**Set-AzImageOsDisk** cmdlet은 이미지 개체의 운영 체제 디스크 속성을 설정합니다.
...
첫 번째 명령은 이미지 개체를 만든 다음, $imageConfig 변수에 저장합니다.
다음 세 가지 명령은 os 디스크의 경로와 두 개의 데이터 디스크를 $osDiskVhdUri, $dataDiskVhdUri 1 및 $dataDiskVhdUri 2 변수에 할당합니다.
이 방법은 다음 명령의 가독성을 위한 것입니다.
다음 세 가지 명령은 각각 os 디스크와 두 개의 데이터 디스크를 저장한 이미지에 $imageConfig.
각 디스크의 URI는 $osDiskVhdUri, $dataDiskVhdUri 1 및 $dataDiskVhdUri 2에 저장됩니다.
마지막 명령은 리소스 그룹 'ResourceGroup01'에서 'ImageName01'이라는 이미지를 만듭니다.
