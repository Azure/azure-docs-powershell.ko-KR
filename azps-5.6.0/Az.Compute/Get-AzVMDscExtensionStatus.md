...
## 설명
**Get-AzVMDscExtensionStatus** cmdlet은 리소스 그룹의 가상 머신에 대한 DSC(원하는 상태 구성) 확장 처리기 상태를 얻습니다.
...
가상 머신에 대한 DSC 확장 처리기 상태를 얻습니다. (자동 재생)
...
Set-AzVMDscExtension cmdlet은 **Get-AzVMDscExtensionStatus에서** 사용하는 동일한 값인 Microsoft.Powershell.DSC로 이 이름을 설정합니다.
설정 cmdlet에서 기본 이름을 변경하거나 Resource Manager 템플릿에서 다른 리소스 이름을 사용한 경우만 이 매개 변수를 지정합니다.
...
### -VMName
이 cmdlet에서 DSC 확장 상태를 얻을 가상 머신의 이름을 지정합니다.
