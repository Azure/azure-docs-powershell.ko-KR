...
이 확장을 사용하면 가상 머신에서 자체 스크립트를 실행할 수 있습니다.
...
스크립트 파일은 contososcript.exe.

### 예제 2

가상 머신에 사용자 지정 스크립트 확장을 추가합니다. (자동 재생)
...
### -SecureExecution
이 cmdlet은 *RUN* 매개 변수의 값이 서버에 기록되지 않은지 또는 GET 확장 API를 사용하여 사용자에게 반환되지 않는지 나타냅니다.
Run *값은* 스크립트 파일에 안전하게 전달할 비밀 또는 암호를 포함할 수 있습니다.
...
### -StorageAccountName
이 cmdlet이 스크립트를 저장하는 Azure Storage 계정의 이름을 지정합니다.
