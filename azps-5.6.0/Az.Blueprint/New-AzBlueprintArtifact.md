...
## 설명
새 아티팩트를 생성합니다. 아티팩트를 만드는 두 가지 방법은 아티팩트 JSON을 입력 파일로 또는 아티팩트에 대한 인라인 매개 변수를 제공하는 것입니다. JSON 메서드는 인라인 매개 변수 메서드를 제공해야 하는 아티팩트 형식을 요구하지는 않는 반면 사용자가 -Type 매개 변수를 통해 아티팩트 형식을 제공해야 합니다.
...
아티팩트 JSON 파일을 통해 새 아티팩트를 생성합니다.
...
템플릿 파일을 통해 새 아티팩트를 ARM 만듭니다.

## 매개 변수

### -ArtifactFile
디스크의 JSON 형식의 아티팩트 파일의 위치입니다.
...
정책 정의 아티팩트에 전달할 매개 변수의 해시테이블입니다.
...
### -Type
아티팩트의 형식입니다.
지원되는 3가지 유형은 RoleAssignmentArtifact, PolicyAssignmentArtifact, TemplateArtifact입니다.
