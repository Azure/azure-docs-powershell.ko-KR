...
## 설명
**Add-AzMetricAlertRule** cmdlet은 메트릭 기반 경고 규칙을 추가하거나 업데이트합니다.
추가된 규칙은 리소스 그룹과 연결되어 있으며 이름이 있습니다.
이 cmdlet은 ShouldProcess 패턴을 구현합니다. 즉, 리소스를 실제로 만들거나 수정하거나 제거하기 전에 사용자로부터 확인을 요청할 수 있습니다.

## 예제

### 예제 1: 웹 사이트에 메트릭 경고 규칙 추가
```
...
이 명령은 웹 사이트에 대한 메트릭 경고 규칙을 만듭니다.
...
이 명령은 규칙을 사용하지 않도록 설정합니다.
규칙이 없는 경우 사용하지 않도록 만듭니다.
규칙이 있는 경우 해당 규칙은 사용하지 않도록 설정됩니다.
...
### -MetricName
규칙이 모니터링하는 메트릭의 이름을 지정합니다.
메트릭 기반 규칙에만 이 매개 변수를 지정합니다.
...
### -TargetResourceId
규칙이 모니터링하는 리소스의 ID를 지정합니다. 참고: 이 속성은 기존 경고 규칙에 대해 업데이트할 수 없습니다.
...
규칙이 평가될 때 시간 창에 적용할 집계 연산자를 지정합니다.
...
### -WindowSize
해당 데이터를 계산하는 규칙의 시간 창 크기를 지정합니다.
