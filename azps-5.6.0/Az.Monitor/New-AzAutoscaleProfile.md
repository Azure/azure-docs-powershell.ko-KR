...
첫 번째 명령은 Requests라는 자동 조정 규칙을 만든 다음, 이 규칙을 $Rule 변수에 저장합니다.
두 번째 명령은 의 규칙을 사용하여 고정된 날짜를 사용하여 Profile01이라는 프로필을 $Rule.
...
첫 번째 명령은 Requests라는 자동 조정 규칙을 만든 다음, 이 규칙을 $Rule 변수에 저장합니다.
두 번째 명령은 규칙의 규칙을 사용하여 다시 일정을 사용하여 SecondProfileName이라는 프로필을 $Rule.
...
마지막 명령은 Rule1 및 Rule2의 규칙을 사용하여 SecondProfileName이라는 프로필을 만든 다음, 해당 프로필을 $Profile 2 변수에 저장합니다.

### 예제 4: 일정이나 고정 날짜가 없는 프로필 만들기
```
PS C:\>$Rule = New-AzAutoscaleRule -MetricName "Requests" -MetricResourceId "/subscriptions/b93fb07a-6f93-30be-bf3e-4f0deca15f4f/resourceGroups/Default-Web-EastUS/providers/microsoft.web/sites/mywebsite" -Operator GreaterThan -MetricStatistic Average -Threshold 10 -TimeGrain 00:01:00 -ScaleActionCooldown 00:05:00 -ScaleActionDirection Increase -ScaleActionScaleType ChangeCount -ScaleActionValue "1" 
...
첫 번째 명령은 Requests라는 자동 조정 규칙을 만든 다음, 이 규칙을 $Rule 변수에 저장합니다.
두 번째 명령은 일정이나 고정된 날짜 없이 프로필을 만든 다음, 해당 프로필을 $Profile 저장합니다.
