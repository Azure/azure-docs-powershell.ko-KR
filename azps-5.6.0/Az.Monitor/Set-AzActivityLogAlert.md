...
태그, 조건 및 작업의 경우 개체를 미리 만들어서 이 호출의 매개 변수로 구분된 매개 변수로 전달해야 합니다(아래 예제 참조).
이 cmdlet은 ShouldProcess 패턴을 구현합니다. 즉, 실제로 리소스를 만들고 수정하기 전에 사용자로부터 확인을 요청할 수 있습니다.
**참고**: 이 cmdlet 및 관련 cmdlet은 사용이 사용 안 된(2017년 11월) **Add-AzLogAlertRule** 을 대체합니다.
...
처음 4개의 명령은 리프 조건 및 작업 그룹을 생성합니다.
마지막 명령은 조건 및 작업 그룹을 사용하여 활동 로그 경고를 만듭니다.

### 예제 2: 활동 로그 경고를 사용하지 않도록 설정
```
PS C:\>$location = 'Global'
PS C:\>$alertName = 'myAlert'
PS C:\>$resourceGroupName = 'theResourceGroupName'
...
처음 4개의 명령은 리프 조건 및 작업 그룹을 생성합니다.
마지막 명령은 조건 및 작업 그룹을 사용하여 활동 로그 경고를 만들고 경고를 사용하지 않도록 만듭니다.
...
첫 번째 명령은 nop와 유사하며, 이미 포함된 동일한 값으로 경고를 설정하고 나머지 명령은 경고 규칙을 검색하고 설명을 변경하고 사용하지 않도록 설정한 다음 InputObject 매개 변수를 사용하여 이러한 변경 내용을 유지

### 예제 4: 파이프에서 ResourceId 값을 사용하여 활동 로그 경고 설정
```
PS C:\>Get-AzResource -ResourceGroupName "myResourceGroup" -Name "myLogAlert" | Set-AzActivityLogAlert -DisableAlert
```

주어진 로그 경고 규칙이 있는 경우 이 명령은 사용하지 않도록 설정됩니다.
...
**참고**: 조건 목록에서 필드가 "범주"와 같아야 합니다. 백end은 이 조건이 없는 경우 400(BadRequest)으로 응답합니다.
...
### -DisableAlert
사용자가 활동 로그 경고를 사용하지 않도록 설정할 수 있습니다. 제공되지 않은 경우 경고가 사용하도록 설정됩니다.
