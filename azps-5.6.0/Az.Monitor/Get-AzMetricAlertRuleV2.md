...
## SYNOPSIS
V2(클래식이 아닌) 메트릭 경고 규칙을 얻습니다.

## 구문

### ByResourceGroupName(기본값)
```
...
## 설명
**Get-AzMetricAlertRuleV2** cmdlet은 해당 이름 또는 URI 또는 지정된 리소스 그룹 또는 구독의 모든 메트릭 경고 규칙에 따라 메트릭 경고 규칙을 얻습니다.

## 예제

### 예제 1: 현재 구독의 모든 메트릭 경고 규칙 사용
```powershell
...
이 명령은 현재 구독의 모든 메트릭 경고 규칙을 얻습니다.

### 예제 2: 리소스 그룹의 모든 메트릭 경고 규칙 사용
...
이 명령은 metricAlertsRG라는 리소스 그룹의 모든 메트릭 경고 규칙을 얻습니다.

### 예제 3: 이름에 따라 메트릭 경고 규칙 얻기
...
이 명령은 metricAlertsRG라는 리소스 그룹에서 PS3182019라는 메트릭 경고 규칙을 얻습니다.

### 예제 4: ruleID에 따라 메트릭 경고 규칙 얻기
...
이 명령은 주어진 리소스 ID를 사용하여 메트릭 경고 규칙을 얻습니다.
