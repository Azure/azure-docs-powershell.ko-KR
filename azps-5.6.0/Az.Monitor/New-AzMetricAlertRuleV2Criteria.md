...
## 설명
**New-AzMetricAlertRuleV2Criteria** cmdlet은 입력 [Add-AzMetricAlertRuleV2](https://docs.microsoft.com/powershell/module/az.monitor/add-azmetricalertrulev2) cmdlet으로 사용할 로컬 메트릭 조건 개체를 만들어 새 메트릭 경고 규칙을 만듭니다.
...
이 명령 집합은 차원 선택을 포함하는 더 복잡한 메트릭 경고 조건을 만듭니다.

### 예제 4: 웹테스트 가용성 조건 만들기
...
이 명령은 메트릭 경고 규칙에 사용할 수 있는 웹 사이트 가용성 조건을 만듭니다.
...
메트릭 유효성 검사가 건너뜁니다. 아직 내보이지 않은 사용자 지정 메트릭에 경고 규칙을 만들 수 있습니다.
...
### -TimeAggregation
창 간격을 통해 여러 메트릭 값을 롤업하는 데 사용되는 집계 작업
...
### -violationCount
경고를 발생하는 데 필요한 선택한 룩백 시간 기간 내에 필요한 최소 위반 수
