...
## 설명
이 명령은 저장소 동기화 서비스에서 서버를 등록을 등록하지 않습니다. 경고: 서버를 등록하지 않은 경우 이 서버의 모든 서버 엔드포인트가 삭제됩니다. 이 서버의 경로가 더 이상 동기화되지 않았다고 확실한 경우 호출해야 합니다.
...
이 명령은 서버를 등록을 등록하지 않습니다. 이 서버의 모든 서버 엔드포인트가 삭제됩니다.
...
### -InputObject
일반적으로 파이프라인을 통해 전달되는 RegisteredServer 입력 개체입니다.
