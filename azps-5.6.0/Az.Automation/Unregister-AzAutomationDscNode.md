...
## 설명
**Unregister-AzAutomationDscNode** cmdlet은 Azure Automation 계정의 관리에서 APS 원하는 상태 구성(DSC) 노드를 제거합니다.
...
이 cmdlet에서 DSC 노드를 제거하는 Automation 계정의 이름을 지정합니다.
...
### -Id
이 cmdlet이 제거한 DSC 노드의 고유 ID를 지정합니다.
...
### -ResourceGroupName
이 cmdlet에서 DSC 노드 등록을 등록하지 않은 리소스 그룹의 이름을 지정합니다.
