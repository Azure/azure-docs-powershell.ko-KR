...
## 설명
**Start-AzAutomationDscCompilationJob** cmdlet은 Azure Automation에서 DSC(원하는 상태 구성) 구성을 컴파일합니다.
...
명령에는 DSC 구성 매개 변수에 대한 $Params 값을 포함합니다.

### 예제 2: 새 노드 구성 빌드 버전으로 Automation에서 Azure DSC 구성을 컴파일합니다.
```
PS C:\>$Params = @{"StringParam"="Hello World";"IntegerParam"=32}
...
첫 번째 예제와 마찬가지로 첫 번째 명령은 매개 변수의 사전을 만들고 변수 변수에 $Params 저장합니다.
두 번째 명령은 Config01이라는 DSC 구성을 컴파일합니다.
명령에는 DSC 구성 매개 변수에 대한 $Params 값을 포함합니다.
Config01[<2>]로 새 노드 구성을 만들어 기존 노드 구성을 <NodeName>>. 버전 번호는 이미 있는 기존 버전 번호에 따라 증분됩니다.
...
### -매개 변수
이 cmdlet이 DSC 구성을 컴파일하는 데 사용하는 매개 변수의 사전을 지정합니다.
