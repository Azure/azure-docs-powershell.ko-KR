...
## 설명
**Get-AzAutomationDscCompilationJob** cmdlet은 Azure Automation에서 APS 원하는 상태 구성(DSC) 컴파일 작업을 얻습니다.

## 예제

### 예제 1: 모든 DSC 컴파일 작업 사용
```
...
이 cmdlet에서 얻을 DSC 컴파일 작업이 포함된 Automation 계정의 이름을 지정합니다.
...
### -ConfigurationName
이 cmdlet에서 컴파일 작업을 얻을 DSC 구성의 이름을 지정합니다.
...
### -EndTime
종료 시간을 지정합니다.
이 cmdlet은 이 매개 변수가 지정하는 시간까지 시작된 컴파일 작업을 제공합니다.
...
### -Id
이 cmdlet이 얻을 DSC 컴파일 작업의 고유 ID를 지정합니다.
...
### -ResourceGroupName
이 cmdlet에서 DSC 컴파일 작업을 얻을 리소스 그룹의 이름을 지정합니다.
...
### -StartTime
시작 시간을 지정합니다.
이 cmdlet은 이 매개 변수가 지정하는 시간 또는 이후에 시작하는 작업을 제공합니다.
