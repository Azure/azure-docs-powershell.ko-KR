...
## 설명
**Import-AzAutomationRunbook** cmdlet은 Azure Automation Runbook을 가져온다. 그래픽 runbook의 wps_2 워크플로 runbook에 대한 wps_2 wps_2(.graphrunbook) 파일 또는 python 2 runbook에 대한 (.py) 파일을 가져올 wps_2 스크립트(.ps1) 파일의 경로를 지정합니다. 워크플로 wps_2 Runbook의 경우 스크립트에 파일의 이름과 일치하는 단일 워크플로 wps_2 워크플로 정의가 포함되어야 합니다.
...
두 번째 명령은 GraphicalRunbook06이라는 그래픽 Runbook을 AutomationAccount01이라는 Automation 계정으로 가져온다.
또한 명령은 에 저장된 태그를 $Tags.

### 예제 2

Automation Runbook을 가져온다. (자동 재생)
...
이 cmdlet에서 Runbook을 가져오는 Automation 계정의 이름을 지정합니다.
...
### -LogProgress
Runbook이 진행률 정보를 기록하는지 여부를 지정합니다.
...
### -LogVerbose
Runbook에서 자세한 정보를 기록할지 여부를 지정합니다.
...
### -ResourceGroupName
이 cmdlet에서 Runbook을 가져오는 리소스 그룹의 이름을 지정합니다.
