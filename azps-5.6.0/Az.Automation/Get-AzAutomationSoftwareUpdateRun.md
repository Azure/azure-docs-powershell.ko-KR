...
## 설명
Get-AzAutomationSoftwareUpdateRun cmdlet은 소프트웨어 업데이트 실행 목록을 반환합니다. 소프트웨어 업데이트 구성에 연결된 일정이 있는 경우 여러 번 트리거할 수 있습니다. 일정이 트리거될 때마다 업데이트 실행이 생성됩니다. 업데이트 실행은 모든 컴퓨터 실행의 결과의 집계입니다. SoftwareUpdateConfigurationName 매개 변수를 전달하여 특정 소프트웨어 업데이트 구성에 대한 실행을 얻을 수 있습니다. 특정 실행을 얻게하려면 이름 매개 변수를 전달해야 합니다. 특정 상태의 실행을 나열하거나, 특정 운영 체제를 대상으로 실행하거나, 적절한 매개 변수를 전달하여 특정 시간 이후에 시작된 실행을 나열할 수도 있습니다.

## 예제

### 예제 1
이 예제에서는 특정 소프트웨어 업데이트 구성에서 트리거되는 모든 업데이트 실행을 나열합니다.
...
소프트웨어 업데이트 구성의 이름이 실행을 트리거했습니다.
