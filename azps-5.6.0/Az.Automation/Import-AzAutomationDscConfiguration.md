...
## 설명
**Import-AzAutomationDscConfiguration** cmdlet은 APS 원하는 상태 구성(DSC) 구성을 Azure Automation으로 가져온다.
...
이 명령은 Contoso17이라는 automation 계정으로 client.ps1 파일에서 DSC 구성을 가져온다. 명령은 Force 매개 *변수를 지정합니다.* 기존 DSC 구성이 있는 경우 이 명령은 대체합니다.

### 예제 2

DSC 구성을 Automation으로 가져온다. (자동 재생)
...
이 cmdlet에서 DSC 구성을 가져오는 Automation 계정의 이름을 지정합니다.
...
### -Force
이 cmdlet은 Automation의 기존 DSC 구성을 대체합니다.
...
### -LogVerbose
이 cmdlet이 이 DSC 구성의 컴파일 작업을 위해 자세한 로깅을 켜거나 끄는지 여부를 지정합니다. 로그온 또는 $True 끄기 위해 $False 값을 지정합니다.
...
### -Published
이 cmdlet은 게시된 상태의 DSC 구성을 가져오는 경우를 나타냅니다.
...
### -ResourceGroupName
이 cmdlet에서 DSC 구성을 가져오는 리소스 그룹의 이름을 지정합니다.
...
### -SourcePath
이 cmdlet에서 가져오는 DSC wps_2 포함하는 스크립트의 경로를 지정합니다.
