...
## 설명
**Get-AzAutomationScheduledRunbook** cmdlet은 하나 이상의 Azure Automation Runbook 및 관련 일정을 얻습니다.
기본적으로 이 cmdlet은 예약된 모든 Runbook을 얻습니다.
Runbook의 이름을 지정하거나 일정 또는 둘 다를 지정하여 특정 Runbook 일정을 표시합니다.

## 예제

### 예제 1: 예약된 모든 Runbook을 다운로드합니다.
```
PS C:\>Get-AzAutomationScheduledRunbook -AutomationAccountName "Contoso17" -ResourceGroupName "ResourceGroup01"
```

이 명령은 Contoso17이라는 Azure Automation 계정에서 예약된 모든 Runbook을 얻습니다.

### 예제 2: Runbook과 연결된 모든 일정을 얻습니다.
```
PS C:\>Get-AzAutomationScheduledRunbook -AutomationAccountName "Contoso17" -ResourceGroupName "ResourceGroup01" -RunbookName "Runbk01"
```

이 명령은 Contoso17이라는 Azure Automation 계정에서 Runbook Runbk01에 대해 예약된 모든 Runbook을 얻습니다.

### 예제 3: 일정과 연결된 모든 Runbook을 다운로드합니다.
```
PS C:\>Get-AzAutomationScheduledRunbook -AutomationAccountName "Contoso17" -ResourceGroupName "ResourceGroup01" -ScheduleName "Schedule01"
```

이 명령은 Contoso17이라는 Azure Automation 계정에서 Schedule01 일정에 대해 예약된 모든 Runbook을 얻습니다.
...
### -JobScheduleId
이 cmdlet이 도착하는 예약된 작업의 ID를 지정합니다.
...
### -ResourceGroupName
이 cmdlet이 도착하는 예약된 Runbook에 대한 리소스 그룹의 이름을 지정합니다.
...
### -RunbookName
이 cmdlet에서 예약된 Runbook을 받을 Runbook의 이름을 지정합니다.
...
### -ScheduleName
이 cmdlet이 예약된 Runbook을 얻을 일정의 이름을 지정합니다.
