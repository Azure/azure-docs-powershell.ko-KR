...
첫 번째 명령은 시스템에서 표준 시간대 ID를 얻게 하여 $TimeZone 변수에 저장합니다.
두 번째 명령은 지정된 표준 시간대의 오후 11:00에 현재 날짜에 한 번 실행되는 일정을 만듭니다.
...
첫 번째 명령은 **Get-Date** cmdlet을 사용하여 날짜 개체를 만든 다음, $StartDate 변수에 저장합니다.
향후 5분 이상인 시간을 지정합니다.
두 번째 명령은 **Get-Date** cmdlet을 사용하여 날짜 개체를 만든 다음, $EndDate 변수에 저장합니다.
명령은 향후 시간을 지정합니다.
마지막 명령은 Schedule02라는 일별 일정을 만들어 에 저장한 $StartDate $EndDate 시작하고 에 저장한 시간에서 만료됩니다.
...
첫 번째 명령은 **Get-Date** cmdlet을 사용하여 날짜 개체를 만든 다음, $StartDate 변수에 저장합니다.
두 번째 명령은 월요일, 화요일, 수요일, 목요일 및 금요일을 포함하는 주일 배열을 만듭니다.
마지막 명령은 매주 월요일에서 금요일까지 매주 13:00에 실행될 Schedule03이라는 일별 일정을 만듭니다. 일정이 만료되지 않습니다.

## 매개 변수

### -AutomationAccountName
이 cmdlet에서 일정을 만드는 Automation 계정의 이름을 지정합니다.
...
### -DayInterval
일정에 대한 간격(일)을 지정합니다.
이 매개 변수를 지정하지 않으면 *OneTime* 매개 변수를 지정하지 않으면 기본값은 1(1)입니다.
...
### -ExpiryTime
일정의 만료 시간을 **DateTimeOffset 개체로 지정합니다.**
유효한 **DateTimeOffset로** 변환할 수 있는 문자열을 지정할 수 있습니다.
...
소프트웨어 업데이트 구성 예약에 이 일정 개체가 사용될 것 을 나타냅니다.
...
### -ResourceGroupName
이 cmdlet에서 일정을 만드는 리소스 그룹의 이름을 지정합니다.
...
### -StartTime
일정의 시작 시간을 **DateTimeOffset 개체로 지정합니다.**
유효한 **DateTimeOffset로** 변환할 수 있는 문자열을 지정할 수 있습니다.
*TimeZone* 매개 변수를 지정하면 오프셋이 무시되고 지정된 표준 시간대가 사용됩니다.
...
### -TimeZone
일정에 대한 표준 시간대를 지정합니다.
이 문자열은 IANA ID 또는 Windows 표준 시간대 ID일 수 있습니다.
