...
## 설명
**Get-AzAutomationJob** cmdlet은 Azure Automation에서 Runbook 작업을 제공합니다.
...
이 cmdlet에서 작업을 얻을 수 있는 Automation 계정의 이름을 지정합니다.
...
이 cmdlet은 이 매개 변수가 지정하는 값에 종료 시간이 있는 작업을 얻습니다.
...
### -ResourceGroupName
이 cmdlet에서 작업을 받을 리소스 그룹의 이름을 지정합니다.
...
### -RunbookName
이 cmdlet에서 작업을 받을 Runbook의 이름을 지정합니다.
...
이 cmdlet은 이 매개 변수가 지정하는 값에 시작 시간이 있는 작업을 제공합니다.
...
### -Status
작업의 상태를 지정합니다.
이 cmdlet은 이 매개 변수와 일치하는 상태가 있는 작업을 제공합니다.
유효한 값은: 
- 활성화
- 완료되었습니다.
- 실패했습니다.
