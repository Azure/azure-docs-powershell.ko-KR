...
## SYNOPSIS
Virtual Network Gateway에 대한 IP 구성을 만듭니다.
...
## 설명
**New-AzVirtualNetworkGatewayIpConfig** cmdlet은 서브넷 ID를 기반으로 (이전에 만든) 공용 IP 주소를 통해 Virtual Network Gateway에 할당된 구성을 만듭니다.

## 예제

### 1: 가상 네트워크 게이트웨이에 대한 IP 구성 만들기
```
$gwIpConfig = New-AzVirtualNetworkGatewayIpConfig -Name myGWIpConfig -SubnetId $myGWsubnet.Id -PublicIpAddressId $myGWpip.Id
```

공용 IP 주소로 Virtual Network Gateway를 구성합니다. 가상 $myGWsubnet 만들 가상 네트워크 내의 "GatewaySubnet"에서 **Get-AzVirtualNetworkSubnetConfig** cmdlet을 사용하여 변수를 얻습니다. 변수 $myGWpip **New-AzPublicIpAddress** cmdlet을 사용하여 얻을 수 있습니다.
