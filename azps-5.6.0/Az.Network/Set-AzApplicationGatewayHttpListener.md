...
## 설명
**Set-AzApplicationGatewayHttpListener** cmdlet은 Azure 애플리케이션 게이트웨이에 대한 HTTP 수신기 수정합니다.
...
두 번째 명령은 게이트웨이의 HTTP 수신기에서 포트 80의 HTTP 프로토콜과 함께 $FIP 01에 저장된 프런트 엔드 구성을 사용합니다.
...
### -ApplicationGateway
이 cmdlet이 HTTP 수신기를 연결하는 애플리케이션 게이트웨이를 지정합니다.
...
### -HostName
이 cmdlet에서 HTTP 수신기를 보내는 호스트 이름을 지정합니다.
...
이 매개 변수에 대해 허용되는 값은 true 또는 false입니다.
...
### -SslCertificateId
HTTP 수신기의 SSL(보안 소켓 계층) 인증서 ID를 지정합니다.
