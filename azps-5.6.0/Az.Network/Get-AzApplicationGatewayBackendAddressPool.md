...
두 번째 명령은 Pool01이라는 이름의 $AppGw 연결된 백 엔드 주소 풀을 $BackendPool 변수에 저장합니다.
...
첫 번째 명령은 ResourceGroup01이라는 리소스 그룹에 ApplicationGateway01이라는 애플리케이션 게이트웨이를 $AppGw 변수에 저장합니다.
두 번째 명령은 와 연결된 백 엔드 주소 풀의 목록을 $AppGw 및 목록은 $BackendPools 저장합니다.
...
### -Name
이 cmdlet이 얻을 백 엔드 주소 풀의 이름을 지정합니다.
