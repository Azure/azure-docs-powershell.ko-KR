...
처음 두 명령은 vnet 및 서브넷을 만들고 초기화합니다. 세 번째 명령은 만든 서브넷을 참조하는 컨테이너 nic ip 구성 프로필을 만듭니다. 네 번째 명령은 이전 명령에서 만든 IP 구성 프로필을 공급하는 컨테이너 네트워크 인터페이스 구성을 만듭니다. 마지막으로, 다섯 번째 명령은 컨테이너 네트워크 인터페이스 구성으로 초기화된 네트워크 프로필을 $containerNicConfig.
