...
두 번째 명령은 애플리케이션 게이트웨이에 대한 요청 라우팅 규칙을 수정하여 $Setting 변수에 지정된 백 엔드 HTTP 설정, $Listener 지정된 HTTP 수신기 및 $Pool 지정합니다.

## 매개 변수

### -ApplicationGateway
이 cmdlet이 요청 라우팅 규칙을 연결하는 애플리케이션 게이트웨이 개체를 지정합니다.
...
### -BackendHttpSettings
애플리케이션 게이트웨이 백end HTTP 설정을 지정합니다.
