...
## SYNOPSIS
ExpressRoute 연결을 이름에 따라 얻거나 ExpressRouteGateway에 연결된 모든 ExpressRoute 연결을 나열합니다.
...
## 설명
ExpressRoute 연결을 이름으로 얻거나 ExpressRouteGateway에 연결된 모든 ExpressRoute 연결을 나열합니다.
...
게이트웨이가 만들어지면 이 게이트웨이는 New-AzExpressRouteConnection 사용하여 프레미스 ExpressRoute 회로에 연결됩니다.
...
이 명령은 ExpressRoute "testExpressRoutegw"에서 "test"로 시작하는 모든 연결을 얻습니다.
