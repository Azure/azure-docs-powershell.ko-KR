...
## 설명
**Get-AzServiceEndpointPolicy** cmdlet은 서비스 엔드포인트 정책을 제공합니다.
...
이 명령은 ResourceGroup01이라는 리소스 그룹에 속하는 ServiceEndpointPolicy1이라는 서비스 엔드포인트 정책을 얻게 $policy 변수에 저장합니다.
...
이 명령은 ResourceGroup01이라는 리소스 그룹의 모든 서비스 엔드포인트 정책 목록을 얻게 $policyList 변수에 저장합니다.
...
이 명령은 "ServiceEndpointPolicy"로 시작하는 모든 서비스 엔드포인트 정책 목록을 제공합니다.
