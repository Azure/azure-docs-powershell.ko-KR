...
## SYNOPSIS
방화벽 정책과 연결하기 위해 새 Azure 방화벽 정책 침입 감지를 만듭니다.
...
## 설명
**New-AzFirewallPolicyIntrusionDetection** cmdlet은 Azure 방화벽 정책 침입 감지 개체를 만듭니다.
...
이 예제에서는 경고(검색) 모드를 통해 침입 감지를 만듭니다.

### 예제 2: 2. 서명을 오버라이드하여 침입 감지 만들기
```powershell
...
### 예제 3: 3. 우회 트래픽 설정으로 구성된 침입 감지를 사용하여 방화벽 정책 만들기
```powershell
