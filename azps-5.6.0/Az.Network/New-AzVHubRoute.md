...
## SYNOPSIS
VHubRoute 개체를 만듭니다. 이 개체를 매개 변수로 전달할 New-AzVHubRouteTable 있습니다.
...
위의 명령은 다음으로 VHubRoute 개체를 만든 다음 VHubRouteTable 리소스에 추가할 수 있는 지정된 방화벽으로 nextHop을 사용하여 VHubRoute 개체를 만듭니다.
...
위의 명령은 nextHop을 지정된 hubVnetConnection으로 VHubRouteTable 리소스에 추가할 수 있는 VHubRoute 개체를 만듭니다.
...
```
위의 명령은 이미 기존 AzVHubRoute의 라우팅 구성을 얻은 다음 연결에 정적 경로를 추가합니다. 또는 정적 경로 내에서 새 연결을 만들 수 있는 경우 여기에서 예제 1을 [참조하세요.](New-AzRoutingConfiguration.md)
## 매개 변수
