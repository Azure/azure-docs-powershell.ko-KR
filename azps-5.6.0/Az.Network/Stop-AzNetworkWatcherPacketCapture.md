...
## 설명
Stop-AzNetworkWatcherPacketCapture 패킷 캡처 세션을 중지합니다. 세션이 중지된 후 패킷 캡처 파일은 해당 구성에 따라 VM에 로컬로 저장 및/또는 저장됩니다.
...
이 예제에서는 "PacketCaptureTest"라는 실행 중인 패킷 캡처 세션을 중지합니다. 세션이 중지된 후 패킷 캡처 파일은 해당 구성에 따라 VM에 로컬로 저장 및/또는 저장됩니다.
