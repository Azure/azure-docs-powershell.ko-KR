...
## SYNOPSIS
새 Azure VpnSite 리소스를 만듭니다. 이는 Cortex 가상 허브와 S2S 연결에 대해 Azure에 업로드된 고객 분기의 RM 표현입니다.
...
위의 리소스 그룹은 Azure에서 "nonlinkSite" 리소스 그룹에서 미국 동부의 Virtual WAN을 생성합니다. 

그런 다음, VpnSite를 만들어 고객 분기를 나타내고 Virtual WAN에 연결합니다.

그런 다음 IPSec 연결을 이 분기 및 VpnGateway를 사용하여 New-AzVpnConnection 수 있습니다.
...
위의 리소스 그룹은 Azure의 "멀티링크" 리소스 그룹에서 미국 동부에 VpnSiteLinks 1개가 있는 VpnSite 그룹, Virtual WAN 및 VpnSite를 만들 것입니다.

### 예제 3

새 Azure VpnSite 리소스를 만듭니다. (자동 재생)
...
### -VirtualWan
VirtualWan 이 VpnSite에 연결해야 합니다.
...
### -VirtualWanId
ResourceId VirtualWan 이 VpnSite에 연결해야 합니다.
...
### -VirtualWanName
VirtualWan의 이름을 이 VpnSite에 연결해야 합니다.
...
VirtualWan의 리소스 그룹 이름 이 VpnSite에 연결해야 합니다.
...
### -VpnSiteLink
이 VpnSite가 있는 VpnSiteLinks 목록입니다.
