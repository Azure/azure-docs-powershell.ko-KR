...
## 설명
Set-AzApplicationGatewayBackendHttpSetting cmdlet은 Azure 애플리케이션 게이트웨이에 대한 HTTP(백 엔드 Hypertext 전송 프로토콜) 설정을 업데이트합니다.
백 엔드 HTTP 설정은 풀의 모든 백 엔드 서버에 적용됩니다.

## 예제

### 예제 1: 애플리케이션 게이트웨이에 대한 백 엔드 HTTP 설정 업데이트
```
...
두 번째 명령은 포트 88$AppGw, HTTP 프로토콜을 사용하기 위해 애플리케이션 게이트웨이의 HTTP 설정을 업데이트하고 쿠키 기반의 에피니티를 사용할 수 있습니다.
...
### -ApplicationGateway
이 cmdlet이 백 엔드 HTTP 설정을 연결하는 애플리케이션 게이트웨이 개체를 지정합니다.
...
### -CookieBasedAffinity
백end 서버 풀에 대해 쿠키 기반의 연결성을 사용하도록 설정하거나 사용하지 않도록 설정해야 하는지 여부를 지정합니다.
이 매개 변수에 대해 허용되는 값은 사용 안 되거나 사용하도록 설정되어 있습니다.
...
### -Port
백 엔드 서버 풀의 각 서버에 사용할 포트를 지정합니다.
...
### -Probe
백 엔드 HTTP 설정과 연결하기 위해 프로브를 지정합니다.
...
### -ProbeId
백 엔드 HTTP 설정과 연결하기 위해 프로브의 ID를 지정합니다.
