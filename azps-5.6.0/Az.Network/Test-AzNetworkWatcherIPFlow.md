...
## SYNOPSIS
패킷을 허용하거나 특정 대상에서 또는 거부할지 여부를 반환합니다.
...
## 설명
지정된 Test-AzNetworkWatcherIPFlow 및 로컬 및 원격, IP 주소 및 포트를 사용하여 지정된 방향이 있는 패킷의 경우 cmdlet은 패킷이 허용되거나 거부될지 여부를 반환합니다.
...
이 구독에 대해 미국 중서부에 있는 Network Watcher를 을인 다음 VM을 얻게 하여 연결된 네트워크 인터페이스입니다. 그런 다음 첫 번째 네트워크 인터페이스의 경우 Test-AzNetworkWatcherIPFlow 첫 번째 네트워크 인터페이스의 첫 번째 IP를 사용하여 인터넷의 IP에 대한 아웃바운드 연결을 실행합니다.
