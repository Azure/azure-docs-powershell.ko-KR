...
## 설명
**Set-AzRouteTable** cmdlet은 경로 테이블을 업데이트합니다.

## 예제

### 예제 1: 경로 구성을 추가하여 경로 테이블 업데이트
```
...
명령은 파이프라인 연산자를 사용하여 Add-AzRouteConfig cmdlet에 테이블을 전달합니다.
**Add-AzRouteConfig는** Route07이라는 경로를 추가한 다음 결과를 현재 cmdlet에 전달하여 변경 내용을 반영하기 위해 테이블을 업데이트합니다.
...
첫 번째 명령은 rtName이라는 경로 테이블을 얻게 하여 $rt 변수에 저장합니다.
두 번째 명령은 DisableBgpRoutePropagation 값을 표시합니다.
세 번째 명령은 DisableBgpRoutePropagation 값을 업데이트합니다.
네 번째 명령은 서버에서 경로 테이블을 업데이트합니다.
다섯 번째 명령은 업데이트된 경로 테이블을 얻게 하여 $rt 변수에 저장합니다.
여섯 번째 명령은 DisableBgpRoutePropagation 값을 표시합니다.
...
### -RouteTable
경로 테이블을 설정해야 하는 상태를 나타내는 경로 테이블 개체를 지정합니다.
