...
## 설명
Remove-AzApplicationGatewayBackendHttpSetting cmdlet은 Azure 애플리케이션 게이트웨이에서 백 엔드 Hypertext HTTP(전송 프로토콜) 설정을 제거합니다.
...
두 번째 명령은 에 저장된 애플리케이션 게이트웨이에서 BackEndSetting02라는 백 엔드 HTTP 설정을 $AppGw.
...
### -Name
이 cmdlet이 제거한 백 엔드 HTTP 설정의 이름을 지정합니다.
