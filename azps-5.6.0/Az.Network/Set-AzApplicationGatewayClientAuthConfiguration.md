...
## SYNOPSIS
ssl 프로필 개체의 클라이언트 공인 구성을 수정합니다.
...
## 설명
**Set-AzApplicationGatewayClientAuthConfiguration** cmdlet은 ssl 프로필 개체의 클라이언트 공인 구성을 수정합니다.
...
첫 번째 명령은 ResourceGroup01이라는 리소스 그룹에 ApplicationGateway01이라는 애플리케이션 게이트웨이를 $AppGw 변수에 저장합니다. 두 번째 명령은 SslProfile01이라는 ssl 프로필을 $AppGw 변수에 $profile 저장합니다. 마지막 명령은 클라이언트에 저장된 ssl 프로필 개체의 클라이언트 $profile.
