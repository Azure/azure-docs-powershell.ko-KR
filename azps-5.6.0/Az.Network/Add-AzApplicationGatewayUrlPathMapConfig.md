...
## 설명
**Add-AzApplicationGatewayUrlPathMapConfig** cmdlet은 백 엔드 서버 풀에 URL 경로 매핑의 배열을 추가합니다.
...
두 번째 명령은 백던드 주소 풀을 얻게 하여 $pool 변수에 저장합니다.
세 번째 명령은 백end http 설정을 얻게 하여 $poolSettings 변수에 저장합니다.
...
### -ApplicationGateway
이 cmdlet에서 URL 경로 맵 구성을 추가하는 애플리케이션 게이트웨이를 지정합니다.
...
*pathRules* 매개 변수에 지정된 규칙이 일치하지 않는 경우 라우팅할 기본 백던드 주소 풀을 지정합니다.
...
### -Name
이 cmdlet이 백던드 서버 풀에 추가하는 URL 경로 맵 이름을 지정합니다.
...
### -PathRules
경로 규칙 목록을 지정합니다.
경로 규칙은 순서가 중요하고 지정된 순서대로 적용됩니다.
