...
## 설명
**Add-AzVirtualNetworkPeering** cmdlet은 두 가상 네트워크 간에 피어링을 만듭니다.
...
피어링이 작동하려면 vnet1에서 vnet2로, 그 반대의 경우도 피어링 링크를 만들어야 합니다.

### 예제 2: 서로 다른 지역의 두 가상 네트워크 간에 피어링 만들기
```
# Variables for common values used throughout the script.
$rgName='myResourceGroup'
...
여기에 미국 서부 중부의 'myVnet1'이 캐나다 중부의 'myVnet2'로 피어링됩니다.

## 매개 변수

### -AllowForwardedTraffic
이 cmdlet은 원격 가상 네트워크의 가상 머신에서 전달된 트래픽을 허용합니다.
...
### -AllowGatewayTransit
이 가상 네트워크에 대한 원격 가상 네트워크의 링크에서 GatewayLinks를 사용할 수 있도록 플래그
...
### -BlockVirtualNetworkAccesss
이 cmdlet은 연결된 가상 네트워크 공간의 가상 머신을 차단하여 로컬 가상 네트워크 공간의 모든 가상 머신에 액세스합니다.
...
### -UseRemoteGateways
이 cmdlet은 이 가상 네트워크의 원격 게이트웨이를 허용합니다.
