...
## 설명
**Move-AzExpressRouteCircuit** cmdlet은 ExpressRoute 회로를 클래식 배포 모델에서 Resource Manager 배포 모델로 이동합니다. 이동 후 ExpressRoute 회로는 Resource Manager 배포 모델에 생성된 다른 ExpressRoute 회로와 같이 행동하고 수행됩니다. 회로 링크, 가상 네트워크 및 VPN 게이트웨이는 이 작업을 통해 이동되지 않습니다. 이러한 리소스는 이동 후 다시 구성해야 합니다.
...
### -ResourceGroupName
이동되는 ExpressRoute 회로를 포함하는 리소스 그룹의 이름입니다.
