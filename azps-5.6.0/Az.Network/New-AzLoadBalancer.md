...
## 설명
**New-AzLoadBalancer** cmdlet은 Azure 부하 분산을 만듭니다.
...
부하 균형 조정기 배포를 위해서는 먼저 여러 개체를 만들어야 합니다. 처음 7개의 명령은 이러한 개체를 만드는 방법을 보여 주야 합니다.
8차 명령은 MyResourceGroup이라는 리소스 그룹에 MyLoadBalancer라는 부하 분산기 를 만듭니다.
9번째 명령과 마지막 명령은 성공적으로 만들어지기 위해 새 부하 저울을 얻습니다.
이 예제에서는 부하 저울을 만드는 방법만 보여줍니다. 또한 다른 가상 머신에 Add-AzNetworkInterfaceIpConfig cmdlet을 사용하여 구성해야 합니다.
...
전역 부하 균형을 배포하려면 먼저 여러 개체를 만들어야 합니다. 처음 5개의 명령은 해당 개체를 만드는 방법을 보여 주야 합니다.
여섯 번째 명령은 MyResourceGroup이라는 리소스 그룹에 MyLoadBalancer라는 부하 분산기 를 만듭니다.
일곱 번째 명령과 마지막 명령은 성공적으로 만들어지기 위해 새 부하 균형 조정을 제공합니다.
이 예제에서는 전역 부하 균형을 만드는 방법만 보여줍니다. 또한 백 엔드 주소 풀에 지역 부하 New-AzLoadBalancerBackendAddressConfig 프런트 엔드 ipconfig ID를 할당하기 위해 New-AzLoadBalancerBackendAddressConfig cmdlet을 사용하여 구성해야 합니다.
...
### -BackendAddressPool
부하 균형 조정기와 연결하기 위해 백던드 주소 풀을 지정합니다.
...
### -Force
이 cmdlet이 동일한 이름의 부하 균형 조정기가 이미 있는 경우에도 부하 균형 조정을 만듭니다.
...
부하 균형 조정기와 연결하기 위해 프런트 엔드 IP 주소 목록을 지정합니다.
...
### -InboundNatRule
부하 저울과 연결하기 위해 NAT(인바운드 네트워크 주소 변환) 규칙 목록을 지정합니다.
...
### -LoadBalancingRule
부하 분산기와 연결하기 위해 부하 분산 규칙 목록을 지정합니다.
...
### -Probe
부하 균형 조정기와 연결하기 위해 프로브 목록을 지정합니다.
