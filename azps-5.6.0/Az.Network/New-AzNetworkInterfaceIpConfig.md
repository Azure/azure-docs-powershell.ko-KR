...
### 1: 네트워크 인터페이스에 대한 공용 IP 주소로 IP 구성 만들기
```
...
처음 두 명령은 이전에 만든 myvnet이라는 가상 네트워크와 mysubnet이라는 서브넷을 각각 얻습니다. 이러한 데이터는 각각 $vnet $Subnet 저장됩니다. 세 번째 명령은 개인 IP 주소 10.0.0.5와 연결된 "IPConfig-2"라는 새 IP 구성을 만듭니다.
그런 다음 마지막 명령은 이 IP 구성을 사용하여 mynic1이라는 네트워크 인터페이스를 만듭니다.
...
이 네트워크 인터페이스 IPConfiguration이 속하는 부하 균형기 인바운드 Nat Rule 참조의 컬렉션을 지정합니다.
