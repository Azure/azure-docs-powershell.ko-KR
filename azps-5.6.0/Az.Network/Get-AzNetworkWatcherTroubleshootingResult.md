...
## SYNOPSIS
이전에 실행되거나 현재 실행 중인 문제 해결 작업에서 문제 해결 결과를 얻습니다.
...
## 설명
Get-AzNetworkWatcherTroubleshootingResult cmdlet은 이전에 실행되거나 현재 실행 중인 작업에서 문제 해결 Start-AzNetworkWatcherResourceTroubleshooting 합니다. 문제 해결 작업이 현재 진행 중인 경우 이 작업을 완료하는 데 몇 분 정도 걸릴 수 있습니다. 현재 Virtual Network Gateway 및 연결이 지원됩니다.

## 예제

### 예제 1: Virtual Network Gateway에서 문제 해결 시작 및 결과 검색
```
...
문제 해결이 시작되면 이 Get-AzNetworkWatcherTroubleshootingResult 결과를 검색하기 위해 리소스에 대한 호출이 호출됩니다. 
