...
첫 번째 명령은 공용 IP 리소스 개체를 만들고 해당 $PublicIP 저장합니다.
두 번째 명령은 $PublicIP 프런트 엔드 IP 구성을 만들고 FrontEndIP01이라는 새 프런트 엔드 IP 구성을 만들고 $FrontEnd 변수에 저장합니다.
...
세 번째 명령은 두 번째 명령 및 개인 IP 주소 10.0.1.1에서 $Subnet 사용하여 FrontEndIP02라는 프런트 엔드 IP 구성을 만든 다음, 해당 $FrontEnd 변수에 저장합니다.
...
### -PrivateIPAddress
이 cmdlet이 애플리케이션 게이트웨이의 프런트 엔드 IP 주소와 연결된 개인 IP 주소를 지정합니다.
서브넷을 지정하는 경우만 지정할 수 있습니다.
...
### -PublicIPAddress
이 cmdlet이 애플리케이션 게이트웨이의 프런트 엔드 IP 주소와 연결된 공용 IP 주소 개체를 지정합니다.
...
### -PublicIPAddressId
이 cmdlet이 애플리케이션 게이트웨이의 프런트 엔드 IP와 연결된 공용 IP 주소 ID를 지정합니다.
...
### -서브넷
이 cmdlet이 애플리케이션 게이트웨이의 프런트 엔드 IP 주소와 연결된 서브넷 개체를 지정합니다.
이 매개 변수를 지정하는 경우 게이트웨이에서 개인 IP 주소를 사용하는 의미입니다.
*PrivateIPAddress* 매개 변수를 지정하는 경우 이 매개 변수에 의해 지정된 서브넷에 속해야 합니다.
...
### -SubnetId
이 cmdlet이 애플리케이션 게이트웨이의 프런트 엔드 IP 구성과 연결된 서브넷 ID를 지정합니다.
*서브넷* 매개 변수를 지정하는 경우 게이트웨이가 개인 IP 주소를 사용하는 의미입니다.
