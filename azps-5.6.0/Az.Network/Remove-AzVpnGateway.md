...
## SYNOPSIS
Remove-AzVpnGateway cmdlet은 Azure VPN 게이트웨이를 제거합니다. Azure Virtual WAN의 소프트웨어 정의 연결과 관련이 있는 게이트웨이입니다.
...
이 예제에서는 리소스 그룹, Virtual WAN, Virtual Hub, 확장 가능한 VPN 게이트웨이를 미국 중부에 만들고 즉시 삭제합니다. Virtual Gateway를 삭제할 때 프롬프트를 표시하지 않습니다.
그러면 VpnGateway 및 연결된 모든 VpnConnections가 삭제됩니다.
...
이 예제에서는 리소스 그룹, Virtual WAN, Virtual Hub, 확장 가능한 VPN 게이트웨이를 미국 중부에 만들고 즉시 삭제합니다. 이 삭제는 powershell 배관을 사용하여 발생합니다. 이 Get-AzVpnGateway 명령에서 반환된 VpnGateway 개체를 사용합니다.
Virtual Gateway를 삭제할 때 프롬프트를 표시하지 않습니다.
그러면 VpnGateway 및 연결된 모든 VpnConnections가 삭제됩니다.
...
### -ResourceId
삭제될 vpnGateway에 대한 Azure 리소스 ID입니다.
