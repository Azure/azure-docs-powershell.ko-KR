...
## 설명
**Get-AzPublicIPAddress** cmdlet은 리소스 그룹에서 하나 이상의 공용 IP 주소를 얻습니다.
...
이 명령은 리소스 그룹 myRg에서 myPublicIp 이름이 있는 공용 IP 주소 리소스를 얻습니다.

### 예제 2: 필터링을 사용하여 공용 IP 리소스 사용
```powershell
...
이 명령은 myPublicIp로 시작하는 모든 공용 IP 주소 리소스를 얻습니다.
...
### -Name
이 cmdlet이 얻을 공용 IP 주소의 이름을 지정합니다.
...
Virtual Machine 네트워크 인터페이스 이름입니다.
...
### -ResourceGroupName
이 cmdlet에서 얻을 공용 IP 주소를 포함하는 리소스 그룹의 이름을 지정합니다.
