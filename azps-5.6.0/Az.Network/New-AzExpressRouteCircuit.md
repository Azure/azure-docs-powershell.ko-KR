...
### -대역폭InMbps
회로의 대역폭입니다. 이 값은 서비스 공급자에서 지원하는 값입니다.
...
### -ExpressRoutePort
ExpressRoutePort 리소스에서 회로를 프로비전할 때 ExpressRoutePort 리소스에 대한 참조입니다.
...
### -ServiceProviderName
회로 서비스 공급자의 이름입니다. 이 이름은 cmdlet에 나열된 이름과 Get-AzExpressRouteServiceProvider 합니다.
...
### -SkuFamily
SKU 패밀리는 청구 유형을 결정합니다. 이 매개 변수에 대한 가능한 값은 또는 `MeteredData` `UnlimitedData` 입니다. 청구 유형을 MeteredData에서 UnlimitedData로 변경할 수 있지만, UnlimitedData에서 MeteredData로 유형을 변경할 수 없습니다.
...
### -SkuTier
회로에 대한 서비스 계층입니다. 이 매개 변수에 대한 가능한 값은 `Standard` , 또는 `Premium` `Local` 입니다.
