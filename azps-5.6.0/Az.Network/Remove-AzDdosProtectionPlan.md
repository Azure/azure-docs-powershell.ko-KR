...
가상 네트워크에 연결된 경우 DDoS 보호 계획을 삭제할 수 없습니다. 따라서 첫 번째 단계는 두 개체를 모두 분해하는 것입니다. 여기에서는 계획과 연결된 가상 네트워크의 가장 업데이트된 버전을 얻게 됐고 **DdosProtectionPlan** 속성을 빈 값으로 설정하고 **EnableDdosProtection** 플래그(이 플래그는 계획 없이는 true일 수 없습니다)로 설정합니다.
그런 다음 **Set-AzVirtualNetwork에** 로컬 변수를 피핑하여 새 상태를 유지합니다. 이 시점에서 계획은 더 이상 가상 네트워크에 연결하지 않습니다.
