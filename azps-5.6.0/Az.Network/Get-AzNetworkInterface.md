...
### 예제 2: 특정 프로비전 상태로 모든 네트워크 인터페이스 사용
```
...
### -ResourceGroupName
이 cmdlet에서 네트워크 인터페이스를 얻을 리소스 그룹의 이름을 지정합니다.
...
### -VirtualMachineIndex
이 cmdlet에서 네트워크 인터페이스를 얻을 가상 머신 확장 집합의 가상 머신 인덱스를 지정합니다.
...
이 cmdlet에서 네트워크 인터페이스를 얻을 가상 머신 확장 집합의 이름을 지정합니다.
