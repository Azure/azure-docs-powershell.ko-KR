...
첫 번째 명령은 ResourceGroup001 리소스 그룹에 속하는 Gateway01이라는 가상 네트워크 게이트웨이를 $Gateway 변수에 저장된 가상 네트워크 게이트웨이를 업데이트하는 $Gateway.
또한 명령은 ASN을 1337로 설정합니다.
...
첫 번째 명령은 리소스 그룹 ResourceGroup001에 속하는 Gateway01이라는 가상 네트워크 게이트웨이를 얻게 $Gateway 두 번째 명령은 지정된 ipsec 매개 변수에 따라 Vpn ipsec 정책 개체를 만듭니다.
세 번째 명령은 변수에 저장된 가상 네트워크 게이트웨이를 $Gateway.
또한 명령은 가상 네트워크 게이트웨이의 $vpnclientipsecpolicy 지정된 사용자 지정 vpn ipsec 정책을 설정합니다.
...
첫 번째 명령은 ResourceGroup001에 속하는 Gateway01이라는 가상 네트워크 게이트웨이를 얻게 $Gateway라는 변수에 저장합니다. 두 번째 명령은 @{testtagKey="SomeTagKey"; testtagValue="SomeKeyValue" }를 사용하여 가상 네트워크 게이트웨이 Gateway01을 업데이트합니다.
...
첫 번째 명령은 리소스 그룹 ResourceGroup001에 속하는 Gateway01이라는 가상 네트워크 게이트웨이를 얻게 $Gateway 두 번째 명령은 AAD 인증 구성을 사용하여 가상 네트워크 게이트웨이 Gateway01을 업데이트합니다. aadTenantUri, aadAudienceId, vpnClient용 aadIssuerUri.
...
첫 번째 명령은 리소스 그룹 ResourceGroup001에 속하는 Gateway01이라는 가상 네트워크 게이트웨이를 얻게 $Gateway 두 번째 명령은 가상 네트워크 게이트웨이 Gateway01 ipConfiguration Id 값을 변수 ipconfigurationId1에 할당합니다.
세 번째 명령은 주소 목록1에 주소 목록을 할당합니다.
네 번째 명령은 PSIpConfigurationBgpPeeringAddress 개체를 만듭니다.
...
가상 네트워크 게이트웨이에서 Active Active 기능을 사용하지 않도록 플래그 지정
...
### -GatewayDefaultSite
힘 터널링에 사용할 기본 사이트입니다.
기본 사이트를 지정하면 게이트웨이의 vnet의 모든 인터넷 트래픽이 해당 사이트로 라우팅됩니다.
...
플래그를 지정하여 가상 네트워크 게이트웨이에서 P2S 클라이언트에 대한 AAD 인증을 제거합니다.
...
기본 수정을 해제할 가상 네트워크 게이트웨이 개체입니다.
이 검색은 Get-AzVirtualNetworkGateway
...
이러한 인증서 중 하나와 일치하는 인증서를 제시하는 VPN 클라이언트는 멀어지라고 합니다.
...
VPN 클라이언트를 연결하려면 이러한 루트 인증서 중 하나에서 생성된 인증서를 제시해야 합니다.
