...
## 설명
**Add-AzApplicationGatewaySslProfile** cmdlet은 애플리케이션 게이트웨이에 SSL 프로필을 추가합니다. SSL 프로필은 HTTPS 수신기에 적용됩니다.
...
두 번째 명령은 새 SSL 정책을 만들고 $sslPolicy 변수에 저장합니다.
세 번째 및 네 번째 명령은 두 개의 신뢰할 수 있는 클라이언트 CA 인증서 체인을 만들고 $ClientCert 01 및 $ClientCert 02 변수에 저장합니다.
다섯 번째 명령은 ssl 정책 및 신뢰할 수 있는 클라이언트 CA 인증서 체인이 있는 SSL 프로필을 애플리케이션 게이트웨이 $AppGw.
