...
## 설명
**New-AzFirewallPolicy** cmdlet은 Azure 방화벽 정책을 만듭니다.
...
### 예제 2: 2. ThreatIntel Mode를 사용하여 빈 정책 만들기
```powershell
PS C:\> New-AzFirewallPolicy -Name fp1 -ResourceGroupName TestRg -ThreatIntelMode "Deny"
```

이 예제에서는 위협 인텔 모드를 사용하여 Azure 방화벽 정책을 만듭니다.

### 예제 3: 3. ThreatIntel 화이트리스트를 사용하여 빈 정책 만들기
```powershell
...
이 예제에서는 위협 인텔 허용 목록이 있는 Azure 방화벽 정책을 만듭니다.

### 예제 4: 4. 침입 감지, ID 및 전송 보안을 사용하여 정책 만들기
```powershell
...
KeyVault에 저장된 비밀 ID(base-64 암호화되지 않은 pfx) 'Secret' 또는 'Certificate' 개체의 비밀 ID
