...
## SYNOPSIS
사이트 연결 지점을 위해 VirtualHub에서 기존 P2SVpnGateway를 업데이트합니다.
...
## 설명
**Update-AzP2sVpnGateway** cmdlet을 사용하면 VirtualHub에서 기존 P2SVppnGateway를 새 VpnClientAddressPool 또는 새 VpnServerConfiguration 또는 VpnGatewayScaleUnit의 변경으로 업데이트할 수 있습니다.
...
**Update-AzP2sVpnGateway** cmdlet을 사용하면 VirtualHub의 기존 P2SVpnGateway를 새 VpnClientAddressPool로 업데이트할 수 있습니다. 지점 지점 클라이언트가 이 P2SVpnGateway와 연결하면 이 VpnClientAddressPool의 IP 주소 중 하나는 해당 클라이언트에 할당됩니다.

### 예제 2

사이트 연결 지점을 위해 VirtualHub에서 기존 P2SVpnGateway를 업데이트합니다. (자동 재생)
...
이 P2SVpnGateway 연결에 대한 인터넷 보안 플래그 비활성화
...
이 P2SVpnGateway Vpn 서버 구성 개체의 ID가 연결됩니다.
