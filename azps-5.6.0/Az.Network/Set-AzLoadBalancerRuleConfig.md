...
## 설명
**Set-AzLoadBalancerRuleConfig** cmdlet은 부하 분산기에 대한 규칙 구성을 업데이트합니다.
...
두 번째 명령은 파이프라인 연산자를 사용하여 부하 분산 $slb Add-AzLoadBalancerRuleConfig에 전달합니다. 이 명령은 NewRule이라는 규칙을 추가합니다.
세 번째 명령은 부하 분산을 **Set-AzLoadBalancerRuleConfig로 전달하여** 새 규칙 구성을 설정합니다.
구성은 이전 명령에서 사용하도록 설정한 부동 IP 주소를 사용하도록 설정하지 않습니다.

## 매개 변수

### -BackendAddressPool
Load Balancer 규칙과 연결하기 위해 **BackendAddressPool** 개체를 지정합니다.
...
### -LoadBalancer
부하 균형 조정기를 지정합니다.
이 cmdlet은 이 매개 변수가 지정하는 부하 저울에 대한 규칙 구성을 업데이트합니다.
...
### -LoadDistribution
부하 분산을 지정합니다.
이 매개 변수에 대해 허용되는 값은 SourceIP 및 SourceIPProtocol입니다.
