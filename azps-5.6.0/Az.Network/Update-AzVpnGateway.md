...
Azure VPN 게이트웨이는 VirtualHub 내부 사이트 연결에 대한 소프트웨어 정의 연결입니다. 이 게이트웨이는 사용자가 지정한 크기 단위에 따라 크기 조정 및 크기 조정 VPN 사이트라고 하는 분기/사이트에서 확장 가능한 게이트웨이로 연결을 설정할 수 있습니다. 각 연결은 2개의 Active-Active 구성됩니다.
...
위의 리소스 그룹인 Virtual WAN, Virtual Network, Virtual Hub는 Azure의 "testRG" 리소스 그룹에서 미국 서부에 생성됩니다. 2개 확장 단위가 있는 Virtual Hub에서 VPN 게이트웨이가 생성됩니다.

게이트웨이를 만든 후 게이트웨이를 Update-AzVpnGateway 3개 확장 단위로 업그레이드합니다.
...
게이트웨이를 만든 후 BgpPeeringAddress를 Set-AzVpnGateway 데 사용할 수 있습니다.
