...
## 설명
**Add-AzLoadBalancerInboundNatRuleConfig** cmdlet은 Azure 부하 분산기에 NAT(인바운드 네트워크 주소 변환) 규칙 구성을 추가합니다.

## 예제

### 예제 1: 부하 저울에 인바운드 NAT 규칙 구성 추가
```
...
첫 번째 명령은 MyloadBalancer라는 부하 분산 장치 를 얻은 다음 변수에 $slb.
두 번째 명령은 파이프라인 연산자를 사용하여 부하 분산 $slb **Add-AzLoadBalancerInboundNatRuleConfig에** 전달하여 부하 분산기에 인바운드 NAT 규칙 구성을 추가합니다.
마지막 명령은 Set-AzLoadBalancer를 수행하지 않는 경우 부하 분산에 변경 내용이 적용되지 않습니다.

## 매개 변수

### -BackendPort
규칙 구성과 일치하는 트래픽에 대한 백던드 포트를 지정합니다.
...
이 cmdlet은 이 매개 변수가 지정하는 부하 저울에 인바운드 NAT 규칙 구성을 추가합니다.
