...
## SYNOPSIS
애플리케이션 게이트웨이에 대한 ssl 프로필을 업데이트합니다.
...
## 설명
**Set-AzApplicationGatewaySslProfile** cmdlet은 Azure 애플리케이션 게이트웨이에 대한 ssl 프로필을 업데이트합니다.
...
첫 번째 명령은 ResourceGroup01이라는 리소스 그룹에 속하는 ApplicationGateway01이라는 애플리케이션 게이트웨이를 $AppGw 변수에 저장합니다. 두 번째 명령은 클라이언트 $AppGw 새 값으로 업데이트하기 위해 애플리케이션 게이트웨이의 ssl 프로필을 $newclientconfig.
