...
## SYNOPSIS 
VPN 클라이언트 연결당 Azure 가상 네트워크 게이트웨이의 VPN 클라이언트 연결 상태 목록 얻기
...
## 설명
연결된 모든 VPN 클라이언트 연결 상태 나열. 여기에는 vpnConnectionId vpnConnectionDuration vpnConnectionTime publicIpAddress privateIpAddress vpnUserName maxBandwidth egressPacketsTransferred egressPacketsTransferred ingressBytesTransferred maxPacketsPerSecond
...
리소스 그룹 리소스Group의 Gatewayname이라는 Azure 가상 네트워크 게이트웨이의 경우 OpenVpn을 사용하여 현재 연결된 VPN 클라이언트 연결을 검색합니다. 

### 예제 2

VPN 클라이언트 연결당 Azure 가상 네트워크 게이트웨이의 VPN 클라이언트 연결 상태 목록을 확인합니다. (자동 재생)
