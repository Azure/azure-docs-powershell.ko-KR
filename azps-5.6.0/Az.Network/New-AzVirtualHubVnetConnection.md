...
## SYNOPSIS
New-AzVirtualHubVnetConnection cmdlet은 Azure Virtual Hub에 가상 네트워크를 피어링하는 HubVirtualNetworkConnection 리소스를 만듭니다.
...
위의 리소스 그룹인 Virtual WAN, Virtual Network, Virtual Hub는 Azure의 해당 리소스 그룹에 미국 중부에 생성됩니다. 이후 가상 네트워크 연결이 만들어지며 가상 네트워크를 Virtual Hub에 피어링합니다.

### 예제 2

New-AzVirtualHubVnetConnection cmdlet은 Azure Virtual Hub에 가상 네트워크를 피어링하는 HubVirtualNetworkConnection 리소스를 만듭니다. (자동 재생)
...
```
위의 경우 새 라우팅 구성을 만들고 지정된 IP 주소로 다음 홉을 사용하여 라우팅 구성에서 정적 경로를 생성합니다. 그런 다음 이 라우팅 구성을 매개 변수로 New-AzVirtualHubVnetConnection 명령으로 전달할 수 있습니다.
