...
## SYNOPSIS
Express 경로 게이트웨이와 프레미스 익스프레스 경로 회로 피어링 간에 생성된 익스프레스 경로 연결을 업데이트합니다.
...
## 설명
**Set-AzExpressRouteConnection** cmdlet은 익스프레스 경로 게이트웨이와 프레미스 익스프레스 경로 회로 피어링 간에 생성된 익스프레스 경로 연결을 업데이트합니다.
...
위의 리소스 그룹, Virtual WAN, Virtual Network, Virtual Hub 및 ExpressRouteSite는 Azure의 "testRG" 리소스 그룹에서 미국 서부에 생성됩니다. 2개 확장 단위가 있는 Virtual Hub에서 ExpressRoute 게이트웨이가 생성됩니다.

게이트웨이를 만든 후 이 게이트웨이는 New-AzExpressRouteConnection 사용하여 프레미스 ExpressRoute 회로 피어링에 연결됩니다.

그런 다음, 연결은 Set-AzExpressRouteConnection 사용하여 다른 RoutingWeight를 Set-AzExpressRouteConnection 업데이트됩니다.
...
이 ExpressRoute Gateway 연결에 대한 인터넷 보안 사용
