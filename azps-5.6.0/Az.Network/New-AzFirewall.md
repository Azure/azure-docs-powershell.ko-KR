...
## 설명
**New-AzFirewall** cmdlet은 Azure 방화벽을 만듭니다.
...
또한 위협 인텔은 기본 모드인 경고에서 실행됩니다. 즉 악의적인 트래픽이 기록되지만 거부되지는 않습니다.
...
이 예제에서는 포트 443의 모든 HTTPS 트래픽을 허용하는 방화벽을 만듭니다.
위협 인텔은 기본 모드인 경고에서 실행됩니다. 즉 악의적인 트래픽이 기록되지만 거부되지는 않습니다.
...
이 예제에서는 10.1.2.3:80에서 10.2.3.4:8080 위협 인텔으로 예정된 모든 패킷의 대상 IP 및 포트를 변환하는 방화벽을 만들었다.

### 예제 4: 경고 모드에서 위협 인텔을 사용하여 규칙이 없는 방화벽 만들기
```powershell
$rgName = "resourceGroupName"
$vnet = Get-AzVirtualNetwork -ResourceGroupName $rgName -Name "vnet"
$pip = Get-AzPublicIpAddress -ResourceGroupName $rgName -Name "publicIpName"
...
이 예제에서는 모든 트래픽(기본 동작)을 차단하고 위협 인텔이 경고 모드에서 실행되는 방화벽을 만듭니다.
즉, 다른 규칙을 적용하기 전에 악의적인 트래픽에 대해 경고 로그가 내보이게 됩니다(이 경우 기본 규칙만 - 모두 거부)

### 예제 5: 포트 8080의 모든 HTTP 트래픽을 허용하지만 Threat Intel에서 식별된 악의적인 도메인을 차단하는 방화벽 만들기
```powershell
$rgName = "resourceGroupName"
$vnet = Get-AzVirtualNetwork -ResourceGroupName $rgName -Name "vnet"
$pip = Get-AzPublicIpAddress -ResourceGroupName $rgName -Name "publicIpName"
...
이 예제에서는 위협 인텔에 의해 악의적인 것으로 간주되지 않는 한 포트 8080의 모든 HTTP 트래픽을 허용하는 방화벽을 만듭니다.
거부 모드에서 실행되는 경우 경고와 달리 Threat Intel에서 악의적인 것으로 간주되는 트래픽은 기록될 뿐만 아니라 차단됩니다.

### 예제 6: 규칙이 없는 방화벽 만들기 및 가용성 영역
```powershell
$rgName = "resourceGroupName"
$vnet = Get-AzVirtualNetwork -ResourceGroupName $rgName -Name "vnet"
$pip = Get-AzPublicIpAddress -ResourceGroupName $rgName -Name "publicIpName"
...
### 예제 8: 특정 데이터베이스 데이터베이스에 MSSQL 트래픽을 허용하는 방화벽 SQL 만들기
```powershell
$rgName = "resourceGroupName"
$vnet = Get-AzVirtualNetwork -ResourceGroupName $rgName -Name "vnet"
$pip = Get-AzPublicIpAddress -ResourceGroupName $rgName -Name "publicIpName"
...
이 예제에서는 가상 허브 "vHub"에 연결된 방화벽을 만듭니다. 방화벽 정책 $fp 방화벽에 연결됩니다. 이 방화벽은 방화벽 정책에 언급된 규칙에 따라 트래픽을 허용/$fp. 가상 허브와 방화벽은 동일한 지역에 있습니다.
...
이 예제에서는 "99.99.99.0/24" 및 "66.66.0.0/16"을 개인 IP 범위로 처리하고 해당 주소에 대한 트래픽을 스나이트하지 않는 방화벽을 만듭니다.
...
"강제 터널링" 시나리오를 지원하기 위해 이 방화벽은 관리 트래픽에 대한 서브넷 "AzureFirewallManagementSubnet"과 관리 공용 IP 주소를 사용하게 됩니다.
...
이 예제에서는 방화벽과 동일한 리소스 그룹에 가상 네트워크 "vnet"에 연결된 방화벽을 만듭니다.
방화벽에 적용되는 규칙 및 위협 인텔리전스가 방화벽 정책에서 적용됩니다.

### 예제 14: DNS 프록시 및 DNS 서버로 방화벽 만들기
```powershell
$rgName = "resourceGroupName"
$vnet = Get-AzVirtualNetwork -ResourceGroupName $rgName -Name "vnet"
$pip = Get-AzPublicIpAddress -ResourceGroupName $rgName -Name "publicIpName"
...
이 예제에서는 방화벽과 동일한 리소스 그룹에 가상 네트워크 "vnet"에 연결된 방화벽을 만듭니다.
DNS 프록시는 이 방화벽에 대해 사용하도록 설정되어 있으며 2대의 DNS 서버가 제공됩니다. 또한 네트워크 규칙에 대한 DNS 프록시 요구가 설정되어 있으므로 FQDNS를 사용하는 네트워크 규칙이 있는 경우 DNS 프록시도 사용됩니다.

### 예제 15: 여러 개의 IPS를 사용하여 방화벽 만들기 방화벽은 Virtual Hub와 연결될 수 있습니다.
```powershell
$rgName = "resourceGroupName"
$vHub = Get-AzVirtualHub -Name "hub"
$vHubId = $vHub.Id
...
이 예제에서는 방화벽과 동일한 리소스 그룹에 가상 허브 "hub"에 연결된 방화벽을 만듭니다.
방화벽은 암시적으로 생성된 2개 공용 IPS가 할당됩니다.

### 16: 활성 FTP 허용을 사용하여 방화벽 만들기
```
$rgName = "resourceGroupName"
$vnet = Get-AzVirtualNetwork -ResourceGroupName $rgName -Name "vnet"
$pip = Get-AzPublicIpAddress -ResourceGroupName $rgName -Name "publicIpName"
...
### -Name
이 cmdlet이 만드는 Azure 방화벽의 이름을 지정합니다.
...
### -VirtualNetworkName
방화벽이 배포될 가상 네트워크의 이름을 지정합니다. 가상 네트워크 및 방화벽은 동일한 리소스 그룹에 속해야 합니다.
...
### -Zone
방화벽에서 제공해야 하는 위치를 표시하는 가용성 영역의 목록입니다.
