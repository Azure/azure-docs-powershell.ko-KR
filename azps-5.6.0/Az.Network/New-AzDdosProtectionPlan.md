...
## 설명
New-AzDdosProtectionPlan cmdlet은 DDoS 보호 계획을 만듭니다.

## 예제

### 예제 1: 새 가상 네트워크와 DDoS 보호 계획 만들기 및 연결
```
...
먼저 **New-AzDdosProtectionPlan** 명령을 사용하여 새 DDoS 보호 계획을 생성합니다.
그런 다음 **New-AzVirtualNetwork를** 사용하여 새 가상 네트워크를 만들고 **DdosProtectionPlanId** 매개 변수에서 새로 만든 계획의 ID를 지정합니다. 이 경우 가상 네트워크를 계획과 연결하기 때문에 **EnableDdoSProtection** 매개 변수를 지정할 수도 있습니다.

### 예제 2: DDoS 보호 계획 만들기 및 기존 가상 네트워크와 연결
```
D:\> $ddosProtectionPlan = New-AzDdosProtectionPlan -ResourceGroupName ResourceGroupName -Name DdosProtectionPlanName -Location "West US"
...
먼저 **New-AzDdosProtectionPlan** 명령을 사용하여 새 DDoS 보호 계획을 생성합니다.
둘째, 계획과 연결하려는 가상 네트워크의 가장 업데이트된 버전을 얻습니다. 새로 만든 계획의 ID에 대한 참조가 포함된 **PSResourceId** 개체로 **속성 DdosProtectionPlan을** 업데이트합니다. 이 경우 가상 네트워크를 DDoS 보호 계획과 연결하면 **EnableDdosProtection** 플래그를 true로 설정할 수도 있습니다.
마지막으로 로컬 변수를 **Set-AzVirtualNetwork** 에 피핑하여 새 상태를 유지합니다.
