...
### 예제 2: 클라이언트 프로토콜 및 eviction 정책 업데이트
```powershell
...
### -EvictionPolicy
Redis eviction 정책 - 기본값은 VolatileLRU입니다.
...
생성을 위해 MODULE 속성에 대한 메모 섹션을 참조하고 해시 테이블을 만드십시오.
...
모듈 <IModule[]>: 이 데이터베이스에서 사용하도록 설정하는 redis 모듈의 선택적 집합 - 모듈은 생성 시에만 추가할 수 있습니다.
