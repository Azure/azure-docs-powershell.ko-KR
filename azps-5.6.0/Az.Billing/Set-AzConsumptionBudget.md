...
## 설명
Set-AzConsumptionBudget cmdlet은 구독 또는 리소스 그룹에서 예산을 업데이트합니다.
...
### 예제 2: 비용 또는 사용량이 구독 수준에서 금액의 90%에 도달하면 알림으로 예산을 업데이트합니다.
```powershell
...
### -ContactEmail
임계값을 초과하는 경우 예산 알림을 보낼 전자 메일 주소입니다.
...
### -NotificationEnabled
알림이 사용하도록 설정되어 있습니다.
지정하지 않으면 알림을 기본적으로 사용하지 않도록 설정됩니다.
...
### -NotificationKey
알림이 설정된 스위치, 알림 임계값, 연락처 전자 메일, 연락처 그룹 또는 연락처 역할을 사용하여 알림을 만드는 데 필요한 예산과 연결된 알림의 키입니다.
...
### -StartDate
예산 기간의 시작 날짜(UTC의 YYYY-MM-DD)입니다.
월별 시간 그레인에 대한 현재 월 이전이 아니었다.
분기별 시간 그레인에 대한 3개월 이전에는 없습니다.
12개월 이전에는 12개월 동안의 시간 입자가 없습니다.
...
### -TimeGrain
예산의 시간 입자는 월별, 분기별 또는 연간일 수 있습니다.
