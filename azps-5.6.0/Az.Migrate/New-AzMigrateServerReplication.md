...
보호해야 하는 단일 디스크가 하나만 있는 시나리오를 위한 것입니다.
...
보호해야 하는 디스크가 여러 개 있는 시나리오를 위한 것입니다.
...
생성을 위해 DISKTOINCLUDE 속성에 대한 노트 섹션을 참조하고 해시 테이블을 만드십시오.
...
서버 개체는 cmdlet을 사용하여 Get-AzMigrateServer 수 있습니다.
...
### -LicenseType
원본 서버에 마이그레이션할 Azure Hybrid 혜택이 적용 가능한지 지정합니다.
...
### -PerformAutoResync
복제에서 원본 서버에 대한 변경 내용 추적이 손실될 경우 복제를 자동으로 복구할지 지정합니다.
...
VM 생성에 사용할 가용성 집합을 지정합니다. VM 만들기에 사용할 가용성 집합을 지정합니다.
...
DISKTOINCLUDE <IVMwareCbtDiskInput[]>: 복제에 포함될 원본 서버의 디스크를 지정합니다.
  ...
  - `LogStorageAccountId <String>`: 로그 저장소 계정은 ARM 합니다.
...
INPUTOBJECT : 마이그레이션할 검색된 서버를 <IVMwareMachine> 지정합니다. 서버 개체는 cmdlet을 사용하여 Get-AzMigrateServer 수 있습니다.
