...
지정 된 AppliedScopeType `Reservation` 를 Single 및 InstanceFlexibility로 업데이트 합니다.
...
지정 된 AppliedScopeType `Reservation` 를 Shared 및 InstanceFlexibility 있게 업데이트 합니다.
...
### -InstanceFlexibility
있는 경우의 InstanceFlexibility 값을 업데이트 `Reservation` 합니다. 지정 하지 않으면 기존 값은 변경 되지 않습니다.
