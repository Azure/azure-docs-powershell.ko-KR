...
### 예제 1: Onboards 연결 된 컴퓨터로 현재 컴퓨터
```powershell
...
Onboards 컴퓨터를 연결 된 컴퓨터로 만듭니다.

### 예제 2: Onboards 원격 컴퓨터를 연결 된 장치로
```powershell
...
Onboards PowerShell remoting을 사용 하 여 원격 컴퓨터를 연결 된 장치로
참고:이 시점에는 대상 으로만 Windows가 지원 됩니다.
...
### -PSSession
이를 지정 하면 onboards에서 Azure로 작동 하는 명령이 각 PSSession 내에서 실행 됩니다.
참고:이는 현재 Windows 에서만 작동 합니다.
