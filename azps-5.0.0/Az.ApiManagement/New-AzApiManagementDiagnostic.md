...
## 설명은
Cmdlet **AzApiManagementDiagnostic** 는 전역 범위 또는 특정 Api 범위에서 진단 엔터티를 만듭니다.
...
이 예제에서는 전역 범위에서 진단 엔터티를 만듭니다.

### 예제 2: Api 범위에서 진단 만들기
```powershell
PS D:\github\azure-powershell> $context = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
위의 예제에서는 API에 대 한 진단 유틸리티를 만들어 `httpbin` 헤더 및 100 바이트의 본문을 `azuremonitor` 로 거에 기록 합니다.
