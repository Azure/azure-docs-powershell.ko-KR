...
## 설명은
**새 AzApiManagementOperation** CMDLET은 API 작업을 만듭니다.

## 예제의

### 예제 1: API management 작업 만들기
```powershell
...
이 명령은 API 관리 작업을 만듭니다.

### 예제 2: 요청 및 응답 세부 정보를 사용 하 여 API 관리 작업 만들기
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
이 예제에서는 요청 및 응답 세부 정보를 사용 하 여 API 관리 작업을 만듭니다.

## 변수

### -ApiId
API management 작업의 식별자를 지정 합니다.
...
### -ApiRevision
API 개정판의 식별자입니다. 이 매개 변수는 선택 사항입니다. 지정 하지 않으면 작업이 현재 활성 api 개정판에 연결 됩니다.
...
### -요청
API management 작업의 세부 정보를 지정 합니다.
...
### -응답
사용할 수 있는 API management 작업 응답의 배열을 지정 합니다.
...
### -서식 매개 변수
매개 변수 *Urltemplate* 에 정의 된 매개 변수의 배열을 지정 합니다.
이 매개 변수를 지정 하지 않으면 *Urltemplate* 에 따라 기본값이 생성 됩니다.
...
### ApiManagement [] ServiceManagement. PsApiManagementParameter []

### ApiManagement. ServiceManagement. \ PsApiManagementRequest

### ApiManagement [] ServiceManagement. PsApiManagementResponse []
