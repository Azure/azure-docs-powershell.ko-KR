...
## 설명은
**AzPrivateDnsVirtualNetworkLink** cmdlet은 지정 된 리소스 그룹 및 개인 영역에 새 DNS (개인 도메인 이름 시스템) 가상 네트워크 링크를 만듭니다. *Name* 매개 변수에 대해 고유한 링크 이름을 지정 해야 하며, cmdlet에서 오류를 반환 합니다. *확인* 매개 변수를 사용 하 여 Windows PowerShell 변수를 $ConfirmPreference 하 여 cmdlet에서 확인을 묻는 메시지를 표시할지 여부를 제어할 수 있습니다.
...
이 명령은 지정 된 리소스 그룹에서 myzone.com 및 가상 네트워크 "myvirtualnetwork" (리소스 그룹에 이미 생성 된) 이라는 개인 DNS 영역과 연결 된 새 가상 네트워크 링크를 만든 다음이를 $Link 변수에 저장 합니다.
...
### -EnableRegistration
링크가 등록 되었는지 여부를 나타내는 Switch 매개 변수입니다.
...
### -RemoteVirtualNetworkId
다른 테 넌 트에 있는 가상 네트워크의 리소스 id입니다.
...
### -ZoneName
가상 네트워크 링크에 연결 될 영역의 이름을 지정 합니다.
