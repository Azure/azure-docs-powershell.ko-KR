...
### 예제 1: 서버 이름에 따라 PointInTime a e b를 복원 합니다.
```powershell
...
이 명령은 PointInTime a 서버 이름을 기준으로 하 여 e b를 복원 합니다.

### 예제 2: 서버 개체를 기준으로 PointInTime을 복원 합니다.
```powershell
...
이 명령을 실행할 때 서버 개체 별로 PointInTime을 복원 합니다.
...
### -RestorePointInTime
리소스가 있는 위치를 PointInTimeRestore 영역입니다.
