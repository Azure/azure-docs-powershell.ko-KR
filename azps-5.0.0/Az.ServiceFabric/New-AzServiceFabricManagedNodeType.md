...
## 설명은
특정 클러스터에 대 한 새 노드 형식 리소스를 만듭니다.
...
노드 5 개를 사용 하 여 기본 노드 유형을 만들고 배치 속성, 용량, 응용 프로그램 및 임시 포트를 지정 합니다.
...
### 용량
노드 형식의 노드에 적용 된 용량 태그는 키/값 쌍으로, 클러스터 리소스 관리자는 이러한 태그를 사용 하 여 노드의 리소스 크기를 파악 합니다.
업데이트 하면 현재 값이 재정의 됩니다.
...
### -DiskSize
GBs의 노드 형식에서 각 vm에 대 한 디스크 크기입니다.
기본 100.
...
### -배치 속성
노드 형식의 노드에 적용 되는 배치 태그로, 키/값 쌍으로 특정 서비스 (작업 부하)가 실행 되는 위치를 나타내는 데 사용 될 수 있습니다.
업데이트 하면 현재 값이 재정의 됩니다.
...
### -기본
노드 형식이 기본 인지 여부를 지정 합니다.
이 노드 형식에서는 시스템 서비스를 실행 합니다.
하나의 노드 유형을 기본으로 표시 해야 합니다.
기본 노드 유형을 삭제 하거나 기존 클러스터에 대해 변경할 수 없습니다.
...
### -VmImageOffer
Azure 가상 컴퓨터 마켓플레이스 이미지의 제안 유형입니다.
...
### -VmImagePublisher
Azure 가상 컴퓨터 마켓플레이스 이미지의 게시자입니다.
기본값: MicrosoftWindowsServer.
...
### -VmImageSku
Azure 가상 컴퓨터 마켓플레이스 이미지의 SKU입니다.
기본값: 2019-Datacenter.
...
### -VmImageVersion
Azure Virtual Machines 마켓플레이스 이미지의 버전입니다.
기본값: 최근.
