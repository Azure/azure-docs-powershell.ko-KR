---
Module Name: Az.Websites
Module Guid: cc69c625-e961-43f4-8b50-0061eba6e4b6
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.websites
Help Version: 1.0.0.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Websites/Websites/help/Az.Websites.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Websites/Websites/help/Az.Websites.md
ms.openlocfilehash: e21d0de8ee0d0fc407208e7d5ef899147670a0aa
ms.sourcegitcommit: b4a38bcb0501a9016a4998efd377aa75d3ef9ce8
ms.translationtype: MT
ms.contentlocale: ko-KR
ms.lasthandoff: 10/27/2020
ms.locfileid: "94310430"
---
# Az 모듈
## 설명은
ARM (Azure Resource Manager) 웹 앱 및 App Service 계획 명령입니다.

## Az-Websites Cmdlet
### [추가-AzWebAppAccessRestrictionRule](Add-AzWebAppAccessRestrictionRule.md)
Azure Web App에 Access Restiction 규칙을 추가 합니다.

### [추가-AzWebAppTrafficRouting](Add-AzWebAppTrafficRouting.md)
슬롯에 라우팅 규칙을 추가 합니다.

### [편집-AzWebAppBackupConfiguration](Edit-AzWebAppBackupConfiguration.md)


### [Enter-AzWebAppContainerPSSession](Enter-AzWebAppContainerPSSession.md)
지정한 사이트 또는 슬롯에 지정 된 windows 컨테이너 및 지정 된 리소스 그룹에 원격 PowerShell 세션을 엽니다.

### [Get-AzAppServicePlan](Get-AzAppServicePlan.md)
지정 된 리소스 그룹에서 Azure App Service 계획을 가져옵니다.

### [Get-AzDeletedWebApp](Get-AzDeletedWebApp.md)
구독에서 삭제 된 웹 앱을 가져옵니다.

### [Get-AzWebApp](Get-AzWebApp.md)
지정 된 리소스 그룹의 Azure 웹 앱을 가져옵니다.

### [Get-AzWebAppAccessRestrictionConfig](Get-AzWebAppAccessRestrictionConfig.md)
Azure Web App에 대 한 액세스 Restiction 구성을 가져옵니다.

### [Get-AzWebAppBackup](Get-AzWebAppBackup.md)


### [Get-AzWebAppBackupConfiguration](Get-AzWebAppBackupConfiguration.md)


### [Get-AzWebAppBackupList](Get-AzWebAppBackupList.md)


### [Get-AzWebAppCertificate](Get-AzWebAppCertificate.md)
Azure Web App 인증서를 가져옵니다.

### [Get-AzWebAppContainerContinuousDeploymentUrl](Get-AzWebAppContainerContinuousDeploymentUrl.md)
Get-AzWebAppContainerContinuousDeploymentUrl에서 컨테이너 연속 배포 url을 반환 합니다.

### [Get-AzWebAppPublishingProfile](Get-AzWebAppPublishingProfile.md)
Azure Web App 게시 프로필을 가져옵니다.

### [Get-AzWebAppSlot](Get-AzWebAppSlot.md)
Azure 웹 앱 슬롯을 가져옵니다.

### [Get-AzWebAppSlotConfigName](Get-AzWebAppSlotConfigName.md)
웹 앱 슬롯 구성 이름 목록 가져오기

### [Get-AzWebAppSlotPublishingProfile](Get-AzWebAppSlotPublishingProfile.md)
Azure 웹 앱 슬롯 게시 프로필을 가져옵니다.

### [Get-AzWebAppSnapshot](Get-AzWebAppSnapshot.md)
웹 앱에 사용할 수 있는 스냅숏을 가져옵니다.

### [Get-AzWebAppSSLBinding](Get-AzWebAppSSLBinding.md)
Azure Web App 인증서 SSL 바인딩을 가져옵니다.

### [Get-AzWebAppTrafficRouting](Get-AzWebAppTrafficRouting.md)
지정 된 슬롯 이름에 대 한 라우팅 규칙을 가져옵니다.

### [새로운 AzAppServicePlan](New-AzAppServicePlan.md)
Azure App Service 계획을 지정 된 지리적 위치에 만듭니다.

### [새로운 AzWebApp](New-AzWebApp.md)
Azure Web App을 만듭니다.

### [새로운 AzWebAppAzureStoragePath](New-AzWebAppAzureStoragePath.md)
웹 앱에 탑재할 Azure 저장소 경로를 나타내는 개체를 만듭니다. 이 매개 변수는 Set-AzWebApp (-AzureStoragePath)로 사용 하기 위한 것입니다 Set-AzWebAppSlot

### [새로운 AzWebAppBackup](New-AzWebAppBackup.md)


### [새로운 AzWebAppContainerPSSession](New-AzWebAppContainerPSSession.md)
New-AzWebAppContainerPSSession는 지정 된 사이트 또는 슬롯 및 지정 된 리소스 그룹에서 지정한 windows 컨테이너에 새 원격 PowerShell 세션을 만듭니다.

### [새로운 AzWebAppDatabaseBackupSetting](New-AzWebAppDatabaseBackupSetting.md)


### [새로운 AzWebAppSlot](New-AzWebAppSlot.md)
Azure 웹 앱 슬롯을 만듭니다.

### [새로운 AzWebAppSSLBinding](New-AzWebAppSSLBinding.md)
Azure Web App에 대 한 SSL 인증서 바인딩을 만듭니다.

### [게시-AzWebApp](Publish-AzWebApp.md)
Zipdeploy를 사용 하 여 ZIP, JAR 또는 WAR 파일에서 Azure 웹 앱을 배포 합니다. 

### [제거-AzAppServicePlan](Remove-AzAppServicePlan.md)
Azure App Service 계획을 제거 합니다.

### [제거-AzWebApp](Remove-AzWebApp.md)
Azure 웹 앱을 제거 합니다.

### [제거-AzWebAppAccessRestrictionRule](Remove-AzWebAppAccessRestrictionRule.md)
Azure 웹 앱에서 액세스 제한 규칙을 제거 합니다.

### [제거-AzWebAppBackup](Remove-AzWebAppBackup.md)


### [제거-AzWebAppSlot](Remove-AzWebAppSlot.md)


### [제거-AzWebAppSSLBinding](Remove-AzWebAppSSLBinding.md)
업로드 된 인증서에서 SSL 바인딩을 제거 합니다.

### [제거-AzWebAppTrafficRouting](Remove-AzWebAppTrafficRouting.md)
슬롯에서 라우팅 규칙을 제거 합니다.

### [다시 설정-AzWebAppPublishingProfile](Reset-AzWebAppPublishingProfile.md)


### [다시 설정-AzWebAppSlotPublishingProfile](Reset-AzWebAppSlotPublishingProfile.md)


### [다시 시작-AzWebApp](Restart-AzWebApp.md)
Azure Web App을 다시 시작 합니다.

### [다시 시작-AzWebAppSlot](Restart-AzWebAppSlot.md)


### [Restore-AzDeletedWebApp](Restore-AzDeletedWebApp.md)
삭제 된 웹 앱을 새 또는 기존 웹 앱으로 복원 합니다.

### [Restore-AzWebAppBackup](Restore-AzWebAppBackup.md)


### [Restore-AzWebAppSnapshot](Restore-AzWebAppSnapshot.md)
웹 앱 스냅숏을 복원 합니다.

### [Set-AzAppServicePlan](Set-AzAppServicePlan.md)
Azure App Service 계획을 설정 합니다.

### [Set-AzWebApp](Set-AzWebApp.md)
Azure Web App을 수정 합니다.

### [Set-AzWebAppSlot](Set-AzWebAppSlot.md)
Azure Web App 슬롯을 수정 합니다.

### [Set-AzWebAppSlotConfigName](Set-AzWebAppSlotConfigName.md)
웹 앱 슬롯 구성 이름 설정

### [시작-AzWebApp](Start-AzWebApp.md)
Azure 웹 앱을 시작 합니다.

### [시작-AzWebAppSlot](Start-AzWebAppSlot.md)
Azure Web App 슬롯을 시작 합니다.

### [중지-AzWebApp](Stop-AzWebApp.md)
Azure 웹 앱을 중지 합니다.

### [중지-AzWebAppSlot](Stop-AzWebAppSlot.md)
Azure Web App 슬롯을 중지 합니다.

### [스위치-AzWebAppSlot](Switch-AzWebAppSlot.md)
두 슬롯을 웹 앱으로 바꾸기

### [업데이트-AzWebAppAccessRestrictionConfig](Update-AzWebAppAccessRestrictionConfig.md)
Azure Web App에 대 한 주 사이트 액세스 Restiction config의 SCM 사이트에 대 한 상속을 업데이트 합니다.

### [업데이트-AzWebAppTrafficRouting](Update-AzWebAppTrafficRouting.md)
라우팅 규칙을 슬롯에 업데이트 합니다.

