...
### 예 15: Azure SQL 관리 인스턴스의 할당량을 늘리려면 할당량 지원 티켓을 만듭니다. Get-AzSupportService 및 Get-AzSupportProblemClassification을 사용 하 여 할당량 SQL 관리 되는 인스턴스 서비스 문제 분류에 대 한 올바른 Guid를 검색 합니다.
```powershell
...
### 예제 16: Customer연락처 정보 개체 대신 개별 고객 연락처 매개 변수를 지정 하 여 지원 티켓을 만듭니다. 
```powershell
...
### 예제 17: Azure에서 24 x 7 응답을 요청 하 여 지원 티켓을 만듭니다. 
```powershell
...
### 예제 18: CSP (Cloud Solution Provider) 인 경우 고객을 대신 하 여 지원 티켓을 만듭니다. CSP는 먼저 테 넌 트에 로그인 한 후 아래 예제에 표시 된 대로 고객의 테 넌 트에 로그인 해야 합니다. 그런 다음 지원 티켓을 만들 때-CSPHomeTenantId 매개 변수를 사용 하 여 홈 테 넌 트 id를 지정 해야 합니다.  
```powershell
