...
관리 디스크를 대상 리소스 그룹으로 복원 하 고 구성 정보를 고객 저장소 계정으로 복원 작업을 완료 한 후에는 가상 컴퓨터를 만들고 시작 해야 합니다. 자세한 내용은 다른 가능한 매개 변수 집합 및 매개 변수 텍스트를 참조 하세요.
...
첫 번째 명령은 RecoveryServices 자격 증명 모음을 가져와 $vault 변수에 저장 합니다.
두 번째 명령은 백업 항목을 가져온 다음 $BackupItem 변수에 저장 합니다.
...
여섯 번째 명령은 복구 시점에서 복원할 디스크를 지정 하 $restoreDiskLUNs 변수에 저장 합니다.
마지막 명령은 디스크를 DestRG 리소스 그룹의 대상 저장소 계정 DestAccount로 복원 합니다.

### 예제 2: AzureFileShare 항목의 여러 파일 복원
...
두 번째 명령은 fileshareitem 이라는 백업 항목을 가져온 다음이를 $BackupItem 변수에 저장 합니다.
세 번째 명령은 특정 백업 항목에 대 한 복구 지점 목록을 가져옵니다.
네 번째 명령은 복원할 파일을 spceifies $files 변수에 저장 합니다.
마지막 명령은 지정 된 파일을 원래 위치로 복원 합니다.

### 예제 3

백업 항목에 대 한 데이터 및 구성을 복구 시점으로 복원 합니다. 생성

```powershell
<!-- Aladdin Generated Example --> 
Restore-AzRecoveryServicesBackupItem -VaultId $vault.ID -WLRecoveryConfig <RecoveryConfigBase>
```
### 예제 4: 관리 되는 VM을 관리 디스크로 복원
...
네 번째 명령은 현재 날짜를 가져온 다음 $EndDate 변수에 저장 합니다.
다섯 번째 명령은 $StartDate 및 $EndDate 필터링 한 특정 백업 항목에 대 한 복구 지점 목록을 가져옵니다.
여섯째 명령은 지정 된 대상 리소스 그룹을 사용 하 여 디스크를 관리 되는 디스크로 복원 합니다.

### 예제 5: 관리 되는 VM을 관리 되지 않는 디스크로 복원
...
네 번째 명령은 현재 날짜를 가져온 다음 $EndDate 변수에 저장 합니다.
다섯 번째 명령은 $StartDate 및 $EndDate 필터링 한 특정 백업 항목에 대 한 복구 지점 목록을 가져옵니다.
여섯 번째 명령은 디스크를 관리 되지 않는 디스크로 복원 합니다.

### 예제 6: 원래 저장소 계정을 사용 하 여 관리 되지 않는 VM을 관리 되지 않는 디스크로 복원 합니다.
...
네 번째 명령은 현재 날짜를 가져온 다음 $EndDate 변수에 저장 합니다.
다섯 번째 명령은 $StartDate 및 $EndDate 필터링 한 특정 백업 항목에 대 한 복구 지점 목록을 가져옵니다.
여섯 번째 명령은 VM의 원래 저장소 계정을 사용 하 여 디스크를 관리 되지 않는 디스크로 복원 합니다.
...
파일 공유에서 여러 파일을 복원 하는 데 사용 됩니다. 파일 공유 내에서 복원할 항목의 경로입니다.
...
이 스위치를 사용 하 여 관리 되지 않는 디스크로 복원을 지정 합니다.
...
### -RestoreOnlyOSDisk
이 스위치를 사용 하 여 백업 된 VM의 OS 디스크만 복원
