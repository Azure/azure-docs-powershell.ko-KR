...
## 설명은
**AzRecoveryServicesAsrProtectionDirection** cmdlet은 커밋 장애 조치 작업이 완료 된 후 지정 된 Azure Site Recovery 개체의 복제 방향을 업데이트 합니다.
...
지정 된 복구 계획에 대 한 방향 업데이트 작업을 시작 하 고 작업을 추적 하는 데 사용 되는 ASR 작업 개체를 반환 합니다.
...
보호 컨테이너 매핑과 동일한 지역에서 캐시 저장소를 사용 하 여 대상 azure 지역에서 지정 된 복제 보호 항목에 대 한 방향 업데이트 작업을 시작 합니다.
...
보호 컨테이너 매핑과 제공 된 디스크 복제 구성에 정의 된 대상 azure 지역에서 지정 된 복제 보호 항목에 대 한 방향 업데이트 작업을 시작 합니다.
...
보호 컨테이너 매핑과 제공 된 디스크 복제 구성에 정의 된 대상 azure 지역에서 지정 된 암호화 된 복제 항목에 대 한 방향 업데이트 작업을 시작 합니다.
...
보호 컨테이너 매핑에 의해 정의 되 고 캐시 저장소 (VM과 동일한 지역) 및 근접 배치 그룹을 사용 하 여 대상 azure 지역에서 지정 된 복제 보호 항목에 대 한 방향 업데이트 작업을 시작 합니다.
...
### -AzureToVMware
Azure에서 vMWare로 전환 시나리오를 지정 합니다.
...
### -데이터 스토어
Vmdisk에 사용 되는 VMware 데이터 저장소입니다.
...
장애 조치 후 복구 VM으로 사용할 디스크 암호화 비밀 자격 증명 모음 ID (Azure 디스크 암호화)를 지정 합니다.
...
### -HyperVToAzure
장애 복구 후 Hyper-v 가상 컴퓨터를 다시 보호 합니다.
...
### -KeyEncryptionKeyUrl
장애 조치 후 복구 VM으로 사용할 디스크 암호화 키 URL (Azure 디스크 암호화)을 지정 합니다.
...
### -LogStorageAccountId
Vm의 복제 로그를 저장할 저장소 계정 ID를 지정 합니다.
...
이 가상 컴퓨터를 장애 조치 (failover) 할 복구 근접 배치 그룹의 리소스 ID입니다.
...
### -VmmToVmm
두 개의 VMM 관리 Hyper-v 사이트 간에 보호 되는 Hyper-v 가상 컴퓨터 장애 조치에 대 한 복제 방향을 업데이트 합니다.
...
### -VMwareToAzure
VMware에서 Azure로 복제 방향을 업데이트 합니다.
