...
## SYNOPSIS
마이그레이션 프로젝트에서 검색 된 모든 서버를 가져옵니다.
...
이름으로 마이그레이션 프로젝트에서 서버를 가져옵니다.
Name은 서버의 고유한 paramenter입니다.
...
프로젝트의 기기에 대 한 서버를 가져옵니다.
Name은 서버의 고유한 paramenter입니다.

### 예제 5: 표시 이름별 목록 및 필터링
```powershell
...
마이그레이션 프로젝트의 서버를 나열 하 고 표시 이름으로 응답을 필터링 합니다.

### 예제 6: 기기의 목록과 표시 이름으로 필터링
```powershell
...
프로젝트 마이그레이션에 기기의 서버를 나열 하 고 표시 이름을 사용 하 여 응답을 필터링 합니다.
...
### -이름
VMware 컴퓨터 이름을 지정 합니다.
내부 이름입니다.
사용자의 경우 표시 이름을 사용 합니다.
...
### Api202001. IVMwareMachine의. PowerShell for.
