...
## 설명은
**AzSqlDatabaseImportExportStatus** cmdlet은 저장소 계정에서 Azure sql 데이터베이스로의 bacpac 파일 가져오기에 대 한 세부 정보를 가져오거나 저장소 계정에 대 한 Bacpac 파일로 Azure sql 데이터베이스 내보내기를 가져옵니다.
이 cmdlet은 Azure의 SQL Server 스트레치 데이터베이스 서비스 에서도 지원 됩니다.

## 예제의

### 예제 1: SQL 데이터베이스의 가져오기 및 내보내기 상태 가져오기
```
...
이 명령은 지정 된 URL에서 데이터베이스에 대 한 가져오기 또는 내보내기 요청의 상태를 가져옵니다.
...
### -OperationStatusLink
New-AzSqlDatabaseExport 또는 New-AzSqlDatabaseImport cmdlet에서 반환 되는 상태 링크를 지정 합니다.
...
### AzureSqlDatabaseImportExportStatusModel을 (를) 내보냅니다.
