...
### UseResourceGroupAndServerNameParameterSet (기본값)
```
...
## 설명은
**AzSqlServerActiveDirectoryOnlyAuthentication** cmdlet은 현재 구독의 AzureSQL 서버에 대 한 azure AD (Active Directory) 인증 요구 사항만 사용할 수 없도록 합니다.
...
이 명령은 ResourceGroup01 이라는 리소스 그룹과 연결 된 Server01 이라는 AzureSQL server에 대해 azure AD (Active Directory)만 인증 요구 사항을 사용 하지 않도록 설정 합니다.
...
### -ServerName
Azure Active Directory만 인증을 하는 Azure SQL Server의 이름입니다.
...
### ServerActiveDirectoryAdministrator. AzureSqlServerActiveDirectoryOnlyAuthenticationModel에 대 한
