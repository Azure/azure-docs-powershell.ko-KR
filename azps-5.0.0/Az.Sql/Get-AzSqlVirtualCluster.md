...
### GetVirtualClusterByResourceGroup (기본값)
```
...
## 설명은
**AzSqlVirtualCluster** cmdlet은 하나 이상의 Azure SQL 가상 클러스터에 대 한 정보를 반환 합니다.
해당 클러스터에 대 한 정보만 보려면 가상 클러스터의 이름을 지정 합니다.

## 예제의

### 예제 1: 리소스 그룹에 할당 된 모든 가상 클러스터 가져오기
```powershell
...
이 명령은 리소스 그룹 ResourceGroup01에 할당 된 모든 가상 클러스터에 대 한 정보를 가져옵니다.

### 예제 2: 특정 가상 클러스터에 대 한 정보 가져오기
```
...
이 명령은 리소스 그룹 ResourceGroup01의 가상 클러스터 VirtualCluster1에 대 한 정보를 가져옵니다.
...
### AzureSqlVirtualClusterModel/. m a m.
