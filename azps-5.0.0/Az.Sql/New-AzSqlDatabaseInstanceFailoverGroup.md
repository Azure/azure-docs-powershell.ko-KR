...
## 설명은
지정 된 영역 간에 관리 되는 인스턴스 쌍이 있는 새 Azure SQL 데이터베이스 인스턴스 장애 조치 그룹을 만듭니다.

SqlDatabaseDnsSuffix (예: Name.database.windows.net) 및 SqlDatabaseDnsSuffix와 같은 두 개의 Azure SQL 데이터베이스 TDS 끝점을 만듭니다. 이러한 끝점은 장애 조치 그룹의 기본 영역과 보조 영역에 각각 연결 하는 데 사용 될 수 있습니다. 기본 영역이 작동 중지의 영향을 받는 경우 끝점 및 데이터베이스의 자동 장애 조치는 인스턴스 장애 조치 그룹의 장애 조치 정책 및 유예 기간에 따라 트리거됩니다.
...
이 명령은 관리 되는 인스턴스 쌍에 대해 장애 조치 정책 ' 자동 '을 사용 하 여 새 인스턴스 장애 조치 (Failover) 그룹을 만듭니다.
...
이 명령은 관리 되는 인스턴스 쌍에 대해 장애 조치 정책 ' 수동 '을 사용 하 여 새 인스턴스 장애 조치 (Failover) 그룹을 만듭니다.

### 예제 3

이 명령은 새 Azure SQL 데이터베이스 인스턴스 장애 조치 (Failover) 그룹을 만듭니다. 생성
...
보조 서버의 작동 중지가 읽기 전용 끝점의 자동 장애 조치를 트리거해야 하는지 여부
이 기능은 아직 지원 되지 않습니다.
...
### -단일 기능을 위한 인스턴스
파트너 영역에서 인스턴스 장애 조치 (Failover) 그룹에 추가할 관리 되는 인스턴스의 이름입니다.
...
### -또는 지역
인스턴스 장애 조치 (Failover) 그룹의 파트너 영역 이름입니다.
...
인스턴스 장애 조치 (Failover) 그룹의 보조 리소스 그룹 이름입니다.
...
### -(또는)-(Subscriptionid)
인스턴스 장애 조치 (Failover) 그룹의 보조 관리 인스턴스에 대 한 구독 id입니다. 이 매개 변수는 교차 구독 설정에만 필요 합니다.
...
로컬 지역에서 인스턴스 장애 조치 (Failover) 그룹에 추가할 관리 되는 인스턴스의 이름입니다.
