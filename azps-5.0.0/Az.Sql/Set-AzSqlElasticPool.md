...
## 설명은
**AzSqlElasticPool** Cmdlet은 Azure SQL Database의 탄력적 풀에 대 한 속성을 설정 합니다. 이 cmdlet은 *Dtu* (eDTUs), 풀 당 저장소 최대 크기 ( *storagemb* ), 데이터베이스당 최대 eDTUs ( *DatabaseDtuMax* ), 데이터베이스당 최소 eDTUs ( *databasedtumin* )를 수정할 수 있습니다.
...
Azure SQL 데이터베이스에서 탄력적 데이터베이스 풀의 속성을 수정 합니다. 생성
...
### -StorageMB
탄력적 풀의 저장소 용량 한도 (mb)를 지정 합니다. 자세한 내용은 New-AzSqlElasticPool cmdlet을 참조 하세요.
...
### 태그
이 cmdlet이 해시 테이블 형식으로 탄력적 풀에 연결 하는 키-값 쌍의 사전을 지정 합니다. 예를 들어: `@{key0="value0";"key 1"=$null;key2="value2"}`
