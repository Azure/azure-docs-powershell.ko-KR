...
### 예제 1: 이름별로 MySql 가상 네트워크 규칙 업데이트
```powershell
...
이 cmdlet은 이름으로 MySql 가상 네트워크 규칙을 업데이트 합니다.

### 예제 2: id를 기준으로 MySql 가상 네트워크 규칙을 업데이트 합니다.
```powershell
...
이러한 cmdlet은 id를 기준으로 MySql 가상 네트워크 규칙을 업데이트 합니다.
...
### IMySqlIdentity (Microsoft. PowerShell)

## 출력

### Api20171201. IVirtualNetworkRule에 대 한 Microsoft.
...
INPUTOBJECT <IMySqlIdentity> : Identity 매개 변수
