...
이 cmdlet은 이름으로 MySql 방화벽 규칙을 업데이트 합니다.

### 예제 2: id를 기준으로 MySql 방화벽 규칙을 업데이트 합니다.
```powershell
...
이러한 cmdlet은 id를 기준으로 MySql 방화벽 규칙을 업데이트 합니다.

### 예제 3:-ClientIPAddress를 기준으로 MySql 방화벽 규칙을 업데이트 합니다.
```powershell
PS C:\> $ID = "/subscriptions/<SubscriptionId>/resourceGroups/PowershellMySqlTest/providers/Microsoft.DBforMySQL/servers/mysql-test/firewallRules/rule"
...
이러한 cmdlet은 MySql 방화벽 규칙을-ClientIPAddress로 업데이트 합니다.
