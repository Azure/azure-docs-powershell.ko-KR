...
## 설명은
New-AzDdosProtectionPlan cmdlet은 DDoS protection 계획을 만듭니다.

## 예제의

### 예제 1: DDoS protection 계획 만들기 및 새 가상 네트워크에 연결
```
...
먼저 **새로운-AzDdosProtectionPlan** 명령을 사용 하 여 새 DDoS Protection 계획을 만듭니다.
그런 다음 **new-AzVirtualNetwork** 를 사용 하 여 새 가상 네트워크를 만들고, **DdosProtectionPlanId** 매개 변수에 새로 생성 된 계획의 ID를 지정 합니다. 이 경우 가상 네트워크를 계획과 연결 하기 때문에 **EnableDdoSProtection** 매개 변수를 지정할 수도 있습니다.

### 예제 2: DDoS protection 계획 만들기 및 기존 가상 네트워크에 연결
```
D:\> $ddosProtectionPlan = New-AzDdosProtectionPlan -ResourceGroupName ResourceGroupName -Name DdosProtectionPlanName -Location "West US"
...
먼저 **새로운-AzDdosProtectionPlan** 명령을 사용 하 여 새 DDoS Protection 계획을 만듭니다.
두 번째로, 계획과 연결 하려는 가상 네트워크의 업데이트 된 버전을 가져옵니다. 새로 만든 계획의 ID에 대 한 참조를 포함 하는 **Psresourceid** 개체를 사용 하 여 **DdosProtectionPlan** 속성을 업데이트 합니다. 이 경우 가상 네트워크를 DDoS protection 계획과 연결 하는 경우 **EnableDdosProtection** 플래그를 true로 설정할 수도 있습니다.
마지막으로 지역 변수를 **Set-AzVirtualNetwork** 에 파이핑 하 여 새 상태를 유지 합니다.
...
### -ResourceGroupName
만들려는 DDoS protection 계획의 리소스 그룹을 지정 합니다.
