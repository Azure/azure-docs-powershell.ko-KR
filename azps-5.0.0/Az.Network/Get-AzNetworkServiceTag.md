...
## 설명은
**Get-AzNetworkServiceTag** cmdlet은 서비스 태그 정보 리소스 목록을 가져옵니다.

지정 하는 Azure 지역 정보는 위치를 기반으로 하는 필터가 아니라 버전에 대 한 참조로 사용 됩니다. 예를 들어 사용자가 지정 하는 경우에도 `-Location eastus2` 모든 지역에서 접두사 세부 정보를 사용 하 여 서비스 태그 목록을 가져오고 구독이 속해 있는 클라우드로 제한 (예: 공개, 미국 정부, 중국 또는 독일) 할 수 있습니다.
...
명령에서 서비스 태그 정보 리소스 목록을 가져와 변수에 저장 `serviceTags` 합니다.

### 예제 2: AzureSQL의 모든 주소 접두사 가져오기
```powershell
PS C:\> $serviceTags = Get-AzNetworkServiceTag -Location eastus2
...
첫 번째 명령은 서비스 태그 정보 리소스 목록을 가져와 변수에 저장 합니다 `serviceTags` .
두 번째 명령은 목록에서 Sql에 대 한 정보 리소스를 선택 하도록 필터링 합니다.

### 예제 3: 미국 2에 대 한 저장소 서비스 태그 정보 리소스 가져오기
```powershell
PS C:\> $serviceTags = Get-AzNetworkServiceTag -Location eastus2
...
첫 번째 명령은 서비스 태그 정보 리소스 목록을 가져와 변수에 저장 합니다 `serviceTags` .
다음 명령은 US 2에서 저장소에 대 한 서비스 태그 정보를 선택 하도록 목록을 필터링 하는 다양 한 방법을 보여 줍니다.

### 예제 4: 모든 글로벌 서비스 태그 정보 리소스 가져오기
```powershell
PS C:\> $serviceTags = Get-AzNetworkServiceTag -Location eastus2
...
첫 번째 명령은 서비스 태그 정보 리소스 목록을 가져와 변수에 저장 합니다 `serviceTags` .
두 번째 명령은 목록에서 set region이 없는 필드만 선택 하도록 필터링 합니다.
...
### -위치
위치를 기반으로 하는 필터가 아니라 버전에 대 한 참조로 사용할 위치 이며, 모든 지역에서 접두사 세부 정보를 사용 하 여 서비스 태그 목록을 가져올 수 있지만 구독이 속해 있는 클라우드로 제한 됩니다.
...
### Microsoft. 네트워크 모델. PSNetworkServiceTag
