...
## SYNOPSIS
VirtualHub와 연결 된 허브 경로 테이블 리소스를 만듭니다.
...
## 설명은
제공 된 경로와 레이블을 사용 하 여 지정 된 가상 허브에 연결 된 지정 된 경로 테이블을 만듭니다.
