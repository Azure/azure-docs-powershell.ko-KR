...
New-AzVpnGateway는 확장 가능한 VPN 게이트웨이를 만듭니다. 이는 VirtualHub 내 사이트 간 연결에 대 한 소프트웨어 정의 연결입니다. 

이 게이트웨이에는이 게이트웨이의 크기를 조정 하 고이를 Set-AzVpnGateway cmdlet에 지정 된 배율 단위를 기준으로 확장 합니다. 

VPNSite로 알려진 지사/사이트에서 확장 가능한 게이트웨이로 연결이 설정 됩니다. 각 연결은 2 개의 Active-Active 터널로 구성 됩니다.

VpnGateway는 참조 되는 VirtualHub와 같은 위치에 있게 됩니다.
