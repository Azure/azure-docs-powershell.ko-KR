...
## 설명은
**AzExpressRouteConnection** cmdlet은 express 경로 게이트웨이와 온-프레미스 express 경로 회로 피어 링 간에 생성 되는 express 경로 연결을 업데이트 합니다.
...
위의 예는 Azure의 "testRG" 리소스 그룹에 미국 내에서 리소스 그룹, 가상 WAN, 가상 네트워크, 가상 허브 및 ExpressRouteSite을 만듭니다. 1 개의 배율 단위를 사용 하 여 가상 허브에 Express 경로 게이트웨이가 생성 됩니다.

게이트웨이가 생성 되 면 New-AzExpressRouteConnection 명령을 사용 하 여 온-프레미스 Express 경로 회로 피어 링에 연결 됩니다.

그런 다음 Set-AzExpressRouteConnection 명령을 사용 하 여 다른 RoutingWeight를 포함 하도록 연결을 업데이트 합니다.
...
### -AuthorizationKey
Express 경로 게이트웨이 연결을 만드는 데 사용할 인증 키입니다.
...
### -InputObject
업데이트할 ExpressRouteConnection 개체입니다.
...
### -ResourceId
삭제할 ExpressRouteConnection 개체의 리소스 id입니다.
...
### -RoutingWeight
패킷 라우팅에 대해이 연결에 할당 해야 하는 가중치입니다.
