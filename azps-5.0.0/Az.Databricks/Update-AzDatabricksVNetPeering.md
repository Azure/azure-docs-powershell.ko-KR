...
### 예제 1: vnet 피어 링 업데이트 AllowForwardedTraffic
```powershell
...
이 명령은 vnet 피어 링의 AllowForwardedTraffic를 업데이트 합니다.

### 예제 2: 개체에의 한 vnet 피어 링 업데이트 AllowForwardedTraffic
```powershell
...
이 명령은 개체에의 한 vnet 피어 링의 AllowForwardedTraffic를 업데이트 합니다.

## 변수

### -AllowForwardedTraffic
[System.webserver 매개 변수] 로컬 가상 네트워크의 Vm 으로부터 전달 된 트래픽이 원격 가상 네트워크에서 허용/허용 되지 않을 지 여부
...
### -Allow게이트웨이 전송
원격 가상 네트워킹에서 게이트웨이 링크를 사용 하 여이 가상 네트워크에 연결할 수 있는 경우 [system.webserver]. e \ 스위치 매개 변수
...
[System.webserver 매개 변수] 로컬 가상 네트워크 공간의 Vm이 원격 가상 네트워크 공간에서 Vm에 액세스할 수 있는지 여부를 지정 합니다.
...
### -UseRemoteGateway
이 가상 네트워크에서 원격 게이트웨이를 사용할 수 있는 경우 [system.webserver] 스위치 매개 변수를 사용 합니다.
플래그를 true로 설정 하 고 원격 피어 링에 대 한 Allow네트워크도 참이 면 가상 네트워크는 전송에 원격 가상 네트워크 게이트웨이를 사용 합니다.
한 번만 피어 링이이 플래그를 true로 설정할 수 있습니다.
가상 네트워크에 이미 게이트웨이가 있는 경우이 플래그를 설정할 수 없습니다.
...
### Databricks. IDatabricksIdentity에 대 한 Microsoft PowerShell

## 출력

### Databricks. Api20180401. IVirtualNetworkPeering에 대 한
...
INPUTOBJECT <IDatabricksIdentity> : Identity 매개 변수입니다.
  - `[Id <String>]`: 리소스 id 경로
  - `[PeeringName <String>]`: 작업 영역 vNet 피어 링의 이름입니다.
  - `[ResourceGroupName <String>]`: 리소스 그룹의 이름입니다. 이름은 대/소문자를 구분 합니다.
  - `[SubscriptionId <String>]`: 대상 구독의 ID입니다.
  - `[WorkspaceName <String>]`: 작업 영역의 이름입니다.
