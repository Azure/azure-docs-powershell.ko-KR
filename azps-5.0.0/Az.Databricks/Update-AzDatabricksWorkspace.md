...
### 예제 1: Databricks 작업 영역의 태그 업데이트
```powershell
...
이 명령은 Databricks workspace의 태그를 업데이트 합니다.

### 예제 2: Databricks 작업 영역에서 암호화 사용
```powershell
...
Databricks 작업 영역에서 암호화를 사용 하도록 설정 하는 단계는 다음과 같이 세 가지입니다.
1.
작업 영역을 `-PrepareEncryption` (만들지 않은 경우)으로 업데이트 합니다.
1.
`StorageAccountIdentityPrincipalId`마지막 단계의 출력을 찾습니다.
보안 주체에 키 사용 권한을 부여 합니다.
1.
작업 영역을 다시 업데이트 하 여 암호화 키에 대 한 정보를 입력 합니다.
    - `-EncryptionKeySource`
    - `-EncryptionKeyVaultUri`
    - `-EncryptionKeyName`
    - `-EncryptionKeyVersion`

### 예제 3: Databricks 작업 영역에서 암호화 사용 안 함
```powershell
...
암호화를 사용 하지 않으려면 `-EncryptionKeySource` 로 설정 `'Default'` 합니다.
...
### -EncryptionKeySource
암호화 keySource (공급자)입니다.
사용할 수 있는 값 (대/소문자 구분 안 함): 기본값, Microsoft. Keyvault
...
### -PrepareEncryption
암호화를 위해 작업 영역을 준비 합니다.
관리 되는 저장소 계정에 관리 되는 Id를 사용 하도록 설정 합니다.
...
### Databricks. Api20180401에 대 한 작업 영역
