...
## 설명은
이 명령은 Azure 파일 동기화 클라우드 끝점을 만듭니다. 클라우드 끝점은 기존 Azure 파일 공유에 대 한 참조입니다. 이는 파일 공유를 나타내며 클라우드 끝점을 만든 동기화 그룹의 모든 파일 부분을 동기화 하는 참여를 정의 합니다.
...
클라우드 끝점은 동기화 그룹의 정수 계열 구성원이 며,이는 "mySyncGroupName" 이라는 기존 동기화 그룹 내에 하나를 만드는 예제입니다.
...
### -이름
클라우드 끝점의 이름입니다. Azure 포털을 통해 생성 되 면 이름이 참조 하는 Azure 파일 공유의 이름으로 설정 됩니다.
