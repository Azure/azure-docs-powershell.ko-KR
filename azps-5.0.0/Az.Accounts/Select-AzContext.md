...
## 설명은
Azure PowerShell cmdlet에서 대상 (또는 계정 또는 테 넌 트)에 대 한 구독을 선택 합니다.  이 cmdlet은 이후 cmdlet이 선택한 컨텍스트를 대상으로 합니다.

## 예제의

### 예제 1: 명명 된 컨텍스트 대상 지정
```powershell
...
후속 Azure PowerShell cmdlet을 ' Work ' 컨텍스트에서 계정, 테 넌 트 및 구독에서 대상으로 지정 합니다.

### 예제 2

이 PowerShell 세션의 Azure PowerShell cmdlet에서 대상으로 할 구독 및 계정을 선택 합니다. 생성
