...
### 피어 링을 얻고 등록 된 접두사 만들기
```powershell
PS C:\>$peering = Get-AzPeering -ResourceGroupName $resourceGroupName -Name $name
PS C:\>$peering | New-AzPeeringRegisteredPrefix -Name $asnName -Asn $asn
```

등록 된 접두사를 추가 하려는 피어 링을 가져옵니다. 그런 다음이를 이상으로 전달 합니다.

### 피어 링 resourceId를 사용 하 여 등록 된 asn 만들기
```powershell
...
### PSPeeringRegisteredPrefix (Microsoft. PowerShell.
