...
## 설명은
**AzCognitiveServicesAccountNetworkRule** Cmdlet은 인지 서비스 계정의 networkrule 속성에서 IpRules 또는 VirtualNetworkRules을 제거 합니다.

## 예제의

### 예제 1: IPAddressOrRange를 사용 하 여 여러 IpRules 제거
```
...
이 명령은 IPAddressOrRange로 여러 IpRules를 제거 합니다.

### 예제 2: JSON을 사용 하 여 VirtualNetworkRule 개체 입력을 사용 하 여 VirtualNetworkRule 제거
```
...
이 명령은 JSON에서 VirtualNetworkRule 개체 입력을 사용 하 여 VirtualNetworkRule를 제거 합니다.

### 예제 3: 파이프라인을 사용 하 여 첫 번째 IpRule 제거
```
...
이 명령은 파이프라인을 사용 하 여 첫 번째 IpRule를 제거 합니다.

### 예제 4: VirtualNetworkResourceID를 사용 하 여 여러 VirtualNetworkRules 제거
```
...
이 명령은 VirtualNetworkResourceID로 여러 VirtualNetworkRules를 제거 합니다. 
...
### -IpAddressOrRange
인지 서비스 계정 네트워크 규칙 IpRules IpAddressOrRange in 문자열.
...
인지 서비스 계정 네트워크 규칙 VirtualNetworkRules VirtualNetworkResourceId in 문자열.
...
### -VirtualNetworkRule
인지 서비스 계정 네트워크 규칙 VirtualNetworkRules.
...
### CognitiveServices를 PSIpRule []으로 합니다.

### CognitiveServices를 PSVirtualNetworkRule []으로 합니다.

## 출력

### CognitiveServices. PSVirtualNetworkRule/. *

### CognitiveServices. PSIpRule/. *
