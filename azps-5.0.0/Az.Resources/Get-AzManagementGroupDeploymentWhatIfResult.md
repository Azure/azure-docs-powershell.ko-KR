...
## 설명은
**AzManagementGroupDeploymentWhatIfResult** cmdlet은 지정 된 관리 그룹 범위에서 템플릿 배포에 대 한 결과 What-If 팔 템플릿을 가져옵니다. 이 메서드는 실제 리소스를 변경 하지 않고 배포를 적용 한 경우 업데이트 되는 리소스를 표시 하는 변경 목록을 반환 합니다. 반환 되는 결과의 형식을 지정 하려면 *Resultformat* 매개 변수를 사용 합니다.

## 예제의

### 예제 1: 관리 그룹 범위에서 What-If 결과 가져오기
```powershell
...
이 명령은 사용자 지정 템플릿 파일과 디스크 상의 매개 변수 파일을 사용 하 여 관리 그룹 범위에서 What-If 결과를 가져옵니다.
이 명령은 *Location* 매개 변수를 사용 하 여 배포 데이터를 저장할 위치를 지정 합니다.
이 명령은 *Managementgroupid* 매개 변수를 사용 하 여 템플릿이 배포 될 관리 그룹을 지정 합니다.
...
### 예제 2: ResourceIdOnly를 사용 하 여 관리 그룹 범위에서 What-If 결과 가져오기
```powershell
PS C:\> Get-AzManagementGroupDeploymentWhatIfResult `
    -DeploymentName "deploy-01" `
    -Location "West US" `
...
### -이름
만들려는 배포의 이름입니다.
지정 하지 않으면 템플릿 파일이 제공 될 때 기본적으로 템플릿 파일 이름이 만들어집니다. 템플릿 개체가 제공 되는 현재 시간을 기본값으로 설정 합니다 (예: "20131223140835").
