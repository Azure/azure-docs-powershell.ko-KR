...
## 설명은
**AzStorageBlobRestorePolicy** Cmdlet은 Azure Storage blob 서비스에 대 한 Blob Restore 정책을 사용 하지 않도록 설정 합니다.

## 예제의

### 예제 1: 저장소 계정에서 Azure Storage Blob 서비스에 대 한 Blob Restore 정책 사용 안 함
```powershell
PS C:\> Disable-AzStorageBlobRestorePolicy -ResourceGroupName "myresourcegoup" -StorageAccountName "mystorageaccount"
```

이 명령은 저장소 계정에서 Azure Storage Blob 서비스에 대 한 Blob Restore 정책을 사용 하지 않도록 설정 합니다.
