...
## 설명은
**AzStorageBlobCopyState** Cmdlet은 Azure 저장소 blob의 복사 상태를 가져옵니다.
복사 대상 blob에서 실행 되어야 합니다.
...
이 명령은 **AzStorageBlob** cmdlet을 사용 하 여 ContosoUploads 이라는 컨테이너에서 ContosoPlanning2015 라는 blob을 가져온 다음 파이프라인 연산자를 사용 하 여 현재 cmdlet에 결과를 전달 합니다.
**AzStorageBlobCopyState** cmdlet은 해당 blob에 대 한 복사 상태를 가져옵니다.
...
이 명령은 **Get-AzStorageBlob** cmdlet을 사용 하 여 명명 된 컨테이너를 가져온 다음 결과를 현재 cmdlet으로 전달 합니다.
**AzStorageContainer** cmdlet은 해당 컨테이너의 ContosoPlanning2015 라는 blob에 대 한 복사 상태를 가져옵니다.

### 예제 4: 복사 및 파이프라인을 시작 하 여 복사 상태 가져오기
```
...
첫 번째 명령은 blob "ContosoPlanning2015"를 "ContosoPlanning2015_copy"으로 복사 하 고 destiantion blob 개체를 출력 합니다. 두 번째 명령 파이프라인은 destiantion blob 개체를 가져오고 AzStorageBlobCopyState를 통해 blob 복사 상태를 가져옵니다. 
