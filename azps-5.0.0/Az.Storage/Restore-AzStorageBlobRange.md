...
## 설명은
**AzStorageBlobRange** cmdlet은 특정 blob 범위에 대해 저장소 계정에 blob을 복원 합니다.
시작 범위가 포함 되며, 끝 범위가 blob restore에서 제외 됩니다.

## 예제의

### 예제 1: 시작 특정 blob 범위를 사용 하 여 저장소 계정에 blob 복원
```powershell
...
이 명령은 먼저 2 개의 blob 범위를 만든 다음 start는 1 일 전 2 개의 blob 범위를 사용 하 여 저장소 계정에 blob을 복원 합니다. 사용자는 Get-AzStorageAccount를 사용 하 여 나중에 복원 상태를 추적할 수 있습니다.

### 예제 2: 백엔드의 저장소 계정에 있는 모든 blob 복원
```powershell
...
이 명령은 저장소 계정의 모든 blob을 30 분 전에 복원 하 고 복원이 완료 될 때까지 기다립니다. Blob 복원에 시간이 오래 걸릴 수 있으므로-Asjob 매개 변수를 사용 하 여 백엔드에서 실행 한 다음 작업이 완료 될 때까지 기다렸다가 결과를 표시 합니다.

### 예제 3: 입력 blob 범위를 기준으로 blob을 직접 복원 하 고 완료 될 때까지 기다립니다.
```powershell
...
이 명령은 1 일 전부터 저장소 계정의 blob을 복원 하 고, 입력 2 blob는 Restore-AzStorageBlobRange cmdlet으로 직접 범위를 지정 합니다. 이 명령은 복원이 완료 될 때까지 기다립니다.
...
### -BlobRestoreRange
복원할 blob 범위입니다.
이 매개 변수를 지정 하지 않으면 모든 blob이 복원 됩니다.
...
### PSBlobRestoreStatus를 통해 관리 합니다.
