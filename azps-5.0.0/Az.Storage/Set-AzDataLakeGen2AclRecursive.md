...
## 설명은
**Set-AzDataLakeGen2AclRecursive** cmdlet은 지정 된 경로에 대해 ACL을 재귀적으로 설정 합니다. 입력 ACL이 원본 ACL을 완전히 대체 합니다.

## 예제의

### 예제 1: directiry에서 재귀적으로 ACL 설정
```
...
이 명령은 먼저 3 개의 acl 항목을 사용 하 여 ACL 개체를 만든 다음 디렉터리에서 재귀적으로 ACL을 설정 합니다.

### 예제 2: 파일 시스템의 루트 directiry에서 ACL을 재귀적으로 설정 합니다.
```
...
이 명령은 먼저 ACL을 루트 디렉터리로 재귀적으로 설정 하 고 실패 한 다음, 사용자가 실패 한 파일을 수정 하면 ContinuationToken으로 다시 시작 합니다.

### 예제 3: 청크를 재귀적으로 청크 하는 ACL 설정
```
$token = $null
...
이 스크립트는 청크 크기를 사용 하 여 디렉터리 청크에 대 한 ACL rescursively를 BatchSize * MaxBatchCount로 설정 합니다. 청크 크기는이 스크립트의 200입니다.

### 예제 4: 디렉터리와 ContinueOnFailure에서 ACL을 재귀적으로 설정한 다음 실패가 하나씩 다시 시작
```
...
이 명령은 먼저 ACL을 재귀적으로 ContinueOnFailure를 사용 하는 디렉터리로 설정 하 고 일부 항목은 실패 한 다음 실패 한 항목을 하나씩 다시 시작 합니다.
