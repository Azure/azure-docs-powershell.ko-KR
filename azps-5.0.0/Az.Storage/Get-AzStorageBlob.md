...
### 예제 5: 컨테이너의 모든 blob 가져오기 blob 버전이 포함 됩니다.
```
...
이 명령은 컨테이너의 모든 blob을 가져옵니다. blob 버전을 포함 합니다.
...
이 명령은 verion를 사용 하 여 단일 blob을 가져옵니다.

### 예제 7: 단일 blob 스냅숏 가져오기
```
...
이 명령은 SnapshotTime를 사용 하 여 단일 blob 스냅숏을 가져옵니다.
...
이 매개 변수에 대 한 값이 지정 된 경우 cmdlet은 이름이이 매개 변수와 일치 하는 모든 blob를 나열 합니다. 이 매개 변수는 문자열의 모든 위치에서 와일드 카드를 지원 합니다.
...
### -IncludeVersion
이 매개 변수가 있는 경우에만 blob 버전이 나열 됩니다. 기본적으로 blob get에는 blob 버전이 포함 되지 않습니다.
