...
이 명령은 시계열 insights 환경을 가져옵니다.

### 예제 2: 모든 시계열 insights 환경 나열
```powershell
...
이 명령은 리소스 그룹의 모든 시계열 insights 환경을 나열 합니다.

### 예제 3: 개체 별로 시계열 insights 환경 가져오기
```powershell
...
### -확장
$Expand = status 설정에는 시계열 Insights 서비스에 환경에 대 한 내부 서비스의 상태가 포함 됩니다.
