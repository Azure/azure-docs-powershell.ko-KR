...
## 설명은
**AzVMDiskEncryptionExtension** cmdlet은 실행 중인 인프라를 Azure의 IaaS (서비스) 가상 컴퓨터에서 암호화를 사용 하도록 설정 합니다.  가상 컴퓨터에 디스크 암호화 확장을 설치 하 여 암호화를 사용 하도록 설정 합니다. 
...
Linux: Linux 가상 컴퓨터를 암호화 하는 경우는 데 필요한 **etype** 매개 변수가 필요 하며, linux 배포에서 지원 되는 값 ("Os", "Data" 또는 "All")으로 설정 해야 합니다. 

Windows: 지 수 **etype** 매개 변수를 생략할 수 있으며,이 경우 작업은 기본적으로 모두 설정 됩니다. Windows 가상 컴퓨터에는 지 수 Etype 매개 변수가 있는 경우 모든 또는 OS 중 하나로 설정 되어야 합니다.
...
이 예제에서는 광고 자격 증명을 지정 하지 않고 VM에서 암호화를 사용 하도록 설정 합니다.
...
이 예제에서는 광고 자격 증명을 지정 하지 않고 파이프라인 입력을 사용 하 여 VM에서 암호화를 사용 하도록 매개 변수를 보냅니다.
...
이 예제에서는 Azure AD 클라이언트 ID와 클라이언트 암호를 사용 하 여 VM에서 암호화를 사용 하도록 설정 합니다.
...
이 예제에서는 Azure AD 클라이언트 ID 및 클라이언트 인증 지문을 사용 하 여 VM에서 암호화를 사용 하도록 설정 합니다.
...
이 예제에서는 Azure AD 클라이언트 ID와 클라이언트 암호를 사용 하 여 VM에서 암호화를 사용 하도록 설정 하 고 키 암호화 키를 사용 하 여 디스크 암호화 키를 래핑합니다.
...
이 예제에서는 Azure AD 클라이언트 ID 및 클라이언트 인증서 지문을 사용 하 여 VM에서 암호화를 사용 하도록 설정 하 고 키 암호화 키를 사용 하 여 디스크 암호화 키를 래핑합니다.
...
### -이름
확장을 나타내는 Azure Resource Manager 리소스의 이름을 지정 합니다. *Name* 매개 변수를 생략 하면 설치 된 확장이 Windows 가상 머신에서 AzureDiskEncryption으로 명명 되 고 linux 가상 머신에는 Azurediskencryption forlinux가 지정 됩니다.
