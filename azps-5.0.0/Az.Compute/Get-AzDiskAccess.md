...
## 설명은
**Get-AzDiskAccess** Cmdlet은 디스크 액세스의 속성을 가져옵니다.
...
이 명령은 리소스 그룹 ' ResourceGroup01 '에 ' DiskAccess01 ' (이) 라는 디스크 액세스 리소스의 속성을 가져옵니다.

### 예제 2: 리소스 그룹별 Get-AzDiskAccess
```
...
이 명령은 리소스 그룹 ' ResourceGroup01 '에 있는 모든 디스크 액세스의 속성을 가져옵니다.


### 예제 3: 모든 디스크 액세스 가져오기
```
...
이 명령은 구독에서 모든 디스크 액세스의 속성을 가져옵니다.

### 예제 4: 와일드 카드 문자를 사용 하 여 모든 디스크 액세스 가져오기
```
...
이 명령은 ' DiskAccessMicrosoft '로 시작 하는 구독 이름 아래에 있는 모든 디스크 액세스의 속성을 가져옵니다.

### 예제 5: ResourceId를 사용 하 여 디스크 액세스 가져오기
```
...
이 명령은 지정 된 ResourceId로 디스크 액세스의 속성을 가져옵니다.
