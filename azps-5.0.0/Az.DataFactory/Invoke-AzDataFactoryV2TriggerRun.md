...
## 설명은
**AzDataFactoryV2TriggerRun** 명령은 새 트리거 실행 id를 사용 하 여 실행 되는 트리거의 다른 인스턴스를 시작 합니다.
...
```
새 트리거 실행 id를 사용 하 여 트리거를 실행 하는 다른 인스턴스를 시작 하 고 원래 트리거로 실행 되는 동일한 windowStartTime 및 Windowstarttime을 유지 합니다.
