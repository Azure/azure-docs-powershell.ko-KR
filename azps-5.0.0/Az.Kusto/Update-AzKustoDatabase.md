...
위의 명령은 리소스 그룹 "testrg"에 있는 클러스터 "testnewkustocluster"에서 "mykustodatabase"에 대 한 소프트 삭제 기간 및 과열 캐시 기간을 업데이트 합니다.
...
### 예제 3: 이름으로 기존 읽기 전용 데이터베이스 업데이트
```powershell
PS C:\> $2ds = New-TimeSpan -Days 2
...
위 명령은 리소스 그룹 "testrg"에 있는 클러스터 "myfollowercluster"의 Kusto 검색 기간을 "mykustodatabase" 데이터베이스로 업데이트 합니다.

### 예제 4: id를 통해 기존 읽기 전용 데이터베이스 업데이트
```powershell
