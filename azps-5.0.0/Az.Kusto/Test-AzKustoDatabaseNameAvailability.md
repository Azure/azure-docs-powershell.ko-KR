...
### 예제 1: 사용 중인 데이터베이스 이름에 대 한 Kusto의 사용 가능 여부 확인
```powershell
...
위의 명령은 "mykustodatabase" 이라는 이름의 데이터베이스에 대 한 Kusto "testnewkustocluster" 클러스터에 있는지 여부를 반환 합니다.

### 예제 2: 사용 하지 않는 Kusto 데이터베이스 이름 사용 가능성 확인
```powershell
...
위의 명령은 "mykustodatabase2" 이라는 이름의 데이터베이스에 대 한 Kusto "testnewkustocluster" 클러스터에 있는지 여부를 반환 합니다.
...
### -Type
리소스 유형으로, 예를 들어 Microsoft Kusto/클러스터/데이터베이스.
