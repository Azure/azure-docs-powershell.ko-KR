...
### 예제 1: 이름으로 데이터베이스에 모든 PrincipalAssignment 배정 나열
```powershell
...
위 명령은 "testnewkustocluster" 클러스터에서 찾은 "mykustodatabase" 데이터베이스의 모든 PrincipalAssignment을 반환 합니다.

### 예제 2: 이름으로 데이터베이스에서 특정 PrincipalAssignment 가져오기
```powershell
...
위 명령은 "testnewkustocluster" 데이터베이스에서 "kustoprincipal1" "이라는 모든 PrincipalAssignment을" mykustodatabase "(으)로 반환 했습니다.
...
### Api20200614. IDatabasePrincipalAssignment의 a에 대 한 대입
