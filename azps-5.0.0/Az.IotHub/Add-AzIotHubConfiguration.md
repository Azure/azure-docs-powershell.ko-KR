...
## 설명은
구성 콘텐츠는 json 이며 장치 또는 모듈 의도에 따라 slighty 달라 집니다. 장치 구성은 {"deviceContent": {...}} 형식입니다.
모듈 구성은 {"moduleContent": {...}} 형식입니다.
구성을 사용 하 여 요청을 평가할 때 사용자가 제공한 메트릭을 정의할 수 있습니다.
사용자 메트릭은 {"쿼리": {...}}의 형식으로 된 json입니다. 또는 {"메트릭스": {"쿼리": {...}}}.

참고: 모듈의 대상 조건은 "on devices. 모듈"로 시작 해야 합니다. https://docs.microsoft.com/azure/iot-hub/iot-hub-automatic-device-management자세한 내용은을 참조 하세요.
...
장치가 빌드 9에 태그가 지정 되 고 환경이 ' 테스트 ' 인 경우 조건에 적용 되는 우선 순위 3을 사용 하 여 디바이스 구성을 만듭니다.
...
### -우선 순위
경합 하는 규칙 (최고 wins)에 대 한 장치 구성의 가중치.
