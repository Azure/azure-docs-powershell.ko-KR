...
## 설명은
IoT Edge 배포에 정의 된 대상 사용자 지정 또는 시스템 메트릭을 평가 합니다.
Iot Hub로 계산 되며 사용자 지정할 수 없는 미리 정의 된 시스템 메트릭이 있습니다.
- "대상"은 배포 대상 지정 조건과 일치 하는 IoT Edge 장치를 표시 합니다.
- "적용 됨"을 지정 하면 우선 순위가 높은 다른 배포의 대상으로 지정 되지 않은 IoT Edge 장치가 표시 됩니다.
- "보고 성공"은 모듈이 성공적으로 배포 되었다고 보고 한 IoT Edge 장치를 보여 줍니다.
- "보고 실패"는 하나 이상의 모듈이 성공적으로 배포 되지 않았다고 보고 한 IoT Edge 장치를 보여 줍니다. 
  오류를 자세히 조사 하려면 해당 장치에 원격으로 연결 하 고 로그 파일을 확인 합니다.
...
사용자 지정 정의 ' warningLimit ' 메트릭을 평가 합니다.
...
### IotHub. PSConfigurationMetricsResult/. *
