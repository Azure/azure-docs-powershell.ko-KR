...
## 설명은
**Get-AzLogicAppTrigger** cmdlet은 논리 앱에서 트리거를 가져옵니다.
이 cmdlet은 **Workflowtrigger** 개체를 반환 합니다.
워크플로, 리소스 그룹 및 트리거를 지정 합니다.
...
이 명령은 LogicApp05 이라는 논리 앱에서 Trigger01 이라는 트리거를 가져옵니다.

### 예제 2: 논리 앱의 모든 트리거 가져오기
```
...
이 명령은 LogicApp07 이라는 논리 앱의 트리거를 가져옵니다.
...
### -이름
이 cmdlet이 트리거를 가져오는 논리 앱의 이름을 지정 합니다.
...
### -ResourceGroupName
이 cmdlet이 트리거를 가져오는 리소스 그룹의 이름을 지정 합니다.
...
### -TriggerName
이 cmdlet이 가져오는 트리거의 이름을 지정 합니다.
...
### Microsoft. Azure. WorkflowTrigger
