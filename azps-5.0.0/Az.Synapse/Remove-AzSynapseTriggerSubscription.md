...
## 설명은
AzSynapseTriggerSubscription cmdlet은 트리거 정의에서 지정 된 외부 서비스 이벤트에 대 한 이벤트 트리거를 구독 **취소** 합니다.
...
이 명령은 트리거 정의에서 지정 된 이벤트에 ContosoTrigger 이라는 트리거를 구독 취소 합니다.
...
이 명령은 트리거 정의에서 파이프라인 까지의 지정 된 이벤트에 ContosoTrigger 이라는 트리거의 구독을 취소 합니다.
